...
SMTP(Simple Mail Transfer Protocol)의 목적은 **메일을 신뢰성 있고 효율적으로 전송**하는 것입니다.
...
1. **사용자**가 메일 발송을 요청하면, **SMTP 클라이언트**가 **SMTP 서버**와 양방향 전송 채널을 설정합니다.
...
> **중요**: SMTP 서버가 메시지에 대해 성공 응답(250)을 보내면, 그 메시지를 배달하거나 배달 실패를 적절히 보고할 **책임을 정식으로 수락**한 것입니다.
...
SMTP 응답은 **3자리 숫자 코드**와 **텍스트 설명**으로 구성됩니다.
