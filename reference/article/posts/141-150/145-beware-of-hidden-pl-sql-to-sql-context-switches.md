...
보시다시피, `USER` 함수는 내부적으로 `SELECT USER INTO c FROM SYS.DUAL` 쿼리를 실행합니다. 이것이 바로 **PL/SQL에서 SQL로의 컨텍스트 스위치**입니다. PL/SQL 코드가 실행 컨텍스트를 벗어나 SQL을 호출한 다음 다시 돌아오는 비용이 발생합니다.
