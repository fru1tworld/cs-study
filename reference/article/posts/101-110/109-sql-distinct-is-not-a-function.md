...
**Joe Celko**는 역사적으로 SQL에 `SELECT DISTINCT`와 `SELECT ALL` 연산자가 모두 포함되어 있었다고 언급했다.

**Peter Tolgyesi**는 `GROUP BY`가 `MIN()` 같은 집계 함수와 함께 유사한 로직을 표현할 수 있다고 제안했다.

**Lukasz**는 `QUALIFY ROW_NUMBER() OVER()`를 사용한 윈도우 함수가 더 나은 대안이라고 주장했다.

**David Fetter**는 `DISTINCT`를 불완전한 제약 조건 분석을 나타내는 "코드 스멜(code smell)"로 특징지었으며, `GROUP BY`와 윈도우 함수가 더 엄밀한 접근 방식이라고 설명했다.
