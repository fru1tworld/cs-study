...
이것이 의미가 있는 이유는 **SQL 쿼리가 튜플 집합의 설명**이기 때문입니다. SQL은 함수형 프로그래밍 언어이며, 일부 동시성 측면을 잊는다면 원칙적으로 부작용이 없습니다. 이는 쿼리가 정말로 튜플의 집합이라는 **것**을 의미합니다. 이 생각을 염두에 두면, 우리는 단순히 그것을 반복할 수 있습니다.
