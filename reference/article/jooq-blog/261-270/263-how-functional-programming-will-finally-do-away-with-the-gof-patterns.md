...
`instanceof`를 사용하는 것이 약간 꺼림칙하게 느껴질 수 있습니다. 하지만 사실 위 코드는 매우 직접적이고 간단합니다. 반면 비지터(Visitor) 패턴을 적용하면 어떻게 될까요? `instanceof`를 사용한 7줄의 코드가 **약 200줄의 이상한 인터페이스들, 추상 클래스들**, 그리고 **암호 같은 `accept()`와 `visit()` 메서드들**로 불어나게 됩니다.
