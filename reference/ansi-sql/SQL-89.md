...
- 관계형 모델의 핵심인 **무결성 제약조건**을 표준에 포함시키려는 학계와 산업계의 요구
...
SQL-89에서의 참조 동작은 **매우 제한적**이었다. SQL-89 원본 표준에서는 참조 동작(ON DELETE, ON UPDATE)에 대한 지원이 다음과 같았다:
...
SQL-89 표준의 **일부 구현체**에서는 제한적인 참조 동작을 지원했으며, 표준 문서의 후반부에서 다음과 같은 동작이 언급되었다:
...
> **중요:** `ON DELETE CASCADE`, `ON UPDATE CASCADE` 등의 참조 동작은 SQL-89에서 논의되었으나, **완전한 표준 지원은 SQL-92에서 공식화**되었다. SQL-89에서의 기본 동작은 사실상 `RESTRICT`와 동일하다.
...
SQL-89 이전에는 데이터 무결성이 **애플리케이션 코드**에 의존했다:
...
SQL-89(ANSI X3.135-1989 / ISO 9075:1989)는 규모 면에서는 SQL-86에 대한 비교적 작은 개정이었으나, 그 **질적 의미**는 매우 크다. **Integrity Enhancement Feature(IEF)**의 도입을 통해 관계형 데이터베이스의 본질적 특성인 **선언적 무결성**이 국제 표준으로 확립되었고, 이는 이후 모든 SQL 표준과 상용 RDBMS의 근간이 되었다. 오늘날 `PRIMARY KEY`, `FOREIGN KEY`, `CHECK`, `UNIQUE`, `DEFAULT`는 SQL을 배우는 모든 사람이 가장 먼저 접하는 기본 개념이며, 이 모든 것의 표준화된 출발점이 바로 SQL-89이다.
