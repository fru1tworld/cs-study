...
이 문서는 패스워드 해싱 및 작업 증명 애플리케이션을 위한 Argon2 메모리 하드 함수를 설명한다.
 우리는 테스트 벡터와 함께 구현자 지향적인 설명을 제공한다.
 이 문서의 목적은 인터넷 프로토콜에서 Argon2의 채택을 간소화하는 것이다.
 이 문서는 IRTF의 암호 포럼 연구 그룹(CFRG)의 산출물이다.

## 이 메모의 상태

이 문서는 인터넷 표준 트랙 규격이 아니며, 정보 제공 목적으로 발행된다.

이 문서는 인터넷 연구 태스크 포스(IRTF)의 산출물이다.
 IRTF는 인터넷 관련 연구 및 개발 활동의 결과를 발행한다.
 이러한 결과는 배포에 적합하지 않을 수 있다.
 이 RFC는 인터넷 연구 태스크 포스(IRTF)의 암호 포럼 연구 그룹의 합의를 나타낸다.
 IRSG에 의해 발행 승인된 문서는 어떤 수준의 인터넷 표준 후보도 아니다; RFC 7841의 섹션 2를 참조하라.

이 문서의 현재 상태, 정오표 및 피드백 제공 방법에 대한 정보는 https://www.rfc-editor.org/info/rfc9106에서 얻을 수 있다.

## 저작권 고지

Copyright (c) 2021 IETF Trust 및 문서 저자로 식별된 사람들.
 모든 권리 보유.

이 문서는 이 문서의 발행일에 유효한 BCP 78 및 IETF 문서에 관한 IETF Trust의 법적 조항(https://trustee.ietf.org/license-info)의 적용을 받는다.
...
이 문서는 패스워드 해싱 및 작업 증명 애플리케이션을 위한 Argon2 [ARGON2ESP] 메모리 하드 함수를 설명한다.
 우리는 테스트 벡터와 함께 구현자 지향적인 설명을 제공한다.
 이 문서의 목적은 인터넷 프로토콜에서 Argon2의 채택을 간소화하는 것이다.
 이 문서는 Argon2 해시 함수의 버전 1.3에 해당한다.

Argon2는 메모리 하드 함수이다 [HARD]. 간소화된 설계이다.
 트레이드오프 공격에 대한 방어를 제공하면서도 최고의 메모리 채우기 속도와 다중 계산 유닛의 효과적인 사용을 목표로 한다.
 Argon2는 x86 아키텍처에 최적화되어 있으며 최신 Intel 및 AMD 프로세서의 캐시 및 메모리 구성을 활용한다.
 Argon2는 하나의 주요 변형인 Argon2id와 두 개의 보조 변형인 Argon2d 및 Argon2i를 가진다.
 Argon2d는 데이터 의존적 메모리 접근을 사용하여, 사이드 채널 타이밍 공격의 위협이 없는 암호화폐 및 작업 증명 애플리케이션에 적합하다.
 Argon2i는 데이터 독립적 메모리 접근을 사용하여, 패스워드 해싱 및 패스워드 기반 키 유도에 선호된다.
 Argon2id는 메모리에 대한 첫 번째 패스의 전반부는 Argon2i로, 나머지는 Argon2d로 작동하여, 사이드 채널 공격 보호와 시간-메모리 트레이드오프로 인한 무차별 대입 비용 절감을 모두 제공한다.
 Argon2i는 트레이드오프 공격으로부터 보호하기 위해 메모리에 대해 더 많은 패스를 수행한다 [AB15].

Argon2id는 이 문서의 모든 구현에서 반드시(MUST) 지원되어야 하며, Argon2d와 Argon2i는 선택적으로(MAY) 지원할 수 있다.

Argon2는 또한 고정 입력 길이 압축 함수 G와 가변 입력 길이 해시 함수 H에 대한 연산 모드이다.
 Argon2는 잠재적으로 임의의 함수 H와 함께 사용될 수 있지만, 최대 64바이트까지의 출력을 제공하는 한, 이 문서에서는 BLAKE2b 함수 [BLAKE2]가 사용된다.
...
- 논스 S, 패스워드 해싱 애플리케이션의 경우 솔트이다. 2^(32)-1 바이트를 초과하지 않는 길이를 가져야(MUST) 한다.
 패스워드 해싱의 경우 16바이트가 권장된다(RECOMMENDED).
 솔트는 각 패스워드에 대해 고유해야(SHOULD) 한다.
...
- 메모리 크기 m은 8*p에서 2^(32)-1 사이의 정수 키비바이트 수여야(MUST) 한다.
 실제 블록 수는 m'이며, 이는 m을 4*p의 가장 가까운 배수로 내림한 것이다.

- 패스 수 t(메모리 크기와 독립적으로 실행 시간을 조정하는 데 사용됨)는 1에서 2^(32)-1 사이의 정수 수여야(MUST) 한다.

- 버전 번호 v는 1바이트 0x13이어야(MUST) 한다.

- 비밀 값 K는 선택사항(OPTIONAL)이다.
 사용되는 경우, 2^(32)-1 바이트를 초과하지 않는 길이를 가져야(MUST) 한다.

- 연관 데이터 X는 선택사항(OPTIONAL)이다.
 사용되는 경우, 2^(32)-1 바이트를 초과하지 않는 길이를 가져야(MUST) 한다.
...
Argon2는 두 개의 1024바이트 입력과 1024바이트 출력을 가진 내부 압축 함수 G와, x가 바이트 단위 출력 길이인 내부 해시 함수 H^x()를 사용한다.
 여기서, 문자열 A에 적용된 H^x()는 BLAKE2b ([BLAKE2], 섹션 3.3) 함수이며, (d,ll,kk=0,nn=x)를 매개변수로 취하고, 여기서 d는 A를 128바이트의 배수로 패딩한 것이고 ll은 d의 바이트 길이이다.
 압축 함수 G는 내부 순열에 기반한다.
 H를 기반으로 구축된 가변 길이 해시 함수 H'도 사용된다.
 G는 섹션 3.5에, H'는 섹션 3.3에 설명되어 있다.

Argon2 연산은 다음과 같다.

1. H_0를 아래에 표시된 64바이트 값으로 설정한다.
 K, X 또는 S의 길이가 0인 경우, 단지 부재하지만 길이 필드는 유지된다.
...
5. 0을 포함하여 p를 포함하지 않는 범위의 모든 i와 2를 포함하여 q를 포함하지 않는 범위의 모든 j에 대해 B[i][j]를 계산한다.
 계산은 반드시(MUST) 슬라이스 단위로(섹션 3.4) 진행되어야 한다: 먼저, 모든 레인에 대해 슬라이스 0의 블록이 계산되고(레인의 임의 순서로), 그 다음 슬라이스 1의 블록이 계산되는 등이다.
 블록 인덱스 l과 z는 Argon2d, Argon2i, Argon2id에 대해 각 i, j에 대해 다르게 결정된다.
...
6. 패스 수 t가 1보다 큰 경우, 단계 5를 반복한다. 0을 포함하여 p를 포함하지 않는 범위의 모든 i와 1을 포함하여 q를 포함하지 않는 범위의 모든 j에 대해 B[i][0]과 B[i][j]를 계산한다.
 그러나, 블록은 이전 값과 새 값이 XOR되므로 다르게 계산된다:
...
V_i를 64바이트 블록으로, W_i를 그 처음 32바이트로 하자.
 그러면 함수 H'를 다음과 같이 정의한다:
...
병렬 블록 계산을 가능하게 하기 위해, 메모리 행렬을 SL = 4개의 수직 슬라이스로 더 분할한다.
 슬라이스와 레인의 교차점을 세그먼트라고 하며, 길이는 q/SL이다.
 동일한 슬라이스의 세그먼트는 병렬로 계산될 수 있으며 서로 블록을 참조하지 않는다.
 다른 모든 블록은 참조될 수 있다.
...
l = J_2 mod p의 값은 블록을 가져올 레인의 인덱스를 제공한다.
 첫 번째 패스(r=0)와 첫 번째 슬라이스(sl=0)의 경우, 블록은 현재 레인에서 가져온다.
...
2. l이 현재 레인이 아닌 경우, W는 레인 l에서 계산되고 완료된 마지막 SL - 1 = 3개 세그먼트의 모든 블록 인덱스를 포함한다.
 B[i][j]가 세그먼트의 첫 번째 블록인 경우, W에서 맨 마지막 인덱스가 제외된다.
...
압축 함수 G는 BLAKE2b 기반 변환 P에 기반하여 구축된다.
 P는 8개의 16바이트 레지스터로 볼 수 있는 128바이트 입력에서 동작한다:
...
압축 함수 G(X, Y)는 두 개의 1024바이트 블록 X와 Y에서 동작한다.
 먼저 R = X XOR Y를 계산한다.
 그 다음 R은 16바이트 레지스터 R_0, R_1, ... , R_63의 8x8 행렬로 본다.
 그 다음 Z를 얻기 위해 P가 먼저 각 행에, 그 다음 각 열에 적용된다:
...
GB의 모듈러 덧셈은 64비트 곱셈과 결합된다.
 곱셈은 원래 BLAKE2b 설계와의 유일한 차이점이다.
 이 선택은 회로 깊이를 증가시켜 ASIC 구현의 실행 시간을 증가시키면서, 병렬 처리 및 파이프라이닝 덕분에 CPU에서는 거의 동일한 실행 시간을 갖도록 하기 위한 것이다.

## 4. 매개변수 선택

Argon2d는 공격자가 시스템 메모리나 CPU에 정기적으로 접근하지 못하는 설정에 최적화되어 있다.
 즉, 타이밍 정보에 기반한 사이드 채널 공격을 실행할 수 없고, 가비지 컬렉션을 사용하여 패스워드를 훨씬 빠르게 복구할 수 없는 설정이다.
 이러한 설정은 백엔드 서버와 암호화폐 채굴에 더 일반적이다.
 실제 사용을 위해, 다음 설정을 제안한다:

- 1코어를 사용하여 2 GHz CPU에서 0.1초가 걸리는 암호화폐 채굴 -- 2개의 레인과 250 MB RAM을 가진 Argon2d.

Argon2id는 공격자가 잠재적으로 같은 머신에 접근하거나, CPU를 사용하거나, 콜드 부트 공격을 마운트할 수 있는 더 현실적인 설정에 최적화되어 있다.
 다음 설정을 제안한다:
...
1. 귀하의 애플리케이션이나 하드웨어에 맞춤화되지 않은 균일하게 안전한 옵션이 허용되는 경우, t=1 반복, p=4 레인, m=2^(21) (2 GiB RAM), 128비트 솔트, 256비트 태그 크기를 가진 Argon2id를 선택하라.
 이것이 첫 번째 권장 옵션이다.

2. 훨씬 적은 메모리를 사용할 수 있는 경우, 균일하게 안전한 옵션은 t=3 반복, p=4 레인, m=2^(16) (64 MiB RAM), 128비트 솔트, 256비트 태그 크기를 가진 Argon2id이다.
 이것이 두 번째 권장 옵션이다.

3. 그렇지 않으면, 타입 y를 선택하는 것부터 시작하라.
 타입 간의 차이를 모르거나 사이드 채널 공격을 실행 가능한 위협으로 고려하는 경우, Argon2id를 선택하라.
...
8. 태그 길이를 선택하라. 128비트 길이는 키 유도를 포함한 대부분의 애플리케이션에 충분하다.
 더 긴 키가 필요한 경우, 더 긴 태그를 선택하라.

9. 사이드 채널 공격이 실행 가능한 위협이거나 확실하지 않은 경우, 라이브러리 호출에서 메모리 와이핑 옵션을 활성화하라.

10. 타입 y, 메모리 m, p 레인의 스킴을 다른 패스 수 t를 사용하여 실행하라.
 실행 시간이 감당 가능한 시간을 초과하지 않는 최대 t를 파악하라. t = 1에서도 초과하는 경우, m을 그에 따라 줄여라.
...
완전한 출력(태그)과 함께 테스트 벡터를 제공한다.
 개발자의 편의를 위해, 일부 중간 변수도 제공한다 -- 구체적으로, 각 패스의 첫 번째와 마지막 메모리 블록이다.
...
Argon2의 충돌 저항성 및 역상 저항성 수준은 기반이 되는 BLAKE2b 해시 함수와 동등하다.
 충돌을 생성하려면 2^(256)개의 입력이 필요하다.
 역상을 찾으려면 2^(512)개의 입력을 시도해야 한다.

KDF 보안은 키 길이와 해시 함수 H'의 내부 상태 크기에 의해 결정된다.
 키가 있는 Argon2의 출력을 무작위와 구별하려면, 최소 (2^(128),2^length(K)) 번의 BLAKE2b 호출이 필요하다.

### 7.2. 시간-공간 트레이드오프 공격에 대한 보안

시간-공간 트레이드오프는 내부 압축 함수에 대한 더 많은 호출 비용으로 더 적은 메모리 블록을 저장하면서 메모리 하드 함수를 계산할 수 있게 한다.
 트레이드오프 공격의 이점은 시간-면적 곱에 대한 감소 계수로 측정되며, 여기서 메모리와 추가 압축 함수 코어가 면적에 기여하고, 누락된 블록의 재계산을 수용하기 위해 시간이 증가한다.
 높은 감소 계수는 잠재적으로 역상 검색을 가속화할 수 있다.

1패스 및 2패스 Argon2i에 대해 가장 잘 알려진 공격은 [CBS16]에 설명된 저장소 공격이며, 시간-면적 곱(피크 메모리 값 사용)을 5배 감소시킨다. 3패스 이상의 Argon2i에 대한 최선의 공격은 [AB16]에 설명되어 있으며, 감소 계수는 메모리 크기와 패스 수의 함수이다(예: 1 기비바이트 메모리의 경우, 3패스에 대해 감소 계수 3, 4패스에 대해 2.5, 6패스에 대해 2). 감소 계수는 메모리 크기가 두 배가 될 때마다 약 0.5씩 증가한다.
 [AB16]의 시간-공간 트레이드오프를 완전히 방지하려면, 패스 수는 반드시(MUST) 메모리의 이진 로그에서 26을 뺀 값을 초과해야 한다.
 점근적으로, 1패스 Argon2i에 대한 최선의 공격은 [BZ17]에 주어지며, 공격자의 최대 이점은 O(m^(0.233))으로 상한되며, 여기서 m은 블록 수이다.
 이 공격은 [BZ17]이 어떤 공격에도 상한이 O(m^(0.25))임을 증명하므로 점근적으로 최적이기도 하다.

t패스 Argon2d에 대한 최선의 트레이드오프 공격은 랭킹 트레이드오프 공격이며, 시간-면적 곱을 1.33배 감소시킨다.

Argon2id에 대한 최선의 공격은 1패스 Argon2i에 대한 최선의 공격과 다중 패스 Argon2d에 대한 최선의 공격을 보완하여 얻을 수 있다.
 따라서, 1패스 Argon2id에 대한 최선의 트레이드오프 공격은 결합된 저장소 공격(메모리의 전반부)과 랭킹 공격(후반부)이며, 약 2.1의 계수를 생성한다. t패스 Argon2id에 대한 최선의 트레이드오프 공격은 랭킹 트레이드오프 공격이며, 시간-면적 곱을 1.33배 감소시킨다.

### 7.3. 시간 제한 방어자를 위한 보안

패스워드 해싱 함수를 사용하는 시스템의 병목 현상은 종종 메모리 비용보다는 함수 지연 시간이다.
 합리적인 방어자는 해시, 솔트, 방어자 머신의 고정된 계산 시간에 대한 타이밍 정보 목록을 갖춘 공격자에 대한 무차별 대입 비용을 최대화할 것이다.
 [AB16]의 공격 비용 추정은 Argon2i의 경우, 3패스가 대부분의 합리적인 메모리 크기에 대해 거의 최적임을 의미한다; Argon2d 및 Argon2id의 경우, 1패스가 일정한 방어자 시간에 대한 공격 비용을 최대화한다.

### 7.4. 권장사항

t=1과 2 GiB 메모리를 가진 Argon2id 변형이 첫 번째 권장 옵션이며 모든 환경의 기본 설정으로 제안된다.
 이 설정은 사이드 채널 공격에 대해 안전하며 전용 무차별 대입 하드웨어에서 공격자 비용을 최대화한다. t=3과 64 MiB 메모리를 가진 Argon2id 변형이 두 번째 권장 옵션이며 메모리 제약 환경의 기본 설정으로 제안된다.
