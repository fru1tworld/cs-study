...
RFC 9568: 로컬 영역 네트워크(LAN)에서 IPv4·IPv6 라우터에 대한 이중화를 제공하는 VRRP 버전 3을 정의
→ RFC 5798을 폐기, 포용적 언어 반영·기술적 명확화 위주로 상당한 업데이트 포함
...
- Advertisement_Interval
  - 공식: 구성값
  - 설명: 광고 전송 간격(기본 100 센티초 = 1초)
- Active_Down_Interval
  - 공식: (3 × Adver_Interval) + Skew_Time
  - 설명: 활성 라우터 실패 판단 시간
- Skew_Time
  - 공식: ((256 - Priority) × Adver_Interval) / 256
  - 설명: 우선순위 기반 지연
...
Skew_Time: 우선순위에 따라 다른 대기 시간 부여
...
수신 라우터가 모두 검증해야 할 항목:

- TTL/홉 제한 = 255 → 원격 공격 방지(로컬 네트워크만 허용)
- VRRP 버전 = 3 → 호환성 확인
- 패킷 유형 = 1(ADVERTISEMENT) → 유효한 메시지 유형
- 패킷 완전성 → 손상된 패킷 거부
- 체크섬 유효 → 데이터 무결성 확인
- VRID 일치 → 구성된 가상 라우터와 매칭
- 수신자 ≠ 주소 소유자 → 자기 자신의 패킷 무시
...
활성 라우터가 전송하는 Router Advertisement
...
일관된 Advertisement_Interval 사용 필요
...
- 광고 간격
  ...
  - VRRPv3: 센티초 단위
- 용어
  - VRRPv2: Master/Backup
  - VRRPv3: Active/Backup
- 인증
  - VRRPv2: 지원(약함)
  - VRRPv3: 제거됨
...
VRRP → 게이트웨이 이중화의 표준 프로토콜, 네트워크 가용성을 높이는 핵심 기술

### 참고 문서

- RFC 5798: Virtual Router Redundancy Protocol (VRRP) Version 3 (본 RFC가 대체)
