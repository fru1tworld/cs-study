...
- RFC 8446: TLS(Transport Layer Security) 프로토콜 최신 버전 TLS 1.3 정의
- HTTPS의 핵심 보안 프로토콜 → 클라이언트-서버 간 통신의 기밀성(confidentiality)·무결성(integrity) 보장

### TLS가 제공하는 세 가지 보안 보장

- 서버 인증: 클라이언트가 서버의 신원 확인 (클라이언트 인증은 선택)
...
TLS 1.3의 표준 핸드셰이크는 1-RTT(1 Round Trip Time)로 완료:
...
이전 연결에서 공유된 PSK 사용 → 첫 메시지와 함께 데이터 전송:
...
- 재전송 공격 보호 없음 (연결 간 non-replay 보장 없음)
...
TLS 1.3은 세 가지 키 교환 모드를 지원:

- (EC)DHE
  - Forward Secrecy: 있음
  - Diffie-Hellman (타원 곡선 또는 유한 필드)
- PSK-only
  - Forward Secrecy: 없음
  - 사전 공유 키만 사용
- PSK + (EC)DHE
...
- x25519: 보안 수준 128비트, 권장·빠름
- x448: 보안 수준 224비트, 고보안
- secp256r1 (P-256): 보안 수준 128비트, 널리 지원
- secp384r1 (P-384): 보안 수준 192비트
- secp521r1 (P-521): 보안 수준 256비트

#### FFDHE (유한 필드)

- ffdhe2048: 크기 2048비트, 보안 수준 ~112비트
- ffdhe3072: 크기 3072비트, 보안 수준 ~128비트
- ffdhe4096: 크기 4096비트, 보안 수준 ~152비트
- ffdhe6144: 크기 6144비트, 보안 수준 ~176비트
- ffdhe8192: 크기 8192비트, 보안 수준 ~200비트

### 4.4 키 유도 (Key Derivation)

TLS 1.3은 HKDF(HMAC-based Extract-and-Expand Key Derivation Function) 사용:
...
`signature_algorithms` 확장으로 지원 알고리즘 협상
...
암호화 키는 일정 횟수 이상 사용하면 갱신 필요:

- AES-GCM: 권장 한계 2^24.5 레코드
...
ServerHello의 random 필드에 다운그레이드 표시자 포함:
...
- 키 교환
  ...
  - TLS 1.3: (EC)DHE만 (Forward Secrecy 필수)
- 암호화
  - TLS 1.2: CBC·GCM·CCM
  - TLS 1.3: AEAD만 (GCM·CCM·ChaCha20)
- 해시
  - TLS 1.2: MD5·SHA-1 사용 가능
  - TLS 1.3: SHA-256+ 만
- 인증서 암호화
  - TLS 1.2: 평문
  - TLS 1.3: 암호화됨
- 세션 재개
  - TLS 1.2: Session ID·Ticket
  - TLS 1.3: PSK 기반
- 압축
  - TLS 1.2: 지원
  - TLS 1.3: 제거됨 (CRIME 공격 방어)
- 재협상
  - TLS 1.2: 지원
  - TLS 1.3: 제거됨
...
TLS 1.3: 보안 강화 + 성능 개선을 동시에 이룬 현대적 보안 프로토콜
