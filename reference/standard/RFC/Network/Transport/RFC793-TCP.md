...
> 폐기: RFC 9293에 의해 대체됨 (최신 통합본)

## 개요

RFC 793: 1981년 9월 발표된 전송 제어 프로토콜(TCP)의 기본 명세서 → TCP의 바이블로 불리며 현대 인터넷 통신의 기반이 되는 핵심 프로토콜을 정의.
...
│    │                                              │          │
│    ↓                                              ↓          │
│  SYN-RECEIVED ←────(동시 열기 예외)───────────────┘          │
│    │                                              │          │
│    ↓                                              ↓ (정상 3-way handshake:│
│  ESTABLISHED ←──────────────────────────────────────rcv SYN,ACK; snd ACK)│
│    │                                                         │
│    ├─────────────┬────────────────────┐                     │
│    ↓             ↓                    ↓                     │
...
TCP 연결 시 사용되는 초기 순서 번호(ISN) → 보안과 신뢰성을 위해 신중하게 선택 필요.

### 선택 방법

RFC 793에서 ISN 생성을 위해 권장하는 방식: 32비트 클럭 기반.
...
보안 강화를 위해 현대 구현에서 사용하는 방식: 암호학적 의사 난수 생성기(CSPRNG).
...
TCP 연결 종료: 4-way 핸드셰이크를 통해 이루어짐.
...
TCP에서 SYN과 FIN 플래그: 각각 시퀀스 번호 공간에서 1바이트 소비.
...
TCP 체크섬 계산 시 실제 TCP 세그먼트 외에 Pseudo Header 포함.
...
TCP 옵션: 헤더의 Options 필드에 위치, 32비트 경계로 패딩.
...
수신자의 버퍼가 가득 차면 → Window = 0을 광고하여 송신자에게 전송 중단 요청.
...
Zero Window 상황에서 교착 상태(Deadlock)를 방지하기 위한 메커니즘.
...
TIME-WAIT: TCP 연결 종료 후 일정 시간 동안 유지되는 상태.
...
- 포트 고갈: 많은 연결 시 TIME-WAIT 소켓 누적 → 해결책: SO_REUSEADDR 옵션
- 메모리 사용: TIME-WAIT 소켓도 자원 소비 → 해결책: tcp_tw_reuse, tcp_tw_recycle
- 서버 재시작: 바인딩 실패 가능 → 해결책: SO_REUSEADDR 옵션
...
Urgent Data(긴급 데이터): 수신자에게 즉시 처리해야 함을 알리는 메커니즘.

### 헤더 필드

- URG 플래그(1비트): 긴급 데이터 존재 표시
- Urgent Pointer(16비트): 긴급 데이터의 위치 표시
...
- RFC 793: Urgent Pointer가 긴급 데이터의 마지막 바이트 다음 위치를 가리킴
- RFC 1122: Urgent Pointer가 긴급 데이터의 마지막 바이트 위치를 가리킴
...
RFC 793: 역사적 맥락과 기초를 이해하는 데 유용하나 → 최신 통합본은 RFC 9293. RFC 9293은 이후의 모든 업데이트와 확장을 통합한 문서.

## 요약

TCP: 인터넷 통신의 근간을 이루는 프로토콜.
...
이러한 특성으로 TCP는 웹 브라우징·이메일·파일 전송 등 신뢰성이 중요한 대부분의 인터넷 애플리케이션에서 사용됨.
