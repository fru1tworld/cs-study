...
- IMAP 클라이언트는 원래 새 메일 도착 여부 확인을 위해 `NOOP`·`SELECT`를 주기적으로 반복 호출하는 폴링(polling) 방식 사용
- IDLE 확장: 클라이언트가 서버에 유휴 상태 대기를 선언 → 서버가 메일함 상태 변화 시마다 즉시 알림 전송 → 폴링 비용 제거
- 푸시 메일(모바일 메일 앱의 즉시 알림 등) 구현의 사실상 표준 메커니즘
