...
DHCP(Dynamic Host Configuration Protocol): TCP/IP 네트워크의 호스트에 설정 정보를 자동으로 전달하는 프로토콜.
- BOOTP를 기반 → 자동 주소 할당과 추가 설정 옵션 기능을 제공
...
`ciaddr`(Client IP Address) 필드: 클라이언트의 현재 IP 주소를 나타냄 → 클라이언트 상태에 따라 엄격한 사용 규칙 적용.

#### 상태별 ciaddr 설정 규칙

- DHCPDISCOVER
  - INIT 상태: ciaddr=0.0.0.0, IP 없음(반드시 0)
- DHCPREQUEST
  - SELECTING 상태: ciaddr=0.0.0.0, OFFER 선택 중이라 아직 IP 미확정
  - INIT-REBOOT 상태: ciaddr=0.0.0.0, 재부팅 중이라 유효성 확인 전
  - RENEWING 상태: ciaddr=현재 IP, 갱신 요청이며 유효한 IP 보유
  - REBINDING 상태: ciaddr=현재 IP, 재바인딩 요청이며 유효한 IP 보유
- DHCPINFORM: ciaddr=현재 IP, 추가 정보만 요청하며 유효한 IP 보유
- DHCPRELEASE: ciaddr=현재 IP, 임대 반환이며 유효한 IP 보유
- DHCPDECLINE: ciaddr=0.0.0.0, 충돌 보고이며 IP 사용 거부

#### ciaddr 사용의 핵심 원칙

- ciaddr이 0.0.0.0이 아닌 경우
...
### 2.5 Option Overload 메커니즘

DHCP 옵션이 options 필드(312바이트)를 초과 → `sname`과 `file` 필드를 추가 옵션 공간으로 사용 가능.
...
- 값 1: `file` 필드(128바이트)가 옵션을 포함
- 값 2: `sname` 필드(64바이트)가 옵션을 포함
- 값 3: `file`과 `sname` 필드 모두 옵션을 포함
...
DHCP 클라이언트는 메시지 손실에 대비 → 지수 백오프(Exponential Backoff) 알고리즘 사용.

재전송 타이밍:
- 초기: 대기 시간 0-1초 · 초기 랜덤 지연(동기화 방지)
- 1차: 대기 시간 4초 ±1초 · 첫 번째 전송
- 2차: 대기 시간 8초 ±1초 · 응답 없을 경우 재전송
- 3차: 대기 시간 16초 ±1초 · 계속 응답 없을 경우
- 4차: 대기 시간 32초 ±1초 · 지수적 증가
- 5차: 대기 시간 64초 ±1초 · 최대 대기 시간 도달
- 이후: 대기 시간 64초 ±1초 · 64초에서 고정
...
RFC 2131은 클라이언트가 여러 DHCPOFFER 중 무엇을 고를지를 "구현에 따라 다름(implementation dependent)"이라고 명시 → 구체적인 선택 알고리즘은 규정하지 않음.
아래는 RFC가 규정한 것이 아니라 일반적인 구현 관행:
...
참고: INIT-REBOOT는 재부팅 시 이전 IP로 직접 REQUEST하는 상태.
...
- 코드 1 · Subnet Mask · 길이 4: 서브넷 마스크
- 코드 3 · Router · 길이 N×4: 기본 게이트웨이 목록
- 코드 6 · DNS Server · 길이 N×4: DNS 서버 목록
- 코드 12 · Host Name · 길이 N: 클라이언트 호스트명
- 코드 15 · Domain Name · 길이 N: DNS 도메인명
- 코드 28 · Broadcast Address · 길이 4: 브로드캐스트 주소
- 코드 50 · Requested IP · 길이 4: 요청하는 IP 주소
- 코드 51 · Lease Time · 길이 4: 임대 기간(초)
- 코드 53 · Message Type · 길이 1: DHCP 메시지 타입
- 코드 54 · Server Identifier · 길이 4: 서버 IP 주소
- 코드 55 · Parameter List · 길이 N: 요청 옵션 목록
- 코드 57 · Max Message Size · 길이 2: 최대 메시지 크기
- 코드 58 · Renewal Time(T1) · 길이 4: 갱신 시작 시간
- 코드 59 · Rebinding Time(T2) · 길이 4: 재바인딩 시작 시간
- 코드 61 · Client Identifier · 길이 N: 클라이언트 고유 ID

### 7.2 DHCP 메시지 타입(옵션 53)

- 값 1 · DHCPDISCOVER: 서버 탐색
- 값 2 · DHCPOFFER: 주소 제안
- 값 3 · DHCPREQUEST: 주소 요청/갱신
- 값 4 · DHCPDECLINE: 주소 거부(충돌)
- 값 5 · DHCPACK: 주소 확인
- 값 6 · DHCPNAK: 요청 거부
- 값 7 · DHCPRELEASE: 주소 반환
- 값 8 · DHCPINFORM: 정보만 요청
...
### 목적
일부 클라이언트는 IP 주소가 설정되기 전에 유니캐스트 수신이 불가능 → BROADCAST 플래그로 응답 방식을 지정.
...
- B값 0: 클라이언트는 유니캐스트 수신 가능 → 서버는 yiaddr로 유니캐스트
- B값 1: 클라이언트는 유니캐스트 수신 불가 → 서버는 255.255.255.255로 브로드캐스트
...
### 9.1 목적
모든 서브넷에 DHCP 서버를 둘 필요 없이, 다른 서브넷의 서버와 통신 가능하게 함.
...
2. 전달 방식 결정
   ...
   - yiaddr != 0.0.0.0 → yiaddr로 유니캐스트(링크 계층 주소 사용)
3. 클라이언트 인터페이스로 전달
   - 목적지 포트: 68(클라이언트 포트)
...
3. 릴레이 동작
   - eth0에서 수신한 브로드캐스트를 giaddr=192.168.1.1로 설정하여 10.0.0.1, 10.0.0.2로 유니캐스트 전달
...
RFC 2131의 DHCP: 기본적인 보안 메커니즘 없음.
