...
- RFC 1035: 도메인 네임 시스템(DNS)의 구현과 명세를 다루는 핵심 기술 문서
- DNS: 서로 다른 호스트·네트워크·프로토콜 패밀리·인터넷 및 행정 조직에서 자원을 명명할 수 있는 메커니즘 제공
...
ID (Identification)
- 16비트 식별자, 프로그램에 의해 할당
- 쿼리 생성 시 설정 → 응답 메시지에 복사
- 여러 미해결 쿼리 구분 용도
- 각 쿼리에 고유 ID 부여 → 요청-응답 매칭에 활용
...
TC (Truncation)
- 메시지가 허용된 크기를 초과해 절단되었는지 표시
- UDP의 경우 512바이트 초과 시 설정
- 설정된 경우 클라이언트는 TCP로 재시도 필요

RD (Recursion Desired)
- 클라이언트가 재귀 쿼리를 원하는지 표시
- 1로 설정 → 네임서버에게 재귀적 해결 요청
- 쿼리에서 설정 → 응답에 복사됨
...
RFC 1035에서 정의된 모든 TYPE 값:
- A · 1 · 호스트 IPv4 주소 · RDATA: 32비트 IP 주소
- NS · 2 · 권한 있는 네임서버 · RDATA: 도메인 명
- MD · 3 · 메일 목적지 (deprecated, MX 사용) · RDATA: 도메인 명
- MF · 4 · 메일 전달자 (deprecated, MX 사용) · RDATA: 도메인 명
- CNAME · 5 · 별칭의 정규 명 · RDATA: 도메인 명
- SOA · 6 · 존 권한 개시 · RDATA: 복합 데이터 (3.3절 참조)
- MB · 7 · 메일박스 도메인 명 (실험적) · RDATA: 도메인 명
- MG · 8 · 메일 그룹 멤버 (실험적) · RDATA: 도메인 명
- MR · 9 · 메일 리네임 도메인 명 (실험적) · RDATA: 도메인 명
- NULL · 10 · 널 RR (실험적) · RDATA: 최대 65535 옥텟의 임의 데이터
- WKS · 11 · Well Known Services · RDATA: 주소 + 프로토콜 + 비트맵
- PTR · 12 · 도메인 명 포인터 (역방향 조회) · RDATA: 도메인 명
- HINFO · 13 · 호스트 정보 · RDATA: CPU, OS 문자열
- MINFO · 14 · 메일박스/메일 리스트 정보 · RDATA: RMAILBX + EMAILBX 도메인 명
- MX · 15 · 메일 교환 서버 · RDATA: 선호도(16비트) + 도메인 명
- TXT · 16 · 텍스트 문자열 · RDATA: 문자열 시퀀스
...
- DNS 메시지에서 도메인 명은 라벨의 시퀀스로 표현 → 각 라벨의 첫 2비트가 라벨 타입을 결정
...
- 포인터가 가리키는 위치부터 도메인 명의 나머지가 시작
- 포인터는 항상 2옥텟, 다른 라벨은 가변 길이
...
- 일반 라벨 뒤에 포인터가 올 수 있지만, 포인터 뒤에 다른 라벨은 불가
...
- RFC 1035는 기본적인 보안 메커니즘을 포함하지 않음
- 이후 확장:
