...
RFC 1035는 도메인 네임 시스템(DNS)의 구현과 명세를 다루는 핵심 기술 문서입니다.
 DNS는 서로 다른 호스트, 네트워크, 프로토콜 패밀리, 인터넷 및 행정 조직에서 자원을 명명할 수 있는 메커니즘을 제공합니다.
...
- BIT MAP: 가변 길이 비트맵.
 N번째 비트가 설정되면 해당 호스트가 프로토콜에서 포트 N의 서비스를 지원함
...
도메인 명 반복을 줄이기 위해 포인터 메커니즘 사용합니다.
 DNS 메시지에서 도메인 명은 라벨의 시퀀스로 표현되며, 각 라벨의 첫 2비트가 라벨 타입을 결정합니다.
...
RFC 1035는 기본적인 보안 메커니즘을 포함하지 않습니다.
 이후 확장:
