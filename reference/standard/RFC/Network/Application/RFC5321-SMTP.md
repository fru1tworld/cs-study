...
SMTP(Simple Mail Transfer Protocol): 메일을 신뢰성 있고 효율적으로 전송하는 목적

- 특정 전송 서브시스템과 독립적 → 신뢰성 있는 순서화된 데이터 스트림 채널만 필요
- TCP(Transmission Control Protocol)가 주요 전송 수단, 다른 전송 서비스도 사용 가능
...
이 문서 키워드 "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "MAY", "OPTIONAL": RFC 2119 기술대로 해석
...
1. 사용자가 메일 발송 요청 → SMTP 클라이언트가 SMTP 서버와 양방향 전송 채널 설정
2. SMTP 클라이언트가 명령 생성 및 서버 응답 처리
3. 연결 설정 후 클라이언트가 MAIL 명령으로 발신자 지정, RCPT 명령으로 수신자 지정
4. 수신자 협상 완료 → DATA 명령으로 메시지 전송
...
메일을 처음 생성하고 SMTP 시스템에 주입하는 시스템

#### 2.3.2 Relay (중계)

메일을 다른 SMTP 서버로 전달하는 중간 시스템

- Received 헤더 추가 외에는 메시지 수정 안 함

#### 2.3.3 Gateway (게이트웨이)

서로 다른 메일 시스템 간에 메시지를 변환하는 시스템

- 메시지 형식 변환 필요 가능

#### 2.3.4 Final Delivery (최종 배달)

메시지를 최종 목적지(사용자 메일박스)에 전달하는 시스템

### 2.4 책임 이전

> 중요: SMTP 서버가 메시지에 대해 성공 응답(250)을 보내면 → 그 메시지를 배달하거나 배달 실패를 적절히 보고할 책임을 정식으로 수락한 것
...
- 대소문자 구분: local-part는 반드시 대소문자를 구분(MUST)
...
SMTP 명령어: 클라이언트가 서버에 보내는 텍스트 기반 명령
...
- EHLO: 확장 지원 (ESMTP) → 현대 사용 권장
- HELO: 확장 미지원 (원본 SMTP) → 폴백용
...
목적: 메일 트랜잭션을 시작하고 발신자(reverse-path) 지정
...
- `SIZE=n` (SIZE 확장): 메시지 크기 (바이트)
- `BODY=8BITMIME` (8BITMIME 확장): 8비트 본문
- `BODY=BINARYMIME` (BINARYMIME 확장): 바이너리 본문
- `RET=FULL|HDRS` (DSN 확장): 반환 유형
- `ENVID=value` (DSN 확장): 봉투 ID
- `AUTH=<addr>` (AUTH 확장): 인증된 발신자
- `SMTPUTF8` (SMTPUTF8 확장): UTF-8 주소

#### 4.1.3 RCPT TO (수신자 지정)

목적: 수신자(forward-path) 지정

- 여러 수신자에게 보내려면 여러 번 사용
...
- `NOTIFY=value` (DSN 확장): 알림 조건 (NEVER, SUCCESS, FAILURE, DELAY)
- `ORCPT=type;addr` (DSN 확장): 원본 수신자
...
- 본문 줄 시작이 점(.)인 경우 → 추가 점을 앞에 붙임(MUST)
...
보안 참고: 많은 서버가 보안상의 이유로 VRFY 비활성화
...
보안 참고: VRFY와 마찬가지로 보안상 이유로 비활성화되는 경우 많음
...
RFC 3463에서 정의된 향상된 상태 코드: 더 세부적인 정보 제공
...
- Date: 메시지 작성 날짜/시간 (예: `Date: Mon, 1 Jan 2024 12:00:00 +0000`)
- From: 메시지 작성자 (예: `From: Alice <alice@example.com>`)

#### 4.4.3 권장 헤더 필드

- Message-ID: 고유 메시지 식별자 (예: `Message-ID: <unique-id@example.com>`)
- To: 기본 수신자 (예: `To: Bob <bob@example.org>`)
- Subject: 메시지 제목 (예: `Subject: Meeting Tomorrow`)
...
SMTP 클라이언트가 목적지 도메인의 MX(Mail Exchanger) 레코드를 조회 → 메일 서버 찾기
...
SMTP 클라이언트와 서버는 적절한 타임아웃을 구현해야 함(MUST)
...
오픈 릴레이 문제:
- 오픈 릴레이는 스팸 전송에 악용 가능
- 서버는 인가된 사용자/도메인만 릴레이해야 함(MUST)
...
- CRAM-MD5: 챌린지-응답 → 보안 수준 중간
- SCRAM-SHA-256: 현대적 챌린지-응답 → 보안 수준 높음
- XOAUTH2: OAuth 2.0 기반 → 보안 수준 높음
...
- 주소 위조: From 헤더 조작 → 대응: SPF·DKIM·DMARC
- 오픈 릴레이 악용: 제3자 서버 이용 → 대응: 릴레이 제어
- 메일 폭탄: 대량 메일 발송 → 대응: 속도 제한
- 디렉토리 수확: 유효 주소 수집 → 대응: VRFY 비활성화
...
- smtp: 포트 25 → 메일 전송 (MTA 간)
- submission: 포트 587 → 메일 제출 (MUA에서 MSA로)
- submissions: 포트 465 → 암시적 TLS 메일 제출

### 8.2 SMTP 확장 레지스트리

IANA는 SMTP 서비스 확장 레지스트리를 유지

- SIZE (RFC 1870): 메시지 크기 선언
- 8BITMIME (RFC 6152): 8비트 MIME 전송
- STARTTLS (RFC 3207): TLS 암호화
- AUTH (RFC 4954): 인증
- PIPELINING (RFC 2920): 명령 파이프라이닝
- DSN (RFC 3461): 배달 상태 알림
- ENHANCEDSTATUSCODES (RFC 2034): 향상된 상태 코드
- SMTPUTF8 (RFC 6531): 국제화 이메일
- CHUNKING (RFC 3030): 청크 전송
- BINARYMIME (RFC 3030): 바이너리 MIME
...
- 연결 거부: 원인 - 방화벽·포트 차단 → 해결 - 포트 587 또는 465 사용
- 인증 실패: 원인 - 잘못된 자격 증명 → 해결 - 사용자명/비밀번호 확인
- 550 오류: 원인 - 수신자 불명 → 해결 - 주소 확인
- 552 오류: 원인 - 메일박스 가득 참 → 해결 - 수신자에게 연락
- 554 오류: 원인 - 스팸으로 거부됨 → 해결 - SPF/DKIM/DMARC 설정 확인
- TLS 오류: 원인 - 인증서 문제 → 해결 - 인증서 유효성 확인
