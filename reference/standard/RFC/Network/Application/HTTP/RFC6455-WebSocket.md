...
RFC 6455는 2011년 12월에 발행되었으며, WebSocket 프로토콜 을 정의합니다.
 이 프로토콜은 브라우저 기반 애플리케이션이 여러 HTTP 연결에 의존하지 않고 서버와 양방향 통신 을 할 수 있는 메커니즘을 제공합니다.
...
### 배경
기존 웹 애플리케이션에서 실시간 업데이트가 필요한 경우 여러 HTTP 연결을 사용하는 폴링(polling) 방식에 의존했습니다.
 WebSocket은 이러한 비효율적인 우회 방법을 대체하여 진정한 실시간 통신 패턴을 가능하게 합니다.
...
연결 상태 확인(keepalive) 용도로 사용됩니다.
 수신자는 Pong으로 응답해야 합니다.

### 5.3 Pong 프레임 (0xA)

Ping에 대한 응답입니다.
 수신된 Ping 데이터를 그대로 에코합니다.
...
> RFC 6455 §7.4.1에 따라 1004, 1005, 1006, 1015는 애플리케이션 내부/API 용도로만 예약된 값이며, Close 프레임을 통해 와이어 상에 실제로 전송되어서는 안 됩니다(MUST NOT).
...
비브라우저 클라이언트가 WebSocket 서버에 접근할 때는 Origin 기반 보호를 우회합니다.
 서버는 이러한 연결에 대해 대체 인증 메커니즘 을 구현해야 합니다.
