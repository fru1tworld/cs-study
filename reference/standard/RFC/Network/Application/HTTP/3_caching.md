...
HTTP 캐시와 캐시 동작을 제어하거나 캐시 가능한 응답 메시지를 나타내는 관련 헤더 필드 정의
...
사용자 A·B·C가 공유 캐시(Proxy/CDN)를 거쳐 원본 서버에 접근하는 구조 → 공유 캐시가 여러 사용자 요청을 동시에 처리 가능
...
`Vary` 헤더 필드: 응답이 요청의 어떤 부분에 따라 달라지는지 나타냄

동작 방식:

- 클라이언트 A(`Accept-Language: ko`)가 `GET /page` 요청 → 캐시가 서버에 전달 → 서버가 `Vary: Accept-Language` 헤더와 한국어 콘텐츠로 응답
- 클라이언트 B(`Accept-Language: en`)가 동일 요청 → 캐시는 `Accept-Language` 값이 다름을 인지 → 저장된 응답 사용 불가 → 서버에 새 요청
...
원본 서버가 명시적 만료 시간을 항상 제공하지 않음 → 캐시는 다른 헤더 필드 값(예: `Last-Modified` 시간)으로 합리적인 만료 시간을 추정하는 휴리스틱 만료 시간을 할당 가능(MAY)
...
캐시된 응답을 직접 제공할 수 없는 경우 → 조건부 요청으로 응답의 유효성 확인 가능
...
안전하지 않은 요청 메서드(PUT, POST, DELETE 등)는 원본 서버의 상태를 변경할 가능성 있음 → 캐시는 이를 감지해 콘텐츠를 최신 상태로 유지 가능
...
캐시가 원본 서버에서 성공적으로 검증하지 않고는 저장된 응답을 사용해서는 안 됨을 나타냄
...
응답이 오래지면 캐시가 원본 서버에서 성공적으로 검증하지 않고는 후속 요청에 응답을 사용해서는 안 됨을 나타냄
...
RFC 7234는 2022년 6월 RFC 9111로 폐기됨. 주요 변경사항:
