...
HTTP/2는 HTTP/1.1의 성능 문제를 해결하기 위해 설계된 프로토콜입니다.
 HTTP의 의미론(메서드, 상태 코드, 헤더)은 유지하면서 전송 계층을 완전히 재설계 했습니다.
...
HPACK은 별도 문서인 RFC 7541에서 정의되며, RFC 7540은 이를 참조해 사용한다.
...
HTTP/3는 QUIC 전송 프로토콜 위에서 HTTP 시맨틱을 매핑하는 프로토콜입니다.
 HTTP/1.1 및 HTTP/2와 동일한 HTTP 시맨틱(메서드, 상태 코드, 헤더 필드)을 사용하면서 TCP 대신 QUIC을 전송 계층으로 사용합니다.
...
| MAX_PUSH_ID | 0x0d | 최대 허용 서버 푸시 ID 제어 | 제어 |
