...
AsyncAPI Specification은 이벤트 기반(Event-Driven)·비동기(Asynchronous) API를 기술하기 위한 표준 인터페이스 정의 언어임. OpenAPI Specification이 RESTful HTTP API의 표준 명세 역할을 수행하는 것과 동일하게, AsyncAPI는 메시지 브로커(Kafka, RabbitMQ 등)·메시징 프로토콜(MQTT, AMQP, WebSocket 등)을 통한 비동기 통신의 모든 측면을 선언적으로 기술함. JSON 또는 YAML 형식으로 작성하며, 메시지의 구조·채널(토픽/큐)·서버 정보·프로토콜 바인딩 등을 정의 가능.

### 왜 만들어졌는가

마이크로서비스 아키텍처와 이벤트 기반 아키텍처(EDA)의 확산으로 비동기 API 사용이 급증했으나, 체계적으로 기술할 수 있는 표준 명세가 부재했음. 다음과 같은 문제가 대두됨.

- 비동기 API 명세의 부재: OpenAPI는 HTTP 요청-응답 패턴만 대상 → Kafka 토픽이나 MQTT 메시지 같은 비동기 통신을 기술할 방법이 없었음. 이벤트의 구조와 채널 정보가 코드 주석이나 위키 문서에 산재 → 서비스 간 통합 시 혼란 발생
- 이벤트 카탈로그 필요: 수십 개의 마이크로서비스가 수백 개의 이벤트를 주고받는 환경에서, 어떤 서비스가 어떤 이벤트를 발행(publish)하고 구독(subscribe)하는지 파악이 어려웠음
- 코드 생성·자동화의 한계: 동기 API는 OpenAPI를 통해 클라이언트 SDK와 서버 스텁을 자동 생성 가능했으나, 비동기 API에는 동등한 자동화 파이프라인이 존재하지 않았음
- 프로토콜 다양성에 대한 추상화 부재: Kafka·RabbitMQ·MQTT·NATS·WebSocket 등 다양한 메시징 프로토콜이 사용되지만, 통합적으로 기술할 수 있는 프로토콜 중립적 명세가 없었음
- 계약 기반 개발의 부재: 동기 API에서는 OpenAPI 명세 기반으로 계약 테스트 수행이 가능했으나, 비동기 API에서는 메시지 형식 변경이 하위 호환성을 깨뜨리는지 검증 불가였음

AsyncAPI는 이러한 문제 해결을 위해 이벤트 기반 아키텍처의 단일 진실 공급원(Single Source of Truth) 역할을 수행하도록 설계됨.

### 핵심 철학

- 프로토콜 중립성: 특정 메시징 프로토콜에 종속되지 않으며, Kafka·MQTT·AMQP·WebSocket·NATS 등 다양한 프로토콜을 동일한 형식으로 기술함
- OpenAPI와의 유사성: OpenAPI에 익숙한 개발자가 최소한의 학습으로 AsyncAPI를 사용할 수 있도록 유사한 구조와 용어를 채택함
- 사람과 기계 모두를 위한 설계: 개발자가 읽을 수 있으면서도 도구가 파싱할 수 있는 형식임
- 선언적 기술: 메시지가 "무엇"인지를 기술하며, "어떻게" 처리하는지는 다루지 않음
- 재사용성: `components`를 통해 메시지·스키마·채널 등을 재사용 가능하게 정의함
- 확장성: 프로토콜별 바인딩(bindings)을 통해 각 프로토콜 고유의 설정을 기술 가능
...
AsyncAPI는 Fran Mendez가 2017년 자신이 근무하던 스타트업 Hitch에서 시작한 프로젝트임. 당시 Hitch는 IoT 장치와 마이크로서비스 간의 비동기 통신을 구축 중이었는데, 이벤트 기반 API를 문서화할 표준적인 방법이 없어 어려움을 겪음. Fran Mendez는 OpenAPI(당시 Swagger)가 REST API에 제공하는 것과 동일한 수준의 명세를 비동기 API에도 제공해야 한다고 판단 → OpenAPI의 구조를 참고하여 AsyncAPI의 첫 버전을 설계함.

### 초기 발전 (2017-2019)

- 2017년: AsyncAPI 1.0.0 공개. OpenAPI 2.0(Swagger) 구조를 기반으로 비동기 API를 기술할 수 있는 최초의 체계적 명세임. `topics`라는 개념으로 메시지 채널을 정의함
- 2018년: AsyncAPI 1.1.0·1.2.0 발표. 프로토콜 바인딩 개념이 도입되기 시작했으며, 커뮤니티 피드백을 반영하여 명세가 정교해짐
- 2019년 9월: AsyncAPI 2.0.0 발표. 대대적인 구조 개편이 이루어짐 → `topics`가 `channels`로 변경, `servers` 개념 도입, 프로토콜 바인딩 공식 체계화. OpenAPI 3.0의 구조적 영향을 받아 `components` 도입됨

### AsyncAPI Initiative와 Linux Foundation (2019-2021)

- 2019년: AsyncAPI 프로젝트가 단일 개인 프로젝트에서 커뮤니티 주도 프로젝트로 전환 시작. AsyncAPI Initiative가 조직되어 거버넌스 체계가 수립됨
- 2020년: AsyncAPI 2.1.0·2.2.0 발표되며 지속 개선. Generator·Parser 등 핵심 도구 개발
- 2021년 3월: AsyncAPI Initiative가 Linux Foundation에 합류 → 벤더 중립적인 오픈 거버넌스 체계를 확보하고, 기업 채택을 위한 신뢰성을 높임

### 성숙기 (2021-현재)

- 2021년: AsyncAPI 2.3.0·2.4.0 발표. 메시지 트레이트(traits)·서버 변수 등 고급 기능 추가
- 2022년: AsyncAPI 2.5.0·2.6.0 발표. 명세의 안정성과 도구 생태계가 성숙해짐
- 2023년 9월: AsyncAPI 3.0.0 발표. 가장 큰 규모의 구조적 변경을 포함하며, `operations`가 `channels`에서 분리되고 `send`/`receive` 개념이 도입됨. Request-Reply 패턴 지원도 추가됨
- 2024년: AsyncAPI 3.0 생태계의 도구 지원이 확대됨. Modelina·Studio 등이 3.0을 완전 지원하기 시작함
...
AsyncAPI 3.0 기준 문서의 구조와 각 구성 요소를 상세히 설명함. AsyncAPI 3.0은 2.x 대비 상당한 구조적 변경이 이루어졌으며, 특히 `operations`가 `channels`에서 분리된 것이 가장 큰 변화임.
...
AsyncAPI 3.0에서는 `channels`와 `operations`가 명확히 분리됨. 채널은 메시지가 전달되는 경로를 정의하고, 오퍼레이션은 해당 채널에서 수행되는 동작을 정의함.

### 3.2 Info Object

API에 대한 기본 메타데이터를 정의함.
...
메시지 브로커 또는 서버의 연결 정보를 정의함. OpenAPI의 `servers`와 유사하지만, 프로토콜(protocol) 필드가 핵심적인 차이임.
...
AsyncAPI 3.0에서 서버 객체의 URL이 `host`와 `pathname`으로 분리됨. 2.x에서는 `url` 하나로 통합되어 있었음.

### 3.4 Channels

채널(channel)은 메시지가 전달되는 경로를 정의함. Kafka에서는 토픽(topic), RabbitMQ에서는 큐(queue) 또는 익스체인지(exchange), MQTT에서는 토픽(topic)에 해당함.

AsyncAPI 3.0에서 채널은 오퍼레이션과 분리되어, 채널 자체는 메시지의 구조와 경로만을 정의함.
...
3.0에서의 주요 변경사항:
- 2.x에서는 `channels`가 `publish`/`subscribe` 오퍼레이션을 직접 포함했으나, 3.0에서는 채널이 메시지와 경로만 정의함
- `address` 필드가 도입되어 채널의 실제 주소(토픽명 등)를 명시함
- 채널 내에서 `messages`를 맵 형태로 정의함

### 3.5 Operations

오퍼레이션(operation)은 채널에서 수행되는 동작을 정의함. AsyncAPI 3.0에서 가장 큰 변화 중 하나로, 기존의 `publish`/`subscribe`가 `send`/`receive`로 대체됨.
...
- `send`: 이 애플리케이션이 해당 채널로 메시지를 보냄 (2.x의 `publish`에 대응, 관점에 따라 혼란이 있었음)
- `receive`: 이 애플리케이션이 해당 채널에서 메시지를 받음 (2.x의 `subscribe`에 대응, 관점에 따라 혼란이 있었음)

2.x에서는 `publish`/`subscribe`의 의미가 "애플리케이션 관점"인지 "채널 관점"인지 혼란이 있었음. 3.0에서는 항상 명세를 작성하는 애플리케이션의 관점에서 `send`/`receive`를 사용하여 모호함을 제거함.

Request-Reply 패턴 (3.0 신규):

AsyncAPI 3.0에서는 오퍼레이션에 `reply` 필드를 추가하여 요청-응답 패턴을 기술 가능함.
...
메시지(message)는 채널을 통해 전달되는 데이터의 구조를 정의함. 헤더·페이로드·상관관계 ID 등을 포함 가능.
...
재사용 가능한 객체들을 정의하는 컨테이너임. OpenAPI의 `components`와 동일한 역할을 수행함.
...
트레이트는 여러 메시지나 오퍼레이션에 공통으로 적용되는 속성을 정의하는 메커니즘임. 코드의 믹스인(mixin)과 유사한 개념으로, 중복을 줄이고 일관성을 유지하는 데 활용됨.
...
- 채널 정의
  - 1.x: `topics`
  - 2.x: `channels` (publish/subscribe 포함)
  - 3.0: `channels` (메시지/주소만 정의)
- 오퍼레이션
  - 1.x: `topics` 내에 포함
  - 2.x: `channels` 내 `publish`/`subscribe`
  - 3.0: 독립된 `operations` (send/receive)
- 서버 정의
  - 1.x: 제한적
  - 2.x: `servers` 객체 (URL 통합)
  - 3.0: `servers` 객체 (host/pathname 분리)
- 서버 URL
  - 1.x: -
  - 2.x: `url` 단일 필드
  - 3.0: `host` + `pathname` 분리
- 메시지 정의
  - 1.x: 인라인
  - 2.x: `components/messages` + 인라인
  - 3.0: `components/messages` + 채널 내 참조
- 프로토콜 바인딩
  - 1.x: 미지원
  - 2.x: `bindings` 지원
  - 3.0: `bindings` 지원 (확장됨)
...
- 보안
  - 1.x: 제한적
  - 2.x: `securitySchemes`
  - 3.0: `securitySchemes` (서버/오퍼레이션 레벨)
- 트레이트
  - 1.x: 미지원
  - 2.x: `messageTraits`, `operationTraits`
  - 3.0: `messageTraits`, `operationTraits`
- $ref
  - 1.x: 제한적
  - 2.x: 지원
  - 3.0: 지원 (확장됨)
...
2.x에서 `publish`와 `subscribe`의 의미가 혼란스러웠음. 예를 들어, 명세를 작성하는 애플리케이션이 Kafka 토픽에 메시지를 보내는 경우 이것이 `publish`인지(애플리케이션이 발행) 아니면 `subscribe`인지(다른 애플리케이션이 구독할 수 있도록) 모호했음.
...
- Request-Reply 패턴: `operations.{name}.reply` 필드로 요청-응답 패턴 기술 가능
- 채널 재사용 강화: 채널과 오퍼레이션이 분리됨으로써, 동일한 채널을 여러 오퍼레이션에서 참조 가능
- 서버-채널 연결: 채널이 특정 서버에만 연결됨을 명시 가능

### 4.3 마이그레이션 시 주의사항

2.x에서 3.0으로 마이그레이션할 때 주요 작업은 다음과 같음.

1. `channels` 내의 `publish`/`subscribe`를 별도의 `operations`로 이동
2. `publish`를 `action: send`로, `subscribe`를 `action: receive`로 변환
3. 서버 `url`을 `host`와 `pathname`으로 분리
4. 채널 키를 논리적 이름으로 변경하고, 실제 주소를 `address` 필드에 기입
5. 채널 내 메시지를 `messages` 맵 형태로 변환

AsyncAPI는 공식적으로 `@asyncapi/converter` 라이브러리를 제공하여 2.x 문서를 3.0으로 자동 변환할 수 있도록 지원함.
...
프로토콜 바인딩은 특정 메시징 프로토콜에 고유한 설정을 기술하기 위한 확장 메커니즘임. AsyncAPI의 핵심 명세는 프로토콜 중립적이지만, 실제 시스템에서는 프로토콜별 세부 설정이 필요함. 바인딩은 서버·채널·오퍼레이션·메시지 네 가지 레벨에서 정의 가능.
...
AsyncAPI에서 메시지는 다음과 같은 구성 요소로 이루어짐.

- `headers`: 메시지 헤더의 스키마. 메타데이터(상관관계 ID·이벤트 타입·타임스탬프 등)를 포함함
- `payload`: 메시지 본문의 스키마. 실제 비즈니스 데이터를 정의함
- `correlationId`: 메시지의 상관관계를 추적하기 위한 식별자. `location`으로 헤더 또는 페이로드 내 위치를 지정함
...
`location` 필드는 런타임 표현식을 사용하여 상관관계 ID의 위치를 지정함.
...
AsyncAPI는 JSON Schema 외에도 다양한 스키마 형식을 지원함.
...
AsyncAPI는 JSON Schema를 기반으로 스키마를 정의함. OpenAPI 3.1과 마찬가지로 JSON Schema Draft 2020-12의 키워드를 사용 가능.
...
AsyncAPI에서 지원하는 보안 스킴 유형은 다음과 같음.
...
- `userPassword`: 사용자명/비밀번호. 주요 사용 프로토콜 - MQTT, AMQP
- `apiKey`: API 키. 주요 사용 프로토콜 - HTTP, WebSocket
- `X509`: X.509 인증서. 주요 사용 프로토콜 - Kafka, MQTT (TLS)
...
마이크로서비스 간에 교환되는 이벤트의 구조·채널·프로토콜을 체계적으로 문서화함. 각 서비스가 발행하는 이벤트와 구독하는 이벤트를 명확히 정의 → 시스템 전체의 이벤트 흐름을 파악 가능.
...
조직 내 모든 이벤트를 중앙에서 관리하는 이벤트 카탈로그를 구축함. 각 서비스별 AsyncAPI 명세를 통합하면 전사적 이벤트 맵 작성 가능 → 새로운 서비스 개발 시 기존에 어떤 이벤트가 사용 가능한지 쉽게 파악 가능.

코드 생성

AsyncAPI 명세로부터 메시지 모델 클래스·프로듀서/컨슈머 코드·유효성 검증 로직을 자동 생성함. 서비스 간 메시지 형식의 일관성을 보장하고, 수작업으로 인한 오류를 줄임.

계약 기반 개발 및 테스트

명세 기반으로 이벤트 생산자와 소비자 간의 계약을 정의하고, 계약 테스트를 수행함. 이벤트 스키마 변경 시 하위 호환성을 자동 검증함.

모킹 및 시뮬레이션

실제 메시지 브로커 없이도 명세 기반의 가상 이벤트를 생성하여 서비스 개발·테스트 진행 가능.

IoT 시스템 설계

MQTT 기반 IoT 장치의 메시지 프로토콜을 정의함. 센서 데이터·명령·상태 보고 등의 메시지 형식을 표준화함.

### 6.2 핵심 도구

AsyncAPI Generator

AsyncAPI 명세로부터 다양한 출력물을 생성하는 코드 생성 도구임.
...
브라우저 기반의 AsyncAPI 명세 편집기임. 실시간 문법 검증·시각화·미리보기를 제공하며, OpenAPI의 Swagger Editor에 해당하는 도구임.
...
AsyncAPI(또는 JSON Schema, OpenAPI) 명세로부터 데이터 모델 클래스를 생성하는 도구임.
...
AsyncAPI 명세를 파싱하고 검증하는 라이브러리임. JavaScript/TypeScript로 구현되어 있으며, 다른 도구들의 기반이 됨.
...
AsyncAPI 명세를 기반으로 이벤트 기반 애플리케이션 프레임워크를 제공함. 명세를 작성하면 이벤트 핸들러만 구현하면 되는 서버리스 스타일의 개발 경험을 제공함.
...
Stoplight의 Spectral 도구는 OpenAPI뿐 아니라 AsyncAPI 명세의 린팅도 지원함. 커스텀 규칙을 정의하여 조직 차원의 명세 작성 표준을 강제 가능.
...
이벤트 기반 아키텍처의 표준화
- 비동기 API를 위한 사실상의 유일한 표준 명세임
- 마이크로서비스 간 이벤트 통신을 체계적으로 문서화 가능
- 이벤트 카탈로그를 통해 조직 전체의 이벤트 흐름 파악 가능

프로토콜 중립적 추상화
- Kafka·MQTT·AMQP·WebSocket·NATS 등 다양한 프로토콜을 동일한 형식으로 기술함
- 프로토콜 변경 시에도 핵심 명세 구조는 유지 → 바인딩만 수정하면 됨
- 멀티프로토콜 환경에서 통합적인 문서화 가능

OpenAPI와의 유사성
- OpenAPI에 익숙한 개발자가 빠르게 학습 가능
- JSON Schema를 공유 → 데이터 모델을 동기/비동기 API 간에 재사용 가능
- 동일한 도구 생태계(Spectral 등) 활용 가능

자동화 지원
- 코드 생성·문서 생성·모델 생성 등 다양한 자동화 가능
- CI/CD 파이프라인에 명세 검증 통합 가능
- 스키마 레지스트리와 연동하여 스키마 호환성 자동 검증 가능

계약 기반 개발
- 이벤트 생산자와 소비자 간의 명확한 계약 수립 가능
- 스키마 변경 시 하위 호환성 자동 검증 가능
- 이벤트 스키마의 버전 관리가 용이함

커뮤니티와 거버넌스
- Linux Foundation 산하의 오픈 거버넌스 체계를 갖추고 있어 장기적 안정성이 보장됨
- 활발한 오픈 소스 커뮤니티가 도구 생태계를 지속적으로 발전시킴

### 7.2 단점

생태계 성숙도
- OpenAPI 대비 도구 생태계가 아직 성숙하지 않음
- 일부 도구가 AsyncAPI 3.0을 완전히 지원하지 않는 경우가 있음
- 프레임워크별 통합이 OpenAPI만큼 풍부하지 않음

학습 곡선
- 비동기 메시징 자체에 대한 이해가 선행되어야 함
- 프로토콜 바인딩의 세부 설정은 해당 프로토콜에 대한 깊은 지식을 요구함
- 3.0의 구조 변경으로 기존 2.x 사용자도 재학습 필요

명세 복잡도
- 여러 프로토콜·다수의 채널·복잡한 바인딩을 포함하면 명세가 매우 방대해짐
- 프로토콜 바인딩 명세가 별도로 관리되어 전체 명세의 파악이 어려울 수 있음
- 트레이트·컴포넌트·참조가 얽히면 명세의 가독성이 떨어짐

실행 시점의 간극
- AsyncAPI는 메시지의 구조를 기술하지만, 메시지의 순서·타이밍·실패 처리 등 런타임 동작은 기술하지 않음
- 이벤트의 인과 관계(causality)나 사가(saga) 패턴 같은 복잡한 워크플로를 표현하기 어려움
- 명세만으로는 시스템의 전체적인 이벤트 흐름을 파악하기 어려울 수 있음

버전 호환성 관리
- 2.x에서 3.0으로의 마이그레이션에 상당한 노력이 필요함
- 프로토콜 바인딩 명세의 버전이 독립적으로 관리되어 호환성 매트릭스가 복잡함
- 도구마다 지원하는 AsyncAPI 버전이 다를 수 있음

채택률
- OpenAPI 대비 산업 전반의 채택률이 낮음
- 많은 조직이 아직 비동기 API를 비공식적인 방법으로 문서화함
- 상용 도구의 AsyncAPI 지원이 제한적인 경우가 있음

---

## 8. 실제 예시

아래는 Kafka 기반 전자상거래 주문 이벤트 시스템의 완전한 AsyncAPI 3.0 명세임. 주문 생성·주문 확인·주문 취소·결제 처리·재고 변경·배송 상태 업데이트 등의 이벤트를 포함함.
...
AsyncAPI는 OpenAPI의 자매 명세(sister specification)로 불림. AsyncAPI의 설계 철학·구조·용어는 의도적으로 OpenAPI와 유사하게 설계되었으며, 다음과 같은 이점을 제공함.

- 학습 곡선 최소화: OpenAPI에 익숙한 개발자가 빠르게 AsyncAPI 습득 가능
- 스키마 공유: JSON Schema 기반의 데이터 모델을 동기/비동기 API 간에 재사용 가능
- 도구 통합: Spectral·Microcks 등의 도구가 OpenAPI와 AsyncAPI를 모두 지원함
- 통합 API 카탈로그: 동기(REST) API와 비동기(이벤트) API를 하나의 카탈로그에서 관리 가능

두 명세의 주요 차이:

- 대상: OpenAPI - RESTful HTTP API / AsyncAPI - 이벤트 기반 비동기 API
- 통신 패턴: OpenAPI - 요청-응답 (동기) / AsyncAPI - 발행-구독, 이벤트 스트림 (비동기)
- 핵심 개념: OpenAPI - Paths, Operations, Responses / AsyncAPI - Channels, Operations, Messages
- 프로토콜: OpenAPI - HTTP/HTTPS / AsyncAPI - Kafka·MQTT·AMQP·WebSocket·NATS 등
- 방향성: OpenAPI - 단방향 (클라이언트 -> 서버 -> 클라이언트) / AsyncAPI - 다방향 (생산자 -> 브로커 -> 소비자)
- 거버넌스: OpenAPI - OpenAPI Initiative (Linux Foundation) / AsyncAPI - AsyncAPI Initiative (Linux Foundation)

### 9.2 CloudEvents와의 관계

CloudEvents는 CNCF(Cloud Native Computing Foundation) 산하의 이벤트 데이터 형식 표준임. AsyncAPI와 CloudEvents는 상호 보완적인 관계에 있음.

- 범위: AsyncAPI - API 명세 (채널·오퍼레이션·메시지 구조) / CloudEvents - 이벤트 데이터 형식 (메타데이터 표준)
- 초점: AsyncAPI - "어떤 채널에서 어떤 메시지를 교환하는가" / CloudEvents - "이벤트의 메타데이터를 어떻게 표현하는가"
- 관계: AsyncAPI 메시지의 페이로드가 CloudEvents 형식을 따를 수 있음
...
AsyncAPI는 직접적으로 CNCF 프로젝트는 아니지만, CNCF 생태계와 밀접한 관련이 있음.

- CloudEvents: CNCF 졸업 프로젝트로, AsyncAPI 메시지 형식으로 사용 가능
- Kubernetes: 클라우드 네이티브 환경에서 이벤트 기반 마이크로서비스 배포에 사용됨
- NATS: CNCF 인큐베이팅 프로젝트로, AsyncAPI가 지원하는 프로토콜 중 하나임
- gRPC: CNCF 인큐베이팅 프로젝트로, 스트리밍 RPC 패턴을 AsyncAPI로 기술 가능
- Dapr: CNCF 인큐베이팅 프로젝트로, Pub/Sub 빌딩 블록의 메시지를 AsyncAPI로 문서화 가능

### 9.4 AsyncAPI Initiative

역할:
- Linux Foundation 산하의 오픈 거버넌스 조직으로, AsyncAPI Specification의 개발과 발전을 주도함
- 명세·도구·커뮤니티의 로드맵을 관리함
...
Postman·Solace·SAP·Salesforce·Slack·eBay·PayPal·LEGO 등이 AsyncAPI Initiative에 참여 중. 이들 기업은 자사의 이벤트 기반 시스템에 AsyncAPI를 채택하거나, 도구 개발에 기여하고 있음.
...
- AsyncAPI Conference: 연례 커뮤니티 컨퍼런스. 온라인으로 진행되며 명세의 발전 방향·사용 사례·도구 소개 등을 다룸
- Open Governance Meetings: 매주 진행되는 공개 미팅으로, 명세 변경 사항과 도구 개발 진행 상황을 논의함
- Mentorship Program: Google Summer of Code(GSoC) 등의 멘토십 프로그램에 참여하여 신규 기여자를 육성함
- AsyncAPI Bounty Program: 특정 기능 개발이나 버그 수정에 대해 보상을 제공하는 프로그램임

### 9.7 AsyncAPI의 미래

- 멀티프로토콜 지원 강화: 신규 메시징 프로토콜(Solace, Amazon EventBridge 등)에 대한 바인딩이 지속적으로 추가 중
- 도구 생태계 확장: IDE 플러그인·CI/CD 통합·모니터링 도구 등의 생태계가 확대 중
- AI/LLM 통합: AsyncAPI 명세를 기반으로 AI가 이벤트 기반 시스템을 이해하고 코드를 생성하는 패턴이 부상 중
- OpenAPI와의 수렴: 동기/비동기 API를 통합적으로 기술할 수 있는 방향에 대한 논의가 진행 중
- 스키마 레지스트리 통합: Confluent Schema Registry·AWS Glue Schema Registry 등과의 네이티브 통합이 강화 중
- 이벤트 카탈로그 표준화: 조직 전체의 이벤트를 관리하는 카탈로그 도구와의 통합이 발전 중
