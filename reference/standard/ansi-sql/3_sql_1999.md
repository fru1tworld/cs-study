...
SQL:1999(공식명: ISO/IEC 9075:1999)는 SQL 표준의 세 번째 주요 개정판으로, SQL3라고도 불림. SQL-92에서 관계형 모델의 기본 기능을 확립한 이후, 객체-관계형(Object-Relational) 기능을 대폭 추가하며 SQL의 표현력을 크게 확장함.
...
SQL-92의 3단계 적합성(Entry/Intermediate/Full)을 폐지하고, Core SQL + 선택적 기능 패키지 방식으로 변경함.
...
- BEFORE ROW: DML 전 행별 실행, NEW 값 수정 가능, 데이터 검증·기본값 설정 용도
- AFTER ROW: DML 후 행별 실행, 데이터 수정 불가, 감사 로그·파생 데이터 갱신 용도
- BEFORE STATEMENT: DML 전 문장별 실행, 데이터 수정 불가, 사전 조건 검증 용도
- AFTER STATEMENT: DML 후 문장별 실행, 데이터 수정 불가, 집계 데이터 갱신 용도
...
기존 타입을 기반으로 새로운 타입을 생성하며, 원본 타입과 호환되지 않음.
...
- 논리형: SQL-92에 없음 → SQL:1999에서 BOOLEAN 추가
- 대용량: SQL-92에 없음 → SQL:1999에서 BLOB·CLOB 추가
- 컬렉션: SQL-92에 없음 → SQL:1999에서 ARRAY 추가
- 구조체: SQL-92에 없음 → SQL:1999에서 ROW 추가
- 사용자 정의: SQL-92에 없음 → SQL:1999에서 DISTINCT TYPE·STRUCTURED TYPE 추가

---

### 9. PSM (Persistent Stored Modules)

SQL:1999 Part 4에서 저장 프로시저와 함수를 표준화함.
...
SQL:1999에서 정규 표현식 기반 패턴 매칭을 도입함.
...
- `_`: 임의의 한 문자 (예: `'A_C'` → ABC, AXC)
- `%`: 0개 이상 임의 문자 (예: `'A%'` → A, AB, ABC)
- `[abc]`: 문자 집합 (예: `'[ABC]%'` → A로 시작)
- `[a-z]`: 문자 범위 (예: `'[0-9]%'` → 숫자로 시작)
- `[^abc]`: 부정 문자 집합 (예: `'[^0-9]%'` → 숫자 외로 시작)
- `|`: 선택 (OR) (예: `'A|B'` → A 또는 B)
- `{n}`: 정확히 n번 반복 (예: `'[0-9]{3}'` → 숫자 3자리)
- `{n,m}`: n~m번 반복 (예: `'[0-9]{2,4}'` → 숫자 2~4자리)
- `*`: 0번 이상 반복 (예: `'A*B'` → B, AB, AAB)
- `+`: 1번 이상 반복 (예: `'A+B'` → AB, AAB)
- `?`: 0 또는 1번 (예: `'A?B'` → B, AB)
...
재귀 쿼리는 CTE의 WITH RECURSIVE를 활용한 고급 기능임.
...
SQL/MED(Management of External Data)는 외부 데이터 소스에 대한 접근을 표준화한 파트임.
...
SQL/OLB(Object Language Bindings)는 SQL을 Java 프로그램에 내장하기 위한 표준임.
...
SQL:1999에서 미지원, 이후 표준에서 도입된 기능:

- 윈도우 함수 (ROW_NUMBER·RANK 등) → SQL:2003에서 도입
- MERGE 문 → SQL:2003에서 도입
- 시퀀스 (CREATE SEQUENCE) → SQL:2003에서 도입
- XML 지원 → SQL:2003에서 도입
- TRUNCATE TABLE → SQL:2008에서 도입
- OFFSET/FETCH → SQL:2008에서 도입
- 시간적 데이터베이스 (Temporal) → SQL:2011에서 도입
- JSON 지원 → SQL:2016에서 도입
- 그래프 쿼리 → SQL:2023에서 도입

---

### 18. 영향과 의의

- 객체-관계형 패러다임: SQL에 객체지향 개념(UDT·메서드·상속)을 도입 → 복잡한 데이터 모델링 가능
- CTE와 재귀 쿼리: 계층적 데이터 처리의 표준 방법 제공, 현재 거의 모든 DBMS가 지원함
- 트리거 표준화: 데이터 무결성과 비즈니스 로직의 데이터베이스 레벨 구현을 표준화함
- PSM 표준화: 저장 프로시저/함수의 표준 문법을 정의 → 이식성 향상
- OLAP 기능: ROLLUP·CUBE·GROUPING SETS는 비즈니스 인텔리전스 분야의 핵심 기능이 됨
- 적합성 모델 개선: Core + 패키지 모델은 이후 모든 SQL 표준에서 채택됨
- SIMILAR TO: SQL 내에서 정규 표현식 패턴 매칭의 기초를 확립함

SQL:1999는 SQL을 단순한 데이터 조회 언어에서 본격적인 프로그래밍 언어로 발전시킨 전환점임.

---

> 참고: SQL:1999 표준의 공식 문서는 ISO/IEC 9075:1999 시리즈로 발행됨.
