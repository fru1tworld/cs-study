...
SQL-92(공식명: ISO/IEC 9075:1992)는 1992년 ISO·IEC가 공식 발표한 SQL 표준의 두 번째 주요 개정판. SQL2라고도 부르며, SQL-86/SQL-89 대비 기능이 대폭 확장됨. 약 600페이지 분량으로 SQL-89(약 120페이지)의 5배 이상.
...
SQL-89는 기본적인 데이터 조작·정의 기능만 제공했으며, 다음과 같은 한계가 있었음.
...
SQL-92는 이러한 한계를 극복하고 관계형 데이터베이스의 실질적인 표준을 확립하기 위해 설계됨.

---

### 3. 적합성 수준

SQL-92는 구현의 현실성을 고려하여 세 가지 적합성 수준(Conformance Level)을 정의함.

#### 수준 비교

- Entry (입문)
  - 복잡도: 낮음
  - 포함 기능: SQL-89 + 약간의 개선. 대부분의 DBMS가 이 수준 충족
- Intermediate (중간)
  - 복잡도: 중간
  - 포함 기능: Entry + JOIN 구문·CASE·CAST·도메인·문자열 함수·동적 SQL 등
- Full (전체)
  - 복잡도: 높음
  - 포함 기능: Intermediate + 스크롤 커서·ASSERTION·BIT 타입·서브쿼리 확장 등
...
SQL-89에서는 테이블 생성 후 구조 변경이 표준에 포함되지 않았으나, SQL-92에서 `ALTER TABLE`이 공식 도입됨.
...
스키마는 데이터베이스 객체의 논리적 그룹. SQL-92에서 처음 표준화됨.
...
도메인은 재사용 가능한 컬럼 정의로, 데이터 타입과 제약사항을 묶어서 정의 가능.
...
어서션은 데이터베이스 전체에 적용되는 제약사항으로, 여러 테이블에 걸친 비즈니스 규칙을 정의 가능.
...
SQL-92는 SQL-89 대비 데이터 타입을 대폭 확장함.
...
SQL-92에서 가장 중요한 변경 중 하나는 명시적 JOIN 구문의 도입.
...
- INNER JOIN: 왼쪽 불일치 미포함, 오른쪽 불일치 미포함, 조건은 ON / USING
- LEFT OUTER JOIN: 왼쪽 불일치 포함, 오른쪽 불일치 미포함, 조건은 ON / USING
- RIGHT OUTER JOIN: 왼쪽 불일치 미포함, 오른쪽 불일치 포함, 조건은 ON / USING
- FULL OUTER JOIN: 왼쪽 불일치 포함, 오른쪽 불일치 포함, 조건은 ON / USING
- CROSS JOIN: 조건 없음 (카테시안 곱)
- NATURAL JOIN: 왼쪽 불일치 미포함, 오른쪽 불일치 미포함, 자동 (동명 컬럼)
...
- UNION: 중복 제거, 합집합
- UNION ALL: 중복 유지, 합집합 (전체)
- INTERSECT: 중복 제거, 교집합
- INTERSECT ALL: 중복 유지, 교집합 (전체)
- EXCEPT: 중복 제거, 차집합
- EXCEPT ALL: 중복 유지, 차집합 (전체)
...
- CHAR / VARCHAR에서 변환: 모든 타입으로 변환 가능
- INTEGER / DECIMAL / FLOAT에서 변환: CHAR·VARCHAR·INTEGER·DECIMAL·FLOAT로 가능, DATE·TIME·TIMESTAMP로는 불가
- DATE에서 변환: CHAR·VARCHAR·DATE·TIMESTAMP로 가능, 숫자형·TIME으로는 불가
- TIME에서 변환: CHAR·VARCHAR·TIME·TIMESTAMP로 가능, 숫자형·DATE로는 불가
- TIMESTAMP에서 변환: CHAR·VARCHAR·DATE·TIME·TIMESTAMP로 가능, 숫자형으로는 불가
...
- SUBSTRING: 부분 문자열 추출. 예) `SUBSTRING('Hello' FROM 2 FOR 3)` → `'ell'`
- UPPER: 대문자 변환. 예) `UPPER('hello')` → `'HELLO'`
- LOWER: 소문자 변환. 예) `LOWER('HELLO')` → `'hello'`
- TRIM: 공백/문자 제거. 예) `TRIM('  hi  ')` → `'hi'`
- POSITION: 위치 찾기. 예) `POSITION('l' IN 'hello')` → `3`
- CHAR_LENGTH: 문자 수. 예) `CHAR_LENGTH('hello')` → `5`
- OCTET_LENGTH: 바이트 수. 예) `OCTET_LENGTH('hello')` → `5`
- `||`: 연결. 예) `'a' || 'b'` → `'ab'`
...
- READ UNCOMMITTED: Dirty Read 가능, Non-Repeatable Read 가능, Phantom Read 가능
- READ COMMITTED: Dirty Read 불가, Non-Repeatable Read 가능, Phantom Read 가능
- REPEATABLE READ: Dirty Read 불가, Non-Repeatable Read 불가, Phantom Read 가능
- SERIALIZABLE: Dirty Read 불가, Non-Repeatable Read 불가, Phantom Read 불가
...
- SQL의 실질적 표준 확립: 현재까지도 대부분의 SQL 교재·교육이 SQL-92를 기반으로 함
- JOIN 구문의 혁신: 명시적 JOIN 구문 도입 → SQL의 가독성·유지보수성 크게 향상
- CASE 표현식의 도입: 조건부 논리를 SQL 내에서 직접 처리 가능
- 날짜/시간 타입 표준화: 이식성이 크게 향상됨
- INFORMATION_SCHEMA: 메타데이터 조회가 표준화됨
- 트랜잭션 격리 수준 표준화: 동시성 제어에 대한 표준 모델 제공

SQL-92에서 정의된 기본 문법과 개념은 이후 모든 SQL 표준의 토대가 되었으며, SQL:1999에서 객체-관계형 기능이 추가되면서 더욱 확장됨.

---

> 참고: SQL-92 표준의 공식 문서는 ISO/IEC 9075:1992로, ISO에서 구매 가능.
