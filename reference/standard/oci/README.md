...
OCI(Open Container Initiative) - 컨테이너 기술에 대한 개방형 업계 표준을 만들기 위해 설립된 오픈소스 프로젝트. Linux Foundation 산하의 거버넌스 구조 아래 운영되며, 컨테이너의 런타임(Runtime)·이미지(Image)·배포(Distribution)에 대한 명세(Specification)를 정의함.

### 왜 만들어졌는가

컨테이너 기술이 급속히 성장 → Docker가 사실상의 표준(de facto standard)으로 자리잡음 → 다음과 같은 문제 발생.

- 벤더 종속(Vendor Lock-in): Docker 하나의 구현에 전체 생태계가 의존하는 구조 → 건강하지 않음
- 상호운용성 부재: 서로 다른 컨테이너 런타임, 이미지 포맷, 레지스트리 간 호환성 미보장
- 경쟁 표준의 등장: CoreOS가 자체적으로 appc(App Container) 명세 발표 → 표준 분열 위험
- 업계 합의의 필요성: 클라우드 네이티브 생태계 성숙 → 특정 벤더에 종속되지 않는 개방형 표준 필수

OCI는 이러한 문제 해결 목적 → 컨테이너 기술의 핵심 요소들을 표준화해 이식성(Portability)과 상호운용성(Interoperability) 보장이 목표.
...
- 2013년: Docker가 PyCon에서 처음 공개됨. Linux 커널의 cgroups와 namespace 기술을 활용한 컨테이너화를 쉽게 사용할 수 있게 만들어, 개발 및 배포 방식에 혁명을 일으킴.
- 2013~2015년: Docker는 폭발적으로 성장 → 컨테이너 이미지 포맷과 런타임의 사실상 표준이 됨. 동시에 Docker라는 단일 벤더에 대한 의존도도 높아짐.

### CoreOS와 appc (App Container Specification)

- 2014년 12월: CoreOS가 Docker의 독점적 지위에 대한 우려 표명 → appc(App Container Specification)라는 대안 컨테이너 명세와 이를 구현한 rkt(로켓) 런타임 발표.
- appc는 컨테이너 이미지 포맷(ACI, App Container Image), 런타임 환경, 이미지 검증 및 서명 등을 정의함.
- 이로 인해 컨테이너 업계에 표준 분열(Fragmentation) 위기 발생. 두 개의 경쟁 표준 존재 → 생태계 전체 혼란 우려.

### OCI 설립 (2015년)

- 2015년 6월: Docker, CoreOS, Google, Microsoft, Amazon, IBM, Red Hat 등 주요 기업들이 모여 Open Container Initiative(OCI)를 Linux Foundation 산하에 설립.
- Docker는 자사의 컨테이너 런타임인 libcontainer의 코드를 기부해 runc 프로젝트의 기반으로 삼음. 이것이 OCI 런타임 명세의 참조 구현(Reference Implementation)이 됨.
- CoreOS는 appc의 아이디어와 경험을 OCI에 반영하기로 합의하고, 별도 표준 추진을 중단함. (이후 rkt도 CNCF에 기부되었다가 2020년에 아카이브됨.)
- 설립 당시 목표는 크게 두 가지.
...
OCI는 컨테이너 생태계의 핵심 영역을 세 가지 명세로 표준화함.
...
- 목적: 컨테이너의 설정(configuration), 실행 환경(execution environment), 생명주기(lifecycle) 표준화
- 핵심 내용: 컨테이너가 어떤 파일시스템 번들(filesystem bundle)을 사용하고, 어떤 격리 기술(namespace, cgroups 등)을 적용하며, 어떤 상태 전이를 거치는지 정의
- 참조 구현: runc

### 3.2 Image Specification (image-spec)

- GitHub: `opencontainers/image-spec`
- 목적: 컨테이너 이미지의 포맷 표준화 → 어떤 빌드 도구로 만들든, 어떤 런타임에서든 실행 가능하게 함
- 핵심 내용: 이미지 매니페스트(Manifest), 이미지 인덱스(Index), 파일시스템 레이어(Layer), 이미지 설정(Configuration)의 구조를 정의
- 설계 원칙: 콘텐츠 주소 지정 방식(Content-addressable) - 모든 구성 요소는 SHA-256 해시로 식별됨

### 3.3 Distribution Specification (distribution-spec)

- GitHub: `opencontainers/distribution-spec`
- 목적: 컨테이너 이미지를 레지스트리(Registry)에 저장하고 가져오는(push/pull) API 표준화
- 핵심 내용: HTTP 기반의 RESTful API를 정의하며, Docker Registry HTTP API V2를 기반으로 발전시킴
- 배경: 원래 Docker Distribution 프로젝트에서 비롯 → OCI가 이를 공식 명세로 채택

---

## 4. Runtime Specification 상세

Runtime Spec은 컨테이너를 실행하기 위한 표준을 정의함. 핵심은 파일시스템 번들(Filesystem Bundle)과 config.json.

### 4.1 파일시스템 번들 (Filesystem Bundle)

OCI 런타임이 컨테이너를 생성하려면, 다음으로 구성된 파일시스템 번들 필요.
...
`config.json`은 컨테이너의 실행 환경을 완전히 기술하는 파일. 주요 필드는 다음과 같음.
...
OCI Runtime Spec은 컨테이너의 상태 전이를 다음과 같이 정의함.
...
OCI 호환 런타임은 다음의 연산을 반드시 지원해야 함.

- `create <container-id> <bundle-path>`: 파일시스템 번들로부터 컨테이너 생성. `config.json`을 읽어 환경을 설정하되, 사용자 프로세스는 아직 시작하지 않음.
- `start <container-id>`: 생성된 컨테이너의 사용자 프로세스 시작
- `state <container-id>`: 컨테이너의 현재 상태를 JSON으로 반환
- `kill <container-id> <signal>`: 컨테이너 프로세스에 시그널 전송
- `delete <container-id>`: 중지된 컨테이너의 리소스 정리 및 삭제

#### Hooks (생명주기 후크)

컨테이너 생명주기의 특정 시점에 사용자 정의 스크립트 실행 가능.
...
네임스페이스는 프로세스가 볼 수 있는 시스템 리소스의 범위를 격리함.

- Mount (설정 값: `mount`): 파일시스템 마운트 포인트 격리. 컨테이너 자체의 마운트 트리를 가짐.
- PID (설정 값: `pid`): 프로세스 ID 격리. 컨테이너 내부에서 PID 1부터 시작.
- Network (설정 값: `network`): 네트워크 인터페이스, IP 주소, 라우팅 테이블, 포트 등을 격리.
- IPC (설정 값: `ipc`): System V IPC, POSIX 메시지 큐 등 프로세스 간 통신을 격리.
- UTS (설정 값: `uts`): 호스트명과 도메인명을 격리.
- User (설정 값: `user`): UID/GID 매핑. 컨테이너 내부의 root를 호스트의 비특권 사용자로 매핑 가능.
- Cgroup (설정 값: `cgroup`): cgroup 루트 디렉터리를 격리.

#### Cgroups (Control Groups)

cgroups는 프로세스 그룹의 리소스 사용을 제한하고 모니터링함.
...
- `memory.reservation`: 소프트 리밋. 시스템 메모리 부족 시 적용됨.
- `cpu.shares`: CPU 상대적 가중치 (다른 컨테이너와의 비율)
- `cpu.quota` / `cpu.period`: CPU 시간 할당. quota/period 비율만큼 CPU 사용.
- `pids.limit`: 최대 프로세스 수. 포크 밤(Fork Bomb) 방지 용도.
- `blockIO`: 블록 I/O 대역폭 제한

참고: Linux 커널 4.5 이후 cgroups v2가 도입됨. OCI Runtime Spec도 cgroups v2를 지원함. cgroups v2는 통합된 계층 구조와 더 세밀한 리소스 제어를 제공함.

#### Seccomp (Secure Computing Mode)

seccomp는 컨테이너 프로세스가 호출할 수 있는 시스템 콜을 제한 → 공격 표면 감소.
...
- `SCMP_ACT_KILL`: 해당 시스템 콜 호출 시 프로세스 종료
- `SCMP_ACT_TRAP`: SIGSYS 시그널 발생
- `SCMP_ACT_LOG`: 허용하되 로그 기록

기본적으로 Docker/containerd는 약 300여 개의 시스템 콜 중 위험한 약 40~50개를 차단하는 기본 seccomp 프로파일을 제공함.

---

## 5. Image Specification 상세

OCI Image Spec은 컨테이너 이미지의 포맷과 구조를 정의함. 핵심 원칙은 콘텐츠 주소 지정(Content Addressability) - 모든 구성 요소가 그 내용의 SHA-256 해시로 고유하게 식별됨.
...
매니페스트는 하나의 컨테이너 이미지를 구성하는 요소들(설정 + 레이어들)을 기술하는 JSON 문서.
...
- `layers`: 파일시스템 레이어 배열. 순서가 중요하며, 아래에서 위로 겹쳐짐.
- `annotations`: 선택적 메타데이터 (키-값 쌍)

### 5.3 이미지 인덱스 (Image Index / Manifest List)

이미지 인덱스는 멀티 플랫폼 이미지를 지원하기 위한 상위 수준의 매니페스트. 하나의 이미지 태그가 여러 아키텍처/OS 조합을 가리킬 수 있게 해줌.
...
클라이언트가 `docker pull nginx:latest`를 실행하면, 레지스트리는 먼저 이미지 인덱스를 반환함. 클라이언트는 자신의 플랫폼(예: `linux/amd64`)에 맞는 매니페스트를 선택해 해당 레이어들을 다운로드함.

### 5.4 레이어 (Layer) 구조

레이어는 컨테이너 이미지의 파일시스템 변경분을 나타내는 tar 아카이브.
...
레이어는 유니온 파일시스템(Union Filesystem, 예: OverlayFS)을 통해 겹쳐짐. 상위 레이어에서 하위 레이어의 파일을 "삭제"하려면 whiteout 파일 사용.
...
이미지 설정은 컨테이너를 실행할 때 적용할 기본 파라미터와 이미지 메타데이터를 담음.
...
Distribution Spec은 OCI 이미지를 레지스트리에 저장하고 가져오는 HTTP API를 정의함.

### 6.1 레지스트리 API 엔드포인트

OCI Distribution Spec이 정의하는 핵심 API 엔드포인트는 다음과 같음.
...
여기서 `<name>`은 리포지토리 이름(예: `library/nginx`), `<reference>`는 태그(예: `latest`) 또는 다이제스트(예: `sha256:abc123...`).

### 6.2 Pull 프로토콜 (이미지 가져오기)

이미지를 가져오는 과정은 다음과 같음.
...
- Monolithic Upload: `POST` 한 번으로 전체 Blob을 업로드. 작은 Blob에 적합.
- Chunked Upload: `POST`로 세션을 시작하고, `PATCH`로 청크를 나누어 전송한 뒤, `PUT`으로 완료. 대용량 레이어에 적합.
...
OCI Distribution Spec은 Docker Registry의 인증 체계를 따름.
...
Distribution Spec v1.1에서 추가된 Referrers API는 특정 매니페스트를 참조하는 아티팩트들(서명, SBOM, 취약점 스캔 결과 등)을 조회할 수 있게 함.
...
- 개발: Docker가 libcontainer를 기부해 시작, 현재 OCI 산하 프로젝트
- 특징: Go로 작성된 저수준(low-level) 컨테이너 런타임. Linux namespace, cgroups, seccomp 등을 직접 다뤄 컨테이너를 생성하고 실행함.
- 사용처: containerd, CRI-O 등 고수준 런타임의 기본 하위 런타임으로 사용됨.
...
- 특징: C로 작성된 경량 OCI 런타임. runc보다 빠르고 메모리 사용량 적음.
- 사용처: Podman, CRI-O에서 runc 대안으로 사용 가능

#### youki

- 특징: Rust로 작성된 OCI 런타임. 메모리 안전성 강조.

#### gVisor (runsc)

- 특징: Google이 개발한 샌드박스 런타임. 사용자 공간에서 Linux 커널 시스템 콜을 가로채어 추가적인 격리 계층을 제공함.
- 사용처: 멀티테넌트 환경에서 보안이 중요한 경우

#### Kata Containers

- 특징: 경량 가상 머신(Lightweight VM) 기반 런타임. 각 컨테이너가 전용 커널을 가짐.
...
- 특징: 이미지 관리, 컨테이너 생명주기, 스토리지, 네트워킹 등 전반적인 컨테이너 관리를 담당함. 하위 런타임으로 runc(또는 다른 OCI 런타임)를 호출함.
...
- 특징: Kubernetes CRI를 직접 구현하며, OCI 이미지와 런타임을 사용함. Docker나 containerd 없이도 Kubernetes에서 컨테이너를 실행할 수 있음.
...
- 특징:
  - 데몬이 없는(daemonless) 아키텍처. 각 명령이 독립적인 프로세스로 실행됨.
  - Rootless 컨테이너 기본 지원
  - OCI 이미지와 OCI 런타임을 직접 사용
  - Pod 개념 기본 지원 (Kubernetes Pod와 유사)
...
- 특징:
  - Dockerfile 없이도 스크립트로 이미지 빌드 가능
  - 데몬 불필요
  - OCI 이미지와 Docker 이미지 포맷 모두 생성 가능
  - CI/CD 파이프라인에 적합
...
- 특징: 레지스트리 간 이미지를 직접 복사할 수 있으며, 로컬에 이미지를 풀(pull)하지 않고도 레지스트리의 이미지를 검사할 수 있음.
...
- Docker는 OCI 설립의 주도적 역할을 했으며, libcontainer 코드를 기부해 runc의 기반을 마련함.
- Docker는 자사의 모놀리식 아키텍처를 분해해, containerd(고수준 런타임)와 runc(저수준 런타임)를 분리함.
- containerd는 CNCF에 기부되어 독립 프로젝트가 됨.
- Docker 이미지 포맷(Docker Image Manifest V2, Schema 2)은 OCI Image Spec의 직접적인 기반이 됨. 두 포맷은 거의 동일하며 상호 변환 가능.
...
- Kubernetes는 OCI 이미지를 표준으로 사용함. `kubectl`로 배포하는 모든 컨테이너 이미지는 OCI 또는 Docker 이미지 포맷을 따름.
- CRI를 통해 OCI 호환 런타임이면 어떤 것이든 Kubernetes에서 사용 가능.
...
- 벤더 중립성: 특정 벤더에 종속되지 않는 개방형 표준. Docker, Podman, containerd 등 어떤 도구를 사용하든 동일한 이미지를 실행할 수 있음.
- 이식성: OCI 호환 이미지는 어디서든 실행 가능. 로컬 개발 환경, CI/CD, 클라우드 등 환경을 가리지 않음.
- 상호운용성: 서로 다른 빌드 도구(Docker, Buildah, kaniko), 런타임(runc, crun, kata), 레지스트리(Docker Hub, GHCR, ECR) 간 호환 보장.
- 생태계 다양성: 표준이 있으므로, 각 레이어에서 다양한 구현체가 경쟁하고 혁신 가능. 보안(gVisor, Kata)·성능(crun)·편의성(Podman) 등 다양한 가치를 추구하는 도구가 공존함.
- 확장성: Image Spec v1.1의 아티팩트 지원, 커스텀 미디어 타입 등으로 컨테이너 이미지 외의 아티팩트(Helm 차트, Wasm 모듈, SBOM 등)도 OCI 레지스트리에 저장 가능.
- 보안: 콘텐츠 주소 지정(Content Addressable)으로 이미지 무결성 보장. Referrers API로 서명, SBOM 등의 보안 메타데이터를 체계적으로 관리 가능.

### 단점

- 표준화의 느린 속도: 합의 기반 프로세스 → 새로운 기능의 표준화가 느림. 예를 들어 Distribution Spec은 수년간 작업됨.
- 복잡성: 세 가지 명세가 별도로 존재하며, 각각의 버전이 다르게 진행됨. 전체 스택을 이해하려면 상당한 학습 필요.
- 최소 공통분모 문제: 다양한 벤더의 합의가 필요 → 혁신적이거나 실험적인 기능이 명세에 포함되기 어려움.
- 사실상 표준과의 간극: Docker 이미지 포맷이 여전히 가장 널리 사용되며, OCI 이미지와 Docker 이미지가 사실상 거의 동일함에도 두 가지가 공존함.
- 플랫폼 의존성: Runtime Spec은 Linux에 가장 최적화되어 있으며, Windows나 다른 OS 지원은 상대적으로 제한적.
- 구현의 일관성 부재: 명세를 해석하는 방식이 구현체마다 미묘하게 다를 수 있어, "OCI 호환"이라 해도 완벽한 호환을 기대하기 어려운 경우 있음.
...
OCI Image Index를 활용하면, 하나의 이미지 태그로 x86 서버, ARM 기반 AWS Graviton, Raspberry Pi 등 다양한 플랫폼을 모두 지원 가능.

### 10.2 OCI 레지스트리를 범용 아티팩트 저장소로 활용

OCI Image Spec v1.1부터 컨테이너 이미지가 아닌 범용 아티팩트도 저장 가능.
...
OCI Referrers API 덕분에 이미지와 서명, SBOM, 취약점 스캔 결과 등을 체계적으로 연결하고 관리 가능.
...
OCI Runtime Spec 덕분에 워크로드의 특성에 따라 런타임 선택 가능. 일반 워크로드는 runc, 보안이 중요한 워크로드는 gVisor나 Kata Containers를 사용하는 식.
...
Docker 데몬 없이 Buildah로 OCI 이미지를 빌드하고, Skopeo나 Buildah로 직접 레지스트리에 푸시 가능. CI/CD 환경에서 Docker-in-Docker(DinD)의 복잡성과 보안 문제 회피 가능.
...
OCI 이미지 레이아웃은 파일시스템에 직접 저장할 수 있는 표준 구조를 정의 → 인터넷이 차단된 보안 환경에서도 이미지를 안전하게 전송 가능.
