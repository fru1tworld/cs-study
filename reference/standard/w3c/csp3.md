...
CSP(Content Security Policy) — 서버가 HTTP 응답 헤더로 "이 페이지가 어떤 출처의 리소스를, 어떤 방식으로 로드/실행할 수 있는지" 화이트리스트를 선언하는 브라우저 보안 메커니즘 → XSS·데이터 주입·클릭재킹 같은 공격 표면 축소.
Level 3 → Level 2 대비 nonce/hash 기반 스크립트 허용, `strict-dynamic`, Trusted Types 연계 등을 정교화.
...
기존 화이트리스트 방식(`script-src https://cdn.example.com`) → CDN 하나 손상 시 전체가 뚫림 · URL 목록 관리 비용도 큼.
`strict-dynamic` → 접근 방식 전환.
...
CSP3 → DOM XSS의 근본 원인인 "문자열을 그대로 innerHTML 등에 대입하는 패턴"을 차단하는 Trusted Types API와 연동.
...
정책 위반 시 브라우저가 보고서를 전송하도록 설정 가능.
...
레거시 `X-Frame-Options` 헤더를 대체 → 더 세밀한 출처 목록과 CSP 정책 하나로 통합 관리 가능.
