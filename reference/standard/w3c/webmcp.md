...
WebMCP: 사이트가 자신의 기능을 AI 에이전트(브라우저에서 동작하는 LLM 기반 어시스턴트 등)에게 구조화된 "도구(tool)"로 노출할 수 있게 하는 웹 플랫폼 API 제안. 이름에서 드러나듯 Anthropic의 [Model Context Protocol(MCP)](../mcp/specification.md) 개념을 브라우저 네이티브 API로 가져오려는 시도.

핵심 아이디어: 에이전트가 웹사이트를 사용할 때 지금처럼 DOM을 스크린 스크래핑하거나 좌표를 클릭하는 대신 → 사이트가 스스로 "내가 제공하는 기능은 이런 것들이다"라고 선언한 함수 목록을 에이전트가 직접 호출.
...
스펙 원문 vs 구현 불일치:
- WICG Explainer 원문: 이 API를 `partial interface Document { readonly attribute ModelContext modelContext; }`로 정의 → `document.modelContext`를 통해 접근하도록 규정
- Chrome의 현재 Origin Trial 구현: `navigator.modelContext` 사용
- 아직 초안 단계 → 스펙 텍스트와 실제 구현이 어긋나 있는 상태, 어느 한쪽이 틀렸다기보다 앞으로 조율될 여지가 있는 부분
- 이 문서는 개발자가 실제로 마주치는 Chrome 구현 기준으로 `navigator.modelContext` 사용
...
- 신뢰성
  - DOM 기반 자동화의 한계: 레이아웃 변경·셀렉터 변경에 취약
  - WebMCP가 노리는 개선: 페이지가 명시한 안정적인 함수 인터페이스 사용
- 성능
  - DOM 기반 자동화의 한계: 스크린샷·접근성 트리 분석 등 비용이 큼
  - WebMCP가 노리는 개선: 함수 호출 한 번으로 처리
- 의도 파악
  - DOM 기반 자동화의 한계: 좌표·텍스트만으로 버튼의 진짜 의미를 추론해야 함
...
즉 WebMCP는 MCP의 "도구 노출" 철학을 웹 콘텐츠 보안 모델(Same-Origin·사용자 동의·권한 프롬프트) 안에서 재구현하려는 시도.
...
- 목표: 좌표·DOM 기반 자동화의 취약성 제거 → 사이트가 통제 가능한 안정적 인터페이스 제공
