...
- **required 필드를 추가하지 말 것**: 하위 호환을 깨뜨립니다. 새 required 필드는 해당 값을 제공하지 못하는 기존 코드를 망가뜨립니다.
...
- **기본값을 바꾸지 말 것**: 기본값에 의존하던 시스템의 동작을 예기치 않게 변경합니다.
- **텍스트 형식 메시지를 데이터 교환에 쓰지 말 것**: 텍스트 형식은 사람이 읽기 위한 용도입니다. 시스템 간 교환에는 바이너리 형식을 사용합니다.
...
- **well-known/공통 타입을 사용할 것**: `Duration`, `Timestamp`, `FieldMask` 같은 표준 타입을 활용해 일관성과 상호운용성을 높이고 커스텀 코드를 줄입니다.
...
- [ ] 시각/기간/필드마스크 등은 well-known types 활용
- [ ] 데이터 교환은 바이너리 형식 사용
