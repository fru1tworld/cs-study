...
`buf breaking`은 현재 스키마를 이전 버전과 비교하여 클라이언트·서버·생성 코드를 깨뜨릴 수 있는 변경을 검출함. 예를 들어 필드 타입을 `int32`에서 `string`으로 바꾸면 wire 타입이 달라져 wire 호환성이 깨짐.
...
CI에서는 보통 머지 대상 브랜치(`main`)와 비교함.

---

## 3. 호환성 카테고리 (FILE / PACKAGE / WIRE_JSON / WIRE)

호환성 규칙은 가장 엄격한 것부터 가장 관대한 것까지 4개의 중첩된 카테고리로 구성됨.
...
핵심 관계: 더 엄격한 카테고리를 통과하면 더 관대한 카테고리도 자동으로 통과함. 즉 `FILE`을 통과하면 `PACKAGE`, `WIRE_JSON`, `WIRE`도 통과함.

카테고리 선택은 소비자가 코드 생성을 필요로 하는지(FILE/PACKAGE), 직렬화 호환성만 중요한지(WIRE/WIRE_JSON)에 따라 결정함.

---

## 4. 설정 (use / except / ignore)

`buf.yaml`의 `breaking` 섹션에서 규칙을 조정함.
...
v2에서 모듈별로 다른 정책을 적용하려면 `modules[].breaking`에 작성함 → 이 설정은 워크스페이스 수준 설정을 완전히 대체함.
