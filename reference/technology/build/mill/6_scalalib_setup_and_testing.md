...
Mill은 Scala 프로젝트를 구성하는 방식을 세 가지 제공 → 프로젝트 복잡도에 따라 선택하거나 섞어 쓸 수 있음.

- 선언적(YAML): `build.mill.yaml`, `package.mill.yaml` 사용 → 의존성·버전 설정만 필요한 단순한 모듈에 적합
- 단일 파일 스크립트: `Foo.scala`(헤더에 `//| ...`) 사용 → 스크립트 하나로 빌드 설정과 코드를 함께 관리
- 프로그래매틱(build.mill): `build.mill`(Scala 코드) 사용 → trait 상속, 커스텀 task 등 복잡한 로직이 필요한 빌드에 적합

세 방식은 서로 완전히 배타적이지 않음. 선언적 모듈이 `build.mill`에 정의된 프로그래매틱 모듈에 `moduleDeps`로 의존 가능 · YAML의 `extends:` 항목에 `mill-build/src/`에 정의한 커스텀 Scala trait을 지정 가능.
...
SBT의 `src/main/scala`, `src/test/scala`와 달리 Mill 기본 레이아웃은 소스를 `src/`, 테스트를 별도 하위 모듈인 `test/`에 배치. `out/`은 모든 task의 캐시된 산출물이 저장되는 디렉터리 → 소스 변경이 없으면 재실행되지 않음.

---

### 3. 다중 모듈 프로젝트와 moduleDeps

각 하위 디렉터리가 하나의 모듈이 됨 → `moduleDeps`로 모듈 간 의존 관계를 선언.
...
build.mill 방식에서는 공통 설정을 trait으로 뽑아 재사용하는 패턴이 일반적.
...
테스트는 별도 하위 모듈(`ScalaTests` 상속)로 정의 · `testFramework`로 사용할 테스트 프레임워크를 지정.
...
여러 Scala 버전을 동시에 빌드해야 할 때 `Cross`와 `CrossScalaModule`을 사용.
...
- `src-2-12-20/`: Scala 2.12.20 정확히 일치할 때만 적용

Cross 모듈 간 의존은 현재 크로스 값을 넘겨 참조.
...
선언적(YAML) 모듈에서 Cross 모듈을 `moduleDeps`로 참조할 때는 Mill의 task 쿼리 문법을 그대로 쓰되, YAML 배열 문법과 겹치지 않도록 따옴표로 감싸야 함.
...
기존 SBT 프로젝트의 표준 디렉터리 구조(`src/main/scala`, `src/test/scala`)를 그대로 유지하려면 `ScalaModule` 대신 `SbtModule`을 상속.
...
Cross 버전 조합에는 `CrossSbtModule` / `CrossSbtTests`를 사용 → 이 경우 `src/main/scala-2.12/`, `src/main/scala-2.13/`처럼 버전별 소스 디렉터리를 SBT 규칙대로 배치.
...
`sources`/`resources`의 경로는 상대 경로면 해당 모듈의 `moduleDir` 기준, `//src/`처럼 슬래시 두 개로 시작하면 워크스페이스 루트 기준으로 해석.

---

### 9. 컴파일러 플러그인 설정

Scala 컴파일러 플러그인은 특정 Scala 전체 버전(예: `2.13.16`)에 맞춰 발행 → 일반 `::` 대신 `:::` 구분자를 사용해 아티팩트를 지정.
...
- `compileMvnDeps`: 컴파일 시점에만 필요한 의존성(런타임 클래스패스에는 미포함)
...
기본 소스/리소스 목록에 디렉터리를 추가하려면 `super`를 호출해 이어붙임.
...
빌드 시점에 소스 코드를 생성해야 하면 `generatedSources`를 오버라이드. 결과는 `Task.dest`(해당 task 전용 출력 디렉터리)에 파일을 쓰고 `PathRef`로 감싸 반환.
...
테스트에서 클래스패스에 없는 별도 파일에 접근해야 하면 `forkEnv`로 경로를 명시적으로 넘김.
...
관련 task: `docJar`(문서를 JAR로 패키징) · `scalaDocGenerated`(패키징 없이 디렉터리 형태). Scala 3에서는 `docResources()`를 정적 사이트의 루트로 취급하며 그 안의 `_docs/`, `_blog/` 디렉터리를 탐색. 소스 코드 링크를 문서에 붙이려면 다음처럼 지정.
...
여러 모듈의 문서를 하나로 합치려면 `UnidocModule`을 추가로 상속.
...
`console` task에서만 별도로 적용할 scalac 옵션을 지정 가능(예: `-Xfatal-warnings` 제외).
...
Ammonite REPL을 쓰려면 버전을 명시. Ammonite는 Scala 컴파일러 내부 API에 의존하고 컴파일러가 이진 호환성을 보장하지 않음 → 반드시 정확한 버전을 맞춰야 함.
...
기본적으로 Mill은 변경된 소스만 재컴파일하는 Zinc 증분 컴파일을 사용. 필요하면 끌 수 있음.
...
나이틀리 저장소를 `JvmWorkerModule`과 모듈 양쪽에 추가해야 함.
...
`mill-build/src/`에 정의한 Scala trait을 YAML의 `extends:`에서 참조 가능.
...
- `PathRef`: 파일/폴더의 내용을 참조값으로 감싸는 개념 → 내용이 바뀌면 하위 task 캐시를 무효화
...
`ScalaModule`은 의존성을 `mvnDeps`(일반 의존성) · `compileMvnDeps`(컴파일 전용) · `runMvnDeps`(런타임 전용)로 구분해 선언. Mill 설정은 `.mill.yaml` 형태로도, `build.mill` Scala DSL로도 작성 가능.
...
- `runMvnDeps`: 런타임에만 필요한 의존성(리플렉션이나 classpath scanning으로 로드되는 구현체 등). `compile` 태스크의 classpath에는 미포함 → `run`/`assembly` 시점에 추가됨

### 2. 컴파일 타임 vs 런타임 의존성

- `runMvnDeps`
  - 전이성(transitive): 전이적
  - 특징: `run`/`assembly` 생성 시 업스트림 모듈의 런타임 의존성까지 함께 포함
- `compileMvnDeps`
  - 전이성(transitive): 비전이적
  - 특징: 업스트림 의존성에서 자동으로 집계되지 않음 → 필요한 각 모듈에서 직접 선언하거나 공통 trait을 상속해 전달해야 함

`compileMvnDeps`로 선언한 의존성은 Maven/Ivy로 퍼블리시할 때 `provided` scope로 변환됨.

### 3. 저장소(repositories) 설정

기본적으로 Mill은 Coursier를 통해 Maven Central에서 의존성을 해석. 추가 저장소가 필요하면 `repositories`를 선언.
...
Scala 컴파일러 나이틀리 빌드 등 특수 저장소도 동일하게 추가.
...
Coursier 미러(mirror) 설정 파일 위치는 OS별로 다름.
...
파일시스템에 이미 존재하는 JAR을 직접 classpath에 넣고 싶을 때 `unmanagedClasspath`를 사용. 다만 대부분의 경우 `mvnDeps`/`moduleDeps`로 해결 가능 → `unmanagedClasspath`는 좌표가 없는 사설 JAR 등 극히 드문 상황에만 사용.
...
`unmanagedClasspath`를 Task로 정의하면 빌드 시점에 JAR을 동적으로 다운로드해 사용 가능.
...
Task는 캐시됨 → JAR은 최초 1회만 다운로드되고 이후 빌드에서는 캐시된 결과가 재사용됨. `Task.offline`으로 오프라인 모드를 감지해 명시적으로 실패시킬 수 있음.

### 6. Scala 의존성 표기법

Scala는 2.13까지 major 버전 간 이진 호환을 보장하지 않음 → 배포되는 아티팩트 이름에 Scala major 버전이 접미사로 붙음(예: `foo_2.13`). Mill은 이를 콜론 개수로 구분해 선언.
...
Scala 3는 Scala 2.13과 같은 표준 라이브러리를 공유 → Scala 2.13용 라이브러리를 Scala 3 프로젝트에서 그대로 사용 가능. 다만 아티팩트 이름 매핑을 위해 `withDottyCompat`을 호출해야 함.
...
`::` 표기법은 ScalaJS 의존성 선언에도 그대로 쓰임. 대상 모듈이 각각 `ScalaJSModule` 또는 `ScalaNativeModule`을 상속해야 플랫폼별 아티팩트 접미사(`_sjs1_2.13` 등)가 올바르게 해석됨.

### 9. 의존성 최신화 (Scala Steward)

GitHub, GitLab, Bitbucket에 호스팅된 프로젝트는 Scala Steward를 연동해 `mvnDeps`에 선언된 라이브러리 버전을 자동으로 최신화하는 PR을 받을 수 있음.

---

### 10. 테스트 모듈 기본 구성

Mill에는 별도의 "test scope" 개념이 없음. 테스트 모듈은 그냥 하나의 일반 모듈 → `ScalaTests` trait과 원하는 `TestModule.*` 프레임워크 trait을 함께 상속해서 만듦.
...
멀티 테스트 슈트도 동일한 패턴으로 만들 수 있음. 예를 들어 `integration` 모듈이 `test` 모듈의 유틸리티를 재사용하려면 `moduleDeps`로 연결.
...
이 경우 의존 관계는 `root → test → integration` 형태가 되어 `integration`에서 `test/src`의 코드를 그대로 가져다 쓸 수 있음.
...
특정 클래스나 케이스만 고를 때는 프레임워크별 선택자 문법을 그대로 인자로 넘김(uTest 예시).
...
- uTest: TestModule trait `TestModule.Utest`, `com.lihaoyi::utest`
- JUnit 4: TestModule trait `TestModule.Junit4`, sbt/junit-interface
- JUnit 5: TestModule trait `TestModule.Junit5`, sbt/sbt-jupiter-interface
- TestNG: TestModule trait `TestModule.TestNg`
- MUnit: TestModule trait `TestModule.Munit`
- ScalaTest: TestModule trait `TestModule.ScalaTest`
- Specs2: TestModule trait `TestModule.Specs2`
- Weaver: TestModule trait `TestModule.Weaver`, weaver-test
- ZIO Test: TestModule trait `TestModule.ZioTest`, zio.dev

`testFramework`를 직접 문자열로 지정 가능(`TestModule.*` trait 없이) · trait을 상속하면 기본값이 채워짐.
...
테스트는 기본적으로 비어있는 `sandbox/` 폴더를 작업 디렉터리로 실행됨 → 리소스 파일에 접근하려면 `MILL_TEST_RESOURCE_DIR` 환경 변수를 사용. 특정 클래스/케이스만 지정하려면 인자를 그대로 이어서 전달.
...
Mill에는 test-scoped dependency가 없음. 테스트 모듈도 일반 모듈이므로 `mvnDeps`, `runMvnDeps`, `moduleDeps`를 그대로 사용해 다른 테스트 모듈에 의존 가능.
...
의존 그래프는 `baz → baz.test`, `qux → baz`, `qux.test → baz.test`가 되어 `qux.test`가 `baz.test`에 있는 공용 테스트 유틸을 그대로 가져다 쓸 수 있음.
...
`--jobs`로 지정한 개수만큼 테스트 클래스를 여러 JVM subprocess에 자동으로 분산 실행. 실행 결과는 워커별로 분리되어 기록됨.
...
`def testParallelism = false`로 끄면 단일 프로세스에서 순차 실행됨.

### 16. 테스트 그룹화 (testForkGrouping)

`testForkGrouping`으로 테스트 클래스를 그룹으로 나누어 각 그룹을 별도 JVM에서 실행 가능. 느리거나 부작용이 있는 테스트를 격리하거나, 그룹별 로그를 따로 관리할 때 유용.
...
이름 패턴으로 그룹을 나누고 병렬화도 함께 켤 수 있음.
...
Mill은 각 subprocess가 GroupX 또는 GroupY 중 한 그룹의 테스트만 독점적으로 실행하도록 보장 → 그룹 간 테스트가 섞이지 않음.

---

### 17. scalafmt 자동 포매팅

프로젝트 루트에 `.scalafmt.conf`를 두면 별도 플러그인 없이 scalafmt를 바로 사용 가능.
...
`.scalafmt.conf`에 `maxColumn` 같은 옵션을 추가하면 그 기준으로 재포매팅됨.
...
scalafix는 서드파티 `mill-scalafix` 플러그인을 통해 연동.
...
acyclic은 모듈 내부 파일 간 순환 의존성을 감지하는 Scala 컴파일러 플러그인.
...
해결 방법은 한쪽 방향으로만 의존하도록 코드를 리팩터링하는 것.

### 20. Spotless 통합 포매팅 & Ratchet

여러 언어/파일 그룹마다 서로 다른 포매팅 규칙이 필요할 때는 Spotless로 하나의 명령에 통합 가능.
...
`.spotless-formats.json`에서 파일 패턴별 포매팅 스텝을 지정.
...
Scala 버전별로 별도의 `.scalafmt.conf` / `.scalafmt3.conf`를 배치.
...
Ratchet은 Git 트리 간 차이가 있는 파일에 대해서만 점진적으로 포매팅을 적용하는 기능. 레거시 코드베이스에 새 포맷 규칙을 한 번에 적용하기 어려울 때, 변경된 파일부터 순차적으로 규칙을 도입 가능.
...
`mill-contrib-scoverage`를 통해 statement/branch 커버리지를 수집하고 최소 기준을 강제 가능.
...
라이브러리를 배포하는 프로젝트에서 이전 버전과의 바이너리 호환성을 검사하려면 Lightbend Migration Manager(MiMa)를 `mill-mima` 플러그인으로 연동. 자세한 설정은 https://github.com/lolgab/mill-mima 참고.
...
- 테스트
  - `./mill foo.test`: testForked로 테스트 실행(기본)
  - `./mill foo.test.testCached`: 캐시된 성공 결과 재사용
  - `./mill foo.test.testLocal`: 메인 프로세스 내 실행(빠름, 격리 낮음)
  - `./mill foo.test.testOnly <class>`: 특정 클래스만 실행
- scalafmt
  - `./mill mill.scalalib.scalafmt/`: 전체 포매팅
  - `./mill mill.scalalib.scalafmt/checkFormatAll`: 포맷 확인만
- scalafix
  - `./mill __.fix`: 전체 모듈 자동 수정
- Spotless
  - `./mill spotless` / `--check`: 다국어 통합 포매팅/확인
- Ratchet
