...
Mill의 표준 설치 방식: `./gradlew`, `./mvnw`와 같은 방식으로 `./mill` 부트스트랩 스크립트를 프로젝트 루트에 두는 것. 이 스크립트는 build 헤더의 `mill-version` 등을 읽어 프로젝트에 맞는 Mill 버전을 자동으로 판단하고, 로컬에 없으면 자동 다운로드함.
...
주의: PowerShell 5.1은 `.mill-version` 파일을 UTF-16LE로 생성해 Mill이 읽지 못함 → PowerShell 7 이상 사용 권장.

프로젝트 루트에 부트스트랩 스크립트를 커밋해두면 신규 기여자가 별도 설치 없이 바로 `./mill`을 실행할 수 있고, CI(Jenkins, GitHub Actions 등)에서도 버전 불일치 문제를 줄일 수 있음.
...
신규 프로젝트를 시작할 때는 `mill init`으로 예제 템플릿을 받아 시작하는 방식 권장.
...
특정 프로젝트에 속하지 않은 Java/Scala/Kotlin 스크립트 파일을 실행할 때 유용함. 다만 프로젝트 내에서는 개별 부트스트랩 스크립트 방식 권장.

#### 1.3 JVM 버전

Mill은 시스템에 설치된 `java`와 무관하게 기본으로 `zulu:25`를 사용함. build 헤더에서 명시적으로 지정 가능.

```scala
//| mill-jvm-version: temurin:24.0.1
```

- 이 값은 `build.mill` 평가와 `JavaModule`/`ScalaModule`/`KotlinModule` 실행 모두에 적용됨.
...
기본 Mill 런처는 GraalVM Native Image로, Java가 없는 환경에서도 빠르게 실행됨.

네이티브 런처 플랫폼별 지원 현황:

- Windows: Intel 지원, ARM 미지원
- Mac: Intel 지원, ARM 지원
- Linux: Intel 지원, ARM 지원

Windows-ARM은 업스트림 Graal Native Image 제약으로 미지원. 네이티브 미지원 플랫폼에서는 JVM 기반 런처를 사용하며, 버전에 `-jvm` 접미사를 붙이면 다른 플랫폼에서도 JVM 런처 선택 가능.

```
1.1.7-jvm
```

JVM 런처는 네이티브 대비 시작 오버헤드가 약 100ms 있고, Java 17 이상이 전역에 설치되어 있어야 함.
...
`~/.local/share/mill/completion/mill-completion.sh`에 스크립트를 생성하고 `~/.bash_profile`, `~/.zshrc`에 자동 등록됨. 이후 `./mill <TAB>`으로 모듈/태스크를, `./mill -<TAB>` 또는 `./mill --<TAB>`으로 플래그를 자동완성 가능. `mainargs` 기반 커스텀 커맨드의 파라미터도 자동완성 대상.
...
GitHub는 아직 `.mill` 파일 문법 강조를 기본 지원하지 않음 → `.gitattributes`에 다음 추가.
...
Mill은 IntelliJ, VSCode를 포함해 표준 Build Server Protocol(BSP)을 지원하는 모든 클라이언트와 연동됨. Eclipse는 BSP를 지원하지 않음 → 별도의 프로젝트 파일 생성 명령 제공.
...
- BSP 모드의 기본 출력 디렉터리는 `.bsp/mill-bsp-out`이며, 일반 빌드와 분리되어 컴파일이 두 번 일어남. `out/`을 공유하려면 `MILL_NO_SEPARATE_BSP_OUTPUT_DIR=1` 환경변수 설정.
- BSP 대신 XML 프로젝트 파일을 직접 생성하려면 아래 명령 사용:

```bash
./mill mill.idea/
```

`.idea/` 하위에 `scala_settings.xml`, `mill_modules/*.iml`, `libraries/*.xml`, `workspace.xml` 등이 생성됨. `build.mill` 변경 후에는 동일 명령 재실행.
...
`.settings/org.eclipse.core.resources.prefs`, `.settings/org.eclipse.jdt.core.prefs`, `.classpath`, `.project` 생성됨. IntelliJ와 달리 프로젝트 구조에 따라 여러 개의 Eclipse 프로젝트가 생성될 수 있고, 이들 간 의존관계가 있을 수 있음. 이후 변경 시 동일 명령 재실행 후 Eclipse에서 F5로 새로고침.

---

### 2. CLI 플래그

Mill 실행 시 플래그를 받을 수 있는 대상은 네 가지로 구분됨.

- 실행할 태스크: 예시 `foo.run --text hello` → 커맨드라인에서 직접 전달
- 태스크를 실행하는 JVM: 예시 `java -Xss10m -Xmx10G` → Compilation and Execution Flags 참고
- Mill 빌드 도구 프로세스: 예시 `./mill --jobs 10` → 커맨드라인, build 헤더의 `mill-opts`, `.mill-opts` 파일에서 설정
- Mill 빌드 도구를 실행하는 JVM: 예시 `java -Xss10m -Xmx10G` → `JAVA_OPTS`, build 헤더의 `mill-jvm-opts`, `.mill-jvm-opts` 파일에서 설정

플래그를 넘길 때 태스크/Mill 자체/JVM 중 어디로 향하는지 명확히 구분 필요.
...
일반적인 사용에는 필요하지 않은 내부/고급 플래그이며 자기 책임 하에 사용 필요.
...
`--interactive` / `-i` / `--no-server`: 현재는 모두 `--no-daemon`의 별칭. Mill 1.1.0부터 `ScalaModule#console`, `ScalaModule#repl` 같은 대화형 태스크에 별도 플래그가 필요 없으며, stdin/stdout 직결이 필요할 때 `--no-daemon`만으로 충분함.
...
`build.mill`과 그 안의 import까지 감시 대상. `runBackground`와 조합하면 코드 변경 시 이전 프로세스를 강제 종료하고 재시작함.

```bash
mill -w foo.runBackground
```

단, CTRL+C로 watch를 중단해도 `runBackground`가 띄운 서버는 백그라운드에 남음. 완전히 종료하려면 다음 명령 사용:
...
빌드 헤더의 `mill-opts`에 설정해두면 개발 머신과 CI 머신처럼 코어 수가 다른 환경에서도 일관되게 적용 가능.

---

### 3. 내장 명령어

Mill은 애플리케이션 코드 빌드와 직접 관련 없는 유틸리티 명령을 다수 내장하고 있음(`mill.main.MainModule` API 문서 참고). 아래 예시는 다음 `build.mill` 기준.
...
- `showNamed`: `show`와 동일하되 결과가 항상 `{태스크명: 값}` 형태의 JSON 딕셔너리로 통일
...
쿼리에 매칭되는 태스크를 나열만 하고 실행하지 않음. 어떤 태스크가 실행될지 미리 확인하거나(dry run), 어떤 모듈/태스크가 있는지 탐색할 때 사용.
...
와일드카드 `_`(단일 세그먼트), `__`(다중 세그먼트), 중괄호 열거 `{}`를 지원함.
...
`resolve`보다 상세하게 태스크의 소스 위치와 입력(inputs) 목록까지 보여줌. 빌드 구조를 디버깅하거나 대화식으로 탐색할 때 유용.
...
`resolve`와 동일한 와일드카드 쿼리 문법을 지원하지만, 보통은 한 번에 한 태스크씩 inspect하는 경우가 많음.

#### 3.3 show / showNamed

Mill은 기본적으로 태스크 평가 메타데이터를 출력하지 않음. `show`로 특정 태스크의 결과값 확인 가능.
...
JSON 출력이므로 `jq`나 외부 스크립트와 연동하기 쉬움. 여러 태스크를 조회하면 `{태스크명: 값}` 형태의 딕셔너리로 바뀜.
...
`showNamed`는 태스크가 하나든 여럿이든 항상 `{"태스크명": 결과}` 형태로 통일해 출력함. 프로그램에서 결과를 다룰 때 일관된 구조가 필요하면 `showNamed` 사용.
...
첫 번째 태스크와 두 번째 태스크 사이의 의존성 체인을 출력함. 데이터가 어떻게 한 태스크에서 다른 태스크로 흘러가는지, 혹은 `mill foo`가 왜 예상 못한 `bar`까지 실행하는지 파악할 때 유용.
...
가능한 경로가 여럿이면 그중 하나가 임의로 선택됨.

#### 3.5 plan

`mill foo`를 실행하면 평가될 태스크들과 순서를 실제 실행 없이 미리 보여줌.
...
병렬성 때문에 출력 순서는 가능한 위상 정렬 중 하나일 뿐이며, 실제 실행 순서는 스케줄링이나 소요 시간에 따라 달라질 수 있음.

#### 3.6 clean

이전에 실행한 태스크의 캐시된 출력을 삭제함.
...
`visualize`는 지정한 태스크 집합의 의존관계를 그래프로 그려줌.
...
`.svg`/`.png`로 눈으로 확인 가능하고, `.txt`/`.dot`/`.json`은 별도 도구로 후처리하기 쉬움.

```bash
./mill visualize __.compile
```

내부적으로 Transitive Reduction(전이 축약)을 적용해 중복 간선을 제거하고 그래프를 단순화함. 따라서 실제로는 존재하지만 표시되지 않는 중복 간선이 있을 수 있음.

`visualizePlan`은 `visualize`와 비슷하지만 직접 resolve된 태스크뿐 아니라 전체 빌드 플랜(간접 의존성 포함)까지 보여줌. 직접 resolve된 태스크는 실선, 의존성은 점선으로 표시됨.
...
Mill 예제 프로젝트나 Giter8 템플릿을 기반으로 프로젝트를 생성함.
...
빈 폴더에 부트스트랩 스크립트를 받은 뒤 `./mill init`으로 원하는 것에 가장 가까운 예제를 풀어 시작하는 방식이 일반적. 기존 Maven/Gradle 빌드를 Mill 설정으로 초기화하는 용도로도 사용 가능.
...
Mill CLI에서 태스크를 지정할 때, 실제로는 단순한 태스크 이름뿐 아니라 와일드카드·타입 필터 등을 포함한 태스크 셀렉터 쿼리를 받음.
...
각 모듈/태스크는 고유한 경로를 가지며, 경로는 `.`으로 구분된 세그먼트로 구성됨.
...
- 세그먼트는 괄호 `(`, `)`로 감쌀 수 있음. 타입 필터처럼 `.`을 포함하는 세그먼트를 쓸 때는 `.`이 경로 구분자로 오인되지 않도록 괄호로 감싸야 함.
...
일부 셸(bash 등)은 중괄호를 자체적으로 확장하므로, 쿼리를 따옴표로 감싸는 것이 안전함.
...
와일드카드는 콜론(`:`) 뒤에 타입 한정자를 붙여 특정 타입의 모듈만 매칭하도록 제한 가능. 타입은 이름, 또는 그 상위 타입/패키지를 `.`으로 이어붙여 지정함. `.`이 포함된 타입 한정자는 괄호로 감싸야 하며, `_root_` 패키지로 완전한 정규 타입명도 표현 가능.
...
타입 한정자 앞에 `^` 또는 `!`를 붙이면 해당 타입의 인스턴스가 "아닌" 것만 매칭함.

```bash
mill resolve __:^TestModule.jar
```

타입 필터는 여러 개를 이어붙일 수 있음.

```bash
mill resolve "__:JavaModule:^ScalaModule:^TestModule.jar"
```

타입 필터는 현재 모듈 선택에만 지원되며, 태스크의 결과 타입 기준 필터링은 지원하지 않음.

`+`로 새 셀렉터 시작:

열거/와일드카드/타입 필터는 Mill이 하나의 복합 셀렉터 경로로 파싱하며, 이후에 오는 파라미터는 resolve된 모든 태스크에 동일하게 적용됨. 반면 `+`는 완전히 새로운 셀렉터 경로를 시작하며, 서로 다른 파라미터 목록을 지정 가능. 이는 `JavaModule.run`처럼 자체 파라미터를 받는 command 태스크에서 중요.
...
`.super`를 사용하면 오버라이드되기 전의 태스크를 호출 가능. 이름이 겹치지 않으면 단독으로, 겹치면 클래스명을 붙여 명시적으로 선택함.
...
Mill은 `build.mill` / `build.mill.yaml` 본문이 컴파일되기 전에 적용되어야 하는 설정을 파일 최상단 헤더에 저장함.
...
`mill-version`, `mill-opts`, `mill-jvm-version`, `mill-jvm-opts`는 구버전 Mill과의 호환을 위해 `.mill-version` 같은 개별 파일이나, XDG Base Directory 표준을 따르는 `.config` 폴더(`.config/mill-version` 등)에도 둘 수 있음.

#### 5.3 mill-opts

프로젝트 태스크가 CPU를 많이 쓴다면, 모든 사용자가 코어당 0.5개 태스크만 동시 실행하도록 강제 가능.

```scala
//| mill-opts: ["--jobs=0.5C"]
```

커맨드라인에서 `--jobs=10`처럼 명시적으로 전달하면 `.mill-opts` 값을 오버라이드함. `mill-opts`는 Mill 자체에 전달하는 옵션, `mill-jvm-opts`는 Mill을 실행하는 JVM에 전달하는 옵션. Mill이 빌드하는 대상 프로젝트에 JVM 옵션을 전달하려면 Compilation and Execution Flags 참고 필요.
...
`build.mill.yaml`에서는 최상위 키로 지정함.
...
지정하지 않으면 기본값 `zulu:25`가 사용됨. Mill 빌드 도구 자체는 최소 Java 17이 필요하지만, 사용자 정의 모듈은 Java 11까지 지원함. 시스템 Java를 쓰려면 `mill-jvm-version: system`으로 지정 가능하나, 개발/CI/운영 환경 간 JVM 불일치로 디버깅이 어려운 문제가 생길 수 있어 권장하지 않음.

#### 5.5 mill-jvm-index-version

Mill은 Coursier JVM Index를 사용해 `temurin:11` 같은 부분 버전을 `temurin:11.0.28` 같은 구체적인 다운로드 버전으로 해석함. JVM 인덱스 버전은 사용 중인 Mill 버전에 따라 정해지지만, 새로 릴리스된 JVM을 쓰고 싶거나 특정 과거 버전을 고정하고 싶을 때 오버라이드 가능.
...
위 예시는 `mill-jvm-index-version`을 과거 버전으로 고정해 `temurin:17`이 최신 `17.0.17`이 아닌, 해당 인덱스 시점에 존재하던 `17.0.14`로 해석되도록 한 경우임.

#### 5.6 mill-repositories

의존성 해석에 사용할 추가 Maven 저장소를 설정함.
...
설정한 저장소는 기본 Maven Central보다 우선순위가 높게(앞에) 추가됨. 개별 모듈 단위 저장소 설정은 Repository Configuration 참고.

#### 5.7 mill-jvm-opts

Mill 런처(빌드 도구 자체를 실행하는 JVM)에 JVM 레벨 플래그를 전달함. `JAVA_OPTS` 환경변수, build 헤더, 또는 프로젝트 루트의 `.mill-jvm-opts` 파일(한 줄에 하나씩) 중 하나로 설정 가능.
...
환경변수 보간을 지원함. 예: `-Dmy.jvm.property=${PWD}`. 존재하지 않는 환경변수는 빈 문자열로 치환됨.

#### 5.8 메타빌드 태스크 오버라이드 (mvnDeps / repositories / 기타)

build 헤더로 The Mill Meta-Build(즉 `build.mill`을 컴파일하는 빌드)의 임의 태스크를 오버라이드 가능. `mvnDeps`, `repositories`로 `build.mill`에서 쓸 라이브러리를 추가하는 것 외에도, `scalacOptions` 같은 다른 태스크도 오버라이드 가능.
...
`build.mill`은 Scala로 작성되므로, `ScalaModule`에서 오버라이드 가능한 대부분의 태스크를 build 헤더에서도 지정 가능.

`build.mill.yaml` 파일에서는 메타빌드 설정을 `mill-build:` 키 아래에 둠.
...
build 헤더 설정은 Single-File Scripts(단일 Java/Scala/Kotlin 파일 스크립트) 설정에도 동일하게 쓰이며, 파일 내부에 설정값을 가볍게 내장하는 문법으로 활용됨.
