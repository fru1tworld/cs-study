...
Mill 빌드는 결국 Task들의 그래프임. 각 Task는 다른 Task를 입력으로 받아 임의의 코드를 실행하고 결과를 반환하는 단위이며, 이 결과를 다른 Task가 다시 입력으로 사용함. Mill은 이 그래프를 위상 정렬해서 실행 순서를 정하고, 각 Task의 입력이 바뀌었는지 감지해서 필요한 부분만 다시 계산함.

Task 간 의존성은 괄호를 붙여 호출하는 문법(`foo()`)으로 선언함. 어떤 Task 정의 안에서 `otherTask()`를 호출하면 Mill이 이를 보고 자동으로 의존성 그래프의 엣지를 구성함. 의존성 중 하나라도 실패 → 그 Task에 의존하는 하위(다운스트림) Task들은 아예 실행되지 않음.

Task는 크게 다음 종류로 나뉨.

- `Task { ... }`: 결과 캐시됨 → 소스 컴파일처럼 입력이 같으면 결과도 같은 순수 계산에 사용
- `Task.Source` / `Task.Sources`: 파일 해시로 추적 → 소스 파일·리소스 폴더 등 외부 입력에 사용
- `Task.Input`: 매번 재평가 → git 커밋 해시·환경변수 등 매번 확인이 필요한 외부 상태에 사용
- `Task.Command`: 캐시 안 됨 → `run`·`test`처럼 실행할 때마다 동작해야 하는 커맨드에 사용
- `Task.Persistent`: dest 폴더가 유지됨 → 증분 컴파일처럼 이전 실행 결과를 디스크에 남겨야 하는 작업에 사용
- `Task.Worker`: 메모리에 객체 유지 → 컴파일러 인스턴스 등 프로세스 생존 기간 동안 재사용할 무거운 객체에 사용
- `Task.Uncached`: 캐시 안 되고 watch도 안 됨 → 외부 서비스 호출처럼 부수효과 자체가 목적인 작업에 사용
- `Task.Anon`: 캐싱 대상 아님 → CLI에서 직접 실행할 수 없는, 여러 Task가 공유하는 헬퍼 로직에 사용

### 2. 캐시되는 Task

가장 기본적인 형태는 `Task { ... }`로 정의하는 캐시된 Task임.
...
이 Task는 반환값이 uPickle로 JSON 직렬화 가능해야 함. Mill은 이 값을 `out/` 아래에 저장해 두고, 입력(위 예제라면 `allSources()`)이 바뀌지 않으면 다시 실행하지 않고 캐시된 값을 그대로 씀. 반대로 입력이 바뀌어 재실행했더라도 결과값의 해시가 이전과 같음 → 그 Task에 의존하는 다운스트림 Task는 다시 실행할 필요 없음. 즉 캐시 무효화는 값 기준이지 실행 여부 기준이 아님.

### 3. Task.Source / Task.Sources

파일이나 폴더를 입력으로 삼는 Task는 `Task.Source`(단일 경로)와 `Task.Sources`(여러 경로)로 정의함.
...
Mill은 지정된 경로 아래 파일들의 내용을 해시로 추적하다가, 파일이 추가/삭제/수정되면 그 해시가 달라졌다고 보고 이 Task에 의존하는 다운스트림 Task들을 다시 계산함.

### 4. Task.Input

`Task.Input`은 매번 무조건 재평가되는 Task임. git 커밋 해시·시스템 프로퍼티·환경변수처럼 "Mill이 파일 감시만으로는 변경을 알 수 없는" 외부 상태를 감지할 때 사용.
...
Task.Input 자체는 매번 실행되지만, 이 결과를 입력으로 받는 캐시된 다운스트림 Task는 값이 실제로 바뀌었을 때만 재실행됨. 즉 재평가 비용은 Task.Input 하나에서 끝남 → 값이 그대로면 이후 그래프는 캐시를 그대로 사용.

### 5. Task.Command

`Task.Command`는 `run`·`test`처럼 호출할 때마다 실행되어야 하는 작업에 사용하며 결과를 캐싱하지 않음. MainArgs 라이브러리를 이용해 커맨드라인 인자를 위치 인자/명명 인자 형태로 그대로 받을 수 있음.
...
`Task.Persistent`는 `Task.dest` 폴더를 매 실행마다 초기화하지 않고 그대로 유지함. 증분 컴파일처럼 이전 실행 결과물을 디스크에 남겨 놓고 다음 실행에서 재사용해야 할 때 유용함. 다만 폴더 정리를 Mill이 대신 해 주지 않으므로, 파일 상태가 항상 일관되도록 만드는 책임은 해당 Task를 작성한 사람에게 있음.

### 7. Task.Worker와 CachedFactory

`Task.Worker`는 파일이 아니라 메모리에 객체를 유지하는 Task임. 컴파일러 인스턴스나 데몬 프로세스처럼 초기화 비용이 크고 여러 번의 실행에 걸쳐 재사용하고 싶은 객체에 적합.
...
Worker 객체가 여러 스레드에서 동시에 쓰일 수 있으므로 스레드 안전성을 직접 챙겨야 함. `AutoCloseable`을 구현해 두면 Mill이 자원 정리 시점에 `close()`를 호출해 줌.

여러 개의 장기 상태를 setup/teardown 패턴으로 관리해야 할 때는 `CachedFactory`를 헬퍼로 사용 가능. 최대 캐시 크기를 지정해 두면 그 크기를 넘길 때 오래된 항목부터 정리됨.

### 8. Task.Uncached와 Task.Anon

`Task.Uncached`는 매번 실행되고 캐시에서 값을 읽지 않는 Task로, 컨테이너 레지스트리 접근처럼 외부 시스템과의 상호작용 자체가 목적일 때 사용. `--watch` 모드에서 파일 변경 트리거로 실행되지도 않음.

`Task.Anon`은 CLI에서 직접 실행할 수 없는 익명 Task로, 여러 Task가 공통으로 쓰는 로직을 재사용하기 위한 헬퍼임. 결과를 직렬화하거나 캐싱할 필요가 없으므로 반환 타입 제약도 없음.

### 9. Module 기본 구조

Module은 두 가지 역할을 함. 하나는 Task들을 논리적으로 묶는 네임스페이스, 다른 하나는 trait을 통해 Task 집합을 복제·커스터마이징할 수 있는 재사용 템플릿.
...
이렇게 정의하면 `mill foo.bar`, `mill foo.qux.baz`로 각 Task를 실행할 수 있고, 결과 메타데이터도 모듈 계층을 그대로 반영해 `out/foo/bar.json`, `out/foo/qux/baz.json` 같은 경로에 저장됨. 즉 Scala 코드상의 중첩 구조 · CLI 호출 경로 · out 디렉터리 경로 세 가지가 항상 일치함.

### 10. trait으로 Module 재사용하기

여러 모듈에 공통 Task 집합을 적용하고 싶으면 class가 아니라 trait으로 정의해야 함.
...
`foo1`은 `qux`를 override해서 대문자로 바꾸고, `foo2`는 `qux`는 그대로 쓰면서 `baz`라는 새 Task를 얹음. 실제 JavaModule, ScalaModule 같은 Mill 내장 모듈들도 이런 방식의 trait 조합으로 구성됨.

### 11. moduleDir로 입력 경로 관리하기

각 Module은 입력 파일이 위치할 기준 경로를 `moduleDir` 필드로 가짐. `Task.Source`, `Task.Sources`에 상대 경로를 넘기면 이 `moduleDir` 기준으로 해석됨.
...
기본적으로 모듈 계층 경로를 그대로 따라가지만, 필요하면 override로 바꿀 수 있음.
...
주의할 점: `moduleDir`은 어디까지나 입력 파일의 기준 경로일 뿐이고, 출력은 항상 `out/` 아래 모듈 계층 경로에 고정되며 바꿀 수 없음. 모듈을 다른 위치로 옮기거나 감쌀 때 소스 경로가 깨지지 않도록 `moduleDir`을 신경 써서 다뤄야 함.

### 12. override와 super

Task를 override할 때는 일반 Scala 메서드처럼 `super`로 상위 구현의 결과에 접근할 수 있음.
...
`Bar`는 `Foo`가 정의한 `src` 경로에 `src2`를 덧붙이는 식으로 확장함. `moduleDeps`처럼 Task가 아니라 일반 메서드로 선언되는 값들도 이 방식으로 override해서 모듈 그래프의 형태(의존 모듈 목록 등)를 바꿀 수 있음.

### 13. Root Module

빌드 파일의 최상위 스코프에 직접 Task나 서브 모듈을 두고 싶으면, `` `package` `` 라는 이름으로 Root Module을 선언.
...
Root Module의 이름은 반드시 `package`여야 하며, 파일 안의 다른 모듈이나 Task는 모두 이 Root Module 내부에 정의해야 함.

### 14. DefaultTaskModule, ExternalModule, ModuleRef

#### DefaultTaskModule

모듈이 `DefaultTaskModule`을 상속하면 Task 이름을 생략하고 모듈 이름만으로 기본 Task를 실행할 수 있음.
...
`mill foo`라고만 호출해도 `mill foo.bar`와 동일하게 동작함.

#### 하이픈이 포함된 이름

모듈이나 Task 이름에 하이픈을 쓰고 싶으면 백틱으로 감싸 선언함.
...
CLI에서 호출할 때는 `mill hyphenated-module.hyphenated-task`처럼 백틱 없이 그대로 씀.

#### ExternalModule

라이브러리가 여러 프로젝트에 공유 제공하는 모듈은 `ExternalModule`로 정의함.
...
`mill foo.Bar/baz`처럼 슬래시 표기로 호출하며, 자주 쓰는 것이면 `def myAutoformat = mill.javalib.palantirformat.PalantirFormatModule`처럼 별칭을 만들어 짧게 쓸 수도 있음.

#### ModuleRef

추상 모듈이 다른 구체 모듈을 참조해야 할 때는 일반 참조 대신 `ModuleRef`를 씀.
...
`ModuleRef`로 감싼 참조는 Mill의 Task 쿼리 해석(예: `mill resolve __.compile` 같은 와일드카드 조회) 대상에서 제외되어, 같은 모듈이 여러 경로에서 중복으로 잡히는 문제를 막아줌.

### 15. out 디렉터리 구조

Mill은 모든 빌드 산출물과 캐시 메타데이터를 프로젝트 루트의 `out/` 폴더 아래에 모듈 계층과 동일한 구조로 저장함. 예를 들어 `foo.compile`이라는 Task를 실행하면 다음 파일들이 생김.

- `out/foo/compile.json`: 캐시 키와 JSON 직렬화된 반환값
  - 반환값에 `PathRef`가 포함되어 있으면 `.dest/` 안의 실제 바이너리 파일을 가리키는 참조가 들어있음
...
`mill show foo.compile` 명령으로 `compile.json`에 저장된 값을 바로 확인할 수 있음.

### 16. 빌드 진단용 JSON 파일들

`out/` 바로 아래에는 Task별 파일 외에 빌드 전체를 진단하기 위한 시스템 파일들도 함께 생성됨.
...
`mill-dependency-tree.json`은 예를 들어 다음과 같은 관계를 보여줌.
...
`mill-invalidation-tree.json`은 소스 파일이 바뀌었을 때 이런 식으로 무효화 경로를 나타냄.

```
foo.sources → foo.allSources → foo.allSourceFiles → foo.compile
```

`build.mill` 자체를 수정한 경우에는 변경된 메서드 단위로 `def`, `call` 노드가 표시됨. 이 무효화 분석은 보수적으로 동작하므로, 실제 실행 결과에는 영향이 없는 코드를 고쳐도 그 메서드를 호출하는 Task들이 전부 무효화 대상으로 잡힐 수 있음.

### 17. out 디렉터리 관리

`out/` 아래 파일을 손으로 정리할 때는 반드시 `.dest/` 폴더와 대응하는 `.json` 파일을 함께 지워야 함. `foo.dest/`만 지우고 `foo.json`을 남겨두면 캐시 메타데이터와 실제 파일 상태가 어긋나 진단하기 어려운 문제가 생길 수 있음. 개별 폴더를 직접 건드리기보다는 Mill이 제공하는 `clean` 명령을 쓰는 편이 안전함.

`out/` 디렉터리 위치는 `MILL_OUTPUT_DIR` 환경변수로 바꿀 수 있음.

```
MILL_OUTPUT_DIR=build-stuff/working-dir ./mill foo.printDest
```

빌드 산출물을 더 빠른 디스크나 별도의 쓰기 가능한 파일시스템에 두고 싶을 때 이 옵션을 활용할 수 있음.
...
Mill은 Coursier로 의존성을 해석하고 다운로드함. 의존성 좌표는 다음 형식을 따름.
...
Maven 생태계 용어로는 `organization`이 group, `name`이 artifact에 해당하고, 이 둘과 `version`을 합쳐 GAV(GroupArtifactVersion)라 부름.
...
Mill에는 다른 빌드 도구가 흔히 쓰는 "test scope" 개념이 없음. 테스트는 scope가 아니라 독립된 모듈로 표현함.
...
컴파일 시에만 필요한 의존성은 `compileMvnDeps`로 선언하며, 생성되는 `pom.xml`에서는 `provided` scope로 매핑됨. provided scope 의존성은 전이적으로 해결되지 않는다는 점에 주의 필요.
...
런타임에만 필요한 의존성은 `runMvnDeps`로 선언함. 컴파일은 최소 API 버전으로 하고 런타임에는 더 새 버전을 쓰는 조합에 유용.
...
출력에서 화살표(`->`)는 버전이 변경되었음을, `(possible incompatibility)` 주석은 잠재적 충돌 가능성을 나타냄.
...
조직/이름 튜플을 지정해 특정 전이 의존성을 제외함. `*`는 모든 조직 또는 이름에 매칭됨.
...
조직 전체 또는 이름 단위로 제외할 수도 있고, 체이닝도 가능.
...
예를 들어 pprint 0.8.1이 전이적으로 fansi 0.4.0을 요구해도, `forceVersion()`을 적용한 fansi 0.2.14가 최종적으로 사용됨.

---

### 5. 의존성 관리(BOM)

의존성 관리는 버전 강제와 exclude를 한꺼번에 적용하는 방식임. 관리 대상 의존성을 직접 선언하지 않아도, 전이적으로 나타나기만 하면 지정한 버전이 강제됨.

#### 외부 BOM 사용

`bomMvnDeps`에 외부 Maven BOM을 지정함.
...
`grpc-protobuf`는 버전을 명시하지 않았으므로 BOM에 정의된 `1.67.1`이 사용됨. BOM이 `protobuf-java`를 `4.28.3`으로 지정 → 원래 `grpc-protobuf`가 요구하는 `3.25.3` 대신 `4.28.3`이 채택됨.

여러 BOM을 동시에 쓸 때 같은 의존성 버전이 겹치면 먼저 선언한 BOM이 우선하고, exclude 항목은 모두 누적됨.
...
`depManagement` 항목에도 exclude를 붙일 수 있음.
...
`BomModule`을 섞어 넣으면 모듈 자체를 BOM으로 사용할 수 있음.
...
정의한 BOM 모듈은 `bomModuleDeps`로 참조함.
...
`BomModule`과 `PublishModule`을 함께 섞으면 BOM을 다른 프로젝트가 소비할 수 있도록 발행할 수 있음.
...
발행된 BOM은 다른 모듈에서 그대로 `bomModuleDeps`로 가져다 씀.
...
`mill.javalib.Dependency/showUpdates` 태스크로 빌드 전체의 의존성 최신 버전을 조회함. JavaModule 계열(ScalaModule, KotlinModule 포함)과 Maven 리포지토리 대상으로만 동작하며, 빌드에 존재하는 모든 모듈에 항상 적용됨.
...
출력 형식을 모듈별이 아니라 의존성별로 묶어서 보려면 `--format PerDependency`를 씀.
...
프리릴리스 버전까지 포함하려면 `--allowPreRelease true`, 메타빌드(`build.mill`을 빌드하는 빌드) 자체의 의존성을 확인하려면 `--meta-level 1`을 붙임.
...
Cross build는 동일한 소스와 설정을 여러 버전이나 플랫폼에 대해 반복 빌드하는 기법임. 같은 Scala 코드를 여러 Scala 버전으로 빌드하거나, 개발용/릴리스용 산출물을 나란히 만드는 상황에 사용.
...
`Cross[T]`는 지정한 값 목록만큼 모듈 인스턴스를 생성함. 각 인스턴스는 `Cross.Module[String]`을 상속하며, `crossValue`로 자신에게 배정된 값에 접근함. 기본 상태에서는 모든 인스턴스가 같은 `foo` 소스 폴더를 공유함.
...
버전 문자열의 점(`.`)은 명령줄에서 언더스코어로 치환해 참조함(`2.10` -> `2_10`).

기본으로 선택되는 값은 목록의 첫 번째지만, `defaultCrossSegments`로 재정의할 수 있음.
...
각 cross 값이 독립된 소스 폴더를 쓰게 하려면 `moduleDir`을 값별로 분기함.
...
이 설정에서는 `foo/2.10`, `foo/2.11`, `foo/2.12` 폴더가 각각 독립적인 소스 루트가 됨. 출력 경로 역시 cross 값별로 분리됨.
...
`foo("2.10")` 형태로 특정 cross 값의 인스턴스에 접근함.
...
`crossValue`를 그대로 다른 Cross 모듈의 키로 전달하면, 같은 값을 가진 인스턴스끼리 자동으로 매칭됨.
...
`CrossValue` 트레이트를 섞으면 Cross 모듈 내부의 중첩 모듈이 상위 `crossValue`를 자동으로 물려받음.
...
암묵적 `Cross.Resolver`를 정의하면 `foo(crossValue)` 대신 `foo()`처럼 짧게 쓸 수 있음.
...
축이 두 개 이상 필요하면 튜플 시퀀스와 `Cross.Module2[T1, T2]`(세 축이면 `Module3`, 이런 식으로 확장)를 사용함.
...
기존 단일 축 Cross 모듈을 다중 축으로 확장할 수도 있음. 이때 값 목록의 튜플 크기가 `Module2`, `Module3` 등의 타입 인자 개수와 맞지 않으면 컴파일 에러가 남.
...
Cross 값 목록을 정적으로 나열하지 않고, 파일시스템 등 런타임 정보로부터 계산할 수도 있음. `BuildCtx.watchValue`로 감싸면 Mill이 해당 값의 변화를 감지해 모듈 구조를 자동으로 재계산함.
...
`modules` 아래에 폴더를 추가하면 다음 빌드 시 자동으로 새 cross 모듈로 인식됨.

#### 실무 예제: 정적 블로그 빌드

마크다운 포스트 목록을 동적 Cross 모듈로 만들어 HTML로 렌더링하는 예시임.
...
포스트 파일을 추가하면 별도 설정 변경 없이 자동으로 인식되고, 변경된 파일만 재처리되며 `-j` 플래그로 병렬 렌더링도 가능함.

---

### 10. Mill에 번들된 라이브러리

Mill은 빌드 도구 내부적으로 여러 오픈소스 라이브러리를 사용하며, 이들은 빌드 스크립트(`build.mill`)에서 직접 import 없이 바로 쓸 수 있음.
...
Mill이 파일 및 프로세스 연산에 전반적으로 사용하는 라이브러리임. 각 태스크는 격리된 작업 디렉터리(`Task.dest`)에서 실행되어 태스크 간 파일 간섭이 발생하지 않음.
...
모든 Mill 태스크의 결과값은 uPickle을 거쳐 JSON으로 저장되고, 이 값을 기준으로 캐시 무효화 여부를 판단함. 기본 타입, 컬렉션, `Path`, `PathRef` 등을 직렬화 대상으로 지원함.
...
컴파일러, 소스코드, 데이터 파일 등을 원격에서 내려받을 때 사용.
...
`Task.Command`의 매개변수로 문자열, 정수, 불린 등 기본 타입을 그대로 받을 수 있게 해줌.
...
Scala, Java 등 JVM 생태계의 서드파티 의존성 해결을 담당하는 엔진으로, [1. 라이브러리 의존성 기본](#1-라이브러리-의존성-기본)에서 다룬 `mvnDeps` 해석의 실질적인 구현체임.

각 라이브러리의 상세 API는 해당 프로젝트의 ScalaDoc과 저장소 페이지에서 확인함.

---

### 11. JVM 버전 설정

Mill은 기본적으로 자체 실행 중인 JVM(예: `zulu:25`)을 Java/Scala/Kotlin 모듈 컴파일에 사용함. 필요한 JVM은 자동으로 다운로드하고 캐싱하므로, 별도의 전역 JDK 설치 없이도 원하는 버전으로 빌드 가능.
...
버전 문자열은 `"{version}"` 또는 `"{name}:{version}"` 형식을 씀. 예: `temurin:11.0.21`, `zulu:25`.
...
사용 가능한 배포판 목록은 [coursier/jvm-index](https://github.com/coursier/jvm-index)에서 확인함. 다운로드 URL을 직접 지정하는 경우 OS/아키텍처별 분기 처리 필요.

#### JVM 옵션 전달

`javacOptions`에 `-J` 접두사를 붙이면 JVM 자체에 옵션을 전달할 수 있음.
...
- 라이브러리: 최대한 낮은(하지만 지원 대상인) 버전 사용 → 예를 들어 17을 타깃으로 컴파일해 호환 범위를 넓힘
- 애플리케이션: 최신 버전을 사용해 성능·보안 개선을 그대로 누림
