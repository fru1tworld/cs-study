...
Go 1.21 전후로 표준 라이브러리에 제네릭 기반의 `slices`, `maps`, `cmp` 패키지 추가됨 → 예전 `sort` 패키지만으로 처리하던 정렬·검색·비교 작업을 훨씬 짧고 타입 안전하게 작성 가능해짐. 이 문서는 네 패키지에서 실무에서 자주 쓰는 함수 위주로 정리.
...
- `sort` 패키지: Go 1.21 이전부터 존재하던 정렬 라이브러리
- 슬라이스가 `sort.Interface`(`Len`, `Less`, `Swap`)를 구현하면 임의의 타입 정렬 가능
- `Slice`/`SliceStable`이나 `Ints`/`Strings`/`Float64s` 같은 타입별 편의 함수: Go 1.22부터 내부적으로 `slices` 패키지에 위임 → 성능 개선됨
- `sort.Sort`/`Stable`처럼 `Interface`를 직접 받는 함수는 별도 구현 유지 → API 자체는 그대로
...
- `Sort(data Interface)`: `Less`에 따라 오름차순 정렬. 정렬 안정성 보장 안 됨
...
구조체마다 `Interface`를 구현하기 번거로울 때 → `less` 함수만 넘기는 편이 간편함.
...
`x`가 `any`이고 리플렉션 기반으로 동작 → 타입이 고정되어 있다면 뒤에서 설명할 제네릭 `slices.SortFunc`가 더 빠르고 타입 안전.

### 기본 타입용 편의 함수

`Ints`, `Strings`, `Float64s`와 짝을 이루는 `*AreSorted` 함수들:
...
Go 1.22부터 이 함수들 내부적으로 `slices` 패키지 호출 → 새 코드에서는 `slices.Sort`를 직접 쓰는 편 권장.

### 이진 탐색

정렬된 슬라이스에서 값을 찾을 때 사용.
...
- `IntSlice`, `StringSlice`, `Float64Slice`: 각각 `[]int`, `[]string`, `[]float64`에 `Len/Less/Swap`과 `Sort()`, `Search()` 메서드를 얹은 타입
  - `sort.Reverse`와 조합해 내림차순 정렬을 만들 때 흔히 쓰임
...
`slices`와 `maps`의 제네릭 함수들이 공통으로 참조하는 기반 패키지. 비교 가능한 타입 제약과 3-way 비교 결과(`-1`, `0`, `1`)라는 관례를 여기서 정의.
...
`<`, `<=`, `>=`, `>` 연산자를 지원하는 정수·부동소수점·문자열 계열 타입을 모두 포함하는 제약. `slices.Sort`, `slices.Max` 같은 함수의 타입 매개변수 제약으로 쓰임.
...
부동소수점에서는 NaN을 "가장 작은 값"으로 취급 → 일관된 전순서를 만듦. 즉 `Compare(NaN, 1.0)`은 -1, `Compare(NaN, NaN)`은 0 → NaN이 섞인 슬라이스도 정렬 시 패닉 없이 한쪽 끝에 몰림.
...
`Or[T comparable](vals ...T) T` (Go 1.22+): 인자를 순서대로 검사해 제로 값이 아닌 첫 값을 반환. 여러 정렬 기준을 우선순위대로 이어 붙일 때 유용.
...
Go 1.21에 도입된 패키지 → `sort`의 기능을 대체하면서 검색·삽입·삭제·비교까지 아우르는 슬라이스 전용 제네릭 함수 모음 제공. 대부분의 함수가 타입 매개변수 `S ~[]E`를 받아 슬라이스 기반 타입(named slice type 포함)에도 그대로 적용됨.

### 정렬

- `Sort[E cmp.Ordered](x []E)`: 오름차순 정렬. `sort.Slice`보다 빠르고 리플렉션 없음
...
`range-over-func`(`iter.Seq`)가 도입되면서 슬라이스와 이터레이터를 오가는 함수들 추가됨.
...
`slices`의 맵 버전 → Go 1.21에 함께 도입됨. 맵은 순서가 없으므로 `Sort` 계열 함수는 없고, 대신 키/값 열거·복사·비교·조건부 삭제에 집중.
...
맵은 순서가 없음 → 정렬된 순서로 순회하고 싶다면 `maps.Keys`로 키를 뽑은 뒤 `slices.Sort`(또는 `slices.Sorted(maps.Keys(m))`)로 정렬하는 조합이 관용구처럼 쓰임.
...
- 기존 코드나 `sort.Interface`를 구현한 커스텀 타입을 다룰 때는 여전히 `sort` 패키지가 유효 → 새 코드에서 일반 슬라이스를 다룰 때는 `slices.Sort` / `slices.SortFunc`가 더 빠르고 타입 안전
- 여러 필드를 기준으로 정렬할 때는 `cmp.Compare`와 `cmp.Or`를 조합해 우선순위 체인을 구성하는 패턴이 관용적
- 맵은 순서가 없으므로, 정렬된 순회가 필요하면 `maps.Keys` + `slices.Sort`(또는 `slices.Sorted`) 조합 사용
- Go 1.23의 `iter.Seq`/`iter.Seq2` 기반 함수(`All`, `Values`, `Collect`, `Sorted` 등)는 `slices`와 `maps` 사이를 자연스럽게 넘나들 수 있게 해줌
...
`math`: 부동소수점 수학 함수와 상수를 제공하는 표준 라이브러리 패키지. 대부분의 함수는 `float64`를 받고 `float64`를 반환 → 특수 케이스(NaN, ±Inf, ±0)에 대한 동작은 IEEE 754 표준을 따름.
...
- 정수 타입의 min/max는 상수 표현식으로 정의됨 → `int8(math.MaxInt8)`처럼 변환 없이 바로 사용 가능
...
`Round`와 `RoundToEven`의 차이는 `.5` 케이스에서 드러남: `Round(2.5) == 3`이지만 `RoundToEven(2.5) == 2`.
...
- 정수 거듭제곱만 필요하면 `Pow` 대신 반복 곱셈이 더 빠름 → `Pow`는 임의의 실수 지수를 다루기 위한 범용 함수
- `0`에 가까운 값에서 정밀도가 중요하면 `Expm1(x)`(= `e^x - 1`), `Log1p(x)`(= `log(1+x)`)를 씀
...
- `Sincos(x float64) (sin, cos float64)`로 sin/cos을 한 번에 계산 → 각각 호출하는 것보다 효율적
...
- 정수 `int`에는 `math.Abs`를 직접 쓸 수 없음(파라미터가 `float64`) → 정수 최대/최소는 Go 1.21부터 추가된 내장 함수 `min`/`max`(제네릭)를 쓰는 것이 일반적이고, `math.Max`/`math.Min`은 float 특유의 NaN 처리가 필요할 때 씀
- `Mod`는 나눗셈의 나머지, `Remainder`는 반올림 나눗셈 기준의 나머지로 서로 다름
...
Bessel 함수(`J0`, `J1`, `Jn`, `Y0`...), 감마·에러 함수(`Gamma`, `Erf`, `Lgamma`)도 제공되지만 일반적인 애플리케이션 코드에서는 거의 쓰이지 않음.
...
- 암호학적으로 안전하지 않음 → 보안이 필요한 곳(토큰, 키 등)에는 반드시 `crypto/rand` 사용 필요
- 시뮬레이션, 테스트 데이터 생성, 게임 로직 등 예측 가능해도 무방한 용도에 적합
...
- `Source`는 난수의 원천을 추상화한 인터페이스, `Rand`는 그 위에 다양한 분포(정수/실수/순열)를 얹은 래퍼
- `Rand`는 여러 고루틴에서 동시에 안전하게 쓸 수 없음. 반면 패키지 최상위 함수들(`rand.Intn` 등)은 내부적으로 잠금 처리된 전역 `Rand`를 사용 → 동시성에 안전
...
- Go 1.20부터는 `Seed`를 호출하지 않아도 프로그램 시작 시 자동으로 무작위 시드가 설정됨 → 재현 가능한 시퀀스가 필요할 때만 명시적으로 `New(NewSource(seed))`를 쓰면 됨
- 전역 `rand.Seed(seed)`는 deprecated이며 더 이상 권장되지 않음 → 독립된 생성기가 필요하면 `rand.New`로 별도 인스턴스를 만드는 편이 낫음
...
`Rand` 값에도 동일한 이름의 메서드가 있음(`r.Intn`, `r.Float64`, `r.Perm`, `r.Shuffle` ...).
...
`Intn`, `Int31n`, `Int63n`은 `n <= 0`이면 panic한다는 점 주의 필요.
...
Zipf-Mandelbrot 분포를 따르는 값을 생성. 순위 기반 빈도 분포(예: 인기도 시뮬레이션)를 모사할 때 씀.
...
- 모듈 버전(v2)이므로 하위 호환 부담 없이 API를 정리할 수 있었음
...
- `PCG`는 128비트 내부 상태를 가진 빠르고 품질 좋은 범용 생성기 → 대부분의 용도에 기본으로 적합
- `ChaCha8`은 스트림 암호 기반이라 통계적으로 더 강력 → 다만 패키지 문서는 여전히 "보안이 필요한 용도에는 `crypto/rand`를 쓰라"고 명시
- 새 `Rand` 타입은 이 `Source`를 받아 생성: `rand.New(rand.NewPCG(1, 2))`
...
`N`은 타입 매개변수 덕분에 `int`, `int64`, `time.Duration` 등 어떤 정수 계열 타입에도 그대로 쓸 수 있음 → v1에서 타입마다 다른 함수(`Intn`, `Int63n` ...)를 골라 쓰던 번거로움 제거. 모든 `*N` 계열 함수는 `n <= 0`일 때 panic한다는 점은 v1과 동일.
...
시그니처와 사용법은 v1과 동일. 최상위 함수는 전역 소스를 사용하며 동시성에 안전하고, `*Rand` 값의 메서드는 v1과 마찬가지로 동시 사용 시 별도 동기화가 필요함.

### 마이그레이션 감

기존 코드의 `rand.Intn(n)` → `rand.IntN(n)`, `rand.Int63n(n)` → `rand.Int64N(n)`처럼 이름만 바뀐 경우가 많아 치환이 비교적 단순. `import "math/rand/v2"`로 바꾸고 컴파일 에러를 따라가며 이름을 맞추는 방식으로 마이그레이션하면 됨.
...
`math/bits`: 부호 없는 정수에 대한 비트 카운팅·회전·반전, 그리고 캐리를 포함한 저수준 산술 연산 제공. 대부분의 함수는 컴파일러가 대상 아키텍처의 전용 명령어(예: `POPCNT`, `BSR`)로 치환해 최적화 → 직접 루프를 짜는 것보다 빠름.

각 함수군은 플랫폼 기본 `uint`용과 `8`/`16`/`32`/`64` 비트 폭이 고정된 변형(`XxxN`)을 함께 제공.
...
`OnesCount`(popcount)는 집합 크기 계산이나 해밍 거리 계산에, `Len`은 값 하나를 저장하는 데 필요한 최소 비트 수를 구할 때 쓰임.
...
`ReverseBytes`는 네트워크 바이트 순서(빅엔디언)와 리틀엔디언 사이 변환에 자주 쓰임.

### 캐리를 포함한 산술

64비트를 넘는 범위의 덧셈·뺄셈·곱셈을 다룰 때, 자리올림/빌림을 명시적으로 주고받는 함수들. 빅넘버 연산이나 암호 구현에서 쓰임.
...
`Div64`는 `y == 0`이거나 몫이 64비트를 넘치면 panic함. 오버플로 여부와 무관하게 나머지만 필요하면 panic하지 않는 `Rem64`를 씀.

### 요약

- `math`: 부동소수점 수치 계산의 기본 도구 상자. 반올림, 거듭제곱/로그, 삼각함수, 특수값 판별이 핵심
- `math/rand`: 레거시 의사난수. 자동 시딩(Go 1.20+)과 `rand.New(rand.NewSource(seed))` 조합으로 충분 → 보안 용도에는 쓰지 않음
- `math/rand/v2`: 더 나은 알고리즘(PCG/ChaCha8)과 제네릭 `N` 기반의 정리된 API. 새 코드에서는 v2를 우선 검토
- `math/bits`: popcount, 비트 길이, 회전/반전, 캐리 산술 등 컴파일러 최적화 혜택을 받는 저수준 연산 모음. 비트 조작이 잦은 알고리즘, 빅넘버, 해시 구현에 사용

---

## 컨테이너 자료구조 (container/list, container/heap, container/ring)

Go 표준 라이브러리는 범용 컨테이너 자료구조로 `container` 하위에 세 패키지 제공.

- 이중 연결 리스트(`list`)
- 힙 인터페이스(`heap`)
- 원형 리스트(`ring`)

세 패키지 모두 제네릭이 아니라 `any` 타입과 인터페이스를 이용해 동작함 → 실무에서는 슬라이스로 충분한 경우가 많아 사용 빈도는 낮지만, 우선순위 큐(heap)나 LRU 캐시·이벤트 루프(list) 구현에 종종 쓰임.
...
- `List`는 이중 연결 리스트(doubly linked list)이며, 각 노드는 `Element` 타입
- `List`의 제로 값은 곧바로 사용 가능한 빈 리스트 → 다만 `list.New()`로 생성하는 것이 관용적
- 슬라이스와 달리 중간 삽입/삭제가 O(1)이라는 점이 핵심 장점 → 대신 인덱스 접근이 없고 순회는 포인터를 따라가야 함
...
`Value`에 원하는 값을 자유롭게 담아 쓰고, `Next`/`Prev`로 앞뒤 요소를 따라감.
...
모두 새로 생긴 `*Element`(또는 void)를 반환하며, `mark`는 반드시 같은 리스트에 속한 요소여야 함.
...
`Move*` 계열은 요소를 새로 만들지 않고 위치만 재배치 → LRU 캐시에서 "최근 사용한 항목을 맨 앞으로 이동"하는 용도로 적합.
...
`for e := l.Front(); e != nil; e = e.Next()` 형태가 표준 순회 패턴이며, 역순 순회는 `l.Back()`과 `e.Prev()`를 사용.
...
- `heap` 패키지는 자료구조를 직접 제공하지 않고, 임의의 타입이 `heap.Interface`를 구현하면 그 타입을 최소 힙(min-heap)으로 다룰 수 있게 해주는 알고리즘 모음
- 루트(인덱스 0)가 항상 최솟값이라는 힙 불변식을 유지 → 내부적으로 `sort.Interface`(`Len`, `Less`, `Swap`)에 `Push`, `Pop`을 더한 인터페이스를 요구
- 최댓값 우선 큐가 필요하면 `Less`의 부등호 방향만 뒤집으면 됨
...
- `Less(i, j)`가 "i가 j보다 우선순위가 높은가"를 정의 → 이 순서가 힙의 루트를 결정
- `Push`/`Pop`은 슬라이스의 맨 끝을 다루도록 구현하는 것이 관례(내부적으로 heap 알고리즘이 앞부분 재배치를 담당)
- 리시버는 슬라이스 자체를 교체해야 하므로 포인터 리시버(`*IntHeap`)로 구현 필요

### 힙 조작 함수

- `Init(h Interface)` — 임의 순서의 데이터를 힙 불변식에 맞게 재배치, O(n)
- `Push(h Interface, x any)` — 힙에 x를 추가 후 재정렬, O(log n)
- `Pop(h Interface) any` — 최솟값 제거 후 반환(`Remove(h, 0)`과 동일), O(log n)
- `Remove(h Interface, i int) any` — 인덱스 i의 요소를 제거 후 반환, O(log n)
- `Fix(h Interface, i int)` — 인덱스 i의 값이 바뀐 뒤 힙 불변식을 재확립, O(log n)

`Fix`는 우선순위 큐에서 항목의 우선순위를 바꿀 때, `Remove` 후 `Push`하는 것보다 저렴하게 재정렬하는 용도로 씀.
...
우선순위가 바뀌는 항목을 다룰 때는 각 항목이 힙 내부 인덱스를 스스로 들고 있게 하고, `Swap`에서 그 인덱스를 갱신함.
...
- `Ring`은 순환 연결 리스트 → 리스트 자체에 시작이나 끝이 없어 임의의 한 요소에 대한 포인터가 곧 전체 링을 가리키는 참조 역할을 함
- `Ring`의 제로 값은 `Value`가 nil인 요소 1개짜리 링. 빈 링은 nil 포인터로 표현
- 라운드로빈 스케줄링, 순환 버퍼처럼 "끝에 도달하면 처음으로 돌아가는" 순회에 적합
...
`r`은 빈 링(nil)이 아니어야 하며, `Len()`은 캐시되지 않으므로 반복 호출을 피하는 것이 좋음.
...
- `Link(s)`: `r.Next()`가 `s`가 되도록 연결하고, 연결 전 `r.Next()`였던 값을 반환
  - `r`과 `s`가 같은 링에 속하면 그 사이 요소들을 링에서 잘라내는 효과
  - 서로 다른 링이면 `s`의 모든 요소를 `r` 뒤에 끼워 넣어 하나의 링으로 합침
- `Unlink(n)`: `r.Next()`부터 `n % Len()`개 요소를 링에서 떼어내고, 떼어낸 부분 링을 반환. `n % Len() == 0`이면 아무 변화 없음
...
링의 모든 요소에 대해 정방향으로 `f(value)`를 호출. `f` 안에서 링 구조를 변경하면 동작이 정의되지 않음.
