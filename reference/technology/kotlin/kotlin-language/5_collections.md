...
> 배열은 컬렉션 타입이 아님 → 자세한 내용은 [배열](https://kotlinlang.org/docs/arrays.html) 참조
...
- 가변 컬렉션 변경 시 반드시 [`var`](https://kotlinlang.org/docs/basic-syntax.html#variables)에 할당해야 하는 것은 아님
...
- 읽기 전용 컬렉션 타입은 [공변](https://kotlinlang.org/docs/generics.html#variance)
...
![컬렉션 인터페이스 계층](https://kotlinlang.org/docs/images/collections-diagram.png)
...
- 두 리스트가 같은 크기 + 같은 위치에 [구조적으로 동일한](https://kotlinlang.org/docs/equality.html#structural-equality) 요소 → 동일한 것으로 간주
...
- `to` 표기법 → 수명이 짧은 `Pair` 객체 생성 → 성능이 중요하지 않은 경우에만 사용 권장
  ...
  - [`apply()`](https://kotlinlang.org/docs/scope-functions.html#apply) 함수가 이러한 초기화를 유연하게 지원
...
- 빌더 함수 [`buildList()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/build-list.html)·[`buildSet()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/build-set.html)·[`buildMap()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/build-map.html) 호출
  - 해당 타입의 새 가변 컬렉션 생성 → [람다 함수](https://kotlinlang.org/docs/lambdas.html#function-literals-with-receiver)로 채움 → 동일 요소를 가진 읽기 전용 컬렉션 반환
...
- [필터링](https://kotlinlang.org/docs/collection-filtering.html): 필터와 일치하는 요소의 리스트 생성
...
- [매핑](https://kotlinlang.org/docs/collection-transformations.html#map): 변환 결과의 리스트 생성
...
- [연관](https://kotlinlang.org/docs/collection-transformations.html#associate): 맵 생성
...
- Kotlin 컬렉션 연산 관련 자세한 내용 → [컬렉션 연산 개요](https://kotlinlang.org/docs/collection-operations.html) 참조
...
- 표준 라이브러리의 컬렉션 연산은 두 가지 방식으로 선언
  - 컬렉션 인터페이스의 [멤버 함수](https://kotlinlang.org/docs/classes.html#class-members)
  - [확장 함수](https://kotlinlang.org/docs/extensions.html#extension-functions)
...
- 공통 연산은 [읽기 전용 및 가변 컬렉션](https://kotlinlang.org/docs/collections-overview.html#collection-types) 모두에서 사용 가능
- 다음 그룹에 속함:
  - [변환](https://kotlinlang.org/docs/collection-transformations.html)
  - [필터링](https://kotlinlang.org/docs/collection-filtering.html)
  - [`plus` 및 `minus` 연산자](https://kotlinlang.org/docs/collection-plus-minus.html)
  - [그룹화](https://kotlinlang.org/docs/collection-grouping.html)
  - [컬렉션 일부 조회](https://kotlinlang.org/docs/collection-parts.html)
  - [단일 요소 조회](https://kotlinlang.org/docs/collection-elements.html)
  - [정렬](https://kotlinlang.org/docs/collection-ordering.html)
  - [집계 연산](https://kotlinlang.org/docs/collection-aggregate.html)
...
- 가변 컬렉션의 경우 컬렉션 상태를 변경하는 쓰기 연산도 존재
  - 요소 추가·제거·업데이트 포함
  - 관련 섹션: [컬렉션 쓰기 연산](https://kotlinlang.org/docs/collection-write.html)·[리스트별 연산](https://kotlinlang.org/docs/list-operations.html#list-write-operations)·[맵별 연산](https://kotlinlang.org/docs/map-operations.html#map-write-operations)
...
- [리스트의 정의](https://kotlinlang.org/docs/collections-overview.html#list)상 리스트는 순서가 있는 컬렉션 → 모든 요소는 참조할 위치를 가짐
  - 인덱스로 요소 조회·검색하는 더 넓은 방법은 [리스트별 연산](https://kotlinlang.org/docs/list-operations.html) 참조

- [정의](https://kotlinlang.org/docs/collections-overview.html#set)상 셋은 순서가 있는 컬렉션이 아님
...
- `elementAt()`은 인덱스 접근이나 `get()`을 지원하지 않는 컬렉션에 유용
  - `List`의 경우 [인덱스 접근 연산자](https://kotlinlang.org/docs/list-operations.html#retrieve-elements-by-index)(`get()` 또는 `[]`)가 더 관용적
...
- Kotlin 컬렉션 패키지는 자연 순서·사용자 정의 순서·랜덤 순서로 컬렉션을 정렬하는 함수 제공
  - 이 페이지는 [읽기 전용](https://kotlinlang.org/docs/collections-overview.html#collection-types) 컬렉션에 적용되는 정렬 함수 설명 → 요청된 순서의 요소를 포함하는 새 컬렉션으로 결과 반환
  - [가변](https://kotlinlang.org/docs/collections-overview.html#collection-types) 컬렉션을 제자리에서 정렬하는 함수 → [리스트별 연산](https://kotlinlang.org/docs/list-operations.html#sort) 참조
...
- [`slice()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/slice.html): 주어진 인덱스에 있는 요소의 리스트 반환
  - 인덱스는 [범위](https://kotlinlang.org/docs/ranges.html)나 정수 값 컬렉션으로 전달 가능
...
- 맵에서 `plus`·`minus`의 세부 사항 → [맵별 연산](https://kotlinlang.org/docs/map-operations.html) 참조
- [증가 대입 연산자](https://kotlinlang.org/docs/operator-overloading.html#augmented-assignments) [`plusAssign`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/plus-assign.html)(`+=`)·[`minusAssign`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/minus-assign.html)(`-=`)도 컬렉션에 정의됨
  - 읽기 전용 컬렉션의 경우 실제로는 `plus`·`minus` 연산자 사용 후 결과를 동일 변수에 재할당하는 것 → `var` 읽기 전용 컬렉션에서만 사용 가능
  - 가변 컬렉션의 경우 `val`로 선언되어도 제자리 수정
  - 자세한 내용은 [컬렉션 쓰기 연산](https://kotlinlang.org/docs/collection-write.html) 참조
...
- `Grouping`이 지원하는 연산:
  ...
  - [`fold()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/fold.html)·[`reduce()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/reduce.html): 각 그룹에 대해 별도 컬렉션으로 [fold 및 reduce](https://kotlinlang.org/docs/collection-aggregate.html#fold-and-reduce) 연산 수행, 결과 반환
...
- [가변 컬렉션](https://kotlinlang.org/docs/collections-overview.html#collection-types) → 컬렉션 내용을 변경하는 연산 지원(요소 추가·제거 등)
  - 이 페이지는 `MutableCollection`의 모든 구현에서 사용 가능한 쓰기 연산 설명
  - `List`·`Map`에 사용 가능한 더 구체적인 연산은 각각 [리스트별 연산](https://kotlinlang.org/docs/list-operations.html)·[맵별 연산](https://kotlinlang.org/docs/map-operations.html)에서 설명
...
- [`plus` 연산자](https://kotlinlang.org/docs/collection-plus-minus.html)의 제자리 버전 [`plusAssign`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/plus-assign.html)(`+=`)으로도 요소 추가 가능
...
- 요소 제거의 또 다른 방법: [`minusAssign`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/minus-assign.html)(`-=`) 연산자 - [`minus`](https://kotlinlang.org/docs/collection-plus-minus.html)의 제자리 버전
...
- 리스트·맵은 요소 업데이트 연산도 제공 → [리스트별 연산](https://kotlinlang.org/docs/list-operations.html)·[맵별 연산](https://kotlinlang.org/docs/map-operations.html)에서 설명
...
- 표준 라이브러리 → 단일 호출로 필터링 가능한 확장 함수 그룹 존재
  - 원래 컬렉션을 변경하지 않음 → [가변 및 읽기 전용](https://kotlinlang.org/docs/collections-overview.html#collection-types) 컬렉션 모두에서 사용 가능
...
- [`List`](https://kotlinlang.org/docs/collections-overview.html#list)는 Kotlin에서 가장 인기 있는 내장 컬렉션 타입
  - 리스트 요소에 대한 인덱스 접근 → 강력한 연산 세트 제공

### 인덱스로 요소 조회

- 리스트는 요소 조회를 위한 모든 일반 연산 지원: `elementAt()`·`first()`·`last()` 등 [단일 요소 조회](https://kotlinlang.org/docs/collection-elements.html)에 나열된 연산
...
- [컬렉션 일부 조회](https://kotlinlang.org/docs/collection-parts.html) 연산 외에 리스트는 [`subList()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/-list/sub-list.html) 함수 제공: 지정된 범위의 요소를 리스트로 반환
...
- 리스트에서 요소를 검색하는 또 다른 방법: [이진 검색](https://en.wikipedia.org/wiki/Binary_search_algorithm)
  - 다른 검색 함수보다 훨씬 빠름
  - 조건: 리스트가 특정 순서(자연 순서 또는 함수 매개변수에 제공된 다른 순서)에 따라 오름차순으로 [정렬](https://kotlinlang.org/docs/collection-ordering.html)되어야 함 → 그렇지 않으면 결과가 정의되지 않음
...
- [컬렉션 쓰기 연산](https://kotlinlang.org/docs/collection-write.html) 외에 [가변](https://kotlinlang.org/docs/collections-overview.html#collection-types) 리스트는 특정 쓰기 연산 지원
...
- [컬렉션 정렬](https://kotlinlang.org/docs/collection-ordering.html)은 정렬된 순서로 요소를 조회하는 연산을 설명
...
- [맵](https://kotlinlang.org/docs/collections-overview.html#map)에서 키·값의 타입은 사용자가 정의
...
- 다른 컬렉션과 마찬가지로 [`filter()`](https://kotlinlang.org/docs/collection-filtering.html) 함수로 맵 필터링 가능
...
- [가변](https://kotlinlang.org/docs/collections-overview.html#collection-types) 맵 → 맵별 쓰기 연산 제공
...
- 시퀀스 연산은 상태 요구 사항에 따라 분류 가능:
  - 무상태 연산: 상태가 필요 없고 각 요소를 독립적으로 처리 - 예: [`map()`](https://kotlinlang.org/docs/collection-transformations.html#map)·[`filter()`](https://kotlinlang.org/docs/collection-filtering.html)
    - 요소 처리에 상수량의 상태가 필요한 경우도 포함 - 예: [`take()`·`drop()`](https://kotlinlang.org/docs/collection-parts.html)
  - 상태유지 연산: 일반적으로 시퀀스 요소 수에 비례하는 상당한 양의 상태 필요

- 시퀀스 연산이 다른 시퀀스를 반환하며 지연 생성되면 중간 연산
  - 그렇지 않으면 터미널 연산(예: [`toList()`](https://kotlinlang.org/docs/constructing-collections.html#copy)·[`sum()`](https://kotlinlang.org/docs/collection-aggregate.html))
...
![리스트 처리](https://kotlinlang.org/docs/images/list-processing.png)
...
![시퀀스 처리](https://kotlinlang.org/docs/images/sequence-processing.png)
...
- 진행은 `Iterable<N>`을 구현(`N`은 각각 `Int`·`Long`·`Char`) → `map`·`filter` 등 다양한 [컬렉션 함수](https://kotlinlang.org/docs/collection-operations.html)에서 사용 가능
