...
Kotlin 클래스: 데이터(프로퍼티)와 동작(함수)을 캡슐화하는 객체의 청사진 · 재사용 가능하고 구조화된 코드를 선언하는 간결한 문법 제공
...
클래스 헤더에서 선언 → 초기 상태 설정

```kotlin
class Person(name: String) { /*...*/ }

// 어노테이션이나 수정자가 없으면 'constructor' 키워드는 선택:
class Person(name: String) { /*...*/ }
```

생성자 매개변수 프로퍼티: 읽기 전용은 `val`, 가변은 `var`
...
초기화 블록(`init {}`): 인스턴스 생성 시 실행 → 복잡한 초기화 로직 포함 가능
...
클래스를 초기화하는 추가 방법 제공 → 반드시 주 생성자에게 위임 필요
...
명시적 생성자가 없는 클래스 → 암시적인 public 매개변수 없는 주 생성자 보유
...
클래스 상속: 기존 기본 클래스에서 파생 클래스 생성 → 프로퍼티·함수를 상속받으면서 동작 추가/수정 가능

자세한 내용은 상속 섹션 참고

### 추상 클래스

추상 클래스: 직접 인스턴스화 불가 · 상속용으로 설계 · 서브클래스가 구현해야 하는 추상 멤버 제공
...
- 추상 멤버는 `open` 키워드 불필요 (암시적으로 상속 가능)
- 서브클래스는 `override`로 추상 멤버 구현

### 동반 객체

동반 객체: 인스턴스 생성 없이 클래스 이름으로 멤버 접근 가능
...
Kotlin 상속: 클래스가 다른 클래스로부터 상속받는 방식과 이를 지배하는 규칙

### 개요

모든 Kotlin 클래스는 기본적으로 `Any` 상속 → 세 가지 메서드 제공:
- `equals()`
- `hashCode()`
- `toString()`
...
기본적으로 Kotlin 클래스는 final → 상속 불가. 상속 가능하게 하려면 `open` 키워드 사용
...
오버라이드 가능하려면 명시적으로 `open`으로 표시 필요
...
`override`로 표시된 멤버는 기본적으로 자체가 open → 추가 오버라이딩 방지하려면 `final` 사용
...
`val` 프로퍼티를 `var` 프로퍼티로 오버라이드 가능(역방향은 불가):
...
기본 클래스 초기화가 먼저 발생 → 그다음 파생 클래스 초기화. 파생 클래스에서 선언된 프로퍼티는 기본 클래스 생성자 실행 시점에 아직 초기화되지 않은 상태
...
상위 클래스 함수·프로퍼티 호출 → `super` 키워드 사용
...
동일한 멤버의 여러 구현을 상속받는 경우 → 반드시 오버라이드 필요
...
주 생성자가 없는 파생 클래스 → 부 생성자에서 기본 타입 초기화 필요
...
- Kotlin 클래스는 기본적으로 final (안전 지향 설계 철학)
- 상속/오버라이딩 허용 → 명시적으로 `open` 사용
- `override` 수정자는 오버라이딩 시 필수
- 기본 클래스 생성자·초기화자에서 `open` 멤버 사용 회피 → 런타임 오류 방지
...
Kotlin 프로퍼티: 데이터 접근/변경용 함수를 별도로 작성하지 않고도 데이터 저장·관리 가능
- 모든 프로퍼티는 이름·타입·자동 생성된 `get()` 함수(getter) 보유
- 가변 프로퍼티는 `set()` 함수(setter)도 보유
- getter·setter를 접근자라 부름

---

### 프로퍼티 선언

프로퍼티는 가변(`var`) 또는 읽기 전용(`val`)
...
접근자는 메모리에 프로퍼티 값을 저장하기 위해 백킹 필드 사용 → Kotlin이 필요 시 자동 생성. `field` 키워드로 백킹 필드 참조
...
참고: Kotlin은 기본 getter/setter를 사용하거나 커스텀 접근자에서 `field`를 사용할 때만 백킹 필드 생성

---

### 백킹 프로퍼티

외부에서는 읽기 전용, 내부에서만 수정하는 패턴:
...
`const`로 읽기 전용 프로퍼티를 컴파일 타임 상수로 표시:
...
요구 사항:
- 최상위 프로퍼티 또는 `object`·companion object의 멤버
- `String` 또는 기본 타입으로 초기화 필요
- 커스텀 getter 불가
...
생성 후 초기화되는 프로퍼티에 `lateinit` 수정자 사용:
...
구현 세부 사항은 [위임된 프로퍼티 문서](https://kotlinlang.org/docs/delegated-properties.html) 참고
...
Kotlin 인터페이스: 추상 메서드 선언과 메서드 구현 모두 포함 가능. 추상 클래스와 달리 상태 저장 불가, 다만 추상이거나 접근자 구현을 제공하는 프로퍼티는 가능
...
인터페이스 프로퍼티: 추상이거나 접근자 구현 제공 가능. 단, 프로퍼티는 지원 필드가 없어 접근자에서 참조 불가:
...
인터페이스는 다른 인터페이스로부터 파생 가능 → 멤버 구현 제공 및 새 함수·프로퍼티 선언 가능. 이런 인터페이스를 구현하는 클래스는 누락된 구현만 정의하면 됨:
...
동일한 메서드를 가진 여러 인터페이스로부터 상속받는 경우 → 구현을 명시적으로 지정 필요:
...
`-jvm-default` 컴파일러 옵션으로 인터페이스 함수 컴파일 동작 제어:
...
클래스·객체·인터페이스·생성자·함수·프로퍼티 및 세터는 가시성 수정자 보유 가능. 게터는 항상 해당 프로퍼티와 동일한 가시성
...
함수·프로퍼티·클래스·객체·인터페이스는 패키지 내부 최상위 수준에서 직접 선언 가능
...
- `protected`: private과 동일 + 서브클래스에서도 볼 수 있음
- `internal`: 선언하는 클래스를 볼 수 있는 같은 모듈 내 모든 클라이언트에서 볼 수 있음
- `public`: 선언하는 클래스를 볼 수 있는 모든 클라이언트에서 볼 수 있음

중요: 외부 클래스는 내부 클래스의 private 멤버를 볼 수 없음

오버라이드 동작: 가시성을 명시적으로 지정하지 않고 `protected`·`internal` 멤버를 오버라이드하면 원본과 동일한 가시성 유지
...
- 명시적 키워드: 가시성 지정 시 `constructor` 키워드 사용

### 지역 선언

지역 변수·함수·클래스는 가시성 수정자 불가

### 모듈

`internal` 가시성 수정자: 멤버가 같은 모듈 내에서 볼 수 있음을 의미
...
Kotlin 확장: 상속·데코레이터 같은 디자인 패턴 없이도 클래스·인터페이스에 새 기능 추가 가능 → 직접 수정 불가능한 서드파티 라이브러리 작업 시 특히 유용

핵심 포인트: 확장은 원본 클래스를 수정하지 않음 → 멤버와 동일한 문법으로 새 함수 호출 가능

### 수신자

확장은 항상 수신자(확장된 타입의 인스턴스)에서 호출 → 수신자 뒤에 `.`, 그 뒤에 함수·프로퍼티 이름
...
수신자 타입 뒤에 `.`을 붙이고 함수 이름을 접두어로 사용 → 확장 함수 생성
...
확장 함수는 널 가능 수신자 타입 가능 → 값이 `null`일 때도 호출 가능:
...
해석 우선순위:
1. 멤버 함수가 우선 - 동일한 이름·시그니처를 가진 확장 함수보다 우선
2. 확장 함수는 정적으로 해석 - 컴파일 시 선언된 타입 기준
...
네임스페이스 혼잡을 피하기 위해 이름 없는 확장 함수 정의:
...
지원 필드 없이 확장 프로퍼티 생성 → 명시적인 게터·세터 필요
...
클래스 이름을 한정자로 사용해 동반 객체에 대한 확장 정의:
...
다른 클래스 내부에 선언된 확장은 두 개의 암시적 수신자 보유:
...
두 수신자 모두 동일한 이름의 멤버를 가질 때 한정된 `this` 문법 사용:
...
멤버 확장은 `open` 가능 → 서브클래스에서 오버라이드 가능:
...
핵심 포인트:
- 디스패치 수신자는 런타임에 해석 (가상 디스패치)
- 확장 수신자는 컴파일 시 해석 (정적 디스패치)

### 확장과 가시성 수정자

확장은 일반 함수와 동일한 가시성 수정자 사용:
...
Kotlin 데이터 클래스: 주로 데이터 보관용 → 컴파일러가 인스턴스 출력·비교·복사 등의 추가 멤버 함수를 자동 생성. `data` 키워드로 표시
...
컴파일러는 주 생성자에 선언된 모든 프로퍼티로부터 다음 멤버 파생:
...
- 주 생성자는 최소 하나의 매개변수 필요
- 모든 주 생성자 매개변수는 `val` 또는 `var`로 표시 필요
- 데이터 클래스는 abstract·open·sealed·inner 불가

### 클래스 본문에 선언된 프로퍼티

클래스 본문 내부에 선언된 프로퍼티(주 생성자가 아닌)는 생성된 함수에서 제외:
...
JVM에서 매개변수 없는 생성자 생성 → 기본값 제공 필요:
...
표준 라이브러리는 `Pair`·`Triple` 클래스 제공 → 코드 가독성을 위해 일반적으로 명명된 데이터 클래스 선호
...
봉인 클래스·인터페이스: 클래스 계층의 제어된 상속 제공. 봉인 클래스의 모든 직접 서브클래스는 컴파일 시점에 알려짐 → 봉인 클래스가 정의된 모듈·패키지 외부에서는 다른 서브클래스 등장 불가

주요 이점:
- 미리 정의된 유한한 서브클래스로 클래스 상속 제한
- 패턴 매칭·상태 관리를 위한 타입 안전 설계
...
봉인 클래스는 항상 추상 → 직접 인스턴스화 불가. 서브클래스를 위한 생성자 포함 가능:
...
생성자는 `protected`(기본값) 또는 `private` 가시성만 가능:
...
제한사항:
- `enum` 클래스는 봉인 클래스 확장 불가, 봉인 인터페이스는 구현 가능
- 간접 서브클래스에는 제한 없음
...
직접 서브클래스는 같은 소스 세트에 있어야 함. 봉인 클래스가 `expect`·`actual` 수정자를 사용하면 두 버전 모두 각각의 소스 세트에서 서브클래스 가능
...
Kotlin에서 클래스는 다른 클래스 내부에 중첩 가능. 클래스·인터페이스의 모든 조합이 중첩 가능
...
`inner` 키워드로 표시된 중첩 클래스는 외부 클래스의 멤버에 접근 가능 → 외부 클래스 객체에 대한 참조 보유:
...
참고: 내부 클래스에서 `this` 명확화는 한정된 `this` 표현식 참고

### 익명 내부 클래스

익명 내부 클래스 인스턴스는 객체 표현식으로 생성:
...
Java 함수형 인터페이스(단일 추상 메서드)는 람다 표현식 사용:
...
Kotlin 열거형 클래스: 상수 집합을 정의하는 타입 안전한 방법 제공 · 각 열거형 상수는 열거형 클래스의 객체 인스턴스
...
열거형 상수는 해당 메서드를 가진 자체 익명 클래스 선언 및 기본 메서드 오버라이드 가능:
...
참고: 상수 정의와 멤버 정의 구분에는 세미콜론 사용

### 인터페이스 구현

열거형 클래스는 인터페이스 구현 가능(클래스에서 파생은 불가):
...
모든 열거형 클래스는 기본적으로 `Comparable` 구현 → 상수들은 자연적인 순서로 정렬
...
제네릭 열거형 접근에는 인라인 구체화 타입 매개변수 사용:
...
- `name` 프로퍼티: 상수의 이름
- `ordinal` 프로퍼티: 상수의 위치(0부터 시작)

### 성능 참고

deprecated된 `enumValues<T>()` 대신 `enumEntries<T>()` 사용 → 매번 동일한 리스트 인스턴스 반환 → 불필요한 배열 생성 방지
...
Kotlin 객체: 클래스 정의와 인스턴스 생성을 한 단계로 처리 → 싱글톤·일회성 객체에 유용. 두 가지 접근 방식 제공:
- 객체 선언: 싱글톤 생성용
...
- 기존 클래스 동작을 일시적으로 수정
- 인터페이스·추상 클래스의 타입 안전 구현

### 객체 선언

`object` 키워드로 단일 인스턴스 생성:
...
핵심 포인트:
- 첫 접근 시 스레드 안전 초기화
- 상위 타입 보유 가능(인터페이스 구현 또는 클래스 상속)
- 지역적일 수 없음(함수 내부에 중첩 불가)
- 할당의 오른쪽에 사용 불가

#### 데이터 객체

`data` 수정자로 표시 → 컴파일러 생성 함수 확보:
...
- 다중 상위 타입 → 반환 타입 명시적 선언 필요

실제 타입에 선언된 멤버만 접근 가능
...
위임 패턴: 구현 상속의 좋은 대안 · Kotlin은 보일러플레이트 코드 없이 기본 지원

### 기본 위임

클래스는 `by` 키워드로 모든 public 멤버를 지정된 객체에 위임 → 인터페이스 구현 가능:
...
상위 타입 목록의 `by` 절: `b`가 내부적으로 저장 → 컴파일러가 `b`로 전달하는 `Base`의 모든 메서드 생성

### 위임된 인터페이스의 멤버 오버라이딩

위임하는 클래스에서 인터페이스 멤버 오버라이드 가능 → 컴파일러는 위임 객체의 구현 대신 `override` 구현 사용:
...
오버라이드된 멤버는 위임 객체 자체의 구현에서 호출되지 않음. 위임은 자체 인터페이스 멤버 구현만 접근 가능:
...
- [인터페이스](https://kotlinlang.org/docs/interfaces.html)
- [상속](https://kotlinlang.org/docs/inheritance.html)
- [위임된 프로퍼티](https://kotlinlang.org/docs/delegated-properties.html)
...
위임된 프로퍼티: 일반적인 프로퍼티 패턴을 한 번 구현해 재사용 가능. 문법:
```kotlin
val/var <프로퍼티 이름>: <타입> by <표현식>
```

`by` 뒤의 표현식은 `getValue()`·`setValue()` 연산을 처리하는 위임

### 기본 위임 구현

위임 클래스는 `getValue()`·`setValue()` 메서드 필요:
...
`Delegates.observable()`은 변경 시 알림:
...
Vetoable 변형: 할당을 가로채서 거부하려면 `Delegates.vetoable()` 사용

### 다른 프로퍼티에 위임

프로퍼티는 `::` 문법으로 다른 프로퍼티에 위임 가능:
...
위임 생성 커스터마이즈에는 `provideDelegate` 사용:
...
인라인 값 클래스: 런타임 오버헤드 없이 값을 래핑하도록 설계된 Kotlin의 특수한 클래스. 런타임에 힙의 객체가 아닌 기본 타입으로 표현 → 기본 타입 래핑의 성능 문제 해결

#### 핵심 개념

인라인 클래스의 데이터는 인라인 함수가 호출 위치에 인라인되는 것과 유사하게 사용 위치에 인라인됨
...
- 주 생성자에서 초기화된 단일 프로퍼티 필요
- 런타임에 인스턴스는 이 단일 프로퍼티만으로 표현
...
Kotlin 컴파일러는 각 인라인 클래스마다 래퍼를 유지하지만 성능을 위해 언박싱된(기본 타입) 표현 선호. 인라인 클래스는 다른 타입으로 사용될 때마다 박싱됨
...
인라인 클래스가 제네릭 타입 매개변수를 가지면 컴파일러는 이를 `Any?` 또는 상한으로 매핑:
...
인라인 클래스는 값·래퍼 모두로 표현될 수 있어 참조 동등성 비교 불가

### 맹글링

플랫폼 시그니처 충돌 방지를 위해 함수 이름이 맹글링됨:
...
해결책: 두 번째 함수는 `public final void compute-<hashcode>(int x)`로 표현되어 충돌 해결

#### Java 코드에서 호출

맹글링을 수동으로 비활성화하려면 `@JvmName` 사용:
...
참고: 기본적으로 인라인 클래스는 언박싱된 표현을 사용해 Java에서 접근하기 어려움. 박싱된 표현은 상호운용성 가이드 참고

### 인라인 클래스 vs 타입 별칭

- 타입 안전성: 인라인 클래스는 새 타입 도입 / 타입 별칭은 기존 타입의 별칭일 뿐
- 할당 호환성: 인라인 클래스는 불가 / 타입 별칭은 가능(기본 타입과)
...
- [열거형 클래스](https://kotlinlang.org/docs/enum-classes.html)
- [중첩 및 내부 클래스](https://kotlinlang.org/docs/nested-classes.html)
...
함수형 인터페이스(단일 추상 메서드(SAM) 인터페이스): 단 하나의 추상 멤버 함수만 가진 인터페이스. 여러 비추상 멤버 함수는 가능하나 추상 멤버는 정확히 하나여야 함

### 선언

함수형 인터페이스 선언에는 `fun` 수정자 사용:
...
SAM 변환: 명시적 클래스 구현 대신 람다 표현식 사용 가능 → 코드가 더 간결·가독성 향상
...
Kotlin 1.6.20부터 함수형 인터페이스 생성자에 대한 호출 가능 참조 사용 가능:
...
바이너리 호환성 유지에는 이전 함수를 deprecated로 표시:
```kotlin
...
비교:
- 타입 생성: 타입 별칭은 단지 이름일 뿐(새 타입 없음) / 함수형 인터페이스는 새 타입 생성
- 멤버: 타입 별칭은 하나만 / 함수형 인터페이스는 여러 비추상 + 하나의 추상
- 확장: 타입 별칭은 인터페이스 특정 확장 제공 불가 / 함수형 인터페이스는 특정 확장 제공 가능
- 상속: 타입 별칭은 인터페이스 구현/확장 불가 / 함수형 인터페이스는 다른 인터페이스 구현/확장 가능
- 런타임 비용: 타입 별칭은 더 낮음 / 함수형 인터페이스는 잠재적으로 더 높음(변환 필요)

#### 각각 언제 사용할까

- 타입 별칭: API가 특정 매개변수·반환 타입을 가진 어떤 함수든 받아들일 때 사용
- 함수형 인터페이스: 함수 타입 시그니처로 표현할 수 없는 복잡한 계약·연산을 가진 엔티티에 사용
...
this 표현식: Kotlin에서 현재 수신자를 나타냄. `this` 키워드는 컨텍스트에 따라 다른 스코프 참조

### 사용 컨텍스트

#### 클래스에서
클래스의 멤버에서 `this`는 해당 클래스의 현재 객체 참조

#### 확장 함수와 함수 리터럴에서
확장 함수나 수신자가 있는 함수 리터럴에서 `this`는 점의 왼쪽에 전달된 수신자 매개변수를 나타냄

#### 기본 동작
`this`에 한정자가 없으면 가장 안쪽 둘러싸는 스코프 참조

---

### 한정된 this

외부 스코프(클래스·확장 함수·레이블이 지정된 수신자가 있는 함수 리터럴)에서 `this`에 접근하려면 `this@label` 구문 사용. `@label`은 스코프의 레이블
...
`this`에서 멤버 함수를 호출할 때 `this.` 부분 생략 가능. 단, 같은 이름의 비멤버 함수가 존재하면 대신 호출될 수 있어 주의 필요
...
Kotlin: 런타임에 객체가 특정 타입인지 확인하고 다른 타입으로 변환 가능. 타입 검사는 객체의 종류 확인, 타입 캐스트는 객체를 다른 타입으로 변환 시도

---

### `is`와 `!is` 연산자로 검사

`is` 연산자(부정은 `!is`)로 런타임에 객체가 타입과 일치하는지 확인:
...
`is`·`!is`로 객체가 하위 타입과 일치하는지도 확인 가능:
...
컴파일러는 불변 값이면 객체를 자동 캐스트하고 암시적(안전한) 캐스트를 자동 삽입:
...
부정 검사가 반환으로 이어지면 컴파일러는 캐스트가 안전함을 파악:
...
다른 타입에 대한 `or` 연산자 사용 시 공통 상위 타입 사용:
...
스마트 캐스트 정보는 `catch`·`finally` 블록으로 전달:
...
스마트 캐스트는 컴파일러가 변수 불변을 보장할 수 있을 때만 작동:
...
`as` 연산자로 캐스트 실패 시 `ClassCastException` 발생:
...
다운캐스팅은 명시적 캐스트 연산자 필요. 실패 시 안전하게 `null` 반환하려면 `as?` 사용:
...
### 관련 주제
- [제네릭 타입 검사와 캐스트](https://kotlinlang.org/docs/generics.html#generics-type-checks-and-casts)
- [리플렉션](https://kotlinlang.org/docs/reflection.html)
- [제어 흐름](https://kotlinlang.org/docs/control-flow.html)
- [null 안전성](https://kotlinlang.org/docs/null-safety.html)
- [인라인 함수](https://kotlinlang.org/docs/inline-functions.html)
- [위임된 프로퍼티](https://kotlinlang.org/docs/delegated-properties.html)
- [가시성 수정자](https://kotlinlang.org/docs/visibility-modifiers.html)
...
정의: 두 객체가 동일한 내용·구조인지 확인

연산자: `==`와 부정 `!=`

작동 방식: `a == b` 표현식은 다음과 같이 변환:
```kotlin
a?.equals(b) ?: (b === null)
```
...
핵심 포인트:
- `equals()` 함수는 기본적으로 `Any` 클래스에서 상속
- 기본 구현은 참조 동등성 제공
- 값 클래스·데이터 클래스는 구조적 동등성을 위해 자동으로 `equals()` 재정의
- 비데이터 클래스는 구조적 동등성 구현에 수동 재정의 필요
- `null`과의 명시적 비교(`a == null`)는 자동으로 `a === null`로 변환
...
모범 사례: `equals()` 재정의 시 일관성을 위해 `hashCode()`도 재정의

---

### 참조 동등성

정의: 메모리 주소를 확인해 두 객체가 동일한 인스턴스인지 확인

연산자: `===`와 부정 `!==`

작동 방식: `a === b`는 `a`·`b`가 동일한 객체를 가리키는 경우에만 `true`로 평가
...
참고: 런타임에 기본 타입(예: `Int`)의 경우 `===` 동등성은 `==`와 동일

---

### 부동 소수점 숫자 동등성

IEEE 754 동작: 피연산자가 `Float`·`Double`로 정적으로 타입 지정되면 IEEE 754 표준 따름

비정적 타이핑: 부동 소수점으로 정적 타입이 지정되지 않으면 구조적 동등성 적용:
- `NaN`은 자기 자신과 같음
...
두 배열이 같은 순서로 같은 요소가 있는지 비교하려면 다음 사용:
```kotlin
contentEquals()
```
...
### 관련 주제
- [널 안전성](https://kotlinlang.org/docs/null-safety.html)
- [제네릭: in, out, where](https://kotlinlang.org/docs/generics.html)
- [부동 소수점 숫자 비교](https://kotlinlang.org/docs/numbers.html#floating-point-numbers-comparison)
- [배열 비교](https://kotlinlang.org/docs/arrays.html#compare-arrays)
...
Kotlin은 타입에 미리 정의된 연산자의 커스텀 구현 허용. 연산자는 미리 정의된 기호 표현(예: `+`, `*`)과 우선순위 보유. 연산자 구현에는 해당 타입에 특정 이름을 가진 멤버 함수·확장 함수 제공
...
연산자 오버로드를 오버라이드할 때 `operator` 생략 가능:
...
`inc()`·`dec()` 함수는 값을 반환해야 하며 객체 변경 금지
...
모든 비교는 `compareTo()`가 `Int` 반환 필요

#### 프로퍼티 위임 연산자

`provideDelegate`·`getValue`·`setValue` 연산자 함수는 위임된 프로퍼티 섹션에서 설명

### 명명된 함수를 위한 중위 호출

중위 함수 호출로 커스텀 중위 연산 시뮬레이션 가능
...
구조 분해 선언: 객체를 한 번에 여러 변수로 분해 가능:

```kotlin
val (name, age) = person
```

독립적으로 사용할 수 있는 두 개의 새 변수(`name`, `age`) 생성

### 작동 방식

구조 분해 선언은 `componentN()` 함수 호출로 컴파일:
...
핵심 포인트:
- `componentN()` 함수는 `operator` 키워드로 표시 필요
- 필요한 component 함수가 있으면 모든 객체 구조 분해 가능
...
데이터 클래스는 자동으로 `componentN()` 함수 선언
...
변수들은 각 요소의 `component1()`·`component2()`에서 값 획득
...
건너뛴 컴포넌트에 대해서는 `componentN()` 함수가 호출되지 않음
...
타입 별칭: 기존 타입에 대한 대체 이름 제공. 새 타입 도입 없이 긴 타입 이름(특히 제네릭 타입) 축약에 유용
...
타입 별칭은 새 타입을 생성하지 않음 → 기본 타입과 동등. Kotlin 컴파일러는 컴파일 중 이를 확장:
...
다른 선언 내부에 타입 별칭 정의 가능, 단 중요한 제한 존재:
...
- 기존의 모든 타입 별칭 규칙 준수 필요
- 참조된 타입이 허용하는 것보다 더 많은 가시성 노출 불가
- 중첩 클래스와 동일한 스코프(클래스 내부에 정의, 동일한 이름의 부모 별칭 숨김)
- 가시성 제어를 위해 `internal`·`private`으로 표시 가능
...
어노테이션: 코드 요소에 메타데이터를 첨부하는 태그. 도구·프레임워크가 이 메타데이터를 컴파일·런타임에 처리해 다양한 작업 수행 → 보일러플레이트 코드 생성, 코딩 표준 적용, 문서 작성 등 단순화

---

### 선언

어노테이션은 `annotation` 키워드로 선언:
...
#### 메타 어노테이션
추가 프로퍼티는 메타 어노테이션으로 지정 가능:

- `@Target`: 어노테이션할 수 있는 요소 지정(클래스·함수·프로퍼티·표현식)
- `@Retention`: 어노테이션이 컴파일된 파일에 저장되고 런타임에 보이는지 지정
- `@Repeatable`: 단일 요소에 동일한 어노테이션을 여러 번 사용 가능하게 함
...
참고: 어노테이션 매개변수는 널 가능 불가(JVM 제한)

#### 어노테이션을 매개변수로 사용
다른 어노테이션의 매개변수로 어노테이션을 사용할 때는 `@` 문자 생략:
...
Java와 달리 Kotlin은 임의의 코드에서 어노테이션 생성자 호출 가능:
...
어노테이션은 람다에 사용 가능 → 생성된 `invoke()` 메서드에 적용:
...
프로퍼티·주 생성자 매개변수 어노테이션 시 어떤 Java 요소가 어노테이션을 받는지 지정하는 구문 사용:
...
#### 파일 수준 어노테이션
파일 맨 위 package 지시자 전에 어노테이션 배치:
...
#### `all` 메타 대상
매개변수·프로퍼티·필드·getter·setter 매개변수·RECORD_COMPONENT(해당되는 경우)에 어노테이션 적용:
...
#### Java 어노테이션의 명명된 인자
명명된 인자 구문 사용(Java에서 매개변수 순서가 정의되지 않음):
...
단일 요소에 여러 번 사용하려면 `@kotlin.annotation.Repeatable`로 어노테이션 표시:
...
#### 반복 가능한 어노테이션 접근
리플렉션에는 `KAnnotatedElement.findAnnotations()` 함수 사용
...
리플렉션: 런타임에 프로그램의 구조를 검사할 수 있게 해주는 언어·라이브러리 기능의 집합. 함수·프로퍼티는 Kotlin에서 일급 시민 → 이들을 검사하는 능력은 함수형·반응형 스타일 사용 시 필수

참고: Kotlin/JS는 리플렉션 기능에 대한 제한적 지원 제공

---

### JVM 의존성

JVM 플랫폼에서 Kotlin 컴파일러 배포판: 리플렉션을 사용하지 않는 애플리케이션의 런타임 라이브러리 크기 축소를 위해 `kotlin-reflect.jar`를 별도 아티팩트로 포함
...
다른 설정은 `kotlin-reflect.jar`가 프로젝트의 클래스패스에 있는지 확인. 필요 없어 제외하려면 `-no-reflect` 컴파일러 옵션 사용
...
클래스 리터럴 구문으로 Kotlin 클래스에 대한 런타임 참조 획득:
...
JVM 참고: Kotlin 클래스 참조는 Java 클래스 참조와 다름. Java 클래스 참조는 `.java` 프로퍼티 사용:
...
수신자 타입에 관계없이 객체의 정확한 런타임 클래스 획득

---

### 호출 가능 참조

함수·프로퍼티·생성자에 대한 참조는 호출하거나 함수 타입 인스턴스로 사용 가능. 공통 슈퍼타입은 `KCallable<out R>`(`R`은 반환 타입)

#### 함수 참조

명명된 함수를 직접 호출하거나 함수 타입 값으로 사용:
...
`::isOdd`는 `(Int) -> Boolean` 타입의 값

함수 참조는 매개변수 수에 따라 `KFunction<out R>` 하위 타입에 속함(예: `KFunction3<T1, T2, T3, R>`)

##### 오버로드된 함수

오버로드된 함수 사용 시 컨텍스트에서 예상 타입이 알려져야 함:
...
클래스 멤버·확장 함수는 한정된 구문 사용: `String::toCharArray`
...
`KProperty0<Int>` 반환. 값을 읽으려면 `get()`, 프로퍼티 이름은 `name` 사용

##### 가변 프로퍼티

가변 프로퍼티는 `::y`가 `set()` 메서드가 있는 `KMutableProperty0<Int>` 반환:
...
표준 라이브러리는 Kotlin·Java 리플렉션 객체 간 매핑을 위한 확장을 `kotlin.reflect.jvm` 패키지에 포함:
...
클래스 이름과 함께 `::` 연산자로 메서드·프로퍼티처럼 생성자 참조:
...
생성자 참조는 매개변수 수에 따라 `KFunction<out R>` 하위 타입으로 타입 지정
...
바운드 참조는 수신자가 첨부되어 매개변수 목록에서 제거:
...
외부 클래스 인스턴스를 제공해 내부 클래스 생성자에 대한 바운드 참조 획득:
...
관련 주제: [코루틴](https://kotlinlang.org/docs/coroutines-overview.html) · [구조 분해 선언](https://kotlinlang.org/docs/destructuring-declarations.html)
...
Kotlin 클래스는 Java와 유사하게 타입 매개변수 보유 가능. 타입 인수는 문맥에서 추론 가능 → 더 깔끔한 문법:
...
Java의 제네릭 타입은 불변 → `List<String>`은 `List<Object>`의 서브타입이 아님. 런타임 오류는 방지되지만 유연성 저하
...
Kotlin은 타입 매개변수 선언 위치에서 변성 어노테이션으로 이 문제 해결:

##### `out` 수정자 (공변성)

타입 매개변수를 생산 전용(반환만, 소비 안 함)으로 지정:
...
타입 매개변수를 소비 전용(받기만, 반환 안 함)으로 지정:
...
생산·소비를 모두 해야 하는 클래스(예: `Array`)는 호출 위치에서 타입 프로젝션 사용:
...
Java 상호운용성을 위해 `T & Any`로 제네릭 타입을 명확히 널이 아닌 것으로 선언:
...
타입 정보는 런타임에 소거 → `Foo<Bar>`·`Foo<Baz?>`의 인스턴스는 모두 `Foo<*>`가 됨
...
`inline` 함수만 런타임 검사를 위해 구체화된 타입 지원:
...
`foo as List<String>` 같은 구체적인 제네릭 타입으로의 캐스트는 런타임에 검사되지 않음:
...
다른 타입이 명시적으로 지정될 때 타입 인수를 자동으로 추론하려면 `_` 사용:
...
- `out T`: 공변성(생산 전용) · 예: `Source<out T>`
- `in T`: 반공변성(소비 전용) · 예: `Comparable<in T>`
- 타입 프로젝션: 사용 위치 변성 · 예: `Array<out Any>`
- 스타 프로젝션: 알 수 없는 타입 인수 · 예: `List<*>`
- 구체화: 인라인 함수의 런타임 타입 정보 · 예: `inline fun <reified T>`
- 상한: 타입 매개변수 제한 · 예: `<T : Comparable<T>>`
