...
CDP `Page` 도메인은 탭(페이지) 단위의 생명주기 제어와 상태 조회를 담당함. 네비게이션, 로드 이벤트, 스크린샷/PDF 캡처, 다이얼로그 처리, 프레임 트리 조회 등 "탭 자체"를 다루는 기능이 여기에 모여 있음.
...
자동화 스크립트는 보통 `navigate()` 호출 후 `loadEventFired` 또는 `lifecycleEvent`(`networkIdle`)를 기다린 다음 DOM 조작을 시작함.
...
크로스 오리진 iframe은 Site Isolation 정책에 따라 별도 렌더링 프로세스(OOPIF)에서 실행될 수 있어, 해당 프레임 내부의 DOM/Accessibility 정보를 얻으려면 [Target 도메인](./target.md)으로 별도 세션을 맺어야 하는 경우가 있음. 자세한 내용은 [OOPIF / Site Isolation](../automation-internals/oopif-site-isolation.md) 문서 참고.

---

## 5. 다이얼로그 처리

페이지가 `window.alert()`, `confirm()`, `prompt()`를 호출하면 브라우저 UI 스레드가 블로킹되어 다른 CDP 명령도 멈출 수 있음. 이를 막기 위해 `Page.javascriptDialogOpening` 이벤트를 구독하고 `Page.handleJavaScriptDialog`로 즉시 응답하는 패턴을 사용함.
...
- `Page` 도메인은 탭 단위 네비게이션, 로드 생명주기, 캡처, 다이얼로그 처리를 담당함
- 로드 완료 판단은 `loadEventFired` 또는 `lifecycleEvent`로 감지하는 것이 일반적임
- 프레임 트리는 `getFrameTree`로 조회하며, 크로스 오리진 자식 프레임은 별도 `Target` 세션이 필요할 수 있음
- `alert`/`confirm` 같은 블로킹 다이얼로그는 이벤트 구독 후 즉시 응답하지 않으면 세션 전체가 멈출 수 있음
