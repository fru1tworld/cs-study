...
Content Script는 확장 프로그램이 방문 중인 웹 페이지의 DOM에 직접 접근할 수 있게 해주는 JavaScript로, 페이지의 렌더링 프로세스(Isolated World)에서 실행됨 → 확장 프로그램 백그라운드(Service Worker)와 페이지 자체 JavaScript 사이의 다리 역할을 함.
...
Content Script는 페이지와 같은 DOM을 보고 조작할 수 있지만, JavaScript 실행 컨텍스트(변수, 함수, 프로토타입 체인)는 페이지와 완전히 분리되어 있음.

- DOM 트리: 공유됨 (같은 노드를 읽고 쓸 수 있음)
...
페이지의 JS 변수/함수에 직접 접근해야 한다면 `<script>` 태그를 페이지에 주입해 Main World에서 실행하거나, `world: "MAIN"` 옵션(scripting API)을 사용해야 함.
...
- 정적(Static)
  - 선언 위치: `manifest.json`의 `content_scripts` 필드
  - 특징: URL 패턴에 매칭되는 모든 페이지에 자동 주입, 코드 변경 시 확장 재로드 필요
- 동적(Dynamic)
  - 선언 위치: `chrome.scripting.executeScript()` 등 런타임 API
  - 특징: 특정 조건(버튼 클릭 등)에서만 원하는 탭에 주입, 세밀한 제어 가능 ([scripting API](./1_scripting_and_service_worker.md) 참고)
...
지속적인 양방향 통신이 필요하면 `chrome.runtime.connect()`로 포트를 열어 사용함.

---

### 5. 페이지 JS와의 통신 (Main World 브리지)

Isolated World는 페이지 JS 변수에 직접 접근할 수 없으므로, 필요하면 `window.postMessage` 또는 커스텀 DOM 이벤트로 우회함.
...
- Content Script는 페이지 DOM에는 접근하지만 페이지 JS 컨텍스트와는 분리된 Isolated World에서 실행됨.
- 정적 주입(`manifest.json`)과 동적 주입(`scripting` API) 두 방식이 있음 → 세밀한 제어가 필요하면 동적 주입을 씀.
- SW와는 메시지 패싱으로, 페이지 JS와는 `postMessage`/DOM 이벤트로 통신함.
- `run_at` 값으로 주입 시점을 제어하고, `all_frames`로 iframe 포함 여부를 결정함.
...
- [scripting API](./1_scripting_and_service_worker.md)
...
`chrome.scripting`은 Manifest V3에서 Content Script를 런타임에 동적으로 주입/제거하고, CSS를 삽입/제거하는 API임. Manifest V2의 `chrome.tabs.executeScript`를 대체하며, 문자열 코드 실행(`code` 파라미터)을 없애고 함수/파일 기반 주입만 허용해 CSP·보안성을 강화함.
...
Content Script의 Isolated World와 달리 `world: "MAIN"`으로 실행한 코드는 페이지 JS와 같은 컨텍스트를 공유하므로, 페이지가 정의한 함수/객체에 바로 접근할 수 있음. 다만 이 경우 페이지의 CSP 정책이 그대로 적용됨.

---

### 5. 권한 요구사항

- `"scripting"`: `chrome.scripting` API 자체 사용 시 필요
...
`activeTab`을 쓰면 광범위한 `host_permissions`를 요청하지 않고도 "사용자가 명시적으로 클릭한 탭"에만 최소 권한으로 스크립트를 주입할 수 있음 → 스토어 심사와 사용자 신뢰 측면에서 권장됨.
...
매니페스트 재배포 없이 조건에 따라 Content Script를 등록/해제할 수 있음 → 사용자가 설정에서 기능을 켜고 끄는 형태의 확장 프로그램에 유용함.
...
- 문자열 코드 실행
  - Manifest V2 (`chrome.tabs.executeScript`): `code: "..."` 문자열 직접 실행 가능
  - Manifest V3 (`chrome.scripting.executeScript`): 제거됨 — `func` 또는 `files`만 허용
- CSP 영향
  - Manifest V2: 문자열 eval에 준하는 위험 존재
  - Manifest V3: 함수 직렬화 방식으로 보안성 강화
- 반환값 처리
  - Manifest V2: 콜백 기반
  - Manifest V3: Promise 기반, `InjectionResult[]` 반환

---

### 8. 요약

- `chrome.scripting`은 Manifest V3에서 동적 스크립트/CSS 주입을 담당하는 API임.
- `func`/`files` 기반 주입만 허용해 문자열 eval 위험을 없앰.
- `world: "MAIN"`으로 페이지 JS 컨텍스트에 직접 접근하는 실행도 가능함.
- `activeTab` 권한과 조합하면 광범위한 host 권한 없이도 최소 권한 원칙을 지킬 수 있음.
...
- [Content scripts](./1_scripting_and_service_worker.md)
...
Manifest V3부터 확장 프로그램의 백그라운드 실행 환경은 상시 상주하던 Background Page(Manifest V2) 대신 이벤트 기반으로 실행/종료되는 Service Worker로 바뀜. Service Worker는 필요할 때 깨어나 이벤트를 처리하고, 일정 시간 유휴 상태가 지속되면 브라우저가 종료해 메모리를 회수함 → 확장 프로그램의 코드가 "항상 살아있다"고 가정할 수 없다는 의미이며, 상태 관리 방식 자체를 바꿔야 함.
...
SW가 종료되면 메모리상의 전역 변수, 타이머, 진행 중인 클로저 상태가 모두 사라짐. 따라서 세션 간 유지해야 하는 데이터는 다음 방법으로 영속화해야 함.
...
- Manifest V3 Service Worker는 이벤트 기반으로 시작/종료되며 상시 상주하지 않음.
- 전역 변수/타이머에 의존한 상태 관리는 동작하지 않으므로 `chrome.storage`, `chrome.alarms`로 대체해야 함.
- 이벤트 리스너는 최상위 스코프에서 동기적으로 등록해야 재시작 후에도 이벤트를 수신할 수 있음.
- `chrome.debugger` 연결, 진행 중인 fetch, 열린 메시지 포트는 SW 종료를 지연시키는 요인임.
