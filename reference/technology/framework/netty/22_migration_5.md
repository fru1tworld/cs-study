...
새 `Buffer` API는 더 이상 그렇게 동작하지 않으며, 용량과 최대 용량의 구분도 없습니다. 코드는 적절한 크기의 버퍼를 할당하거나(크기 인자가 이제 필수), 필요할 때 `ensureWritable()`을 호출해야 합니다. `ensureWritable()` 메서드는 단일 메모리 복사로 compaction(과거 `discardReadBytes()`와 동일)과 확장을 동시에 수행할 수 있는 인자를 받을 수 있게 되었습니다.

### 어댑터

두 API 사이를 변환할 수 있는 어댑터 세트가 포함되어 있어, 모든 핸들러와 관련 코드가 마이그레이션될 때까지 두 API가 공존할 수 있게 합니다.

`ByteBufBuffer.wrap()` 메서드는 `ByteBuf` 인스턴스를 받아 새 API의 `Buffer` 인스턴스를 반환합니다. 반대로 `ByteBufAdaptor.intoByteBuf()` 메서드는 `Buffer`를 받아 `ByteBuf`를 반환합니다. 두 메서드는 가능한 한 효율적인 방법으로 변환을 수행하며, 반복 변환 시 어댑터가 중첩되지 않도록 서로 상쇄됩니다.

서로 다른 API에 의존하는 핸들러 사이에 삽입할 수 있는 `BufferConversionHandler`도 포함되어 있습니다. 단, `BufferConversionHandler`는 `ByteBufHolder`나 `BufferHolder` 객체는 변환하지 못합니다. `Buffer`나 `ByteBuf` 인스턴스를 포함하는 객체는 커스텀 `MessageToMessageCodec` 등을 사용해 변환해야 합니다.
...
`retain()`을 사용하던 대부분의 위치는 슈퍼/서브 클래스의 무조건적 `release()` 효과를 상쇄하기 위한 것이었습니다. 이런 경우 `split()`으로 대체할 수 있습니다. 일반적으로 버퍼의 readable 부분을 전달하는 패턴이며, `split()`은 두 개의 버퍼를 생성하고 각각이 별도로 닫혀야 합니다.
...
netty 4.1.x에서는 `voidPromise()` 메서드로 특수한 `ChannelPromise` 구현을 얻어 다양한 IO 연산(`write` 등)에 사용함으로써 객체 생성 수를 줄일 수 있었습니다. 취지는 좋았지만 이 특수 `ChannelPromise`는 실제로 많은 문제를 야기했습니다.
...
* `EventLoop`을 실수로 블록하기 쉬운 구조였기 때문에, `Future` 인터페이스에서 모든 블로킹 메서드가 제거되었습니다. `EventLoop` 외부에서 블로킹이 필요하다면 `Future.asStage()`로 변환해야 합니다. 반환된 `FutureCompletionStage`가 블로킹 메서드를 제공합니다.
...
이러한 문제를 검토한 결과, 사용자가 실제로 원하는 것은 비즈니스 로직 처리를 위해 수신 메시지를 다른 스레드에서 처리하는 것임을 알게 되었습니다. 이는 커스텀 구현으로 처리하는 편이 낫습니다. 무엇이 언제 소멸될 수 있는지는 사용자가 더 잘 알기 때문입니다.
...
netty 4.x에서는 전송별로 서로 다른 `EventLoopGroup`/`EventLoop` 구현이 있었습니다(예: `NioEventLoopGroup`, `EpollEventLoopGroup` 등). netty 5에서는 이를 단일 구현인 `MultiThreadEventLoopGroup`으로 통합했습니다. `MultiThreadEventLoopGroup`은 전송에 특화된 `IoHandlerFactory`를 인자로 받습니다(예: `NioHandler.newFactory()`, `EpollHandler.newFactory()` 등). 이 변경은 많은 이점을 제공합니다. 예를 들어 `MultiThreadEventLoopGroup`을 상속해 데코레이터나 커스텀 메트릭을 추가하기 쉬우며, 동일한 구현을 다른 전송에서도 재사용할 수 있습니다. 이는 JDK의 `ThreadPoolExecutor`가 제공하는 커스터마이징 방식과 매우 유사합니다.

### `EventLoopGroup.isCompatible(...)` 메서드 추가

사용 전에 `Channel` 서브타입이 `EventLoopGroup`/`EventLoop`과 호환되는지 미리 확인할 수 있습니다. 적절한 `Channel` 서브타입을 선택하는 데 도움이 됩니다.

### `EventLoop.registerForIo(...)`와 `EventLoop.deregisterForIo` 추가

`EventLoop` 인터페이스에 `Channel` 등록·해제용 새 메서드가 추가되었습니다. 이 메서드는 사용자가 직접 호출하는 것이 아니라 `Channel` 구현체 내부에서만 사용해야 합니다.

## 거의 쓰이지 않는 코덱이 Netty Contrib로 이동

코드베이스를 가볍게 유지하고 유지보수 부담을 줄이기 위해 다음 코덱과 핸들러가 [Netty Contrib](https://github.com/netty-contrib)으로 이동했습니다.
