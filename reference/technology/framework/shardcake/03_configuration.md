...
파드 쪽에서 설정할 수 있는 항목은 다음과 같음.

- `numberOfShards`: 샤드 개수

> **샤드 개수는 어떻게 정할까?**
>
> `numberOfShards`는 엔티티 ID에서 샤드 ID를 계산하는 데 사용 → 모든 파드에서 같은 값이어야 하며 앱 실행 도중에는 변경 불가.
>
> 이 값이 파드 수보다 작으면 일부 파드는 샤드가 없어(엔티티를 호스팅하지 못해) 비효율적. 값이 너무 작으면 두 파드 간 샤드 1개 차이가 상당히 커져서 파드마다 호스팅하는 엔티티 수가 불균형. 반대로 샤드가 너무 많으면 파드마다 많은 샤드를 떠안아 불필요한 오버헤드 발생.
>
> 좋은 경험칙(rule of thumb): 샤드 개수를 예상되는 최대 파드 수의 10배로 설정.
...
> **버전이란?**
>
> 롤링 업데이트(다운타임 없이 파드를 하나씩 업그레이드)를 할 때는 곧 중지될 파드에 샤드를 할당하지 않는 것이 바람직. 이상적으로는 전체 과정에서 각 샤드를 한 번만 이동.
>
> `serverVersion`이 있으면 Shard Manager는 어떤 파드가 오래된 것이고 어떤 파드가 새것인지 구분 → 새 파드를 골라 샤드를 할당.

- `entityMaxIdleTime`: 이 시간 동안 메시지를 전혀 받지 못하면 엔티티가 중지되는 비활성 시간

> **종료 메시지(Termination Message)**
>
> `registerEntity`는 `terminationMessage`라는 선택적 매개변수를 받음. 여기에 엔티티가 중지되기 전에(리밸런스 때문이든 비활성 때문이든) 엔티티에 보낼 메시지를 정의.
>
> 종료 메시지를 지정하지 않으면 엔티티 큐가 그냥 종료. 다만 마지막 메시지를 처리한 뒤 엔티티를 "깔끔하게" 중지하고 싶다면, 종료 메시지를 정의하고 그 메시지를 받은 뒤(`ZIO.interrupt`를 호출해) 직접 동작을 중지하면 됨.
>
> 종료 메시지에는 종료가 완료되었음을 알리기 위해 직접 완료(complete)해야 하는 프로미스(promise)가 들어 있어야 함. [예제는 여기](https://github.com/devsisters/shardcake/tree/series/2.x/examples/src/main/scala/example/complex) 참고.
...
Shard Manager 쪽에서 설정할 수 있는 항목은 다음과 같음.
...
>
> 리밸런스 비율은 새 파드에 너무 많은 샤드가 한꺼번에 할당되는 것을 방지. 곧바로 완벽한 분배에 도달하는 대신 여러 번 반복하며 새 파드가 새 샤드를 감당할 여유를 부여. 엔티티를 시작하는 데 성능 비용이 드는 경우(예: 상태 로딩)나 한 번에 너무 많은 엔티티를 시작하고 싶지 않을 때 특히 유용.
>
> 파드가 떠날 때는 그 샤드를 즉시 리밸런스해야 하므로 이 비율은 적용되지 않음.
