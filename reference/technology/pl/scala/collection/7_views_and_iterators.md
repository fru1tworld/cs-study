...
이전 버전에서는 `(1 to 10)`이 만드는 범위(range)가 뷰처럼 동작했기 때문에, map의 결과 역시 뷰였습니다. 즉 어떤 요소도 계산되지 않았고, 그 결과 어떤 액터도 만들어지지 않은 것입니다! 표현식 전체의 범위를 강제했다면 액터가 만들어졌겠지만, 액터를 일하게 만들기 위해 그렇게 해야 한다는 것은 전혀 직관적이지 않습니다.

이런 뜻밖의 상황을 피하기 위해, 현재의 스칼라 컬렉션 라이브러리는 더 일관된 규칙을 따릅니다. 지연 리스트(lazy list)와 뷰를 제외한 모든 컬렉션은 엄격합니다. 엄격한 컬렉션에서 지연 컬렉션으로 가는 유일한 방법은 `view` 메서드이고, 되돌아오는 유일한 방법은 `to`입니다. 따라서 위의 `actors` 정의는 이제 기대한 대로 동작해서, 액터 10개를 만들고 시작시킵니다. 예전의 그 뜻밖의 동작을 다시 얻으려면 명시적으로 `view` 메서드 호출을 추가해야 합니다.
