...
- 관련 구현
  - `inline`은 Scala 3에서 [구현](../metaprogramming/inline.html)
  - 인용(Quotes) `'{ ... }`과 스플라이스(splices) `${ ... }`는 Scala 3에서 [구현](../metaprogramming/macros.html)
  - [TASTy 리플렉션(TASTy reflect)](../metaprogramming/reflection.html): 인용된 코드(quoted code)를 검사·생성하기 위한 더 복잡한 트리(tree) 기반 API
