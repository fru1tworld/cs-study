...
- `createElement`, `querySelector`, `querySelectorAll`은 태그 이름을 제네릭 키로 받아 `HTMLElementTagNameMap`을 조회하는 오버로드가 있다. 그래서 `"a"`를 넘기면 `HTMLAnchorElement`, `"div"`를 넘기면 `HTMLDivElement`가 나오고, 맵에 없는 임의 문자열을 넘기면 범용 `HTMLElement`로 떨어진다.
