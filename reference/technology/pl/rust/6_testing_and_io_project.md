...
- Rust는 정확성(Correctness)에 높은 관심을 두고 설계 → 타입 시스템만으로는 모든 오류를 잡아낼 수 없음
- 이를 보완하기 위해 자동화된 소프트웨어 테스트 작성 기능을 기본 제공
...
- 예시: 숫자를 전달받아 2를 더하는 `add_two` 함수(정수를 매개변수로 받아 정수를 반환)
- 컴파일 시 Rust가 수행하는 것: 타입 검사(Type Checking)·대여 검사(Borrow Checking) → `String` 값이나 유효하지 않은 참조 전달을 방지
- Rust가 확인하지 못하는 것: 이 함수가 정확히 의도한 대로 동작하는지(예: 10을 더하거나 50을 빼는 게 아니라 2를 더하는지) → 이 지점에서 테스트가 필요
...
- `add_two` 함수에 `3`을 전달했을 때 반환값이 `5`인지 단언(assert)하는 테스트 작성 가능
- 코드 변경 시마다 테스트를 실행 → 기존의 올바른 동작이 유지되는지 확인 가능

---

### 이 장에서 다루는 내용

- 테스트는 복잡한 기술 → 이 장은 모든 세부사항이 아닌 Rust 테스트 기능의 메커니즘 위주로 다룸
- 구체적으로 다루는 내용:
...
- Rust에서 테스트: `#[test]` 속성(attribute)으로 주석 처리된 함수
- `cargo test` 실행 시 → 주석이 달린 함수를 실행하고 통과/실패 결과를 보고하는 테스트 러너 바이너리를 빌드
...
- 더 나은 디버깅을 위해 `assert!`, `assert_eq!`, `assert_ne!`에 사용자 정의 메시지 추가 가능:
...
- 기본 동작: 스레드를 사용해 병렬로 실행 → 더 빠른 피드백 제공
- 단, 테스트가 독립적이고 상태를 공유하지 않아야 함
...
- 단위 테스트는 테스트 대상 코드와 함께 `src` 디렉토리에 배치
- 관례적으로 `#[cfg(test)]` 속성이 붙은 `tests` 모듈 생성
- `#[cfg(test)]` 어노테이션 → `cargo test` 실행 시에만 테스트 코드 컴파일·실행 지시
- `cargo build` 시에는 컴파일 안 됨
...
- Rust의 프라이버시 규칙: 비공개 함수를 직접 테스트하는 것도 허용
- 자식 모듈은 부모 모듈의 항목에 접근 가능
...
- `tests` 디렉토리의 각 파일은 별도 크레이트로 컴파일
- `tests/common.rs` 파일을 만들면 테스트 출력에 `running 0 tests`로 표시 → 헬퍼 함수에는 바람직하지 않음
...
- Rust의 속도·안전성·단일 바이너리 출력·크로스 플랫폼 지원 → 커맨드 라인 도구 제작에 이상적
- 이 프로젝트에서는 클래식 커맨드 라인 검색 도구 `grep`(globally search a regular expression and print)의 자체 버전을 구현
...
- Rust에서 커맨드 라인 인수를 읽으려면 표준 라이브러리의 `std::env::args` 함수 사용
- 이 함수는 커맨드 라인 인수의 반복자(iterator) 반환
...
- 이 장에서 구현하는 것: 커맨드라인 인수로 지정된 파일을 읽는 기능
...
- 이러한 문제점은 개발 초기에 리팩토링하는 것이 좋음 → 다음 섹션에서 오류 처리·모듈성 개선을 다룸
...
- `std::env` 모듈은 환경 변수 관리를 위한 추가 유틸리티 제공 → 자세한 옵션은 공식 문서 참고
