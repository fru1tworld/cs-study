...
`fmt` 패키지 — C의 `printf`/`scanf`와 유사한 방식으로 값을 포맷팅해 출력하거나, 텍스트를 파싱해 값을 읽어들이는 기능 제공. 크게 세 부류로 나뉨.

- 출력 함수: `Print` 계열 — 표준 출력·문자열·`io.Writer`·바이트 슬라이스로 값을 씀
- 입력 함수: `Scan` 계열 — 표준 입력·문자열·`io.Reader`에서 값을 읽음
...
- `Print(a ...any)`: 인수를 기본 형식으로 출력. 피연산자 사이에는 둘 다 문자열이 아닐 때만 공백 삽입
- `Println(a ...any)`: 항상 인수 사이에 공백 삽입, 끝에 개행 추가
- `Printf(format string, a ...any)`: 포맷 문자열의 verb(`%v`, `%d` 등)에 맞춰 인수 대입 후 출력
- 나머지(`Sprint*`, `Fprint*`, `Append*`)는 출력 대상만 다를 뿐 동작 규칙은 동일
...
- `Errorf(format string, a ...any) error`: `Sprintf`처럼 문자열을 만들되 `error` 값으로 반환
- `%w` verb 사용 → 인수로 넘긴 에러를 감싸(wrap) `errors.Is`/`errors.As`/`Unwrap()`이 원본 에러를 찾을 수 있게 함
- Go 1.20부터 `%w`를 여러 번 써서 에러를 여러 개 감쌀 수 있음 → 이 경우 `Unwrap() []error`를 구현한 값 반환
...
- `Scan(a ...any)`: 공백으로 구분된 값을 읽어 각 인수(포인터)에 채움. 개행도 공백으로 취급
- `Scanln(a ...any)`: `Scan`과 비슷하지만 개행에서 멈춤 → 마지막 항목 뒤에 개행이나 EOF가 와야 함
- `Scanf(format string, a ...any)`: 포맷 문자열의 verb에 맞춰 입력 파싱
- 모든 함수는 성공적으로 읽어들인 항목 수 `n`과 `error` 반환
...
- 너비: 최소 출력 폭 → 부족하면 공백(또는 `0` 플래그면 0)으로 채움
- 정밀도: `.` 뒤에 지정 → 부동소수점은 소수 자릿수, 문자열은 최대 출력 룬 수를 의미
- `*` 사용 → 다음 인수(`int`)를 너비/정밀도 값으로 사용
...
`String()` 구현 시 `%v`, `Print` 계열 등 서식이 없는 verb에서 이 값을 사용.
...
`%#v`에서 사용할 Go 소스 코드 형태 표현을 직접 정의할 때 구현.
...
verb·플래그·너비·정밀도까지 전부 직접 제어하고 싶을 때 구현. `f State`를 통해 `f.Width()`, `f.Precision()`, `f.Flag('-')` 등 조회 가능.
...
`Scan`/`Scanf` 계열이 값을 읽을 때 커스텀 파싱 로직을 쓰고 싶을 때 구현. `ScanState`는 `ReadRune`, `SkipSpace`, `Token` 등 제공.

#### 재귀 주의

`String()` 안에서 자기 자신을 다시 `%s`나 `%v`로 포맷 → 무한 재귀 발생. 기반 타입으로 변환해서 사용해야 함.
...
- 패닉 복구: `String()`, `Error()`, `GoString()`이 패닉을 일으키면 `fmt`가 이를 잡아 `%!s(PANIC=메시지)` 형태로 출력에 남김
- 인터페이스 언랩핑: 인터페이스 값을 출력할 때는 (`%T`, `%p` 제외) 내부의 구체 타입 값을 기준으로 포맷
- SQL 문 조립 금지: `database/sql` 사용 시 `Sprintf`로 쿼리 문자열을 직접 조립하면 SQL 인젝션 위험 → 플레이스홀더(`?`, `$1` 등) 사용 필요
...
`strings` 패키지 — UTF-8로 인코딩된 문자열을 다루는 함수 모음. 문자열은 불변(immutable) → 대부분의 함수는 원본을 수정하지 않고 새 문자열을 반환. 많이 쓰는 것 위주로만 정리.
...
- `Cut(s, sep string) (before, after string, found bool)` — sep 기준으로 딱 한 번만 자름. `found`로 존재 여부 확인 후 바로 분기 처리 가능 → `Index` + 슬라이싱 조합보다 간결
...
Go 1.24부터 `SplitSeq`, `FieldsSeq` 등 `iter.Seq[string]`을 반환하는 이터레이터 버전도 추가 → `for range`로 직접 순회 가능.
...
- 단어 첫 글자만 대문자로 바꾸는 `Title`은 단어 경계 판정이 부정확해 deprecated 상태 → 대신 `golang.org/x/text/cases` 패키지 권장
...
- `Compare(a, b string) int` — 사전식 비교 (0/-1/+1). 대부분 `==`, `<`가 더 간단 → 정렬 등 특수한 경우에만 사용
...
문자열을 `+`로 계속 이어 붙이면 매번 새 메모리를 할당 → 느림. 반복적으로 문자열을 조립할 때는 `Builder` 사용.
...
- 값 복사 시 패닉을 일으키도록 설계 → 실수로 복사해 쓰는 것을 방지

### strings.Reader / strings.Replacer

- `NewReader(s string) *Reader` — 문자열을 `io.Reader`, `io.Seeker` 등으로 감싸 스트림처럼 읽을 수 있게 함
- `NewReplacer(oldnew ...string) *Replacer` — 여러 쌍의 old→new 치환을 한 번에 등록. 동시성 안전하며 여러 치환을 한 번에 순회 → `ReplaceAll`을 여러 번 호출하는 것보다 효율적
...
기본 자료형과 문자열 표현 사이의 변환을 담당. 숫자 파싱/포매팅, 문자열 인용(quote)/해제(unquote) 두 축으로 나뉨.
...
`base`를 0으로 주면 `ParseInt`/`ParseUint`가 접두사(`0x`, `0o`, `0b` 등)를 보고 진법을 자동으로 판별.
...
반복문 안에서 바이트 슬라이스에 계속 이어붙여야 한다면 문자열을 새로 할당하는 `Format*` 대신 `AppendInt`, `AppendFloat`, `AppendBool`처럼 기존 슬라이스에 추가하는 `Append*` 계열 사용 → 할당 감소.

### 오류 처리: NumError

Parse 계열 함수가 실패하면 `*strconv.NumError`를 반환.
...
`ErrSyntax`(형식이 잘못됨)와 `ErrRange`(값이 타입 범위를 벗어남)로 원인 구분 가능.
...
로그 출력이나 디버깅 시 문자열에 포함된 제어 문자를 눈에 보이게 표시할 때 유용.
...
`bytes` 패키지 — `strings`와 거의 동일한 API를 `[]byte`에 대해 제공. 문자열 대신 바이트 슬라이스를 다뤄야 할 때(예: 네트워크 I/O, 파일 처리, 불필요한 문자열 변환을 피하고 싶을 때) 사용. 함수 이름과 동작이 `strings`와 대응 → 겹치는 부분은 생략하고 `bytes`만의 특징 중심으로 정리.

### strings와 대응되는 함수들

`Contains`, `Index`, `Split`, `Join`, `Trim*`, `Replace(All)`, `HasPrefix/Suffix`, `Cut`, `Fields`, `Count`, `Equal`, `Compare`, `ToUpper/Lower` 등이 동일한 이름과 시맨틱으로 존재. 차이는 인자와 반환값이 `string` 대신 `[]byte`라는 점뿐.
...
`strings.EqualFold`에 대응하는 `bytes.EqualFold(s, t []byte) bool`도 있음.

### bytes.Buffer

`strings.Builder`의 상위 호환에 가까운, 읽기와 쓰기를 모두 지원하는 가변 크기 버퍼. 제로 값 그대로 바로 사용 가능.
...
`io.Writer`를 요구하는 함수(`fmt.Fprintf` 등)에 그대로 넘길 수 있음 → 문자열 조립 후 I/O로 넘기는 흔한 패턴에 잘 맞음.

### bytes.Reader

읽기 전용으로 `[]byte`를 감싸며 `io.Reader`, `io.ReaderAt`, `io.WriterTo`, `io.Seeker`, `io.ByteScanner`, `io.RuneScanner`를 모두 구현. `Buffer`와 달리 쓰기 메서드가 없고, 대신 `Seek`으로 임의 위치 이동이 가능하며 `UnreadByte`/`UnreadRune`으로 직전에 읽은 바이트·rune을 되돌릴 수 있음.
...
개별 `rune`(유니코드 코드 포인트) 하나의 성질을 판정하고 대소문자를 변환하는 저수준 함수들 제공. `strings`/`bytes`의 `*Func` 계열 함수(`TrimFunc`, `IndexFunc` 등)에 조건 함수로 자주 넘겨짐.
...
`unicode.Is(rangeTab *RangeTable, r rune) bool`로 임의의 유니코드 범위 테이블에 대해 소속 여부 검사 가능. `unicode.Letter`, `unicode.Han`처럼 카테고리/스크립트별로 미리 정의된 `*RangeTable` 값들이 패키지에 노출 → 특정 문자 집합(한글, 한자, 이모지 등)만 걸러낼 때 활용 가능.
...
`rune`과 UTF-8 바이트 인코딩 사이의 저수준 인코딩/디코딩 담당. Go 문자열은 내부적으로 UTF-8 바이트 시퀀스 → 바이트 단위가 아니라 문자(rune) 단위로 정확히 다뤄야 할 때 이 패키지 필요.
...
빈 슬라이스면 `(RuneError, 0)`, 잘못된 인코딩이면 `(RuneError, 1)`을 반환 → `size`로 오류 여부 판별 가능.
...
for-range로 문자열을 순회하면 Go 런타임이 내부적으로 이와 동일한 디코딩을 자동으로 수행.
...
문자열의 "길이"가 필요할 때 `len()`과 `utf8.RuneCountInString()`을 혼동하면 멀티바이트 문자(한글, 한자, 이모지 등)에서 버그가 생기기 쉬움 → 용도에 맞게 구분해서 사용 필요.

### RuneStart

`RuneStart(b byte) bool` — 어떤 바이트가 rune 인코딩의 시작 바이트일 수 있는지 판정. UTF-8 바이트 스트림을 임의 위치에서 잘랐을 때, 문자 경계를 찾아 안전하게 자르는 위치를 재조정하는 데 사용.
...
`regexp` 패키지 — RE2 문법(Perl 호환 정규식이 아님)으로 정규표현식 검색 구현.

- 선형 시간 보장: 백트래킹 기반 엔진과 달리 입력 길이에 비례하는 시간 안에 매칭이 끝남
- 매칭 방식: 기본은 Perl/Python 스타일의 leftmost-first(가장 왼쪽에서 시작하되, 우선순위가 높은 대안을 먼저 선택). POSIX ERE 스타일인 leftmost-longest로 바꾸려면 `CompilePOSIX`를 쓰거나 `Longest()` 호출
- UTF-8 문자열을 다루며, 잘못된 바이트 시퀀스는 `utf8.RuneError`로 취급
- `*Regexp` 값은 컴파일 후에는 여러 고루틴에서 동시에 읽기 전용으로 안전하게 사용 가능. 단 `Longest()`처럼 설정을 바꾸는 메서드는 동시 호출에 안전하지 않음

### 컴파일: Compile / MustCompile

- `Compile(expr string) (*Regexp, error)`
  - 매칭 방식: leftmost-first
  - 실패 시 동작: 오류 반환
- `CompilePOSIX(expr string) (*Regexp, error)`
  - 매칭 방식: leftmost-longest
  - 실패 시 동작: 오류 반환
...
패턴이 상수 리터럴이라 컴파일에 실패할 일이 없다면(패키지 전역 변수 초기화 등) `MustCompile` 사용이 관례. 사용자 입력처럼 실패 가능성이 있는 패턴은 `Compile`로 오류 처리.
...
컴파일 없이 바로 확인하고 싶다면 패키지 레벨 함수 사용. 같은 패턴을 반복 사용한다면 매번 파싱 비용이 들기 때문 → `Compile` 후 메서드를 쓰는 편이 낫음.
...
컴파일된 `*Regexp`의 동일한 메서드도 있음: `Match(b []byte) bool`, `MatchString(s string) bool`, `MatchReader(r io.RuneReader) bool`.
...
캡처 그룹이 있는 패턴은 `Submatch` 계열로 그룹별 값을 얻음. 반환 슬라이스의 인덱스 0은 항상 전체 매칭이고, 1부터가 첫 번째, 두 번째 … 캡처 그룹. 매칭에 실패한 옵셔널 그룹은 빈 문자열(또는 Index 계열에서는 `-1`)이 됨.
...
이름 붙인 캡처 그룹은 `(?P<이름>...)` 문법으로 선언하며, 아래 메서드로 이름과 인덱스를 매핑.
...
- `ReplaceAllStringFunc(src string, repl func(string) string) string`
  - `$` 확장: X
  - repl 종류: 함수(매칭된 전체 문자열을 받아 대체값 반환)

`[]byte` 버전(`ReplaceAll`, `ReplaceAllLiteral`, `ReplaceAllFunc`)도 동일한 규칙으로 존재.

`$` 확장 템플릿에서는 `$1`, `$name`, `${name}`으로 캡처 그룹을 참조하고 `$$`는 리터럴 `$` 하나를 의미.
...
직접 인덱스 슬라이스를 다루고 싶으면 `Expand` / `ExpandString`으로 `FindSubmatchIndex` 결과에 템플릿 적용 가능.
...
매칭되는 부분을 구분자 삼아 문자열을 나눔. `n < 0`이면 제한 없이 모두 분리.
...
- `QuoteMeta(s string) string` — 문자열에 포함된 정규식 메타문자를 이스케이프해, `s`를 리터럴 그대로 매칭하는 패턴으로 변환(사용자 입력을 패턴 일부로 끼워 넣을 때 유용)
...
- `(re *Regexp) Longest()` — 이 인스턴스를 leftmost-longest 매칭으로 전환(POSIX와 동일한 방식). 호출 자체가 동시성 안전하지 않음 → 다른 고루틴이 아직 쓰기 전에 호출해야 함
- Go 1.21부터 `MarshalText` / `UnmarshalText`, Go 1.24부터 `AppendText`를 지원 → `encoding.TextMarshaler` 등으로 직렬화 가능
...
- `FindAll*` 계열은 앞선 매칭 바로 뒤에 붙어 발생하는 빈 매칭은 건너뜀 → 그렇지 않으면 무한히 빈 매칭만 쌓일 수 있기 때문
- 캡처 그룹은 여는 괄호 `(` 의 등장 순서대로 왼쪽부터 번호가 매겨짐. `(?:...)` 처럼 캡처하지 않는 그룹은 번호에 포함되지 않음
...
`regexp/syntax`는 `regexp` 패키지가 내부적으로 쓰는 저수준 패키지 → 정규식 문자열을 파싱해 구문 트리(AST)로 만들고 그 트리를 실행 가능한 프로그램으로 컴파일. 일반적인 정규식 사용에는 필요 없고, 정규식을 분석·변형하는 도구를 만들 때 사용.
...
`regexp.Regexp`(컴파일된 정규식 핸들)과 이름은 같지만 별개 타입. 이쪽은 순수한 파스 트리 노드.
...
`Parse`에 넘기는 비트플래그로, 어떤 문법 방언으로 해석할지 결정.
...
파싱된 AST는 `Compile`로 실행 가능한 명령어 시퀀스(`Prog`)로 바뀜.
...
`Inst`는 개별 명령(문자 매칭, 분기, 캡처 기록 등)을 나타내며 `InstOp` 상수(`InstRune`, `InstCapture`, `InstMatch` 등)로 종류를 구분. 이 계층은 정규식 엔진 내부 구현에 해당해 일반 애플리케이션 코드에서 직접 다룰 일은 거의 없음.
...
패턴이 잘못됐을 때 `Parse`/`Compile`이 반환하는 오류로, `Error()`가 어떤 부분(`Expr`)이 왜(`Code`) 문제인지 알려줌. `ErrorCode`의 대표 값: `ErrMissingParen`(괄호 안 닫힘), `ErrInvalidCharClass`(잘못된 `[...]`), `ErrTrailingBackslash`(패턴이 `\`로 끝남), `ErrNestingDepth`(중첩이 너무 깊음).

### 언제 이 패키지를 직접 쓰나

대부분의 코드는 `regexp.Compile`/`MustCompile`만으로 충분. `regexp/syntax`는 다음과 같은 상황에서만 필요.
