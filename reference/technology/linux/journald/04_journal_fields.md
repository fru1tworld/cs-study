...
128비트 UUID. 메시지의 종류를 식별. 같은 종류의 메시지는 같은 UUID를 가짐. catalog와 결합해 다국어 설명·해결책 제공 가능.

```
MESSAGE_ID=abc12345-...
```

systemd가 발생시키는 표준 메시지 ID는 [systemd 카탈로그](https://github.com/systemd/systemd/tree/main/catalog)에 정의됨.
...
소스 위치 정보. `sd_journal_send` 매크로가 자동으로 채움.
...
cursor를 파일에 저장해 뒀다가 재시작 후 그 시점부터 이어서 읽는 방식으로 활용 가능.
