...
세 개 다 "임시 파일 둘 곳"인데 수명·백엔드·용도가 다름. `hier(7)`과 `tmpfs(5)` 기준 정리.

## 한눈에

- `/tmp`
  - 수명: 재부팅 시 정리(또는 더 짧게)
  - 백엔드: 보통 tmpfs(RAM), 또는 디스크
  - 용도: 일반 임시 파일
- `/var/tmp`
  - 수명: 재부팅 후에도 유지
  - 백엔드: 디스크
...
보안 주의:
- 다른 사용자가 같이 쓰는 디렉터리이므로 race condition에 취약
- 안전한 임시 파일은 `mkstemp(3)` / `mkdtemp(3)` 사용(커널이 atomic하게 unique 파일/디렉터리 생성)
...
이렇게 하면 그 서비스는 자기만의 mount namespace 안에서 새 tmpfs를 `/tmp`와 `/var/tmp`에 마운트함 → 서비스가 종료되면 그 tmpfs는 사라짐(mount namespace에 대한 자세한 내용은 [namespaces.md](namespaces.md) 참고).
