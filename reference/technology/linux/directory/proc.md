...
휘발성 → 디스크에 존재하지 않으며, 읽을 때마다 커널이 즉석에서 생성함. 대부분 read-only지만 `/proc/sys/` 일부는 쓰기로 커널 변수를 변경하는 인터페이스.
...
- `net/`: 프로세스의 네트워크 namespace 정보(`tcp`, `udp`, `unix`, `route`, `dev` 등)
- `ns/`: namespace 핸들(자세한 건 [namespaces.md](namespaces.md) 참고)
- `root`: 프로세스의 루트 디렉터리 심볼릭 링크(chroot/pivot 후 다름)
- `stat`: 한 줄에 모든 상태(PID, comm, state, ppid, utime, stime, vsize, rss 등)
- `statm`: 메모리 사용량 요약(size, resident, shared, text 등)
- `status`: 사람이 읽기 좋은 status(Name, State, Pid, Tgid, Uid, Gid, VmRSS, Threads 등)
...
- `/proc/meminfo`: 메모리 통계(MemTotal, MemFree, MemAvailable, Buffers, Cached, Slab 등)
...
- `/proc/vmstat`: VM 통계(pgfault, pgsteal, pgscan, oom_kill 등)
...
읽기/쓰기 가능한 커널 튜닝 노브. `sysctl(8)` 명령이 이 트리에 mapping됨.
...
`/proc`은 마운트 시점의 PID namespace를 기준으로 프로세스를 노출함.
