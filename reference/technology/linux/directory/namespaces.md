...
컨테이너의 핵심 기반. cgroup이 "얼마나 쓸 수 있냐"라면 namespace는 "무엇이 보이냐"임. 호스트 커널은 그대로 공유하면서, 각 프로세스에게 자기만의 글로벌 자원이 있는 것처럼 보이게 함.

## 1. 8가지 namespace 종류

- Mount
  - CLONE 플래그: `CLONE_NEWNS`
  - 격리되는 자원: 마운트 포인트
  - 도입: 2.4.19(가장 오래됨)
- UTS
  - CLONE 플래그: `CLONE_NEWUTS`
  - 격리되는 자원: hostname, NIS domain name
  - 도입: 2.6.19
- IPC
  - CLONE 플래그: `CLONE_NEWIPC`
  - 격리되는 자원: System V IPC, POSIX message queues
  - 도입: 2.6.19
- PID
  - CLONE 플래그: `CLONE_NEWPID`
  - 격리되는 자원: 프로세스 ID 공간
  - 도입: 2.6.24
- Network
  - CLONE 플래그: `CLONE_NEWNET`
  - 격리되는 자원: 네트워크 디바이스·스택·포트·라우팅, `/proc/net`, `/sys/class/net`
  - 도입: 2.6.29
- User
  - CLONE 플래그: `CLONE_NEWUSER`
  - 격리되는 자원: UID/GID, capabilities, keyring
  - 도입: 3.8
- Cgroup
  - CLONE 플래그: `CLONE_NEWCGROUP`
  - 격리되는 자원: cgroup 루트 디렉터리(가상화된 cgroup path)
  - 도입: 4.6
- Time
  - CLONE 플래그: `CLONE_NEWTIME`
  - 격리되는 자원: boot/monotonic clock 오프셋
  - 도입: 5.6
...
새 프로세스 생성과 동시에 namespace를 새로 만듦.
```c
...
주의(PID/Time): `setns`/`unshare`로 PID 또는 Time namespace를 바꿔도 호출 프로세스 자신은 새 namespace로 들어가지 않음 → 이후 fork되는 자식부터 새 namespace 소속이 됨 — 이미 살아있는 프로세스의 PID 일관성을 보호하기 위함. `/proc/<pid>/ns/pid_for_children`, `time_for_children`이 그 "다음 자식이 들어갈 곳"을 가리킴.

## 3. `/proc/<pid>/ns/`

각 namespace 핸들이 심볼릭 링크로 노출됨:
...
- bind mount로 namespace 파일 박아두기(`mount --bind /proc/1234/ns/net /var/run/netns/foo`)
- 계층 관계(자식 user/PID namespace가 살아있으면 부모 유지)
- 다른 namespace가 그 user namespace를 owner로 참조

이 메커니즘 덕에 프로세스가 없어도 네트워크 namespace를 유지할 수 있음 → `ip netns add foo`가 정확히 이걸 수행.
...
- 가시성: 자기 namespace + 조상만 보임 → 자식에서 부모는 안 보임(단방향)
...
- init이 죽으면 namespace의 모든 프로세스에 SIGKILL 발생 → 이후 fork는 `ENOMEM`
- 시그널 보호: namespace 내부 다른 프로세스는 핸들러가 등록된 시그널만 init에 보낼 수 있음 → 단 조상 namespace에서 보낸 SIGKILL/SIGSTOP은 통과됨
...
- 프로세스는 자식 namespace로 내려갈 수 있지만 부모로는 못 올라감

### `/proc` 동작

> "the /proc filesystem ... only displays processes visible in the PID namespace of the process that performed the mount"

새 PID namespace 진입 후 procfs를 새로 마운트해야 `ps`가 정상 동작 → 컨테이너 이미지가 항상 `/proc`을 새로 마운트하는 이유.

`/proc/sys/kernel/ns_last_pid`: 마지막 할당 PID → CRIU의 PID 복원에 쓰임.

### 시그널 변환

UNIX 도메인 소켓의 SCM_CREDENTIALS에 담긴 PID는 수신자의 PID namespace 기준으로 자동 변환됨.
...
- 32단계 한계, 초과 시 `EUSERS`
- 새 user namespace에 들어가면 그 안에서 모든 capability를 가짐 → 단 부모 namespace에서는 아무 권한 없음
...
- 매핑 안 된 파일의 set-UID/set-GID 비트는 조용히 무시됨
...
- 클럭 변경, 커널 모듈 로드 → 초기 namespace 권한 필요
- 그래서 컨테이너 안에서 "root"여도 호스트 커널 자체는 건드릴 수 없음

## 7. Network Namespace

각 namespace가 자기만의 다음 요소를 가짐:
...
- systemd 유닛의 `PrivateTmp=`, `ProtectSystem=`, `ProtectHome=`, `ReadOnlyPaths=` 등은 전부 mount namespace로 구현됨

## 9. UTS Namespace

가장 단순한 형태 → `hostname`, `domainname`만 격리.
...
서로 다른 IPC namespace의 프로세스는 같은 키로 만든 SysV 객체에 접근하지 못함.

## 11. Cgroup Namespace

`/proc/<pid>/cgroup`이 보여주는 경로가 가상화됨.
...
- `CLOCK_BOOTTIME`, `CLOCK_MONOTONIC`에 오프셋을 적용
...
- cgroups
  - 무엇을 격리하나: 얼마나 쓸 수 있나(리소스 양)
  - 트리 구조: 단일 cgroup 트리
  - 인터페이스: `/sys/fs/cgroup/` 파일 R/W
  - 컨테이너에서 역할: CPU/메모리/IO 쿼터
- namespaces
  - 무엇을 격리하나: 무엇이 보이나(시야)
  - 트리 구조: 종류별 트리(PID, user는 진짜 트리)
  - 인터페이스: syscall(`clone`/`setns`/`unshare`), `/proc/<pid>/ns/`
  - 컨테이너에서 역할: 컨테이너 경계(PID 1, 분리된 네트워크 등)

둘 다 같이 써야 비로소 컨테이너가 됨.
