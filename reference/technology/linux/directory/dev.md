...
커널이 노출하는 디바이스를 파일처럼 다루는 디렉터리. 실제 디스크 블록이 아니라 `devtmpfs` — 커널이 부팅 시 자동으로 만드는 가상 파일시스템임. `udev`(또는 systemd-udevd)가 그 위에 디바이스 노드를 동적으로 추가/이름 부여함.
...
핵심 차이:
- `/dev/random`은 엔트로피 부족 시 블록됨 → Linux 5.6부터는 초기 부팅 이후엔 사실상 블로킹 안 함
...
초기 부팅 시 난수가 필요하면 → `getrandom(2)` syscall(Linux 3.17+) 사용 → 엔트로피 풀 초기화까지 블록하므로 안전.

### 터미널 디바이스

- `/dev/tty`: 호출한 프로세스의 제어 터미널
- `/dev/console`: 시스템 콘솔
- `/dev/pts/*`: 의사 터미널(pseudo terminal) → SSH/tmux 세션마다 하나씩 생성
...
- `/dev/loop0`, `/dev/loop1` 등: 파일을 블록 디바이스처럼 마운트할 수 있게 해줌
...
- `/dev/fd` → `/proc/self/fd`: Bash 프로세스 치환 `<(cmd)`이 이를 활용

## 디바이스 파일의 정체

`ls -l`로 보면 일반 파일과 다른 타입 식별자가 붙음:
...
- major number(앞 숫자): 어떤 드라이버가 다룰지 결정
- minor number(뒤 숫자): 같은 드라이버 내 인스턴스 구분
...
- 컨테이너는 보통 자체 `/dev`를 마운트함(devtmpfs를 새로 또는 일부만 bind-mount)
- 보안을 위해 `/dev/mem`, `/dev/kmsg`, 디스크 노드는 보통 노출하지 않음
- `--device` 옵션(Docker/Podman)은 특정 노드만 컨테이너 안으로 통과시킴
