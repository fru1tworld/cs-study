...
systemd는 시스템 매니저(PID 1) 외에 각 사용자별 매니저도 실행함. 사용자가 처음 로그인하면 `user@<UID>.service`가 시작 → 그 안에서 별도의 systemd 인스턴스가 사용자 unit을 관리.
...
모든 systemctl 명령에 `--user` 플래그를 붙여 사용.
...
기본적으로 사용자 매니저는 사용자가 로그인해 있는 동안만 실행됨. SSH 세션이 종료되면 매니저도 함께 종료 → 모든 사용자 unit이 중지됨.

부팅부터 종료까지 사용자 unit을 계속 실행하려면 linger를 활성화:
...
홈 서버에서 사용자 권한으로 백그라운드 작업을 돌릴 때 필수 사항.

---

### 환경 변수

사용자 매니저는 시스템 매니저와 별도의 환경을 가짐. 일반적인 셸 rc 파일(`.bashrc` 등)은 systemd가 unit을 실행할 때 source되지 않음 → 필요한 환경 변수는 명시적으로 전달 필요.
...
`~/.config/environment.d/*.conf` 파일에 다음 형식 사용:
...
이 변수들은 사용자 매니저와 모든 자식 unit에 자동 적용됨.
...
데스크탑 세션에서 GUI 앱을 systemd로 실행할 때 `DISPLAY`, `XAUTHORITY`, `WAYLAND_DISPLAY` 등을 import하는 패턴이 흔히 쓰임.

---

### Logind 세션

`systemd-logind` 는 사용자 로그인 세션을 관리함.
...
물리적인 입력/출력 장치 그룹. 보통 `seat0` 하나지만, 멀티-시트 데스크탑(키보드/모니터 여러 세트)에서 의미가 있음.
...
`KillUserProcesses=yes` (Debian/Ubuntu 기본값)이면 로그아웃 시 사용자의 모든 프로세스를 종료함 → `nohup`이나 `screen` 같은 도구도 무력화됨. linger를 활성화하면 예외 처리됨.

#### Inhibit

상태 전환을 일시적으로 차단하는 메커니즘. 백업 작업 중 절전 방지 등에 활용.
...
generator는 부팅 초기에 실행되어 unit 파일을 동적으로 생성하는 작은 바이너리. systemd가 unit을 본격적으로 처리하기 전에 호출 → 결과를 임시 디렉터리에 기록함.
...
unit 이름 뒤에 `.d/` 디렉터리를 만들고 그 안에 `.conf` 파일을 둠.
...
`/etc/`, `/run/`, `/usr/lib/` 의 모든 `.d/` 디렉터리가 검색됨.
...
`/etc/systemd/system/nginx.service.d/override.conf` 파일을 자동으로 생성하고 편집기를 엶.
...
원본 nginx.service의 `[Service]` 에 위 옵션이 추가됨.
...
같은 디렉터리 안의 `.conf` 파일은 알파벳 순으로 적용됨 → `10-`, `20-`, `90-` 같은 숫자 접두사를 붙이는 관습이 일반적.
...
대부분의 리스트형 옵션(`ExecStart=`, `Environment=`, `After=` 등)은 누적됨.
...
이렇게 하면 ExecStart가 두 개가 됨 (Type=oneshot이 아닌 한 오류) → 교체하려면 빈 값으로 한 번 초기화 필요:
...
`Environment=` 등 다른 옵션도 동일하게 적용.

#### 검증

drop-in이 어떻게 합쳐졌는지 확인 방법:
...
`service.d` 디렉터리는 모든 `.service` unit에 적용되는 특별한 drop-in임.
...
`Default*=` 접두사가 붙은 옵션은 unit이 별도 지정하지 않을 때의 기본값을 지정.

---

### Preset

배포판이 패키지 설치 시 자동으로 enable 또는 disable할지를 정하는 정책임.
...
`preset-all`은 기존 설정을 일괄 변경하므로 신중하게 사용 필요. 주로 새 시스템 초기 설정이나 컨테이너 이미지 구성 시에 활용.

---

### systemd-run으로 즉석 unit

unit 파일을 별도로 작성하지 않고 명령어를 즉석에서 unit으로 감싸 실행.
...
이름이 지정되므로 이후 systemctl 명령으로 관리 가능:
...
- 기본 (service): systemd가 직접 자식 프로세스로 fork함
