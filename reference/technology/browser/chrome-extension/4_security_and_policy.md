...
Manifest V3는 확장 프로그램의 공격 표면을 줄이기 위해 Content Security Policy(CSP) 적용 방식을 Manifest V2보다 훨씬 엄격하게 바꿈. 핵심 변화는 "원격 코드 실행(Remote Code Execution)을 원천적으로 금지"하는 방향임.

---

### 2. Manifest V2 → V3 CSP 변화

- `content_security_policy` 형식
  - Manifest V2: 문자열 하나로 확장 페이지 전체에 적용
  - Manifest V3: `extension_pages`와 `sandbox` 키로 분리
- 원격 스크립트 로드
  - Manifest V2: CSP에서 완화하면 CDN 등에서 JS를 불러와 실행 가능
  - Manifest V3: 사실상 금지 (모든 코드는 패키지 내부에 있어야 함)
- `eval()` / `new Function()`
  - Manifest V2: CSP 완화 시 허용 가능
  - Manifest V3: 확장 페이지에서는 기본적으로 차단
- 인라인 스크립트
  - Manifest V2: 완화 가능
  - Manifest V3: 기본 정책에서 항상 차단 (변경 불가)
...
Manifest V2 시절 일부 확장 프로그램은 CDN에서 JS 라이브러리를 실시간으로 내려받아 실행함. 편리하지만, 해당 CDN이 침해당하면 사용자 브라우저에 설치된 모든 확장 프로그램이 즉시 악성 코드를 실행하는 공급망 공격 경로가 됨. Manifest V3는 확장 프로그램 패키지에 포함된 코드만 실행하도록 강제해 이 경로를 차단함.
...
확장 프로그램 CSP는 웹 페이지의 [CSP3](../../../standard/w3c/csp3.md) 명세와 문법은 유사하지만, 브라우저가 확장 프로그램 자체에 강제하는 정책이라 사이트 운영자가 완화할 수 없는 고정 제약(원격 코드 실행 금지 등)이 추가로 걸린다는 점이 다름.

---

### 7. 요약

- Manifest V3 CSP는 원격 코드 실행을 원천 차단해 공급망 공격 표면을 줄임.
- `content_security_policy`가 `extension_pages`/`sandbox`로 분리됨.
- CDN 의존 라이브러리는 번들링, `eval` 사용 코드는 안전한 대체 로직으로 재작성해야 함.
...
`ExtensionInstallForcelist`는 기업/조직이 관리하는 Chrome 브라우저에 특정 확장 프로그램을 자동으로 설치하고, 사용자가 임의로 제거하지 못하도록 강제하는 엔터프라이즈 정책임. Chrome Enterprise/그룹 정책(Windows GPO), macOS의 관리형 환경설정(managed preferences), 또는 클라우드 기반 Chrome Browser Cloud Management를 통해 배포됨.
...
Chrome Web Store에 공개하지 않은 사내 전용 확장 프로그램도 자체 호스팅한 update manifest XML을 `update_url`로 지정해 강제 설치할 수 있음. 기업 내부 도구(사내 SSO 확장, 보안 에이전트 확장 등)를 일반 사용자에게 노출하지 않고 배포하는 표준적인 방법임.
...
기업 입장에서는 필수 보안 도구(엔드포인트 보호 확장, DLP 확장)를 사용자가 임의로 끄지 못하게 강제하는 용도로 쓰이지만, 반대로 이 정책이 침해되면(관리 콘솔 계정 탈취 등) 조직 내 모든 브라우저에 악성 확장을 강제 배포할 수 있는 경로가 됨 → 관리 콘솔 접근 권한 자체의 보호가 중요함.

---

### 8. 요약

- `ExtensionInstallForcelist`는 관리형 Chrome 환경에서 특정 확장을 강제 설치·제거 불가 상태로 만드는 정책임.
- 신규 배포에는 더 세분화된 제어가 가능한 `ExtensionSettings` 사용이 권장됨.
- 사설 확장 프로그램도 자체 update URL로 강제 배포할 수 있음.
