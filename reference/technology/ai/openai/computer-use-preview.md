...
`computer-use-preview`는 OpenAI가 제공하는, 화면 스크린샷을 관찰해 GUI 조작 액션을 계획하는 데 특화된 모델임. Anthropic [Claude Computer Use](../anthropic/computer-use.md), Google [Gemini Computer Use](../gemini/computer-use.md)와 마찬가지로 관찰(스크린샷)-행동(좌표 클릭/타이핑)-재관찰 루프 구조를 따름.

---

## 2. Responses API와의 통합

OpenAI의 Computer Use 기능은 Responses API의 `computer_use` 도구 타입으로 노출되며, 모델이 반환하는 액션을 클라이언트가 실행하고 그 결과(새 스크린샷)를 다시 모델에 전달하는 루프를 애플리케이션이 직접 구성 필요.
...
이름의 "preview"가 나타내듯, 이 모델은 아직 프로덕션 안정성이 완전히 보장된 정식 GA(General Availability) 단계가 아니라 초기 공개 단계에 해당함. 프리뷰 단계 모델은 다음과 같은 특징을 갖는 경우가 많음.
...
Computer Use류 기능을 프로덕션에 도입할 때는 벤더와 무관하게 다음 원칙이 공통적으로 적용됨.
...
세 벤더 모두 근본적으로 같은 관찰-행동 루프 패러다임을 각자의 API 컨벤션에 맞춰 노출함.

---

## 6. 요약

- `computer-use-preview`는 OpenAI Responses API에서 스크린샷 기반 GUI 자동화를 담당하는 프리뷰 단계 모델임
- 액션 실행 루프(모델 제안 → 클라이언트 실행 → 결과 재전달)를 애플리케이션이 직접 구성 필요
- 프리뷰 단계이므로 API 변경 가능성을 고려해 프로덕션 핵심 경로에는 폴백을 함께 설계 필요
