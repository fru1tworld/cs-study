...
Tool Use(함수 호출)는 Claude에게 사용 가능한 도구(함수) 목록과 각 도구의 입력 스키마를 알려주면, 모델이 사용자 요청을 해결하기 위해 어떤 도구를 어떤 인자로 호출할지 결정해 구조화된 형태로 응답하는 기능임. 모델이 직접 부수효과를 실행하지는 않으며, "이 도구를 이 인자로 호출하고 싶다"는 요청까지만 생성 → 실제 실행은 호출하는 애플리케이션이 담당.
...
이 구조는 [MCP](../../../standard/mcp/specification.md)의 Tool 정의 형식과 개념적으로 동일한 철학(이름/설명/JSON Schema)을 공유함.
...
애플리케이션은 `tool_use` 블록을 파싱해 실제 함수를 실행한 뒤, 결과를 `tool_result` 콘텐츠 블록으로 다음 요청에 포함시켜 대화를 이어감.
...
[Computer Use](./computer-use.md)는 Tool Use의 특수한 사전 정의 도구 집합(스크린샷, 클릭, 타이핑 등)으로 볼 수 있음. 구조화된 API가 있는 시스템은 일반 Tool Use로 직접 함수를 노출하는 편이 더 정확함 → GUI 외에 접근 수단이 없는 경우에만 Computer Use를 사용하는 것이 일반적인 선택 기준.

---

## 7. 요약

- Tool Use는 모델이 구조화된 함수 호출 요청을 생성하고, 실제 실행은 애플리케이션이 담당하는 협업 패턴임
- 도구는 `name`/`description`/`input_schema`로 정의 → 이 구조는 MCP의 Tool 개념과 철학을 공유
- `tool_choice` 옵션으로 도구 사용 강제 여부 제어 가능
