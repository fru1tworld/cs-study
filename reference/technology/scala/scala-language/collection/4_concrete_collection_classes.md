...
배열의 성능 특성은 원하지만 요소의 타입을 알 수 없고 실행 시점에 제공할 `ClassTag`도 없는 상태에서 시퀀스의 제네릭(generic) 인스턴스를 만들고 싶을 때 보통 `ArraySeq` 사용. 이런 문제는 [배열](5_arrays_and_strings.md) 절에서 설명.
...
- 자바의 `java.util.concurrent.ConcurrentMap` — [표준 자바/스칼라 컬렉션 변환](8_creating_and_converting.md)을 사용해 스칼라 맵으로 자동 변환 가능
