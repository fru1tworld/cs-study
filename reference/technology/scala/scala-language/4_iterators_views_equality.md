...
Scala 컬렉션은 기본적으로 즉시(strict) 평가.
- `map`, `filter` 등 호출 시점에 결과 컬렉션이 전부 계산되어 메모리에 생성됨
- 이 문서에서 다루는 세 가지 — 이터레이터·뷰·LazyList — 는 모두 "당장 다 계산하지 않고 필요할 때만 계산"하는 지연(lazy) 평가와 관련
  - 다만 서로 방식과 목적은 다름
...
참고 — 세 개념 구분법: "이 값, 다시 한번 순회할 수 있는가?"
- Iterator: 아니오(한 번 쓰고 버림)
- View·LazyList: 예
  - View: 안 쓰면 계산 자체를 안 함
  - LazyList: 한 번 계산한 값은 기억해 둠(캐시)
...
`Iterator`는 원소를 담아 두는 자료구조가 아니라, 컬렉션의 원소를 하나씩 순서대로 꺼내는 방법. 핵심 메서드는 두 개.

- `next()`: 다음 원소를 꺼내고 커서를 한 칸 전진. 더 꺼낼 게 없으면 `NoSuchElementException`
...
이터레이터는 "커서" → 한 번 지나간 자리는 되돌아갈 수 없음. 아래처럼 이미 다 꺼낸 이터레이터를 다시 쓰려 하면 예외 발생.
...
계산 자체는 지연됨. `filter`를 호출한다고 그 자리에서 바로 다 걸러내는 게 아니라, `next()`가 호출될 때마다 필요한 만큼만 검사를 진행 → 덕분에 무한한 이터레이터에도 `filter`, `map`을 적용 가능.

주의 — 이터레이터는 "한 번만" 순회한다는 원칙
- 같은 이터레이터 변수를 여러 메서드에 재사용하면 두 번째부터는 빈 결과만 나옴
- 이터레이터를 여러 갈래로 처리해야 한다면 아래 `duplicate`를 사용하거나, 애초에 컬렉션(`List` 등)에서 새 이터레이터를 다시 얻어야 함
...
이터레이터의 계산 결과를 실제 컬렉션으로 확정하려면 `toList`, `toVector`, `toArray`, `toSet`, `toMap` 등 사용. 이 시점에 비로소 남은 모든 원소가 계산되어 메모리에 실체화됨.
...
일반 컬렉션에서 `map`, `filter` 같은 변환 메서드는 즉시(strict) 실행되어 매번 새 컬렉션을 만듦. `.view`를 붙이면 같은 메서드 호출이 실제 계산 없이 "이런 변환을 하겠다"는 계획만 기록하고, 최종적으로 결과가 필요할 때(강제할 때)에야 한 번에 계산.
...
`v.map(f).map(g)`처럼 변환을 체이닝하면, 즉시 평가 컬렉션은 매 단계마다 전체 크기의 새 컬렉션을 만듦. 뷰는 이 단계들을 하나로 합쳐 두었다가, 강제될 때 원소 하나당 `f`와 `g`를 순서대로 한 번씩만 적용 → 뷰의 변환 자체는 원소 개수와 무관하게 O(1)로 취급됨(계획만 세우는 것이므로).

큰 컬렉션에서 앞쪽 일부만 필요한 경우에도 뷰가 유리.
- 예: 백만 개짜리 컬렉션에서 조건에 맞는 원소를 100개만 찾고 싶은 경우
  - 즉시 평가로 `filter`부터 하면 전체를 다 훑어 새 컬렉션을 만듦
  - 뷰로 `filter`한 뒤 `take(100)`을 하면 조건에 맞는 100개를 찾는 순간 나머지는 건드리지 않음

```scala
words.view.filter(_.length > 3).take(2).toList
```

다만 `sorted`처럼 전체 원소를 다 봐야만 답이 나오는 연산은 뷰를 써도 지연의 이점이 없음 — 결국 전부 순회해야 하기 때문.
...
또한 뷰를 강제(force)할 때마다 계획된 변환이 매번 다시 실행됨. 부작용(출력, 액터 생성, DB 호출 등)이 있는 함수를 뷰에 얹으면, 강제하는 횟수만큼 부작용이 반복될 수 있음.

주의 — 뷰를 강제하지 않은 채 부작용을 기대하지 말 것
- 뷰는 "결과를 실제로 꺼내 쓸 때"에만 동작이 일어남
- 부작용을 목적으로 한 코드(로깅, 외부 호출 등)를 뷰 체인 안에 넣었다면, 그 뷰를 `toList`나 `foreach`로 강제하기 전까지는 아무 일도 일어나지 않는다는 점을 꼭 확인해야 함

---

### 4. LazyList

`LazyList`(Scala 2.13에서 옛 `Stream`을 대체)는 `List`와 마찬가지로 컬렉션 그 자체. 순서가 있는 원소들의 목록이며, 여러 번 순회할 수 있고 저장해 둘 수도 있음. 다만 원소를 필요한 시점에만 계산하고, 한 번 계산한 값은 다시 계산하지 않도록 기억(메모이제이션)해 둔다는 점이 `List`와의 차이.

#### 4.1 머리도 꼬리도 지연된다

과거 `Stream`은 `head`를 만드는 시점에 바로 계산해 버려서(꼬리만 지연) 종종 예상 밖의 즉시 평가가 일어남. `LazyList`는 이 문제를 고쳐 `head`와 `tail` 모두 실제로 접근하기 전까지는 계산하지 않음. `LazyList(1, 2, 3)`을 만드는 것만으로는 아무 원소도 계산되지 않고, `head`나 `tail`을 불러야 그 자리의 값이 비로소 계산됨.
...
계산을 미룰 수 있다는 성질 덕분에 끝이 없는 목록도 정의 가능. 필요한 만큼만 `take`로 잘라내면 무한 목록이라도 문제없이 다룰 수 있음.
...
`#::`는 `LazyList`에서 `::`(cons) 대신 쓰는 지연 연결 연산자. 오른쪽 항인 `naturals(n + 1)`은 실제로 `tail`에 접근하기 전까지 평가되지 않음.

#### 4.3 메모이제이션(memoization)에 따른 메모리 누수 주의

`LazyList`는 한 번 계산한 원소를 캐시에 계속 들고 있음. 이 목록을 가리키는 참조를 오래 붙들고 있으면, 이미 다 쓴 앞부분 원소도 메모리에서 해제되지 않고 계속 쌓일 수 있음. 무한열이나 아주 긴 `LazyList`를 순회할 때는, 처음 몇 개를 담아 둔 `val` 참조를 계속 유지하지 않도록 주의해야 함.
...
Scala 컬렉션 라이브러리는 두 가지를 문서 두 페이지로 나누어 설명.
...
둘 다 "컬렉션 타입을 무엇으로 고를지"에 직접 영향을 주는 실전 지식. 동등성 규칙을 모르면 해시맵 키로 가변 컬렉션을 썼다가 원인 모를 버그를 만나고, 성능 특성을 모르면 `List`에 인덱스로 접근하는 코드를 반복문 안에 넣어 놓고 왜 느린지 못 찾게 됨.
...
같은 카테고리 안에서는 구체적인 클래스가 달라도 원소만 같으면 동등. Seq는 순서까지 같아야 하고, Set/Map은 순서와 무관하게 원소 집합만 같으면 됨.
...
주의
- "구현이 다르면 안 같다"고 생각하기 쉽지만 정반대. `List`와 `Vector`처럼 내부 구조가 완전히 다른 두 타입도 카테고리(Seq)와 내용(원소, 순서)만 맞으면 서로 `==`로 비교했을 때 `true`
- 배열(`Array`)만은 예외 — Scala 2.13 기준 `Array`의 `==`는 참조 동등성(reference equality)을 그대로 물려받으므로 원소가 같아도 다른 배열 인스턴스면 `false`
- 배열 내용 비교에는 `sameElements`를 쓰거나 `.toSeq`로 감싸 비교해야 함

#### 2.3 가변 컬렉션과 동등성 — 해시 키로 쓸 때의 함정

가변(mutable) 컬렉션은 "지금 이 순간 담고 있는 원소" 기준으로 동등성이 결정됨. 즉 컬렉션 내용이 바뀌면 `==` 결과와 해시코드(`hashCode`)도 함께 바뀜 → 가변 컬렉션을 `HashMap`의 키로 쓰면 다음과 같은 사고가 남.
...
`HashMap`은 키를 넣을 때의 `hashCode`로 버킷 위치를 정해 두는데, 이후 그 키(가변 컬렉션)의 내용이 바뀌면 `hashCode`도 달라져 원래 저장했던 버킷과 지금 계산되는 버킷이 어긋남. 데이터는 그대로 맵 안에 있지만 찾을 수 없게 되는 것.

실전 원칙: 해시 기반 컬렉션(`HashMap`, `HashSet`)의 키·원소로는 불변 컬렉션만 사용. 가변 컬렉션을 키로 써야 한다면 넣은 뒤 다시는 그 내용을 바꾸지 않는다는 확신이 있을 때만 허용.
...
- `List`: "머리와 꼬리로 이루어진 연결 리스트" → 앞쪽 연산(`head`, `tail`, `prepend`)은 전부 상수 시간. 인덱스로 임의 위치에 접근(`apply`)하거나 끝에 추가(`append`)하려면 리스트를 처음부터 끝까지 훑어야 해 선형 시간
- `Vector`: 내부적으로 갈래가 32개인 트리 구조 → 이론적으로는 `Log`지만, 트리 깊이가 실전 데이터 크기에서 사실상 상수(트리 깊이 ≤ 6~7 정도)라 모든 주요 연산이 `eC`로 균형 잡혀 있음. "이것도 저것도 적당히 빠른" 범용 시퀀스가 필요할 때의 기본 선택지
...
- 트리 기반(`TreeSet`/`TreeMap`): 균형 이진 트리(레드-블랙 트리)로 구현되어 모든 연산이 `Log`로 고르지만, 그 대가로 원소가 항상 정렬된 순서를 유지. 정렬된 순회나 범위 조회(`range`, `from`, `until`)가 필요할 때 선택
- `BitSet`: 정수 집합을 비트 배열로 표현 → 값의 범위가 촘촘하게 몰려 있을 때(dense) 조회(`C`)와 최솟값 조회(`eC`)가 빠름. 추가(`add`)는 불변은 새 비트 배열을 만들어야 해 `L`, 가변은 배열을 가끔만 늘리면 되어 `aC`. 희소(sparse)하고 값의 범위가 아주 넓으면 이 장점이 사라지므로 주의 필요
- `VectorMap`/`ListMap`: `VectorMap`은 삽입 순서를 기억하면서도 조회·추가는 해시 기반이라 `eC`를 유지. `ListMap`은 내부가 연결 리스트라 대부분의 연산이 `L` → 삽입 순서 보존이 꼭 필요한 게 아니라면 `VectorMap`이 더 나은 선택
...
1. 동등성: 카테고리(Seq/Set/Map)가 같고 내용이 같으면 구현 타입과 무관하게 `==`. 단, 가변 컬렉션은 내용이 바뀌면 동등성과 해시코드도 함께 바뀌므로 해시 키로 쓰지 않아야 함
2. 성능: "어떤 연산을 자주 하는가"가 컬렉션 선택을 결정. 앞쪽 연산 위주면 `List`, 인덱스 접근 위주면 `Vector`, 뒤쪽 누적이면 `ArrayBuffer`, 정렬 유지가 필요하면 `TreeMap`/`TreeSet`
