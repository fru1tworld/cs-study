...
특정 `cqlsh` 버전은 공식적으로 그에 대응하는 Cassandra 릴리스(release)와만 동작하도록 보장됩니다. 다른 버전과도 비공식적으로(unofficially) 동작할 수는 있습니다.

#### 선택적 의존성(Optional Dependencies)

`cqlsh`는 일부 기능을 위해 선택적 Python 모듈을 사용합니다.

- **pytz**: Python 3.9 이상에서는 `cqlshrc` 설정 또는 `TZ` 환경 변수(environment variable)로 타임존(timezone) 표시를 활성화합니다. Python 3.8 이하에서는 `pytz`를 직접 설치해야 합니다.
...
- **credentials**: 사용자 이름(username)과 비밀번호(password)를 담으며, `cqlshrc`의 `auth_provider` 클래스명(classname)과 일치해야 합니다. 이 파일은 반드시 사용자 소유(user-owned)여야 하며 접근 권한(permissions)을 제한해야 합니다.
...
명령 출력을 콘솔 대신 파일로 캡처(capture)합니다.
...
스키마 요소(schema element)에 대한 DDL(Data Definition Language) 문을 출력합니다. `DESC`는 `DESCRIBE`의 축약형(substitute)으로 사용할 수 있습니다.
...
단순 텍스트(simple text)는 따옴표 없이 반환되지만, 복합 타입(complex type) 텍스트는 따옴표와 이스케이프가 적용된 형태로 반환됩니다. 컬렉션(collection)과 사용자 정의 타입(user-defined type)에서는 이스케이프가 명시적으로 표시됩니다.
...
- `-pp, --print-port` — 4.0 모드로 동작하여 포트 번호로 호스트를 구분(disambiguate)합니다.
...
- **옵션**:
  ...
  - `-ns, --no-snapshot` — `disableSnapshot`이 false이면 스크럽 대상 CF(Column Family)를 먼저 스냅샷합니다(기본값 false).
...
- **기능**: 디렉터리의 SSTable을 설정된 클러스터로 일괄 적재(bulk-load)합니다. 경로의 상위 디렉터리(parent directory)가 대상 키스페이스와 테이블 이름으로 사용됩니다(예: `/path/to/keyspace1/standard1/`의 파일은 `keyspace1.standard1`로 적재됩니다). 원본 파일을 삭제하지 않고 새 SSTable을 생성합니다.
- **사용법**:
  ```
...
- **기능**: 관련된 `Statistics.db`, `Summary.db` 파일에서 SSTable 정보를 표준 출력에 출력합니다. 실행 전 Cassandra를 중지해야 합니다(스크립트가 이를 확인하지는 않습니다).
- **사용법**:
  ```
...
- **기능**: 손상된(broken) SSTable을 복구합니다. SSTable을 다시 작성하며 손상된 행은 건너뜁니다. 먼저 Cassandra를 중지해야 합니다.
- **사용법**:
  ```
...
- **기능**: 제공된 테이블의 SSTable에서 오류 또는 손상 여부를 검사합니다. 실행 전 Cassandra를 중지해야 합니다. 기본 검증(basic verification)은 표준 검사를 수행하고, 확장 검증(extended verification)은 개별 값(value)의 오류 또는 손상까지 검증합니다(추가 시간과 리소스가 필요합니다).
- **사용법**:
  ```
...
- **기능**: 큰 SSTable 파일을 더 작은 파일로 분할(split)합니다. 일종의 안티컴팩션(anticompaction)으로 볼 수 있습니다. 먼저 Cassandra를 중지해야 합니다(스크립트가 이를 확인하지는 않습니다).
- **사용법**:
  ```
...
- **기능**: 주어진 테이블(또는 스냅샷)의 SSTable을 현재 Cassandra 버전으로 업그레이드합니다. 일반적으로 버전 업그레이드 후에 사용하며, 이전 버전으로 다운그레이드(downgrade)하는 것도 가능합니다. 현재 실행 중인 Cassandra보다 오래된 메이저 버전의 스냅샷을 복원하기 전에 필요합니다. 먼저 Cassandra를 중지해야 합니다.
- **사용법**:
  ```
...
- **기능**: 주어진 SSTable 집합에 `repairedAt` 상태를 설정하여, 리페어되지 않은(un-repaired) SSTable만 선택적으로 리페어할 수 있게 합니다(리페어에 오랜 시간이 걸리는 대용량 데이터셋에 유용합니다). 먼저 Cassandra를 중지해야 합니다.
- **사용법**:
  ```
...
- **기능**: 다른 SSTable이 드롭(drop)되지 못하도록 막고 있는 SSTable을 모두 나열합니다(만료된 SSTable의 가장 최근 툼스톤보다 오래된 데이터를 보유해 드롭을 차단하는 경우). 만료된 툼스톤만 가진 SSTable은 컴팩션 중에 폐기되어야 하지만, 다른 SSTable에 더 최신 데이터가 존재하면 제거할 수 없습니다. 먼저 Cassandra를 중지해야 합니다.
- **사용법**:
  ```
...
- **기능**: SSTable에서 큰 파티션(large partition)을 식별하고 파티션 크기(바이트), 행 수(row count), 셀 수(cell count), 툼스톤 수(tombstone count)를 출력합니다. 하나 이상의 SSTable 파일/디렉터리를 각각 분석합니다. 임계값(threshold)을 지정하면 해당 임계값을 초과하는 파티션만 표시됩니다. 추정 백분위수(estimated percentile)와 정확한 최솟값/최댓값/개수를 포함한 요약 통계를 제공합니다. 기본적으로 사람이 읽기 쉬운 형식이며, `--csv`로 기계 판독(machine-readable) 형식을 출력할 수 있습니다. Cassandra가 실행 중일 필요는 없습니다.
- **사용법**:
  ```
...
User 모드를 사용하면 실제 스키마로 직접 부하 테스트를 수행할 수 있어 장기적으로 효율적입니다. 애플리케이션이 확장 가능한지 검증하는 데 유용합니다. User 모드는 YAML 파일로 프로파일(profile)을 정의합니다.
...
Cassandra는 풍부한 로깅 지원을 제공하며, 운영 가시성을 확보하면서도 불필요한 노이즈는 최소화하도록 설계된 세 가지 주요 로그 파일을 둡니다.
