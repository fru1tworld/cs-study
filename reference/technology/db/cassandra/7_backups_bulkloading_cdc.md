...
Apache Cassandra는 데이터를 불변(immutable) SSTable 파일에 저장함. Apache Cassandra 데이터베이스에서 백업(backup)이란 SSTable 파일로 저장되어 있는 데이터베이스 데이터의 백업 복사본을 의미함.

백업의 핵심 목적:

- 내구성(durability): 데이터 복사본을 보관하여 내구성 확보
- 테이블 복원: 노드 장애·파티션 장애·네트워크 장애로 테이블 데이터가 손실되었을 때 복원
- 이식성(portability): SSTable 파일을 다른 머신으로 전송 가능

#### 1.2 백업의 종류

Cassandra는 두 가지 백업 전략을 지원함.

##### 스냅샷(Snapshots)

스냅샷(snapshot)은 특정 시점에 하드 링크(hard link)를 통해 생성한 테이블의 SSTable 파일 복사본. 테이블의 DDL도 함께 보존됨. 스냅샷은 수동 또는 자동으로 생성 가능.

- `snapshot_before_compaction` 설정(cassandra.yaml): 컴팩션(compaction) 작업 전 자동 스냅샷 생성 여부를 제어(기본값: `false`)
- `auto_snapshot` 설정: 키스페이스 truncate(잘라내기) 또는 테이블 drop(삭제) 전 자동 스냅샷 생성을 활성화(기본값: `true`)
- 기본적으로 Cassandra는 truncate 작업 중 자동 스냅샷이 완료되기를 60초 동안 대기함

##### 증분 백업(Incremental Backups)

증분 백업(incremental backup)은 멤테이블(memtable)이 SSTable로서 디스크에 플러시(flush)될 때 하드 링크로 생성되는 테이블 SSTable 파일의 복사본. 증분 백업은 스냅샷과 함께 사용하여 백업 소요 시간과 저장 공간 사용량을 최소화하는 용도로 일반적으로 활용됨.

증분 백업은 기본적으로 비활성화되어 있으며, cassandra.yaml의 `incremental_backups` 설정 또는 nodetool 명령을 통해 명시적으로 활성화 필요. 활성화되면 Cassandra는 로컬에서 플러시되거나 스트리밍되는 각 SSTable에 대해 해당 키스페이스 데이터 디렉터리 내의 `backups/` 하위 디렉터리에 하드 링크를 생성함.

#### 1.3 데이터 디렉터리 구조

Cassandra의 디렉터리 구조는 데이터를 키스페이스 디렉터리(keyspace directory), 테이블 디렉터리(table directory), 백업/스냅샷 하위 디렉터리(backup/snapshot subdirectory)로 구성함. 각 테이블 디렉터리 안에는 각 백업 유형을 저장하기 위해 별도의 `backups/` 디렉터리와 `snapshots/` 디렉터리가 존재함.

#### 1.4 예제 테이블 준비

아래 예제는 두 개의 키스페이스(`cqlkeyspace`와 `catalogkeyspace`)와 샘플 테이블을 사용함.

먼저 `cqlkeyspace` 키스페이스와 두 개의 테이블 `t`, `t2`를 생성하고 데이터를 삽입함.
...
다음으로 두 번째 키스페이스 `catalogkeyspace`와 테이블 `journal`, `magazine`을 생성하고 데이터를 삽입함.
...
스냅샷을 생성하는 명령은 `nodetool snapshot`. 전체 사용법 정보:
...
- `-cf <table>`, `--column-family <table>`, `--table <table>`: 테이블 이름을 지정(이 옵션을 사용하려면 반드시 키스페이스를 하나만 지정)
- `-h <host>`, `--host <host>`: 노드의 호스트명 또는 IP 주소
- `-kt <ktlist>`, `--kt-list <ktlist>`, `-kc <ktlist>`, `--kc.list <ktlist>`: 스냅샷을 생성할 `Keyspace.table` 목록(키스페이스만 단독으로 지정 금지)
- `-p <port>`, `--port <port>`: 원격 JMX 에이전트 포트 번호
- `-pp`, `--print-port`: 포트 번호로 호스트를 구분하는 4.0 모드로 동작
- `-pw <password>`, `--password <password>`: 원격 JMX 에이전트 비밀번호
- `-pwf <passwordFilePath>`, `--password-file <passwordFilePath>`: JMX 비밀번호 파일의 경로
- `-sf`, `--skip-flush`: 스냅샷 생성 전 멤테이블을 플러시하지 않음(스냅샷에는 플러시되지 않은 데이터 미포함)
- `-t <tag>`, `--tag <tag>`: 스냅샷의 이름(태그)
- `-u <username>`, `--username <username>`: 원격 JMX 에이전트 사용자명
- `--`: 명령줄 옵션과 인자 목록을 구분하는 용도(인자가 명령줄 옵션으로 잘못 해석될 수 있을 때 유용)
- `[<keyspaces...>]`: 키스페이스 목록, 기본적으로 모든 키스페이스 대상

##### 1.5.2 스냅샷을 위한 설정

자동 스냅샷을 비활성화하려면 cassandra.yaml에서 다음과 같이 설정.
...
`catalogkeyspace`의 모든 테이블에 대해 `catalog-ks` 태그를 가진 스냅샷을 생성.
...
이 명령은 스냅샷 이름(Snapshot name), 키스페이스 이름(Keyspace name), 컬럼 패밀리(테이블) 이름(Column family name), 실제 크기(True size), 디스크상의 크기(Size on disk)를 표시함.

##### 1.5.5 스냅샷 디렉터리 찾기

`find` 명령을 사용하여 스냅샷 디렉터리를 찾음.
...
각 스냅샷에는 복원 시 CQL로 테이블을 다시 생성하기 위한 DDL을 담고 있는 `schema.cql` 파일이 포함됨.

##### 1.5.6 스냅샷 삭제

`nodetool clearsnapshot` 명령으로 스냅샷을 삭제. 스냅샷 이름을 지정하거나 `--all` 옵션 사용 가능.
...
cassandra.yaml에서 증분 백업을 활성화.

```yaml
incremental_backups: true
```

이 설정은 기본적으로 비활성화되어 있는데, 데이터가 플러시될 때마다 새로운 SSTable 파일이 생성되어 상당한 저장 공간을 소비할 수 있기 때문. 증분 백업은 nodetool을 통해서도 관리 가능.
...
추가로 플러시를 수행하면, 타임스탬프가 다른 SSTable 파일들이 연속적인 백업을 구분함.
...
삭제(drop)된 테이블을 복원하기 위한 주요 도구:

- `sstableloader`
- `nodetool refresh`

스냅샷은 증분 백업과 유사하게 동작하지만 추가 파일을 포함함. 중요한 점은, 스냅샷은 CQL에서 테이블을 생성하기 위한 스키마 DDL을 담은 `schema.cql` 파일을 포함한다는 것. 반면 테이블 백업(증분 백업)은 DDL을 포함하지 않으므로, 증분 백업으로부터 복원할 때는 스냅샷에서 DDL을 얻어야 함.
...
Apache Cassandra는 SSTable을 통해 벌크 로딩(bulk loading)을 지원하며, SSTable이 유일하게 지원되는 형식. 공식 문서는 다음과 같이 명시함: "Cassandra는 CSV, JSON, XML과 같은 다른 형식의 데이터를 직접 로드하는 것을 지원하지 않습니다."

벌크 로딩의 주요 목적 세 가지:

- 증분 백업과 스냅샷 복원: 이미 SSTable 형식인 증분 백업과 스냅샷을 복원
- 기존 SSTable을 다른 클러스터로 로드: 노드 수나 복제 전략(replication strategy)이 다른 클러스터로 기존 SSTable을 로드
- 외부 데이터 가져오기(import): 외부 데이터를 클러스터로 가져옴

#### 2.2 주요 도구

Cassandra는 두 가지 주요 메커니즘을 제공함.

1. sstableloader: 기본 벌크 업로드 도구, 복제 전략과 복제 인수(replication factor)를 준수하면서 실행 중인 클러스터로 SSTable 데이터를 스트리밍
2. nodetool import: 더 이상 사용이 권장되지 않는(deprecated) `nodetool refresh` 명령을 대체하는 권장 도구
...
- 링(ring) 정보를 얻기 위한 하나 이상의 초기 호스트(initial hosts)를 쉼표로 구분하여 지정
- SSTable을 담고 있는 디렉터리 경로를 지정
- 디렉터리 구조는 반드시 `keyspace/table/` 형식을 따라야 함

##### 2.3.2 주요 옵션

- `-d,--nodes <initial hosts>` (필수): 연결할 초기 호스트
- `-k,--target-keyspace <name>`: 대상 키스페이스 지정
- `-p,--port <port>`: 네이티브 트랜스포트(native transport) 포트(기본값 9042)
- `-sp,--storage-port <port>`: 노드 간(internode) 통신 포트(기본값 7000)
- `--throttle-mib <speed>`: 아웃바운드 스로틀(throttle)을 MiB/s 단위로 지정
- `--inter-dc-throttle-mib <speed>`: 데이터센터 간(cross-datacenter) 스로틀을 MiB/s 단위로 지정
- `-f,--conf-path <path>`: 스트리밍 및 암호화 설정을 위한 cassandra.yaml 경로
- `--no-progress`: 진행 상황 표시를 억제
- `-v,--verbose`: 자세한 출력(verbose output)을 활성화

##### 2.3.3 추가 SSL/인증 옵션

이 도구는 키스토어 경로(keystore path), 인증 자격 증명(authentication credentials), 알고리즘 지정(algorithm specification), 암호 스위트 선택(cipher suite selection)을 포함한 포괄적인 보안 설정을 지원함.

> 참고: `sstableloader`는 노드 간 통신을 위해 포트 7000을 사용함. 또한 로드하려는 SSTable은 로드 대상이 되는 Cassandra 버전과 호환되어야 함.
...
- 특정 디렉터리 구조가 요구되지 않음
- 백업/스냅샷 디렉터리를 복사하지 않고도 직접 작업 가능
- 명령줄에서 테이블과 키스페이스를 지정 가능

##### 2.4.2 주요 옵션

- `-c, --no-invalidate-caches`: 행 캐시(row cache)를 보존(캐시를 무효화하지 않음)
- `-e, --extended-verify`: 모든 SSTable 값을 검증
- `-l, --keep-level`: 기존 SSTable 레벨(level)을 유지
- `-q, --quick`: 검증을 건너뛰어 더 빠르게 가져옴
- `-r, --keep-repaired`: 복구(repair) 상태를 보존
- `-t, --no-tokens`: 토큰 소유권(token ownership) 검증을 건너뜀
- `-v, --no-verify`: SSTable 검증을 비활성화

#### 2.5 sstableloader 실습 워크플로

##### 2.5.1 증분 백업으로부터 로드

1. 대상 디렉터리 구조를 생성: `/catalogkeyspace/magazine/`
2. 대상 테이블이 존재하는지 확인(비어 있어도 됨)
3. 백업 SSTable을 대상 디렉터리로 복사
4. 다음 명령을 실행
...
1. 디렉터리 구조를 생성하거나, 스냅샷 폴더에 대한 심볼릭 링크(symlink)를 생성
2. 테이블이 삭제된 경우, 스냅샷의 `schema.cql`을 사용하여 테이블을 다시 생성
3. 동일한 명령 구조로 `sstableloader`를 실행

> 참고: 다른 클러스터로 로드한 테이블을 복구(repair)하더라도 원본 테이블은 복구되지 않음.
...
외부 데이터(CSV, JSON, XML)의 직접적인 벌크 로딩은 지원되지 않음. 이를 위한 워크플로:

1. `CQLSSTableWriter` Java API를 사용하여 SSTable을 생성
2. 그런 다음 `sstableloader` 또는 `nodetool import`를 사용하여 벌크 로딩을 수행
...
- `addRow(List/Map/Object...)`: CQL 컬럼에 대응하는 타입으로 새 행을 삽입
- `rawAddRow()`: 사전 직렬화된(pre-serialized) 바이너리 값을 추가
- `getUDType(String)`: UDTValue 생성을 위한 사용자 정의 타입(user-defined type)을 가져옴
- `close()`: SSTable 생성을 마무리

##### 2.7.4 CQLSSTableWriter.Builder 메서드

- `inDirectory(File/String)`: 필수, 출력 위치를 설정
- `forTable(String schema)`: 필수, 완전한 정규 이름(fully-qualified name)으로 테이블을 정의
- `using(String insert)`: 필수, 바인드 변수(bind variable)가 포함된 `INSERT` 구문을 지정
- `withBufferSizeInMiB(int)`: 버퍼 크기를 설정(기본값 128MB)
- `withPartitioner(IPartitioner)`: 기본 `Murmur3Partitioner`를 재정의
- `sorted()`: 입력이 파티션 키 순서로 미리 정렬되어 있을 것을 기대
- `build()`: `CQLSSTableWriter` 인스턴스를 생성

#### 2.8 주의사항

- 공식 문서는 다음을 강조함: "증분 백업을 복원하기 전에, 멤테이블에 있는 데이터를 백업하기 위해 `nodetool flush`를 실행하십시오."
- "다른 클러스터로 로드한 테이블을 복구(repair)하더라도 원본 테이블은 복구되지 않습니다."
- 증분 백업은 DDL을 포함하지 않으므로, 대상 테이블이 미리 존재해야 함
- 스냅샷은 테이블 재생성을 위한 `schema.cql`을 포함함
- 두 도구 모두 특정 호스트와 포트로 연결하여 클러스터를 대상으로 지정 가능
...
변경 데이터 캡처(CDC)는 운영자에게 특정 테이블을 아카이빙(archival) 대상으로 표시하고, CDC 로그가 설정 가능한 디스크 크기 임계값에 도달하면 쓰기를 거부하는 기능을 제공함.

테이블에서 CDC가 활성화되면, 지정된 디렉터리에 CommitLogSegment에 대한 하드 링크(hard-link)가 생성됨. 정수 오프셋(integer offset)이 담긴 인덱스 파일이 디스크에 영속화된(persisted) 데이터를 추적하며, 'COMPLETED' 마커가 최종 처리 완료를 나타냄.

#### 3.2 CDC 동작 방식

CDC는 일련의 파일 기반 메커니즘을 통해 동작함.

##### 3.2.1 CommitLogSegment 처리

테이블에 CDC가 활성화되어 있으면, "cassandra.yaml에 지정된 디렉터리에 해당 세그먼트에 대한 하드 링크가 생성됩니다." 세그먼트가 디스크에 fsync될 때, 만약 해당 세그먼트에 CDC 데이터가 존재하면 `<segment_name>_cdc.idx` 파일이 생성되며, 이 파일에는 "원본 세그먼트의 데이터 중 얼마만큼이 디스크에 영속화되었는지를 나타내는 정수 오프셋"이 담김.

##### 3.2.2 인덱스 파일 메커니즘

메모리 매핑된(memory-mapped) 핸들에서 로그를 실시간으로 파싱하지 않고 인덱스 파일을 사용하는 이유는, 데이터가 아직 영속화되지 않고 커널 버퍼(kernel buffer)에 존재할 수 있기 때문. 소비자(consumer)가 `_cdc.idx` 파일에 명시된 오프셋까지만 읽도록 함으로써, 내구성이 보장된(durable) CDC 데이터만 파싱하도록 보장함.
...
- `cdc_enabled`
  - 기본값: `false`
  - 설명: 노드 전역(node-wide)으로 CDC 동작을 활성화하거나 비활성화
- `cdc_raw_directory`
  - 기본값: `$CASSANDRA_HOME/data/cdc_raw`
  - 설명: 모든 멤테이블이 플러시된 후 CommitLogSegment가 이동할 목적지
- `cdc_total_space`
  - 기본값: min(4096MiB, 볼륨 공간의 1/8)
  - 설명: "CDC를 허용하는 모든 활성 CommitLogSegment와 `cdc_raw_directory`에 있는 모든 플러시된 CDC 세그먼트의 합으로 계산됩니다"
- `cdc_free_space_check_interval`
  - 기본값: 250ms
  - 설명: "용량에 도달했을 때, 불필요하게 CPU 사이클을 소모하지 않도록 `cdc_raw_directory`가 차지하는 공간을 다시 계산하는 빈도를 제한합니다"

각 파라미터에 대한 상세 설명:

- `cdc_enabled`(기본값: `false`): CDC 동작을 노드 전역으로 활성화하거나 비활성화
- `cdc_raw_directory`(기본값: `$CASSANDRA_HOME/data/cdc_raw`): 모든 멤테이블이 플러시된 후 CommitLogSegment가 이동할 목적지 디렉터리
- `cdc_total_space`(기본값: 4096MiB와 볼륨 공간의 1/8 중 작은 값): CDC를 허용하는 모든 활성 CommitLogSegment의 합과, `cdc_raw_directory`에 있는 모든 플러시된 CDC 세그먼트의 합으로 계산
- `cdc_free_space_check_interval`(기본값: 250ms): 용량에 도달한 상태에서 공간 사용량을 재계산하는 빈도

#### 3.4 테이블에 CDC 활성화/비활성화

CDC는 `cdc` 테이블 속성(table property)을 통해 관리함.
...
테이블은 생성 시점에 CDC를 활성화하거나, 이후에 변경 가능. 전체 구문은 테이블 생성(`creating tables`) 및 테이블 변경(`altering tables`) 관련 문서 참고.

#### 3.5 CommitLogSegment 읽기

CDC 데이터는 Cassandra의 Java API를 사용해 프로그래밍 방식으로 읽음.

##### 3.5.1 CommitLogReader

`CommitLogReader.java` 클래스를 사용함. 사용법은 비교적 직관적이며 다양한 시그니처(signature)를 지원함. 참조 구현(reference implementation)은 세그먼트 처리를 위한 표준 패턴을 보여줌.

##### 3.5.2 CommitLogReadHandler

"디스크에서 읽은 변형(mutation)을 처리하려면 `CommitLogReadHandler`를 구현하십시오." 이 인터페이스는 커밋 로그 세그먼트에서 추출된 변형을 처리하기 위한 계약(contract)을 정의함.
...
소비 프로세스가 없으면 디스크 공간이 빠르게 가득 차고, CDC가 활성화된 테이블에 대한 쓰기가 거부됨.
