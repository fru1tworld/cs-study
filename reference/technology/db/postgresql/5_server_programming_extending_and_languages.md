...
PostgreSQL의 가장 강력한 특징 중 하나는 확장성(extensibility)임. 이 장에서는 PostgreSQL의 SQL을 확장하는 다양한 방법을 다룸.

---

### 36.1 확장성의 작동 원리 (How Extensibility Works)

PostgreSQL은 카탈로그 주도(catalog-driven) 방식으로 동작하기 때문에 확장 가능함. 이는 일반적인 관계형 데이터베이스와 근본적으로 다른 설계임.

#### 핵심 원리

1. 카탈로그 주도 아키텍처
   - PostgreSQL은 시스템 카탈로그(메타데이터 테이블)에 광범위한 정보를 저장함
...
이러한 유연성 덕분에 PostgreSQL은 새로운 애플리케이션의 빠른 프로토타이핑, 새로운 저장 구조 실험, 커스텀 확장에 매우 적합함.

---

### 36.2 PostgreSQL 타입 시스템 (The PostgreSQL Type System)

PostgreSQL 데이터 타입은 크게 네 가지 범주로 나뉨.

#### 36.2.1 기본 타입 (Base Types)

SQL 언어 수준 아래에서 구현되며, 일반적으로 C로 작성됨.

- PostgreSQL은 사용자가 제공한 함수를 통해서만 기본 타입을 조작함
- 내장 기본 타입은 Chapter 8에 문서화됨
- 열거형 타입(enum types): SQL 명령으로만 생성 가능한 기본 타입의 하위 카테고리

#### 36.2.2 컨테이너 타입 (Container Types)

여러 값을 보유하는 세 종류의 컨테이너 타입이 있음.
...
기본 타입에 제약 조건을 추가하여 유효한 값의 부분집합으로 제한함.
...
PostgreSQL은 네 가지 종류의 사용자 정의 함수를 제공함.
...
SQL 함수는 임의의 SQL 문 목록을 실행하고 마지막 쿼리의 결과를 반환함.
...
동일한 SQL 이름으로 여러 함수를 정의할 수 있음(인수가 다른 경우).
...
C 언어 함수 오버로딩 시 각 함수의 C 이름이 모두 달라야 함:
...
모든 PostgreSQL 함수는 최적화기에게 함수의 동작을 알려주는 휘발성 분류를 가짐.
...
PostgreSQL의 사용자 정의 집계는 상태 값과 상태 전환 함수를 사용하여 정의됨.
...
윈도우 함수에서 이동 프레임 시작점을 효율적으로 처리하기 위해 역 전환 함수를 사용함.
...
병렬 집계를 위해 서로 다른 데이터 하위 집합의 부분 상태 값을 결합함.
...
PostgreSQL은 SQL 언어 수준 아래에서 정의되는 새로운 기본 타입의 생성을 허용함.

#### 핵심 요구사항

모든 사용자 정의 타입은 다음이 필요함:
...
1. 입출력 함수는 서로 역함수 관계여야 하며, 이 관계가 깨지면 데이터 덤프/복원 시 심각한 문제가 발생함.
...
사용자 정의 연산자는 함수 호출의 "구문적 설탕(syntactic sugar)"으로, 쿼리 플래너가 쿼리를 최적화하는 데 활용할 수 있는 추가 정보를 담을 수 있음.
...
PostgreSQL 연산자 정의에는 쿼리 실행을 최적화하는 선택적 절이 포함될 수 있음.

#### 36.15.1 COMMUTATOR (교환자)

정의되는 연산자의 교환자인 연산자를 식별함.

정의: 모든 가능한 입력 값에 대해 `(x A y)`와 `(y B x)`가 동일하면 연산자 A는 연산자 B의 교환자임.
...
사용 사례: 인덱스 및 조인 절 최적화에 중요 — 쿼리 최적화기가 다른 계획 유형을 위해 절의 순서를 뒤집을 수 있음.

#### 36.15.2 NEGATOR (부정자)

정의되는 연산자의 부정자인 연산자를 식별함.

정의: 모든 가능한 입력에 대해 `(x A y)`와 `NOT (x B y)`가 동일하면 연산자 A는 연산자 B의 부정자임.
...
사용 사례: 최적화기가 `NOT (x = y)`를 `x <> y`로 단순화할 수 있음.

#### 36.15.3 RESTRICT (제한)

연산자에 대한 제한 선택도 추정 함수를 지정함.
...
이 연산자를 기반으로 한 조인에 해시 조인 방법을 사용할 수 있음을 시스템에 알림.
...
이 연산자를 기반으로 한 조인에 병합 조인 방법을 사용할 수 있음을 시스템에 알림.
...
연산자 클래스는 인덱스 메서드가 특정 데이터 타입과 작동하는 데 필요한 연산 집합을 정의함.
...
PostgreSQL 확장을 사용하면 관련 SQL 객체를 단일 설치 가능한 단위로 패키징할 수 있음.
...
PostgreSQL은 필요한 경우 업데이트 스크립트를 자동으로 체인 연결함 (예: 1.0 → 1.1 → 2.0).
...
이러한 확장성 덕분에 PostgreSQL은 다양한 도메인별 요구사항을 충족하는 유연하고 강력한 데이터베이스 시스템으로 자리 잡음.
...
트리거(Trigger)는 특정 유형의 작업이 수행될 때마다 데이터베이스가 자동으로 특정 함수를 실행하도록 하는 명세임. 트리거를 연결할 수 있는 객체는 다음과 같음.
...
> 주의: 순수 SQL 함수 언어는 트리거 함수로 사용 불가.
...
뷰에서는 `INSTEAD OF` 트리거를 사용해 `INSERT`, `UPDATE`, `DELETE` 작업을 처리함.
...
- 트리거 이름의 알파벳 순서로 실행됨
- 각 트리거에서 수정된 행이 다음 트리거의 입력이 됨
- `BEFORE` 또는 `INSTEAD OF` 트리거가 `NULL`을 반환하면 해당 행에 대한 작업이 취소됨

---

### 3. 데이터 변경의 가시성 (Visibility of Data Changes)

트리거 내에서 데이터 변경의 가시성은 트리거 유형과 실행 시점에 따라 달라짐.
...
- BEFORE
  - 트리거를 발동시킨 데이터 변경을 볼 수 없음 (아직 발생하지 않음)
  - 같은 외부 명령에서 이전에 처리된 행의 변경 효과는 볼 수 있음
...
표준 절차적 언어로 작성된 트리거 함수에는 다음 규칙이 적용됨.

- VOLATILE (기본값): 위의 가시성 규칙이 적용됨
- STABLE 또는 IMMUTABLE: 트리거 유형에 관계없이 호출 명령에 의한 변경을 볼 수 없음

> 주의: 행 처리 순서는 명령이 여러 행에 영향을 미칠 때 예측할 수 없으므로, BEFORE 트리거에서 이전에 처리된 행의 변경 효과에 의존하는 것은 권장되지 않음.
...
PL/pgSQL 함수가 트리거로 호출될 때 다음 특수 변수들이 자동으로 생성됨.
...
반환 값은 무시됨 (NULL 반환 가능). 단, 오류를 발생시켜 작업 전체를 중단할 수 있음.
...
> 성능 팁: `WHEN` 조건을 사용하면 불필요한 트리거 실행을 방지해 성능을 크게 향상 가능.
...
`AFTER STATEMENT` 트리거에서 전이 테이블을 사용하면 영향받은 모든 행을 한 번에 처리 가능. 대량 데이터 처리 시 행 수준 트리거보다 훨씬 효율적임.
...
트리거 함수에 인자를 전달하면 범용으로 재사용 가능한 트리거를 만들 수 있음.
...
트리거 함수 내에서 다른 트리거를 발동시키는 SQL 문을 실행 가능.
...
> 주의: 캐스케이딩 트리거 사용 시 무한 재귀가 발생하지 않도록 주의 필요. 무한 루프 방지는 개발자의 책임임.

#### 7.5 생성된 컬럼과 트리거

저장된 생성 컬럼(Stored Generated Columns)은 `BEFORE` 트리거 실행 이후, `AFTER` 트리거 실행 이전에 계산됨.

- BEFORE 트리거: `OLD` 행에는 이전 생성 값이 포함되지만, `NEW` 행에는 아직 새 생성 값이 없음.
- 가상 생성 컬럼(Virtual Generated Columns): 트리거 발동 시점에는 계산되지 않음.

---

### 요약

PostgreSQL 트리거는 데이터베이스에서 자동화된 로직을 구현하는 강력한 도구임.
...
트리거를 효과적으로 활용하면 애플리케이션 코드를 단순화하고 데이터베이스 수준에서 일관된 비즈니스 로직을 유지 가능.
...
이벤트 트리거(Event Trigger)는 PostgreSQL에서 일반 트리거 메커니즘을 보완하는 기능임. DDL(Data Definition Language) 이벤트를 캡처하여 데이터베이스 수준의 스키마 변경 작업을 모니터링하고 제어 가능.
...
- 범위
  - 일반 트리거(Regular Trigger): 특정 테이블에 연결
  - 이벤트 트리거(Event Trigger): 데이터베이스 전역
- 이벤트 유형
  - 일반 트리거: DML (INSERT, UPDATE, DELETE)
  - 이벤트 트리거: DDL (CREATE, ALTER, DROP 등)
- 작성 언어
  - 일반 트리거: 모든 프로시저 언어 + SQL
  - 이벤트 트리거: 프로시저 언어 또는 C (SQL 제외)
- 반환 타입
  - 일반 트리거: `trigger`
  - 이벤트 트리거: `event_trigger`
- 데이터 접근
  - 일반 트리거: 테이블의 행 데이터
  - 이벤트 트리거: DDL 명령 메타데이터

---

### 3. 이벤트 종류

PostgreSQL은 현재 5가지 이벤트 트리거 유형을 지원함.

#### 3.1 ddl_command_start

DDL 명령이 실행되기 직전에 발생함.
...
DDL 명령이 완료된 직후에 발생함. 명령의 효과는 반영되었지만 트랜잭션은 아직 커밋되지 않은 상태.
...
객체가 시스템 카탈로그에서 삭제된 직후, `ddl_command_end` 이벤트 직전에 발생함.

이 이벤트는 `DROP` 명령과 일부 `ALTER` 명령(예: `ALTER TABLE DROP COLUMN`)에서 발생함.
...
테이블이 재작성(rewrite)되기 직전에 발생함. `ALTER TABLE` 또는 `ALTER TYPE` 명령으로 인해 테이블이 물리적으로 재작성될 때 발생함.

참고: `CLUSTER`나 `VACUUM`에 의한 재작성에는 트리거되지 않음.
...
인증된 사용자가 데이터베이스에 로그인할 때 발생함.

##### 주의사항
- 스탠바이 서버에서도 실행됨
- 버그가 있으면 시스템 로그인이 불가능해질 수 있음
- 스탠바이 서버에서는 데이터베이스 쓰기를 피해야 함
- 장기 실행 쿼리를 피해야 함
...
1. 반환 타입이 `event_trigger`인 함수를 먼저 생성해야 함
2. 함수는 값을 반환할 필요 없음 (반환 타입은 신호 역할만)
3. 동일 이벤트에 대한 여러 트리거는 트리거 이름의 알파벳 순서로 실행됨

#### 4.3 WHEN 조건 사용

특정 조건에서만 트리거를 실행하도록 필터링 가능.
...
PostgreSQL은 이벤트 트리거 내에서 정보를 얻기 위한 여러 지원 함수를 제공함.

#### 5.1 pg_event_trigger_ddl_commands()

`ddl_command_end` 이벤트에서 실행된 DDL 명령 목록을 반환함.
...
`sql_drop` 이벤트에서 삭제된 모든 객체 목록을 반환함.
...
`table_rewrite` 이벤트에서 재작성될 테이블의 OID를 반환함.
...
C에서 이벤트 트리거 함수를 작성할 때 `EventTriggerData` 구조체를 사용함.
...
- 이벤트 트리거는 중단된 트랜잭션에서 실행될 수 없음
- DDL 명령이 실패하면 `ddl_command_end` 트리거는 실행되지 않음
...
다음 객체에 대한 DDL 명령은 이벤트 트리거를 발생시키지 않음.
...
- 이벤트 트리거를 생성하려면 수퍼유저 권한이 필요
- 일반 사용자는 이벤트 트리거를 생성, 수정, 삭제할 수 없음
...
PostgreSQL의 규칙 시스템(Rule System)은 쿼리 재작성 규칙 시스템(Query Rewrite Rule System)임. 다른 데이터베이스 시스템에서 일반적으로 사용하는 저장 프로시저나 트리거와는 근본적으로 다른 접근 방식임.

#### 1.1 규칙 시스템의 작동 방식

규칙 시스템은 다음과 같이 작동함:

1. 쿼리 수정: 규칙을 반영하여 쿼리를 수정함
2. 수정된 쿼리 전달: 수정된 쿼리를 쿼리 플래너에 전달하여 계획 및 실행함

#### 1.2 규칙 시스템의 용도

규칙 시스템은 다음과 같은 용도로 사용됨:
...
쿼리 트리는 PostgreSQL 규칙 시스템에서 사용하는 SQL 문의 내부 표현임. 파서(Parser)와 플래너(Planner) 사이에 위치하며, 파싱된 쿼리와 사용자 정의 재작성 규칙을 0개 이상의 쿼리 트리로 변환함.

#### 2.1 쿼리 트리의 주요 구성 요소

##### 명령 유형 (Command Type)

쿼리 트리를 생성한 명령을 나타내는 단순 값임:
...
쿼리 결과가 기록될 위치를 식별하는 범위 테이블의 인덱스임:
...
`WHERE` 절에 해당하는 불리언 표현식으로, 작업 실행 여부를 결정함:
...
쿼리 트리는 다음 설정 매개변수를 사용하여 서버 로그에 표시할 수 있음:
...
규칙 액션은 `pg_rewrite` 시스템 카탈로그에 쿼리 트리로 저장됨.

---

### 3. 뷰와 규칙 시스템

PostgreSQL에서 뷰는 규칙 시스템을 사용하여 구현됨. 뷰는 본질적으로 `ON SELECT DO INSTEAD` 규칙(관례적으로 `_RETURN`이라는 이름)이 있는 빈 테이블임.
...
규칙 시스템은 쿼리 최적화 측면에서 다음과 같은 이점을 제공함:
...
PostgreSQL은 세 가지 메커니즘을 통해 뷰 업데이트를 지원함 (평가 순서대로):

##### 1. INSTEAD 규칙 (가장 먼저 평가)

`INSERT`, `UPDATE`, `DELETE` 명령을 기본 테이블에 대한 작업으로 재작성하는 사용자 정의 규칙임.
...
단일 기본 릴레이션에서 선택하는 간단한 뷰는 자동으로 기본 테이블을 업데이트하도록 재작성될 수 있음.

중요: 규칙은 트리거보다 먼저 평가되며, 규칙이나 트리거가 존재하면 자동 재작성이 우회됨.

오류 처리: 이러한 메커니즘 중 어느 것도 적용되지 않으면 실행기가 뷰를 직접 업데이트할 수 없어 오류가 발생함.

---

### 4. 구체화된 뷰 (Materialized Views)

구체화된 뷰는 규칙 시스템을 사용하여 쿼리 결과를 테이블과 유사한 형태로 저장함. 가상적인 일반 뷰와 달리 데이터를 물리적으로 저장함.
...
- 직접 업데이트: 구체화된 뷰는 불가능, 일반 테이블은 가능
- 쿼리 저장: 구체화된 뷰는 저장됨, 일반 테이블은 저장되지 않음
- 새로고침: 구체화된 뷰는 `REFRESH` 명령으로 가능, 일반 테이블은 수동 갱신 필요
...
구체화된 뷰와 인덱스를 사용하면 외부 데이터 래퍼를 통해 원격 데이터에 직접 접근하는 것에 비해 극적인 성능 향상을 얻을 수 있음 (예: 188ms vs 0.117ms).

---

### 5. INSERT, UPDATE, DELETE 규칙

`INSERT`, `UPDATE`, `DELETE`에 대한 규칙은 뷰 규칙과 크게 다름.

#### 5.1 뷰 규칙과의 주요 차이점

- 액션 수: 뷰 규칙은 하나의 `SELECT` 액션, UPDATE 규칙은 없음·하나·또는 여러 개
- 실행 방식: 뷰 규칙은 `INSTEAD` 전용, UPDATE 규칙은 `INSTEAD` 또는 `ALSO`
- 의사 릴레이션: 뷰 규칙은 없음, UPDATE 규칙은 `NEW`와 `OLD` 지원
- 규칙 자격 조건: 뷰 규칙은 없음, UPDATE 규칙은 지원됨
- 쿼리 트리 처리: 뷰 규칙은 기존 트리 수정, UPDATE 규칙은 새 쿼리 트리 생성

#### 5.2 중요 주의사항

> 권장사항: 많은 작업에 대해 규칙 대신 트리거 사용을 권장함.
...
여기서 `event`는 `INSERT`, `UPDATE`, `DELETE` 중 하나임.
...
규칙에 의해 쿼리가 재작성되면 원래 쿼리에 명시적으로 지정된 것 외의 테이블/뷰에도 접근하게 됨.
...
표준 뷰는 함수 실행 순서 조작을 통해 데이터를 유출할 수 있음:
...
`security_barrier`를 사용하더라도 뷰는 다음에 대해 완전히 안전하지 않음:
...
매우 민감한 데이터는 뷰에만 의존하기보다 접근 자체를 제한하는 방안을 고려할 필요가 있음.

---

### 7. 규칙과 명령 상태

PostgreSQL이 쿼리를 실행할 때 명령 상태 문자열(예: `INSERT 149592 1`)을 반환함. 규칙은 어떤 명령 상태가 반환되는지에 영향을 미칠 수 있음.
...
> 활성 규칙들 사이에서 알파벳 순으로 마지막이 되는 규칙 이름을 부여하여 마지막에 적용되도록 함.
...
규칙과 트리거는 모두 데이터베이스에서 자동화된 동작을 구현하지만, 서로 다른 수준에서 작동함.

#### 8.1 주요 차이점

- 실행 수준: 트리거는 영향받는 각 행마다 한 번 실행, 규칙은 쿼리 수준에서 작동
- 개념적 복잡도: 트리거는 더 단순하고 초보자에게 쉬움, 규칙은 더 복잡하고 학습 곡선이 있음
- 작동 방식: 트리거는 행별 트리거 함수 호출, 규칙은 쿼리 수정 또는 추가 쿼리 생성
- 효율성 (대량 작업): 트리거는 행 수에 비례하여 작업량 증가, 규칙은 행 수와 무관하게 하나의 쿼리
...
> 규칙이 트리거보다 현저히 느린 경우는, 규칙 액션이 조건이 불명확한 대규모 조인을 생성하여 플래너가 좋은 계획을 수립하지 못할 때뿐임.

---

### 요약

PostgreSQL의 규칙 시스템은 쿼리 재작성을 통해 강력한 기능을 제공함:
...
규칙 시스템을 잘 활용하면 복잡한 데이터베이스 로직을 쿼리 수준에서 처리할 수 있음. 그러나 대부분의 일반적인 사용 사례에서는 트리거가 더 단순하고 예측 가능한 선택임.
...
PostgreSQL은 SQL과 C 외에도 다른 언어로 사용자 정의 함수(User-Defined Functions) 작성 가능. 이러한 언어들을 일반적으로 절차적 언어(Procedural Languages, PL)라고 함.

절차적 언어로 작성된 함수의 경우, 데이터베이스 서버는 함수의 소스 텍스트를 해석하는 방법에 대한 기본 제공 지식이 없음. 대신 이 작업은 해당 언어의 세부 사항을 알고 있는 특수한 핸들러(Handler)에게 전달됨.

#### 1.2 핸들러의 작동 방식

핸들러 작동 방식:

- 직접 처리: 파싱(Parsing), 구문 분석(Syntax Analysis), 실행(Execution) 등 모든 작업 수행
- 중개 역할: PostgreSQL과 기존 프로그래밍 언어 구현 간의 "접착제(Glue)" 역할

핸들러 자체는 공유 객체(Shared Object)로 컴파일되고 필요 시 로드되는 C 언어 함수임.

#### 1.3 표준 배포판의 절차적 언어

PostgreSQL 표준 배포판에는 현재 4가지 절차적 언어 포함:
...
> 참고: 핵심 배포판에 포함되지 않은 추가 절차적 언어는 PostgreSQL의 External Projects에서 확인 가능.
...
절차적 언어는 사용할 각 데이터베이스에 설치 필요. `template1` 데이터베이스에 설치된 언어는 이후 생성되는 모든 데이터베이스에서 자동 사용 가능.

#### 2.2 간편 설치 방법 (권장)

표준 배포판에 포함된 언어의 경우, `CREATE EXTENSION` 명령 사용:
...
확장(Extension)으로 패키지되지 않은 언어의 경우, 다음 5단계 수행 필요. 이 작업은 데이터베이스 슈퍼유저(Superuser)만 수행 가능.

##### 단계 1: 공유 객체 컴파일 및 설치

언어 핸들러를 공유 객체로 컴파일하고 적절한 라이브러리 디렉토리에 설치.
...
- PL/pgSQL: 기본 설치됨 (모든 데이터베이스)
- PL/Tcl, PL/TclU: 기본 설치 안 됨 (설정 시)
- PL/Perl, PL/PerlU: 기본 설치 안 됨 (설정 시)
- PL/PythonU: 기본 설치 안 됨 (설정 시)
...
PL/pgSQL은 PostgreSQL 데이터베이스 시스템을 위한 로드 가능한 절차적 언어임. PostgreSQL 9.0 이상에서는 기본적으로 설치됨.
...
문제점: SQL은 각 문장을 개별적으로 데이터베이스 서버에서 실행해야 함. 클라이언트 애플리케이션이 각 쿼리를 전송 → 처리 대기 → 결과 수신 → 처리 후 다시 서버로 쿼리를 보내야 하므로 프로세스 간 통신과 네트워크 오버헤드 발생.
...
PL/pgSQL은 블록 구조 언어임. 함수 본문은 반드시 블록이어야 함:
...
모든 PL/pgSQL 표현식은 메인 SQL 실행기를 통해 처리됨. 표현식은 내부적으로 `SELECT` 명령으로 변환됨.
...
PL/Tcl은 PostgreSQL에서 [Tcl 언어](https://www.tcl.tk/)를 사용하여 함수와 프로시저를 작성할 수 있게 해주는 로드 가능한 절차적 언어임.
...
PL/Perl은 PostgreSQL 함수와 프로시저를 Perl 프로그래밍 언어로 작성할 수 있게 해주는 로드 가능한 절차적 언어임.
...
> 참고: 소스 패키지에서는 설치 과정 중 PL/Perl을 특별히 활성화 필요. 바이너리 패키지에서는 별도의 서브패키지에 포함될 수 있음.
...
PL/Python은 PostgreSQL 함수와 프로시저를 Python으로 작성할 수 있게 해주는 절차적 언어임.
...
모범 사례: 함수 매개변수를 읽기 전용으로 취급하여 구현 세부 사항에 의존하지 않도록 함.
...
함수 작성자는 신뢰할 수 없는 PL/Python 함수가 악의적인 목적으로 악용되지 않도록 보장 필요. PL/Python 함수는 데이터베이스 관리자와 동일한 권한을 가짐.
...
PL/pgSQL은 PostgreSQL 데이터베이스 시스템을 위한 로드 가능한 절차적 언어임. PL/pgSQL의 설계 목표는 다음과 같음:
...
PL/pgSQL로 작성된 함수는 내장 함수가 사용될 수 있는 모든 곳에서 사용할 수 있음. 예를 들어, 복잡한 조건부 계산 함수를 만들고 이를 연산자 정의나 인덱스 표현식에서 사용할 수 있음.

PostgreSQL 9.0 이상에서 PL/pgSQL은 기본적으로 설치됨. 그러나 여전히 로드 가능한 모듈이므로, 특히 보안에 민감한 관리자는 이를 제거할 수 있음.
...
SQL은 PostgreSQL 및 대부분의 다른 관계형 데이터베이스가 쿼리 언어로 사용하는 언어임. 이식 가능하고 배우기 쉬움. 그러나 모든 SQL 문은 데이터베이스 서버에서 개별적으로 실행되어야 함.

이는 클라이언트 애플리케이션이 각 쿼리를 데이터베이스 서버로 보내고, 처리되기를 기다린 후 결과를 받고, 일부 계산을 수행한 다음 서버에 추가 쿼리를 보내야 함을 의미함. 클라이언트가 데이터베이스 서버와 다른 머신에 있는 경우 이 모든 것이 프로세스 간 통신을 수반하며 네트워크 오버헤드를 발생시킴.

PL/pgSQL을 사용하면 계산 블록과 일련의 쿼리를 데이터베이스 서버 내에서 그룹화하여 절차적 언어의 강력함과 SQL의 사용 용이성을 결합하면서 상당한 클라이언트/서버 통신 오버헤드를 절약할 수 있음.

- 클라이언트와 서버 간의 추가 왕복 제거
- 클라이언트에 필요하지 않거나 단순히 조건을 테스트하기 위해 전송해야 하는 중간 결과를 서버와 클라이언트 간에 전송할 필요 없음
- 여러 번의 쿼리 파싱을 피할 수 있음

이로 인해 순수 SQL을 사용하는 것에 비해 상당한 성능 향상을 얻을 수 있음.

또한 PL/pgSQL을 사용하면 SQL에서 사용 가능한 모든 데이터 타입, 연산자, 함수를 사용할 수 있음.

#### 43.1.2 지원되는 인자 및 결과 데이터 타입

PL/pgSQL으로 작성된 함수는 서버가 지원하는 모든 스칼라 또는 배열 데이터 타입을 인자로 받아들이고 결과로 반환할 수 있음. 또한 지정된 행 타입의 복합 타입(행 타입)을 받아들이거나 반환할 수 있음. 함수가 반환 타입으로 `record`를 선언하면 익명의 복합 타입도 반환할 수 있음.

PL/pgSQL 함수는 `VARIADIC` 마커를 사용하여 가변 개수의 인자를 받아들이도록 선언할 수 있음. 이는 SQL 함수와 정확히 동일한 방식으로 작동함.

PL/pgSQL 함수는 다형성 타입 `anyelement`, `anyarray`, `anynonarray`, `anyenum`, `anyrange`, `anycompatible`, `anycompatiblearray`, `anycompatiblenonarray`, `anycompatiblerange`를 사용하여 다형성으로 선언할 수도 있음.

PL/pgSQL 함수는 집합(또는 테이블)을 반환하도록 선언할 수도 있음. 이러한 함수는 모든 집합을 생성하기 위해 원하는 각 행에 대해 `RETURN NEXT`를 실행하거나 `RETURN QUERY`를 사용하여 쿼리 결과를 출력함.

---

### 43.2 PL/pgSQL의 구조 (Structure of PL/pgSQL)

PL/pgSQL로 작성된 함수는 다음과 같은 형식으로 정의됨:
...
함수 본문은 단순히 `CREATE FUNCTION`에 대한 문자열 리터럴임. 달러 인용(dollar quoting)을 사용하여 함수 본문을 작성하는 것이 더 도움이 됨. 달러 인용이 없으면 함수 본문의 작은따옴표나 백슬래시를 이중으로 사용하여 이스케이프해야 함.

#### 43.2.1 블록 구조

PL/pgSQL은 블록 구조 언어임. 함수 본문의 전체 텍스트는 블록이어야 함. 블록은 다음과 같이 정의됨:
...
블록 내의 각 선언과 각 문은 세미콜론으로 종료됨. 위에 표시된 대로 다른 블록 내에 나타나는 블록은 세미콜론을 가져야 하지만, 함수 본문을 종료하는 최종 `END`는 세미콜론이 필요하지 않음.

> 팁: 일반적인 실수는 `BEGIN`/`END` 바로 앞에 세미콜론을 쓰는 것임. 이는 올바르지 않으며 구문 오류가 발생함.

레이블(label)은 `EXIT` 문에 의해 종료되거나 블록에서 선언된 변수의 이름을 한정하는 데에만 필요함. 레이블이 `END` 뒤에 지정되면 블록의 시작 레이블과 일치해야 함.

모든 키워드는 대소문자를 구분하지 않음. 식별자는 큰따옴표로 묶이지 않는 한 암묵적으로 소문자로 변환됨.

PL/pgSQL 코드에서 주석은 SQL에서와 동일한 방식으로 작동함. 이중 대시(`--`)는 줄 끝까지 확장되는 주석을 시작함. `/*`는 `*/`가 나타날 때까지 확장되는 블록 주석을 시작함. 블록 주석은 중첩됨.

블록의 문 섹션에 있는 모든 문은 중첩 블록일 수 있음. 블록은 컨트롤 구문의 논리적 그룹화 또는 변수를 작은 그룹의 문에 지역화하는 데 사용할 수 있음. 선언 섹션에서 선언된 변수는 블록 내의 문을 처리하기 전에 기본값으로 초기화되며, 매번 블록에 진입할 때마다 초기화됨.
...
> 참고: `BEGIN`/`END` 쌍과 트랜잭션 제어를 위한 같은 이름의 SQL 명령 사이에는 실제로 혼동이 없음. PL/pgSQL의 `BEGIN`/`END`는 그룹화만을 위한 것이며 트랜잭션을 시작하거나 종료하지 않음. PL/pgSQL에서 트랜잭션을 관리하는 것은 나중에 설명됨.

---

### 43.3 선언 (Declarations)

블록에서 사용되는 모든 변수는 블록의 선언 섹션에서 선언되어야 함. (유일한 예외는 `FOR` 루프의 루프 변수로, 정수 범위를 반복하는 경우 자동으로 정수 변수로 선언되고, 커서 결과를 반복하는 경우 레코드 변수로 선언됨.)

PL/pgSQL 변수는 `integer`, `varchar`, `char`와 같은 모든 SQL 데이터 타입은 물론, 사용자 정의 타입도 가질 수 있음.

변수 선언의 일반적인 구문은 다음과 같음:

```sql
name [ CONSTANT ] type [ COLLATE collation_name ] [ NOT NULL ] [ { DEFAULT | := | = } expression ];
```

`DEFAULT` 절이 주어지면 블록에 진입할 때 변수에 할당될 초기값을 지정함. `DEFAULT` 절이 주어지지 않으면 변수는 SQL 널 값으로 초기화됨. `CONSTANT` 옵션은 변수가 값을 할당받지 못하도록 하여 블록이 지속되는 동안 값이 일정하게 유지되도록 함. `COLLATE` 옵션은 변수에 사용할 조합(collation)을 지정함. `NOT NULL`이 지정되면 널 값을 할당하면 런타임 오류가 발생함. `NOT NULL`로 선언된 모든 변수는 널이 아닌 기본값을 지정해야 함. `=`는 PL/SQL 규정을 따르기 위해 `:=` 대신 사용할 수 있음.

변수의 기본값은 함수가 호출될 때마다가 아니라 블록에 진입할 때마다 평가되고 변수에 할당됨. 예를 들어, `integer` 변수에 `now()`를 할당하면 `now()`가 호출되는 순간의 현재 타임스탬프가 할당됨.
...
변수의 기본값 표현식은 블록에 진입할 때마다 실행됨. 최상위 블록 변수라도 함수가 호출될 때마다 기본 표현식이 평가됨.

#### 43.3.1 함수 인자 선언

함수에 전달된 인자는 식별자 `$1`, `$2` 등으로 명명됨. 선택적으로 가독성 향상을 위해 인자 값에 대한 별칭을 선언할 수 있음. 그런 다음 별칭이나 숫자 식별자를 사용하여 인자 값을 참조할 수 있음.

별칭을 만드는 두 가지 방법이 있음. 선호되는 방법은 `CREATE FUNCTION` 명령에서 인자에 이름을 지정하는 것임:
...
다른 방법은 선언 구문으로 별칭을 명시적으로 선언하는 것임:
...
> 참고: 이 두 예제는 완전히 동일하지 않음. 첫 번째 경우, `subtotal`은 `sales_tax.subtotal`로 참조될 수 있지만, 두 번째 경우에는 그렇지 않음. (별칭을 외부 블록에 레이블을 붙인 경우 별칭을 한정할 수 있음.)

출력 인자는 입력 인자와 동일한 방식으로 PL/pgSQL 함수에서 처리됨. 출력 인자는 함수의 시작 부분에서 NULL로 시작되며, 함수 실행 중에 할당되어야 함. 최종 출력 인자의 값은 반환되는 값임. 예를 들어, 판매세 예제는 다음과 같이 작성할 수도 있음:
...
`ALIAS` 구문은 함수 인자에 별칭을 할당하는 것보다 더 일반적임: 이전에 선언된 변수에 다른 이름을 선언할 수 있음. 이는 주로 트리거 함수와 같이 미리 정의된 이름을 가진 변수에 더 의미 있는 이름을 할당하는 데 유용함.
...
`ALIAS`는 새 변수를 만듦; 같은 변수를 참조하는 두 가지 방법이 아님.
...
`%TYPE`은 변수나 테이블 열의 데이터 타입을 제공함. 이를 사용하여 데이터베이스 객체의 데이터 타입을 보유하는 변수를 선언할 수 있음. 예를 들어, `users` 테이블에 `user_id`라는 열이 있다고 가정하면 다음과 같이 작성할 수 있음:

```sql
user_id users.user_id%TYPE;
```

`%TYPE`을 사용하면 참조하는 데이터베이스 객체의 정의를 알 필요가 없으며, 가장 중요한 것은 참조된 객체의 데이터 타입이 미래에 변경되더라도(예: `user_id`의 타입을 `integer`에서 `real`로 변경) 함수 정의를 변경할 필요가 없다는 것임.

`%TYPE`은 함수 인자에도 사용할 수 있음:
...
복합 타입의 변수를 행 변수(또는 행-타입 변수)라고 함. 이러한 변수는 `SELECT` 또는 `FOR` 쿼리 결과의 전체 행을 보유할 수 있음. 단, 해당 쿼리의 열 집합이 변수의 선언된 타입과 일치해야 함. 행 값의 개별 필드는 일반적인 점 표기법을 사용하여 액세스됨(예: `rowvar.field`).

행 변수는 복합 타입의 이름을 사용하여 선언하거나 `table_name%ROWTYPE` 표기법을 사용하여 테이블 또는 뷰의 행과 동일한 타입을 가진 변수를 선언할 수 있음. `%ROWTYPE`과 함께 사용되는 테이블 또는 뷰 이름은 스키마로 한정될 수 있음.

함수의 인자는 복합 타입(테이블 행 포함)일 수 있음. 이 경우 해당 식별자 `$n`은 행 변수가 되며, 필드는 점 표기법을 사용하여 선택할 수 있음(예: `$1.user_id`).
...
레코드 변수는 행 타입 변수와 비슷하지만 미리 정의된 구조가 없음. 레코드 변수는 `SELECT` 또는 `FOR` 명령 중에 할당된 행의 실제 행 구조를 취함. 레코드 변수의 하위 구조는 할당될 때마다 변경될 수 있음.

레코드 변수는 처음 할당되기 전까지 하위 구조가 없다는 결과가 있음. 할당되지 않은 레코드 변수의 필드에 액세스하려고 하면 런타임 오류가 발생함.

`RECORD`는 진정한 데이터 타입이 아니라 플레이스홀더일 뿐임. PL/pgSQL 함수가 `record` 타입을 반환하도록 선언된 경우 이는 레코드 변수와 정확히 동일한 개념이 아님.

#### 43.3.6 PL/pgSQL 변수의 데이터 정렬 (Collation of PL/pgSQL Variables)

PL/pgSQL 함수가 데이터 정렬(collation)이 가능한 타입의 인자를 받는 경우, 함수 호출 시 인자로부터 적용할 collation이 결정됨. 인자 간에 암묵적 collation 충돌이 없으면, 모든 해당 인자가 그 collation을 암묵적으로 갖는 것으로 처리됨.

---

### 43.4 표현식 (Expressions)

PL/pgSQL에서 사용되는 모든 표현식은 PostgreSQL의 메인 SQL 실행기에서 처리됨. 예를 들어 다음과 같이 작성할 때:

```sql
IF expression THEN ...
```

PL/pgSQL은 내부적으로 다음과 같은 쿼리를 실행함:

```sql
SELECT expression
```

표현식에서 선언된 PL/pgSQL 변수에 대한 참조를 매개변수로 대체함. 이렇게 하면 `SELECT`에 대한 쿼리 계획이 한 번만 준비되고 이후 호출에서 재사용됨.

이러한 방식으로 표현식을 평가하면 일반적인 SQL 쿼리가 할 수 있는 모든 것을 의미함:

- 산술, 문자열, 비교 연산자를 사용하여 복잡한 표현식 계산
- 함수 호출
- 스칼라 서브쿼리 포함

그러나 표현식이 스칼라 값으로 평가되어야 한다는 요구 사항도 있음(복합 타입을 반환하는 경우도 단일 값).
...
변수 또는 행/레코드 필드에 대한 할당은 다음과 같이 작성됨:

```sql
variable { := | = } expression;
```

앞서 언급했듯이 이러한 문의 표현식은 PostgreSQL 메인 SQL 엔진에 전송된 `SELECT` 명령을 통해 평가됨. 표현식은 단일 값을 생성해야 함(변수가 행 또는 레코드 변수인 경우 행 값일 수 있음).

대상 변수가 단순 변수(행/레코드 변수가 아님)인 경우 `=`를 `:=` 대신 사용할 수 있음.
...
단일 행(아마도 여러 열)을 생성하는 SQL 명령의 결과는 레코드 변수, 행 타입 변수 또는 스칼라 변수 목록에 할당될 수 있음. 이는 기본 SQL 명령에 `INTO` 절을 작성하여 수행됨:
...
여기서 `target`은 레코드 변수, 행 변수 또는 쉼표로 구분된 단순 변수와 레코드/행 필드 목록일 수 있음.

`SELECT`를 사용하는 경우 `INTO` 절은 쿼리의 `select_expressions` 목록 바로 뒤, 또는 `FROM` 절 바로 앞에 작성할 수 있음.

`STRICT`가 지정되지 않으면 `target`은 쿼리에서 반환된 첫 번째 행으로 설정되고, 쿼리가 행을 반환하지 않으면 널로 설정됨. (첫 번째 행은 `ORDER BY`를 사용하지 않는 한 "정의되지 않음"임.) 첫 번째 행 이후의 모든 결과 행은 버려짐.
...
`STRICT` 옵션이 지정되면 쿼리가 정확히 하나의 행을 반환해야 함. 그렇지 않으면 `NO_DATA_FOUND`(행이 없음) 또는 `TOO_MANY_ROWS`(둘 이상의 행)와 같은 런타임 오류가 발생함.
...
`STRICT`가 지정되지 않은 쿼리의 경우 `FOUND`는 행이 반환되면 true로, 반환되지 않으면 false로 설정됨.

#### 43.5.3 결과가 없는 명령 또는 동적 명령 실행 (Executing a Command with No Result)

관심 있는 결과를 반환하지 않는 SQL 명령의 경우(예: `INSERT` without `RETURNING`, 또는 결과가 필요 없는 다른 DML), 일반 SQL 명령으로 실행할 수 있음:

```sql
INSERT INTO mytable (id, value) VALUES (nextval('myseq'), 'hello');
```

또는 `PERFORM` 문으로 표현식을 평가하고 결과를 버릴 수 있음:

```sql
PERFORM query;
```

`SELECT` 대신 `PERFORM` 키워드를 사용하는 것 외에는 일반 `SELECT` 문과 동일하게 작성하며, 실행 결과는 모두 버려짐.
...
가변 테이블이나 열에 대해 동적 명령을 생성해야 할 때가 많음. PL/pgSQL 함수 내에서 동적 명령을 실행하려면 `EXECUTE` 문을 사용함:

```sql
EXECUTE command-string [ INTO [STRICT] target ] [ USING expression [, ... ] ];
```

여기서 `command-string`은 실행할 명령을 포함하는 문자열(텍스트 타입)을 생성하는 표현식임.
...
`USING` 절을 사용하여 명령에 매개변수 값을 삽입함. 이는 명령 문자열에 값을 텍스트로 직접 삽입하는 것보다 선호되는 방식으로, 값을 텍스트로 변환하고 다시 되돌리는 런타임 오버헤드를 줄이고 SQL 인젝션 위험도 낮춤.

식별자나 리터럴을 동적으로 삽입해야 할 경우 `format` 함수를 함께 사용하면 편리함:
...
`%I` 지정자는 식별자로 처리되어야 하는 값을 삽입하고, `%L` 지정자는 리터럴로 처리되어야 하는 값을 삽입함.

#### 43.5.5 쿼리 결과의 상태 얻기 (Obtaining the Result Status)

다음과 같은 명령을 실행한 후 영향을 받은 행 수를 확인할 수 있음:

```sql
GET DIAGNOSTICS variable { = | := } item [ , ... ];
```

현재 사용 가능한 상태 항목은 다음과 같음:
...
아무 작업도 수행하지 않는 플레이스홀더 문이 필요할 때가 있음. 예를 들어, if/then/else 체인의 한 분기를 의도적으로 비워 두고 싶을 때는 다음과 같이 작성함:

```sql
NULL;
```

예를 들어, 다음 두 코드 조각은 동일함:
...
제어 구조는 아마도 PL/pgSQL의 가장 유용하고 중요한 부분임. PL/pgSQL의 제어 구조를 사용하면 매우 유연하고 강력한 방식으로 PostgreSQL 데이터를 조작할 수 있음.

#### 43.6.1 함수에서 반환 (Returning from a Function)

함수에서 데이터를 반환하는 두 가지 명령이 있음: `RETURN`과 `RETURN NEXT`(+ `RETURN QUERY`).
...
`RETURN`과 함께 표현식을 사용하면 함수가 종료되고 `expression`의 값이 호출자에게 반환됨. 이 형식은 스칼라 타입을 반환하는 PL/pgSQL 함수에서 사용됨.

```sql
RETURN;
```

표현식 없는 `RETURN`을 사용하여 `void`를 반환하도록 선언된 함수를 종료함.

프로시저에서 반환할 때는 표현식 없는 `RETURN`을 사용함.

복합 타입을 반환하도록 선언된 함수의 경우 표현식은 적절한 복합 값을 생성해야 함:
...
함수가 `SETOF sometype`을 반환하도록 선언된 경우 따라야 할 절차가 약간 다름. 개별 항목은 `RETURN NEXT` 명령의 시퀀스를 통해 반환되고, 마지막에 표현식 없는 `RETURN`이 함수가 실행을 완료했음을 나타내는 데 사용됨.
...
`RETURN QUERY`는 쿼리 결과의 모든 행을 함수의 결과 집합에 추가함:
...
프로시저에는 반환 값이 없음. 따라서 프로시저는 표현식 없는 `RETURN` 문으로 종료할 수 있음:

```sql
RETURN;
```

`RETURN` 문은 프로시저가 자연스럽게 끝나기 전에 종료하려는 경우에만 필요함.

프로시저의 출력 인자는 프로시저가 정상적으로 종료되거나 `RETURN` 문에 도달하면 호출자에게 반환됨.

#### 43.6.3 호출자에 대한 제어 반환 (Returning Control to the Caller)

프로시저 본문 내부에서 `COMMIT` 또는 `ROLLBACK`을 호출하면 트랜잭션이 종료되고 새 트랜잭션이 자동으로 시작됨. 이러한 명령 후에 실행이 계속됨.

#### 43.6.4 조건문 (Conditionals)

`IF`와 `CASE` 문을 사용하면 특정 조건에 따라 명령을 실행할 수 있음. PL/pgSQL에는 세 가지 형태의 `IF`가 있음:

- `IF ... THEN ... END IF`
- `IF ... THEN ... ELSE ... END IF`
- `IF ... THEN ... ELSIF ... THEN ... ELSE ... END IF`

그리고 두 가지 형태의 `CASE`가 있음:
...
`IF-THEN` 문은 가장 간단한 형태의 `IF`임. `THEN`과 `END IF` 사이의 문은 조건이 참이면 실행됨. 그렇지 않으면 건너뜀.
...
`IF-THEN-ELSE` 문은 조건이 참이 아닐 때 실행되어야 할 대체 문 집합을 지정할 수 있도록 `IF-THEN`에 추가함.
...
가끔 두 가지 이상의 대안이 있음. `IF-THEN-ELSIF`는 이를 가능하게 함. 조건은 참이 될 때까지 연속적으로 테스트됨.
...
간단한 형태의 `CASE`는 검색 표현식의 값을 기반으로 조건부 실행을 제공함.
...
검색된 형태의 `CASE`는 부울 표현식의 진리값을 기반으로 조건부 실행을 제공함.
...
`LOOP`, `EXIT`, `CONTINUE`, `WHILE`, `FOR`, `FOREACH` 문을 사용하면 PL/pgSQL 함수가 일련의 명령을 반복하도록 할 수 있음.
...
`LOOP`는 `EXIT` 또는 `RETURN` 문에 의해 종료될 때까지 무한히 반복되는 무조건적인 루프를 정의함. 선택적 레이블은 중첩된 루프 내의 `EXIT` 및 `CONTINUE` 문에서 종료하거나 계속할 루프를 지정하는 데 사용할 수 있음.
...
레이블이 지정되지 않은 경우 가장 안쪽의 루프가 종료되고 `END LOOP` 다음 문이 실행됨. 레이블이 지정되면 해당 레이블이 있는 루프가 종료됨.
...
`WHEN`이 있으면 부울 표현식이 참인 경우에만 루프 종료가 발생함:
...
레이블이 지정되지 않은 경우 가장 안쪽 루프의 다음 반복이 시작됨. 레이블이 지정되면 해당 레이블이 있는 루프의 다음 반복이 시작됨.
...
`WHILE` 문은 부울 표현식이 참으로 평가되는 한 일련의 문을 반복함. 각 루프 본문에 진입하기 직전에 표현식이 확인됨.
...
이 형태의 `FOR`는 정수 값의 범위를 반복하는 루프를 만듦. 변수 `name`은 자동으로 `integer` 타입으로 정의되며 루프 내에서만 존재함(기존 변수의 정의는 루프 내에서 무시됨). 범위의 하한과 상한을 지정하는 두 표현식은 루프에 진입할 때 한 번 평가됨.
...
다른 타입의 `FOR` 루프를 사용하면 쿼리의 결과를 반복하고 해당 데이터를 그에 따라 조작할 수 있음.
...
`target`은 레코드 변수, 행 변수 또는 쉼표로 구분된 스칼라 변수 목록임. `target`은 `query`의 각 행에 연속적으로 할당되고 루프 본문이 각 행에 대해 실행됨.
...
`FOR-IN-EXECUTE`를 사용하여 동적 쿼리의 결과를 반복할 수도 있음:
...
`FOREACH` 루프는 `FOR` 루프와 매우 비슷하지만 쿼리에서 반환된 행을 반복하는 대신 배열 값의 요소를 반복함.
...
`SLICE` 없이(또는 `SLICE 0`) `FOREACH`는 표현식을 평가하여 생성된 배열의 개별 요소를 반복함.
...
양수 `SLICE` 값을 사용하면 `FOREACH`는 개별 요소가 아니라 배열의 슬라이스를 반복함.
...
기본적으로 PL/pgSQL 함수에서 발생하는 모든 오류는 함수 실행을 중단하고 실제로 주변 트랜잭션도 중단함. `BEGIN` 블록에 `EXCEPTION` 절을 사용하여 오류를 트랩하고 복구할 수 있음:
...
`EXCEPTION` 절이 있는 경우, 블록에 진입하기 전에 암묵적인 서브트랜잭션(subtransaction)이 설정됨. 블록이 성공적으로 완료되면 서브트랜잭션이 커밋됨. 오류가 발생하면 블록에서 수행한 모든 데이터베이스 변경 사항이 롤백되고 적절한 예외 핸들러로 제어가 전달됨.
...
특수 조건 이름 `OTHERS`는 `QUERY_CANCELED`와 `ASSERT_FAILURE`를 제외한 모든 오류 타입과 일치함. `OTHERS` 핸들러 끝에는 일반적으로 `RAISE`를 사용하여 원래 오류를 다시 발생시킴.

예외 핸들러 내에서 `GET STACKED DIAGNOSTICS` 명령을 사용하여 현재 예외에 대한 정보를 얻을 수 있음:
...
전체 결과 집합을 한꺼번에 처리하는 대신, 커서를 선언하여 쿼리를 캡슐화하고 결과를 몇 행씩 순차적으로 읽을 수 있음. 결과 행 수가 매우 많을 때 메모리 오버플로를 방지하는 데 유용함.

커서를 함수의 반환값으로 사용하는 방법도 있음. 함수에서 대량의 행 집합을 효율적으로 반환할 때 유용하며, 호출자는 반환된 커서를 통해 행을 직접 읽음.

#### 43.7.1 커서 변수 선언 (Declaring Cursor Variables)

PL/pgSQL에서 커서에 대한 모든 액세스는 커서 변수를 통해 이루어지며, 이는 항상 특수 데이터 타입 `refcursor`임. 커서 변수를 만드는 한 가지 방법은 `refcursor` 타입의 변수로 선언하는 것임. 다른 방법은 커서 선언 구문을 사용하는 것임:

```sql
name [ [ NO ] SCROLL ] CURSOR [ ( arguments ) ] FOR query;
```

`SCROLL`이 지정되면 커서는 역방향으로 스크롤할 수 있음. `NO SCROLL`이 지정되면 역방향 페치가 거부됨. 지정되지 않은 경우 쿼리에 따라 역방향 페치가 허용될 수 있음.
...
세 가지 모두 데이터 타입은 `refcursor`이지만, 첫 번째는 모든 쿼리에 사용할 수 있고, 두 번째는 이미 완전히 지정된 쿼리에 바인딩되어 있으며, 세 번째는 매개변수화된 쿼리에 바인딩되어 있음.

#### 43.7.2 커서 열기 (Opening Cursors)

커서를 사용하여 행을 검색하기 전에 열어야 함. PL/pgSQL에는 세 가지 형태의 `OPEN` 문이 있음.
...
커서 변수가 열리고 지정된 쿼리가 실행됨. 쿼리는 문자열 표현식으로 지정됨.
...
이 형태의 `OPEN`은 선언 시 쿼리에 바인딩된 커서 변수에 사용됨.
...
커서가 열리면 여기에 설명된 문을 사용하여 조작할 수 있음.
...
`FETCH`는 커서에서 다음 행을 가져와 `target`(행 변수, 레코드 변수 또는 쉼표로 구분된 단순 변수 목록)에 저장함.
...
`direction` 절은 SQL의 `FETCH` 명령에서 허용된 모든 변형이 될 수 있음:
...
성공적인 페치 후 `FOUND`는 true로 설정되고, 더 이상 행이 없으면 false로 설정됨.
...
`MOVE`는 데이터를 반환하지 않고 커서를 재배치함. `FETCH`와 정확히 동일하게 작동하지만 행을 반환하지 않음.
...
커서가 테이블에 단순히 위치해 있으면 `WHERE CURRENT OF` 조건을 사용하여 가장 최근에 페치된 행을 업데이트하거나 삭제할 수 있음.
...
`CLOSE`는 열린 커서의 기반이 되는 포털을 닫음. 커서가 닫히면 트랜잭션이 끝나기 전에 다시 열 수 있으며, 동일하거나 다른 쿼리에 바인딩될 수 있음.
...
PL/pgSQL 함수는 호출자에게 커서를 반환할 수 있음. 대량의 행 집합을 반환할 때 유용하며, 함수는 커서 이름 문자열을 반환함.
...
함수가 아닌 저장 프로시저에서는 `COMMIT` 및 `ROLLBACK` 명령으로 현재 트랜잭션을 종료하고 새 트랜잭션을 시작할 수 있음.
...
새로운 트랜잭션은 기본 트랜잭션 특성(예: 격리 수준)으로 시작됨. `COMMIT AND CHAIN` 및 `ROLLBACK AND CHAIN` 변형을 사용하면 종료된 트랜잭션과 동일한 트랜잭션 특성을 가진 새 트랜잭션을 시작함.

특별한 고려 사항:

- 트랜잭션 제어는 최상위 레벨의 `CALL`에서 호출되거나 중첩된 `CALL` 또는 `DO` 호출에서 직접 호출된 프로시저에서만 사용할 수 있음.
- 커서 루프는 트랜잭션 종료로 인해 암묵적으로 닫히지만, `WITH HOLD` 커서는 열린 상태로 유지될 수 있음.
- `EXCEPTION` 절이 있는 블록에서는 트랜잭션 제어 명령을 사용할 수 없음.

---

### 43.9 오류 및 메시지 (Errors and Messages)

`RAISE` 문을 사용하여 메시지를 보고하고 오류를 발생시킬 수 있음.
...
`level` 옵션은 오류 심각도를 지정함. 허용되는 수준은 `DEBUG`, `LOG`, `INFO`, `NOTICE`, `WARNING`, `EXCEPTION`이며, 기본값은 `EXCEPTION`임. `EXCEPTION`은 오류를 발생시키고(일반적으로 현재 트랜잭션을 중단), 다른 수준은 다양한 우선순위의 메시지만 생성함.

`format` 문자열 내에서 `%`는 다음 선택적 인자 값의 문자열 표현으로 대체됨. 리터럴 `%`를 내보내려면 `%%`를 작성함. 인자 수는 `format` 문자열의 플레이스홀더 수와 일치해야 함.
...
매개변수 없이 `RAISE;`를 사용하면 현재 처리 중인 예외가 다시 발생함(예외 핸들러에서만 의미가 있음):
...
`ASSERT` 문은 PL/pgSQL 함수에 디버깅 검사를 삽입하는 편리한 방법임:

```sql
ASSERT condition [ , message ];
```

`condition`은 항상 true로 평가될 것으로 예상되는 부울 표현식임. 그렇다면 `ASSERT` 문은 아무 작업도 수행하지 않음. 결과가 false이거나 널이면 `ASSERT_FAILURE` 예외가 발생함.

```sql
ASSERT quantity > 0, 'quantity must be positive';
```

> 참고: `ASSERT`는 절대 실패하지 않아야 하는 조건을 검사하기 위한 것임; 프로그래머가 실수했음을 나타내며 일반적인 데이터 또는 사용자 오류 보고에는 사용하지 마세요.

---

### 43.10 트리거 함수 (Trigger Functions)

PL/pgSQL은 트리거 함수를 정의하는 데 사용할 수 있음. 트리거 함수는 `CREATE FUNCTION` 명령을 사용하여 생성되며, 인자 없이 반환 타입 `trigger`(행 수준 트리거의 경우) 또는 `event_trigger`(이벤트 트리거의 경우)를 반환함.

#### 43.10.1 행 수준 트리거에 대한 데이터 변경 트리거

트리거 함수가 실행될 때, PL/pgSQL 런타임 환경에서는 자동으로 여러 특수 변수가 생성됨:
...
행 수준 트리거 함수의 반환값은 작업 종류와 트리거 타이밍에 따라 의미가 달라짐:

- `BEFORE` 트리거: `NULL`을 반환하면 해당 행에 대한 작업이 건너뜀. 행 값을 반환하면 그 값이 실제로 삽입되거나 업데이트되는 행으로 사용됨.
- `AFTER` 트리거: 반환값은 무시됨.
- `INSTEAD OF` 트리거: `NULL`을 반환하면 뷰의 해당 행에 대한 작업이 건너뜀.
...
문 수준 트리거는 `NEW` 또는 `OLD`를 가지지 않지만, `AFTER` 트리거와 관련된 모든 행에 액세스하려면 전환 테이블(transition tables)을 사용할 수 있음.
...
이벤트 트리거는 `CREATE TABLE`이나 `DROP INDEX`와 같은 DDL 문이 실행될 때 호출됨. 이벤트 트리거 함수의 반환 타입은 `event_trigger`임.
...
PL/pgSQL 인터프리터가 함수를 처리할 때 SQL 문과 표현식에서 PL/pgSQL 변수 이름을 찾아 쿼리 매개변수로 대체함.
...
PL/pgSQL 인터프리터는 함수 내의 개별 SQL 문에 대한 실행 계획을 캐시함. 계획은 함수가 처음 실행될 때 각 문에 대해 준비되고 이후 실행에서 재사용됨.

이로 인해 성능이 크게 향상되지만, 기반이 되는 데이터베이스 객체가 변경되면(예: 인덱스 추가, 테이블 변경) 캐시된 계획이 최적이 아닐 수 있음. 이런 경우 PostgreSQL은 자동으로 계획을 무효화함.

몇 가지 주의점:

1. 같은 세션 내에서 여러 번 호출되는 함수의 경우 처음 실행이 계획 생성으로 인해 약간 느릴 수 있음.
2. 매개변수 값에 따라 최적 계획이 달라질 수 있는 경우 일반 계획(generic plan)이 사용됨.

동적 SQL(`EXECUTE`)을 사용하면 매번 계획을 새로 만들므로 이러한 문제를 피할 수 있지만 오버헤드가 발생함.
...
PL/pgSQL 코드를 작성할 때 달러 인용(dollar quoting)을 사용하는 것이 좋음:
...
`$PROC$`와 같은 태그를 사용하면 중첩된 달러 인용이 가능함.

#### 추가 검사 활성화

`plpgsql.extra_warnings` 및 `plpgsql.extra_errors` 구성 매개변수를 사용하여 컴파일 타임에 추가 검사를 활성화할 수 있음:
...
`RAISE NOTICE` 문은 PL/pgSQL 함수를 디버깅하는 간단하지만 효과적인 방법임:
...
PL/pgSQL은 PostgreSQL의 강력한 절차적 언어로, 다음과 같은 주요 기능을 제공함:
...
PL/pgSQL을 사용하면 복잡한 비즈니스 로직을 데이터베이스 내에서 직접 구현하여 클라이언트-서버 통신 오버헤드를 줄이고, 데이터 무결성을 보장하며, 재사용 가능한 코드를 작성할 수 있음.
...
PL/Tcl 은 PostgreSQL을 위한 로드 가능한 절차적 언어(loadable procedural language)로, PostgreSQL 함수와 프로시저 작성에 [Tcl 언어](https://www.tcl.tk/)를 사용할 수 있게 해줍니다. PL/Tcl은 C 언어 함수의 대부분의 기능을 제공하면서 Tcl의 강력한 문자열 처리 라이브러리의 이점을 추가로 제공함.

#### 1.2 주요 특징

보안상의 이점:
- 안전한 Tcl 인터프리터(safe Tcl interpreter) 컨텍스트 내에서 실행됨
- 제한된 명령 집합과 제한된 데이터베이스 접근만 가능함
- 특정 SPI 명령과 `elog()` 메시지 기능만 사용 가능함
- 데이터베이스 서버 내부나 OS 수준 권한에 대한 접근이 없음
- 비권한 데이터베이스 사용자도 이 언어를 안전하게 사용할 수 있음

#### 1.3 구현 제한사항

1. 새로운 데이터 타입에 대한 입출력 함수(input/output functions) 생성에 사용할 수 없음
2. 안전한 Tcl 명령 집합으로 제한됨

#### 1.4 PL/TclU (비신뢰 Tcl 변형)

제한 없는 Tcl 기능(예: 이메일 전송)이 필요한 경우 PL/TclU를 사용함:
- 안전 모드 대신 전체 Tcl 인터프리터를 사용함
- 비신뢰 언어(untrusted language)로 설치해야 함 - 슈퍼유저만 함수를 생성할 수 있음
- 전체 시스템 접근이 가능하므로 함수 작성자가 보안을 보장해야 함
- 관리자 수준의 권한을 가짐

#### 1.5 설치

공유 객체 코드(shared object code)는 설치 시 Tcl 지원이 설정되면 자동으로 빌드됨.
...
PL/Tcl 함수는 표준 SQL 구문을 사용하여 생성함:
...
PL/TclU의 경우 언어 지정만 다르게 `pltclu`를 사용함.

#### 2.2 함수 인수

- 함수 인수는 `$1`, `$2` 등의 이름을 가진 Tcl 변수로 전달됨
- 결과는 표준 Tcl `return` 문을 사용하여 반환됨
- 프로시저에서는 Tcl 코드의 반환 값이 무시됨
...
`STRICT` 절은 null 인수로 함수가 호출되는 것을 방지함.

#### 2.4 NULL 값 처리

비엄격(non-strict) 함수에서 null 인수는 빈 문자열이 됨. null을 감지하려면 `argisnull`을 사용함:
...
null 값을 반환하려면 `return_null`을 사용함.

#### 2.5 복합 타입 인수 (Composite-Type Arguments)

복합 타입은 속성 이름과 일치하는 요소 이름을 가진 Tcl 배열로 전달됨. Null 속성은 배열에 나타나지 않음:
...
예상 결과 타입과 일치하는 열 이름/값 쌍의 리스트를 반환함:
...
배열 기반으로 반환할 때는 `array get`을 사용함:
...
PL/Tcl 함수에 전달되는 입력 인수는 텍스트 형식으로 변환됨. 이 변환 결과는 `SELECT` 문으로 값을 출력했을 때와 동일함.

#### 3.2 반환 값

`return`과 `return_next` 명령은 다음에 대해 유효한 입력 형식인 모든 문자열을 허용함:
- 함수의 선언된 결과 타입
- 또는 복합 결과 타입의 지정된 열

#### 3.3 요약

PL/Tcl에서 데이터 값은 텍스트 변환을 통해 처리됨:
- 입력 → 텍스트: 함수 인수는 텍스트 표현으로 변환됨
- 텍스트 → 출력: return 문은 예상 출력 타입의 입력 형식에 맞는 텍스트 문자열을 허용함

값은 텍스트 문자열로 교환되며, PostgreSQL이 텍스트 표현과 실제 데이터 타입 간의 타입 변환을 처리함.
...
PL/Tcl의 전역 데이터를 사용하면 함수 호출 간에 데이터를 유지하거나 여러 함수 간에 공유할 수 있음. 단, 보안 및 범위와 관련된 중요한 제약사항이 있음.

#### 4.2 주요 보안 제한

SQL 역할(Role)별 인터프리터 분리:
- 보안상의 이유로 각 SQL 역할마다 별도의 Tcl 인터프리터를 가짐
- 이는 사용자 PL/Tcl 함수 간의 우발적이거나 악의적인 간섭을 방지함
- 전역 Tcl 변수는 동일한 SQL 역할에 의해 실행되는 함수 간에만 공유됨

역할 간 데이터 공유:
애플리케이션이 여러 SQL 역할에서 코드를 실행하는 경우(`SECURITY DEFINER` 함수, `SET ROLE` 등을 통해):
- 통신해야 하는 함수가 동일한 사용자가 소유하도록 해야 함
- `SECURITY DEFINER`로 표시해야 함
- 이러한 함수가 오용되지 않도록 주의해야 함

#### 4.3 PL/TclU vs PL/Tcl

PL/TclU 함수:
- 모든 함수가 동일한 Tcl 인터프리터에서 실행됨(세션 내에서 공유)
- 전역 데이터가 PL/TclU 함수 간에 자동으로 공유됨
- 모든 PL/TclU 함수가 슈퍼유저 신뢰 수준에서 실행되므로 보안 위험으로 간주되지 않음

#### 4.4 GD 배열 사용 (권장 방식)

특별한 전역 배열 `GD`가 `upvar` 명령을 통해 각 함수에 제공됨:
- 전역 이름: 함수의 내부 이름
- 로컬 이름: `GD`
...
모범 사례:
- 함수의 영구 프라이빗 데이터에는 `GD`를 사용함
- 여러 함수 간에 특별히 공유하려는 값에만 일반 Tcl 전역 변수를 사용함
...
설명:
- 문자열로 주어진 SQL 명령을 실행함
- 처리된 행 수(선택, 삽입, 업데이트 또는 삭제)를 반환함
- 유틸리티 문의 경우 0을 반환함
- 명령 실패 시 오류를 발생시킴

주요 옵션:
- `-count n`: n개 행을 검색한 후 중지함(LIMIT과 유사). n이 0이면 완료까지 실행함
- `-array name`: 개별 변수 대신 연관 배열에 열 값을 저장함
- `loop-body`: 결과의 각 행에 대해 한 번씩 실행되는 선택적 Tcl 스크립트

동작:
- `loop-body` 없는 SELECT 문: 첫 번째 행만 저장되고 나머지 행은 무시됨
- 열 이름을 따라 명명된 Tcl 변수에 열 값이 저장됨
- 행 번호는 배열 요소 `.tupno`에 저장됨(`-array` 사용 시)
- NULL 열은 "unset" 변수가 됨
...
설명:
- 나중에 실행할 쿼리 계획을 준비하고 저장함
- 계획은 현재 세션 기간 동안 유지됨
- 플레이스홀더로 `$1`, `$2` 등을 사용하는 매개변수화된 쿼리를 지원함
- `spi_execp`와 함께 사용할 쿼리 ID를 반환함
...
설명:
- `spi_prepare`로 이전에 준비한 쿼리를 실행함
- 쿼리/매개변수 지정을 제외하고 `spi_exec`와 동일하게 작동함

옵션:
- `-count n`, `-array name`, `loop-body`: `spi_exec`와 동일
- `-nulls string`: 어떤 매개변수가 null인지 나타내는, 공백과 `n` 문자로 구성된 문자열. `value-list`의 길이와 일치해야 함
...
장점: `spi_exec`와 달리 매개변수에 수동 따옴표 처리가 필요하지 않음
...
설명:
- SQL 서브트랜잭션 내에서 Tcl 스크립트를 실행함
- 스크립트가 오류를 반환하면 전체 서브트랜잭션이 롤백됨
- 오류가 외부 트랜잭션에 영향을 미치는 것을 방지함
...
설명:
- 모든 작은따옴표와 백슬래시 문자를 두 번 반복함
- `spi_exec` 또는 `spi_prepare`의 SQL 명령에서 문자열을 안전하게 따옴표 처리함
...
설정:
- 출력은 `log_min_messages` 및 `client_min_messages` 설정에 의해 제어됨
...
PL/Tcl의 트리거 함수는 인수 없이 선언하고 반환 타입이 `trigger`여야 함.

#### 6.2 트리거 변수

트리거 관리자가 다음 변수를 통해 정보를 전달함:
...
트리거 함수는 다음 중 하나를 반환해야 함:

- `OK`: 작업이 정상적으로 진행됨
- `SKIP`: 이 행에 대한 작업을 조용히 억제함
- 열 이름/값 쌍의 리스트: 수정된 행을 반환함(행 수준 `BEFORE INSERT/UPDATE` 또는 `INSTEAD OF INSERT/UPDATE` 트리거에만 의미 있음)

> 팁: 배열 표현을 리스트 형식으로 변환하려면 `array get`을 사용함
...
이 예제는 삽입 시 카운터를 0으로 초기화하고 각 업데이트마다 증가시킴. 트리거는 열 이름 인수를 통해 다른 테이블에서 재사용할 수 있음.
...
이벤트 트리거 함수는 PL/Tcl로 작성할 수 있음. 이벤트 트리거로 호출될 함수는 다음 조건을 만족하도록 선언해야 함:
- 인수 없음
- 반환 타입: `event_trigger`

#### 7.2 사용 가능한 변수

트리거 관리자가 다음 변수를 통해 함수 본문에 정보를 전달함:
...
지원되는 명령이 실행될 때마다 `NOTICE` 메시지를 발생시키는 간단한 이벤트 트리거 함수임:
...
이 예제는:
1. 인수를 받지 않고 `event_trigger`를 반환하는 `tclsnitch()` 함수를 정의함
2. `elog`를 사용하여 이벤트 이름과 명령 태그가 포함된 알림 메시지를 기록함
3. DDL 명령 시작 이벤트에서 실행되는 이벤트 트리거를 생성함
...
PL/Tcl 함수 내의 Tcl 코드는 잘못된 작업이나 명시적 오류 명령을 통해 오류를 발생시킬 수 있음. 오류는 Tcl의 `catch` 명령으로 잡거나, 잡히지 않으면 SQL 오류로 전파됨.
...
잡히지 않은 오류는 최상위 수준으로 전파되어 호출하는 쿼리에서 SQL 오류로 보고됨.

#### 8.3 오류 정보: errorCode 변수

Tcl의 `errorCode` 변수는 리스트 형식으로 추가 오류 정보를 포함함:
...
이중 콜론(`::`)은 `errorCode`를 전역 변수로 명시적으로 참조함.
...
PL/Tcl의 명시적 서브트랜잭션은 여러 데이터베이스 작업이 단일 단위로 함께 성공하거나 실패해야 할 때 데이터 일관성을 유지하는 수단임.

#### 9.2 문제점

명시적 서브트랜잭션 없이는 한 작업의 오류가 이전 작업을 자동으로 롤백하지 않음. 예:
...
두 번째 UPDATE가 실패하면 첫 번째는 여전히 커밋되어 계좌 잔액이 일관되지 않게 됨.

#### 9.3 해결책: `subtransaction` 명령

여러 작업을 `subtransaction` 블록으로 감싸서 모두 성공하거나 모두 롤백되도록 함:
...
- 오류를 처리하고 전파되는 것을 방지하려면 `catch`가 여전히 필요함
- 모든 오류에서 롤백 트리거: 데이터베이스 오류와 일반 Tcl 예외 모두 롤백을 유발함
- 비오류 종료는 롤백하지 않음: 서브트랜잭션 내에서 `return`을 사용해도 롤백이 트리거되지 않음
- subtransaction 명령 자체는 오류를 트랩하지 않음 - 원자적 롤백 동작만 보장함
...
최상위 수준에서 호출된 PL/Tcl 프로시저 또는 익명 코드 블록(`DO` 명령)에서 두 가지 명령을 사용하여 트랜잭션을 제어할 수 있음:

- `commit`: 현재 트랜잭션을 커밋함
- `rollback`: 현재 트랜잭션을 롤백함

#### 10.2 중요 참고사항

1. SQL 명령 사용 불가: `spi_exec` 또는 유사한 함수를 통해 SQL `COMMIT` 또는 `ROLLBACK` 명령을 사용할 수 없음. 대신 `commit` 및 `rollback` PL/Tcl 명령을 사용해야 함.

2. 자동 트랜잭션 시작: 트랜잭션이 종료된 후 새 트랜잭션이 자동으로 시작됨. 새 트랜잭션을 시작하는 별도의 명령이 필요하지 않음.

3. 서브트랜잭션 제한: 명시적 서브트랜잭션이 활성화되어 있으면 트랜잭션을 종료할 수 없음.
...
이 예제는 `test1`에 10개의 행을 삽입하고, 짝수 반복에서 커밋하고 홀수 반복에서 롤백함.
...
목적: PL/Tcl용 새 Tcl 인터프리터가 생성될 때마다 실행할 매개변수 없는 PL/Tcl 함수의 이름을 지정함.

주요 세부사항:
- 세션별 초기화를 활성화함(예: 추가 Tcl 코드 로딩)
- 새 인터프리터가 생성되는 시점:
  - 데이터베이스 세션에서 PL/Tcl 함수가 처음 실행될 때
  - 새 SQL 역할에 대해 추가 인터프리터가 필요할 때
- 참조된 함수는:
  - `pltcl` 언어로 작성되어야 함
  - `SECURITY DEFINER`로 표시되면 안 됨
  - 현재 사용자가 호출할 수 있어야 함
- 슈퍼유저만 이 설정을 변경할 수 있음
- 초기화 함수가 실패하면 호출 함수를 중단하고 오류를 전파하여 현재 트랜잭션/서브트랜잭션을 중단함
- 세션 내 변경은 이미 생성된 인터프리터에 영향을 미치지 않음

##### 11.1.2 pltclu.start_proc (문자열)

목적: `pltcl.start_proc`과 동일하지만 PL/TclU 에 적용됨.

- 참조된 함수는 `pltclu` 언어로 작성되어야 함
- 다른 모든 동작과 제한은 `pltcl.start_proc`과 동일함
...
PostgreSQL의 PL/Tcl에서 Tcl 프로시저 이름은 Tcl이 고유한 프로시저 이름을 요구하기 때문에 일반 PostgreSQL 함수와 다르게 관리됨.

#### 12.2 핵심 사항

##### 12.2.1 이름 생성 전략

PL/Tcl은 다음과 같이 내부 Tcl 프로시저 이름을 생성함:
1. 내부 Tcl 프로시저 이름에 인수 타입 이름 포함
2. 동일한 Tcl 인터프리터에서 이전에 로드된 함수 간의 고유성을 보장하기 위해 필요시 함수의 객체 ID(OID) 추가

##### 12.2.2 함수 오버로딩 처리

- PostgreSQL은 다른 스키마나 다른 인수 타입/개수로 동일한 함수 이름을 허용함
- Tcl은 모든 프로시저 이름이 구별되어야 함
- PL/Tcl은 이름이 같지만 인수 타입이 다른 PostgreSQL 함수에 대해 다른 Tcl 프로시저를 생성하여 이를 해결함

##### 12.2.3 중요 제한: 직접 함수 호출

PL/Tcl 함수는 Tcl 코드 내에서 다른 PL/Tcl 함수를 직접 호출할 수 없음.

다른 PL/Tcl 함수를 호출해야 하는 경우 SQL을 통해 다음을 사용해야 함:
- `spi_exec` 명령, 또는
- 관련 SPI(서버 프로그래밍 인터페이스) 명령

#### 12.3 실질적 영향

- 내부 Tcl 프로시저 이름 지정은 일반적으로 PL/Tcl 프로그래머에게 투명함
- 디버깅 세션 중에 보일 수 있음
- 개발자는 함수 간 직접 호출 제한을 인지하고 대신 SQL 중개자를 사용해야 함
...
PL/Perl은 PostgreSQL 함수와 프로시저를 [Perl 프로그래밍 언어](https://www.perl.org)로 작성할 수 있게 해주는 로드 가능한 절차적 언어(loadable procedural language)임.

#### 1.1 주요 장점

PL/Perl의 주요 장점은 저장 함수(stored functions)와 프로시저 안에서 Perl의 광범위한 "문자열 처리(string munging)" 연산자와 함수를 활용할 수 있다는 점임. 복잡한 문자열 파싱(parsing)은 PL/pgSQL의 문자열 함수와 제어 구조보다 Perl로 처리하는 편이 훨씬 쉬울 수 있음.

#### 1.2 설치

특정 데이터베이스에 PL/Perl을 설치하려면 다음 명령을 사용함:

```sql
CREATE EXTENSION plperl;
```

팁: `template1`에 언어를 설치하면 이후에 생성되는 모든 데이터베이스에 자동으로 설치됨.

참고:
- 소스 패키지 사용자는 설치 과정에서 PL/Perl 빌드를 특별히 활성화해야 함 (17장 소스 코드로부터 설치 참고)
- 바이너리 패키지 사용자는 별도의 서브패키지에서 PL/Perl을 찾을 수 있음
...
PL/Perl 함수는 표준 `CREATE FUNCTION` 구문을 사용하여 생성함:
...
핵심 사항:
- 함수 본문은 서브루틴으로 래핑된 일반 Perl 코드임
- 함수는 스칼라 컨텍스트(scalar context)에서 동작함 (직접 리스트를 반환할 수 없음)
- 비스칼라 값(배열, 레코드, 집합)은 참조(reference)로 반환해야 함
- 인수는 `@_`로 전달되고 결과는 `return` 또는 마지막 표현식을 통해 반환됨
...
익명 블록은 인수를 받지 않으며 반환 값은 무시됨.

#### 2.3 인수 처리

##### NULL 값 처리

SQL NULL 값은 Perl에서 "undefined"로 나타남. `STRICT` 속성을 사용하거나 `defined()`로 확인함:
...
- 인수는 데이터베이스 인코딩에서 UTF-8로 변환됨
- 비참조(non-reference) 인수는 PostgreSQL 외부 텍스트 표현 형식의 문자열임
- `bytea` 타입 변환에는 `decode_bytea()`를 사용함
- 반환 값은 외부 텍스트 표현 형식이어야 함

##### 불리언 값 (Boolean Values)

기본적으로 `bool` 값은 텍스트(`'t'` 또는 `'f'`)임. 올바른 처리를 위해 변환(transforms)을 사용함:
...
Perl 5.10.0 이상에서 사용 가능함.

#### 2.9 중요 참고사항

- 중첩된 명명 서브루틴(nested named subroutines)은 위험함 → 대신 익명 서브루틴 사용 권장
- 함수 본문에는 달러 따옴표(`$$...$$`)를 권장함
- 인수는 자동으로 UTF-8로/에서 변환됨
- 누락된 복합 타입 속성은 NULL로 반환됨
...
- PL/Perl 함수에 전달되는 입력 인수는 텍스트 형식으로 변환됨 (`SELECT` 문에 표시되는 것과 동일)
- `return` 및 `return_next` 명령은 함수에 선언된 반환 타입의 허용 입력 형식에 해당하는 모든 문자열을 받아들임
...
텍스트 변환이 불편한 경우, PostgreSQL은 이 동작을 개선할 수 있는 변환 모듈을 제공함.
...
PostgreSQL 배포판에는 여러 예제 변환 모듈이 포함되어 있음.
...
SQL 명령을 실행하고 전체 행 집합을 해시 참조의 배열에 대한 참조로 반환함.
...
대용량 행 집합을 처리하거나 행이 도착하는 대로 반환하는 데 함께 사용됨.
...
참고: `spi_fetchrow`는 더 이상 행이 없을 때 `undef`를 반환함. 모든 행을 읽지 않는 경우 `spi_cursor_close()`를 호출하여 커서를 수동으로 해제 필요 (메모리 누수 방지).

##### 4.1.3 준비된 쿼리 (Prepared Queries)

번호가 매겨진 인수 플레이스홀더를 사용하는 준비된 쿼리(prepared query)를 지원함.
...
현재 트랜잭션을 커밋하거나 롤백함. 프로시저 또는 익명 코드 블록(`DO` 명령) 내의 최상위 레벨에서만 호출할 수 있음.
...
참고: `ERROR`는 오류 조건을 발생시킴. 나머지 레벨은 `log_min_messages` 및 `client_min_messages` 설정 변수에 따라 다양한 우선순위로 메시지를 출력함.

##### 4.2.2 quote_literal()

주어진 문자열을 SQL 문자열 리터럴로 사용할 수 있도록 따옴표로 묶어 반환함.

구문: `quote_literal(string)`

`undef` 입력에 대해 `undef`를 반환함. 인수가 `undef`일 수 있는 경우 `quote_nullable()` 사용 권장.

##### 4.2.3 quote_nullable()

주어진 문자열을 SQL 문자열 리터럴로 사용할 수 있도록 따옴표로 묶어 반환하거나, 인수가 `undef`인 경우 `"NULL"`을 반환함.

구문: `quote_nullable(string)`

##### 4.2.4 quote_ident()

주어진 문자열을 SQL 식별자로 사용할 수 있도록 따옴표로 묶어 반환함.

구문: `quote_ident(string)`

비식별자 문자가 포함되거나 대소문자 변환이 필요한 경우에만 따옴표가 추가됨.

##### 4.2.5 decode_bytea()

`bytea` 인코딩된 문자열을 이스케이프 해제한 원시 이진 데이터로 반환함.

구문: `decode_bytea(string)`

##### 4.2.6 encode_bytea()

이진 데이터를 `bytea` 인코딩 형식으로 변환하여 반환함.

구문: `encode_bytea(string)`

##### 4.2.7 encode_array_literal()

배열 내용을 배열 리터럴 형식의 문자열로 반환함.

구문: `encode_array_literal(array [, delimiter])`

구분자를 지정하지 않거나 `undef`로 지정하면 기본값인 `", "`가 사용됨.

##### 4.2.8 encode_typed_literal()

Perl 변수를 지정된 데이터 타입으로 변환해 문자열 표현으로 반환함.

구문: `encode_typed_literal(value, typename)`

중첩 배열과 복합 타입을 올바르게 처리함.

##### 4.2.9 encode_array_constructor()

배열 내용을 배열 생성자 형식의 문자열로 반환함.

구문: `encode_array_constructor(array)`

개별 값은 `quote_nullable()`로 따옴표 처리됨.

##### 4.2.10 looks_like_number()

문자열이 숫자처럼 보이면 true, 그렇지 않으면 false를 반환함.

구문: `looks_like_number(string)`

인수가 `undef`이면 `undef`를 반환함. 앞뒤 공백은 무시하며, `Inf`와 `Infinity`는 숫자로 취급함.

##### 4.2.11 is_array_ref()

인수를 배열 참조로 취급할 수 있으면 true를 반환함.

구문: `is_array_ref(argument)`

`ref()`의 반환값이 `ARRAY` 또는 `PostgreSQL::InServer::ARRAY`이면 true를 반환함.
...
전역 해시 `%_SHARED`를 사용하면 현재 세션이 유지되는 동안 함수 호출 간에 코드 참조를 포함한 데이터를 저장할 수 있음.
...
- 각 SQL 역할(role)은 자체 `%_SHARED` 인스턴스를 가진 별도의 Perl 인터프리터에서 실행됨
- 두 PL/Perl 함수는 동일한 SQL 역할로 실행될 때만 같은 `%_SHARED`를 공유함
- 이를 통해 사용자 간의 의도치 않은 간섭이나 악의적인 접근을 방지함

##### 역할 간 데이터 공유

서로 다른 SQL 역할 간에 데이터를 공유해야 하는 경우:
1. 함수들이 동일한 사용자 소유여야 함
2. `SECURITY DEFINER`로 표시해야 함
3. 의도치 않은 용도로 오용되지 않도록 해야 함
...
PostgreSQL의 PL/Perl은 서로 다른 보안 수준을 가진 두 가지 변형으로 제공됨:

#### 6.2 신뢰 PL/Perl (`plperl`)

- 기본 설치되는 "신뢰" 변형임
- 보안 유지를 위해 특정 Perl 작업을 제한함
- 권한이 없는 데이터베이스 사용자에게도 안전함
- 모든 데이터베이스 사용자가 함수를 생성할 수 있음
...
- 제한 없이 Perl 언어를 전부 사용할 수 있음
- 파일 작업, 시스템 호출, 외부 모듈 임포트가 가능함
- 데이터베이스 슈퍼유저만 이 언어로 함수를 생성할 수 있음
- 메일 전송 등 민감한 작업에 활용됨

#### 6.4 보안 경고

> 신뢰 PL/Perl은 보안 유지를 위해 Perl `Opcode` 모듈에 의존함. Perl 문서에 따르면 이 모듈이 신뢰 PL/Perl 사용 사례에 완전히 효과적이지 않을 수 있음. 보안 요구 사항이 해당 경고의 불확실성을 허용할 수 없는 경우 `REVOKE USAGE ON LANGUAGE plperl FROM PUBLIC` 실행 고려.

#### 6.5 코드 예제

이 함수는 `plperl`에서 실패함 (파일 작업이 금지됨):
...
동일한 함수가 `plperlu`에서는 성공함 (슈퍼유저가 생성한 경우):
...
- 별도의 인터프리터: PL/Perl 함수는 SQL 역할마다 별도의 Perl 인터프리터에서 실행됨
- 공유 상태: 세션 내의 모든 PL/PerlU 함수는 단일 Perl 인터프리터를 공유함 (데이터 공유 가능)
- 교차 통신 없음: PL/Perl과 PL/PerlU 함수는 서로 통신할 수 없음
- Perl 빌드 요구 사항: 여러 인터프리터를 사용하려면 Perl이 `usemultiplicity` 또는 `useithreads`로 컴파일되어야 하며, 그렇지 않으면 세션당 하나의 인터프리터만 사용됨

---

### 7. PL/Perl 트리거 (PL/Perl Triggers)

PL/Perl로 트리거 함수를 작성할 수 있음. 해시 참조 `$_TD`는 현재 트리거 이벤트 정보를 담으며, 각 트리거 호출마다 별도의 로컬 값을 받는 전역 변수임.
...
- `$_TD->{event}`: 트리거 이벤트
  - `INSERT`, `UPDATE`, `DELETE`, `TRUNCATE`, 또는 `UNKNOWN`
- `$_TD->{when}`: 트리거가 호출된 시점
  - `BEFORE`, `AFTER`, `INSTEAD OF`, 또는 `UNKNOWN`
- `$_TD->{level}`: 트리거 레벨
  - `ROW`, `STATEMENT`, 또는 `UNKNOWN`
...
PL/Perl로 이벤트 트리거 함수를 작성할 수 있음. 이벤트 트리거 함수는 특별한 해시 참조 `$_TD`를 통해 현재 트리거 이벤트 정보에 접근함.

#### 8.1 $_TD 해시 참조

`$_TD`는 각 트리거 호출마다 별도의 로컬 값을 받는 전역 변수로, 다음 필드를 포함함:
...
Perl 인터프리터가 처음 초기화될 때 `plperl` 또는 `plperlu`로 특수화되기 전에 실행할 Perl 코드를 지정함. 이 단계에서는 SPI 함수를 사용할 수 없음.

핵심 사항:
- 단일 문자열로만 지정할 수 있으며, 긴 코드는 모듈에 작성하고 로드해야 함
- 초기화가 실패하면 인터프리터 초기화 전체가 중단됨
- 로드된 모든 모듈은 `plperl`에서도 사용 가능해짐
...
중요 고려 사항:
- `shared_preload_libraries`에 포함된 경우 초기화가 postmaster에서 이루어짐 (성능 이점이 있으나 보안 위험 존재)
- Windows에서는 postmaster의 Perl 인터프리터가 자식 프로세스로 전파되지 않으므로 사전 로드 이점이 없음
- `postgresql.conf` 또는 서버 명령줄에서만 설정할 수 있음

##### 9.1.2 `plperl.on_plperl_init` / `plperl.on_plperlu_init` (string)

인터프리터가 각각 `plperl` 또는 `plperlu`로 특수화될 때 실행되는 Perl 코드임. 세션에서 첫 번째 함수가 실행될 때나 추가 인터프리터가 생성될 때 호출됨.

핵심 사항:
- `plperl.on_init` 실행 이후에 실행됨
- 실행 중에는 SPI 함수를 사용할 수 없음
- `plperl.on_plperl_init` 코드는 인터프리터 "잠금" 이후에 실행됨 (신뢰된 작업만 허용)
- 슈퍼유저만 이 설정을 변경할 수 있음
- 변경 사항은 현재 세션에서 이미 사용 중인 인터프리터에는 영향을 미치지 않음

##### 9.1.3 `plperl.use_strict` (boolean)

true이면 이후 PL/Perl 함수 컴파일 시 `strict` pragma가 활성화됨. 현재 세션에서 이미 컴파일된 함수에는 영향을 미치지 않음.

#### 9.2 제한 사항 및 누락된 기능 (Limitations and Missing Features)

1. 직접 함수 호출: PL/Perl 함수끼리 직접 호출할 수 없음

2. SPI 구현: SPI가 아직 완전히 구현되지 않음

3. 대용량 데이터 집합:
   - `spi_exec_query`는 전체 결과 집합을 메모리에 로드함
   - 대용량 데이터셋에는 `spi_query`/`spi_fetchrow` 사용 권장
   - 집합 반환 함수에서 대용량 결과를 반환할 때는 `return_next`를 사용해야 함

4. 세션 종료:
   - `END` 블록은 세션이 정상 종료될 때 실행됨 (치명적 오류 시에는 실행되지 않음)
   - 파일 핸들이 자동으로 플러시되지 않음
   - 객체가 자동으로 소멸되지 않음
...
PL/Python은 PostgreSQL 함수와 프로시저를 [Python 프로그래밍 언어](https://www.python.org)로 작성할 수 있도록 해주는 로드 가능한 절차적 언어(loadable procedural language)임.

#### 1.1 설치

특정 데이터베이스에 PL/Python을 설치하려면 다음 명령 사용:

```sql
CREATE EXTENSION plpython3u;
```

팁: `template1`에 언어를 설치하면, 이후에 생성되는 모든 데이터베이스에 해당 언어가 자동으로 설치됨.

참고:
- 소스 패키지 사용자는 설치 과정에서 PL/Python 빌드를 특별히 활성화해야 함
- 바이너리 패키지 사용자는 별도의 서브패키지에서 PL/Python을 찾을 수 있음

#### 1.2 보안 고려사항

PL/Python은 "비신뢰(untrusted)" 언어임:

- 사용자 행동에 대한 제한 없음
- 언어 이름의 "u"는 "untrusted"를 의미 (`plpython3u`)
- 슈퍼유저(superuser)만 비신뢰 언어로 함수를 생성할 수 있음
- 데이터베이스 관리자 권한으로 실행되므로 코드가 오용되지 않도록 주의 필요

> 참고: 향후 안전한 실행 메커니즘이 개발되면 신뢰 버전인 `plpython`이 제공될 수 있음
...
PL/Python 함수는 표준 `CREATE FUNCTION` 구문을 사용하여 생성함:
...
- 인수는 `args` 리스트의 요소로 전달됨
- 이름 있는 인수는 일반 변수로도 전달됨 (가독성을 위해 권장)
- 인수는 함수 범위 내에서 전역 변수로 설정됨

#### 2.3 반환 값

- 표준 Python `return` 문 사용
- 결과 집합 문(result-set statements)에는 `yield` 사용
- 반환 값이 없으면 Python의 `None`이 반환됨
- `None`은 SQL `NULL`로 변환됨
- 프로시저(Procedures)는 `None`을 반환해야 함 (return 없이 종료하거나 인수 없는 `return` 사용)
...
Python 스코핑 규칙으로 인해, 인수 변수를 자기 자신을 포함하는 표현식으로 재할당하지 말 것:
...
SQL NULL 값은 Python에서 `None`으로 나타남:
...
PL/Python은 PostgreSQL 세션 내 함수 호출 간 데이터 공유를 위한 전역 딕셔너리 두 개를 제공함:
...
각 PL/Python 함수는 Python 인터프리터에서 자체 실행 환경을 가짐:

- 한 함수(예: `myfunc`)의 전역 데이터와 인수는 다른 함수(예: `myfunc2`)에서 사용할 수 없음
- 예외: `GD` 딕셔너리에 저장된 데이터는 세션의 모든 함수에서 공유됨

---

### 5. 익명 코드 블록 (Anonymous Code Blocks)

PL/Python은 `DO` 문을 통한 익명 코드 블록을 지원함. 익명 코드 블록은 함수로 저장되지 않고 즉시 실행되는 이름 없는 코드 블록임.
...
- 인수 없음: 익명 코드 블록은 입력 매개변수를 받지 않음
- 반환 값 없음: 블록이 반환할 수 있는 모든 값은 무시됨
- 동작: 함수처럼 동작하지만 저장되지 않음
- 언어: `LANGUAGE plpython3u`를 지정해야 함
...
PL/Python에서 함수가 트리거로 사용될 때, `TD`(트리거 데이터) 딕셔너리에 트리거 관련 값이 담겨 트리거 이벤트에 대한 컨텍스트를 제공함.
...
PL/Python 언어 모듈은 데이터베이스 접근 기능을 제공하는 `plpy` 모듈을 자동으로 임포트함.
...
참고: 전체 결과 집합을 메모리에 올려 놓음. 대용량 데이터셋에는 `plpy.cursor()` 사용 권장.

#### 7.2 준비된 쿼리 (Prepared Queries)

매개변수화된 쿼리를 위해 재사용 가능한 실행 계획을 준비함.
...
대용량 결과 집합을 청크 단위로 처리할 수 있는 커서를 반환함.
...
- `plpy.SPIError`와 그 하위 클래스는 표준 Python 예외처럼 잡을 수 있음
- 예외 클래스 이름은 PostgreSQL 조건 이름에서 파생됨 (예: `division_by_zero` -> `DivisionByZero`)
- 예외 객체 내에서 `sqlstate` 속성으로 오류 코드 확인
- 모든 `plpy.spiexceptions` 클래스는 `SPIError`를 상속함

---

### 8. 명시적 서브트랜잭션 (Explicit Subtransactions)

명시적 서브트랜잭션은 데이터베이스 작업을 원자적으로 처리하므로, 모든 작업이 성공하거나 모두 롤백됨을 보장함.

#### 8.1 문제점

서브트랜잭션 없이 일련의 데이터베이스 작업 도중 오류가 발생하면, 일부 변경사항은 커밋되고 나머지는 실패할 수 있음:
...
두 번째 UPDATE가 실패하면 Joe의 계정에서는 출금됐지만 Mary의 계정에는 입금되지 않아 데이터가 불일치하게 됨.

#### 8.2 해결책: 서브트랜잭션 컨텍스트 관리자

`plpy.subtransaction()`을 사용하여 데이터베이스 작업을 원자적 블록으로 묶음:
...
- 컨텍스트 관리자 인터페이스: `plpy.subtransaction()`은 Python 컨텍스트 관리자 프로토콜을 구현함
- 오류 처리: 예외를 처리하려면 여전히 `try`/`except` 필요
- 원자적 동작: 서브트랜잭션 블록 내의 모든 작업은 함께 성공하거나 함께 실패함
- 예외 시 롤백: 예외가 발생하면 서브트랜잭션이 롤백됨 (데이터베이스 오류뿐 아니라 일반 Python 예외도 포함)

---

### 9. 트랜잭션 제어 (Transaction Control)

프로시저 또는 익명 코드 블록에서 `plpy.commit()`과 `plpy.rollback()`으로 트랜잭션을 제어할 수 있음.
...
- `plpy.error`와 `plpy.fatal`은 호출 쿼리로 전파되어 트랜잭션을 중단시키는 Python 예외를 발생시킴
- 대체 구문: `raise plpy.Error(msg)` 및 `raise plpy.Fatal(msg)` (키워드 인수 미지원)
- 나머지 함수는 다양한 우선순위 레벨로 메시지를 생성함
- 메시지 가시성은 `log_min_messages` 및 `client_min_messages` 설정 변수로 제어됨

#### 10.2 키워드 인수

로깅 함수는 오류 메시지를 보강하기 위해 다음 키워드 인수를 받음:
...
이 함수들은 PostgreSQL 내장 인용 함수와 동일하며, 동적 쿼리 구성 시 SQL 인젝션을 방지함.

---

### 11. 환경 변수 (Environment Variables)

다음 환경 변수는 PL/Python 동작에 영향을 줄 수 있음. 이 변수들은 PostgreSQL 서버 프로세스 환경에서 설정해야 함 (예: 시작 스크립트):
...
1. 서버 프로세스 요구사항: 이 변수들은 개별 클라이언트 연결이 아닌 PostgreSQL 서버 프로세스 자체에 적용됨
2. 버전 종속성: 사용 가능한 변수는 Python 버전에 따라 다름. 자세한 내용은 Python 문서 참고
3. 임베디드 인터프리터 제한: `python` man 페이지에 나열된 일부 환경 변수는 명령줄 인터프리터에서만 유효하며, PL/Python이 사용하는 임베디드 Python 인터프리터에서는 작동하지 않을 수 있음

가장 흔한 사용 사례로, `PYTHONPATH`는 PL/Python 검색 경로에 사용자 정의 모듈 디렉토리를 추가하는 데 활용됨.
