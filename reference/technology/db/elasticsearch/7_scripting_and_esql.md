...
스크립팅을 사용하면 Elasticsearch에서 사용자 정의 표현식 평가 가능 → 스크립트로 계산된 값을 필드로 반환하거나 쿼리에 대한 사용자 정의 점수 평가에 활용

#### 1.2 기본 스크립팅 언어

Elasticsearch의 기본 스크립팅 언어는 Painless → Elasticsearch를 위해 특별히 설계된 언어로, 스크립팅 API의 모든 용도에 사용 가능하며 가장 높은 유연성 제공

#### 1.3 사용 가능한 스크립팅 언어

- Painless: 샌드박스 적용, 내장 플러그인, Elasticsearch 전용 범용 스크립팅 용도
- Expression: 샌드박스 적용, 내장 플러그인, 사용자 정의 랭킹 및 정렬 용도
- Mustache: 샌드박스 적용, 내장 플러그인, 템플릿 작업 용도
- Java: 샌드박스 미적용, 사용 불가한 플러그인 유형, 전문가 수준 API 용도

Painless 외의 다른 언어는 특정 목적에는 유용하나 유연성 제한
...
스크립팅을 지원하는 모든 API에서 스크립트는 일관된 패턴을 따름 → 기본 구조는 다음 세 가지 구성 요소로 구성:
...
스크립트 언어를 명시적으로 지정하지 않으면 기본값은 `painless` → 구문과 사용 가능한 함수를 결정
...
이 섹션에는 스크립트에 변수로 전달되는 명명된 매개변수 포함
...
스크립트에서 하드코딩된 값 대신 매개변수 사용 → 컴파일 시간 단축 가능. Elasticsearch는 스크립트를 컴파일해 캐시에 저장 → 매개변수만 다른 스크립트는 동일한 컴파일된 스크립트 재사용 가능
...
Painless는 Elasticsearch 5.0에서 Groovy를 대체하기 위해 도입된 Elasticsearch의 기본 스크립팅 언어. JVM 위에 구축되어 Java와 유사한 구문에 Elasticsearch 작업에 특화된 보안 기능과 샌드박스 보호를 결합

#### 3.2 핵심 특성

##### 보안 및 성능

Painless는 "제한된 Java API에 대한 접근을 방지하는 세분화된 허용 목록"을 통해 동작 → 바이트코드로 직접 컴파일되어 해석 오버헤드 제거 및 JVM 최적화 가능

##### 목적에 맞게 설계됨

Painless는 범용 스크립팅 솔루션이 아니라 "시스템 리소스에 대한 무단 접근을 방지하면서 네이티브 성능을 제공하는 Elasticsearch 전용 언어"로 설계
...
개발자가 Painless 스크립트를 작성할 수 있는 위치:
- Dev Tools 콘솔
- 인제스트 파이프라인
- 검색 쿼리
- 런타임 필드
...
Painless의 구문은 Java 구문과 유사, 동적 타이핑·Map 및 List 접근자 단축키·배열 초기화자와 같은 추가 기능 포함

구현은 파싱에 ANTLR4, 바이트코드 생성에 ASM 사용 → 스크립트는 실행을 위해 JVM 바이트코드로 직접 컴파일

#### 4.2 주석

Painless는 Java 스타일 주석 지원:
...
참고: Painless는 표준 Java와 달리 `switch` 문 미지원

#### 4.4 데이터 타입

##### 기본 타입

Painless는 기본 JVM 데이터를 나타내는 8가지 기본 타입 제공:
...
참조 타입은 잠재적인 멤버 필드와 메서드를 가진 명명된 구조를 나타내며, 힙 메모리에 할당:
...
`def` 타입은 런타임 타입 유연성을 제공하며, 단일 식별자를 통해 모든 기본 또는 참조 타입 값 표현:
...
참고: 성능 고려사항 → 명시적 타입에 비해 약간의 오버헤드 존재
...
Painless는 연산자를 6가지 주요 카테고리로 구성:
...
연산자는 0(가장 높음)에서 17(가장 낮음)까지의 숫자 우선순위 척도를 따름:
- 레벨 0: 괄호
...
함수는 특정 작업을 수행하기 위한 하나 이상의 문으로 구성된 코드 조각:
...
Painless는 Java의 함수형 프로그래밍 기능과 동일한 구문을 가진 람다 표현식과 메서드 참조 지원:
...
Painless는 정규 표현식 상수를 패턴 생성의 기본 메커니즘으로 지원:

```painless
Pattern p = /[aeiou]/
```

주의: 잘못 작성된 정규 표현식은 성능을 크게 저하시킬 수 있음 → 가능하면 특히 자주 실행되는 스크립트에서 정규 표현식 사용 회피 필요

##### 패턴 플래그

- `c` (CANON_EQ): 예 `'a' ==~ /a/c`
- `i` (CASE_INSENSITIVE): 예 `'A' ==~ /a/i`
- `l` (LITERAL): 예 `'[a]' ==~ /[a]/l`
- `m` (MULTILINE): 예 `'a\nb\nc' =~ /^b$/m`
- `s` (DOTALL): 예 `'a\nb\nc' =~ /.b./s`
- `U` (UNICODE_CHARACTER_CLASS): 예 `'E' ==~ /\\w/U`
- `u` (UNICODE_CASE): 예 `'E' ==~ /e/iu`
- `x` (COMMENTS): 예 `'a' ==~ /a #comment/x`

여러 플래그 결합 가능 (예: `/foo/iUx`)
...
update, update-by-query 및 reindex API 스크립트는 다음을 노출하는 `ctx` 변수에 접근:

- `ctx._source`: 문서 `_source` 필드에 대한 접근
- `ctx.op`: 작업 유형 (`index` 또는 `delete`)
- `ctx._index`: 문서 메타데이터 필드에 대한 접근

참고: 이러한 스크립트는 `doc` 변수 사용 불가 → 문서 접근은 `ctx`에 의존
...
필드 값에 접근하는 세 가지 주요 방법:

##### Doc 값 (가장 빠름)

`doc['field_name']` 구문은 컬럼형 필드 값 검색 → "스크립트에서 필드 값에 접근하는 가장 빠르고 효율적인 방법"
...
- JSON 객체 반환 불가
- 누락된 필드에서 오류 발생
- 텍스트 필드는 `fielddata` 활성화 필요 (성능 집약적)

##### _source 필드

`params._source.field_name` 구문을 통해 접근 → doc 값보다 느리지만 결과당 여러 값을 반환하는 스크립트 필드에 유용
...
매핑에서 `"store": true`로 표시된 필드는 `params._fields['field_name'].value` 구문으로 접근
...
`_score`: `function_score` 쿼리, 스크립트 기반 정렬 및 집계에서 사용 가능 → 문서 관련성 표현

`_termStats`: `script_score` 쿼리에서 용어 통계 데이터 제공 → `termFreq()`, `docFreq()`, `totalTermFreq()`, `uniqueTermsCount()` 및 `matchedTermsCount()`와 같은 메서드 포함

---

### 6. 스크립팅 실행 컨텍스트

Painless 스크립트는 사용 가능한 변수, 허용된 API 및 반환 유형을 결정하는 특정 컨텍스트 내에서 실행

#### 6.1 데이터 수정 컨텍스트

##### 런타임 필드

스크립트는 인덱스에 저장하지 않고 검색 작업 중 동적으로 필드 값 계산
...
`emit` 메서드는 필수이며 필드 유형에 따라 값을 받음:
...
중요: `emit` 메서드는 `null` 값 허용 불가 → 참조된 필드에 값이 없으면 이 메서드 호출 금지
...
스크립트는 인덱싱 전에 인제스트 파이프라인을 통해 삽입 중인 문서 수정
...
스크립트는 Elasticsearch의 업데이트 작업을 통해 개별 문서를 수정 → 단일 문서 내 필드 추가·수정·삭제 가능
...
변경 가능한 소스:
- `ctx['_source']`: 저장된 문서에 존재하는 필드에 대해 추출된 JSON 포함
...
스크립트는 표준 필드 정렬 메커니즘을 넘어 결과 순서를 사용자 정의

반환 값: 구성된 `type` 매개변수에 따라 `double`(숫자 정렬용) 또는 `String`(문자열 기반 정렬용) 반환 필요
...
스크립트는 스크립트 필드를 통해 검색 결과에서 반환되는 계산된 값 생성
...
스크립트는 스크립트 쿼리를 사용해 bool 쿼리 내에서 사용자 정의 필터링 로직 구현
...
스크립트는 function score 쿼리에서 관련성 계산을 사용자 정의
...
반환 값: 현재 문서의 재계산된 점수를 나타내는 `double` 반환 필요
...
스크립트는 관련성 랭킹을 위한 사용자 정의 텍스트 유사도 알고리즘 정의

#### 6.4 집계 컨텍스트

##### 메트릭 집계

분산 메트릭 계산을 지원하는 4개의 특수 컨텍스트(initialization, map, combine, reduce) 존재
...
스크립트는 파이프라인 작업을 위해 집계 버킷 경로에서 값 계산
...
스크립트는 파이프라인 집계에서 계산된 조건에 따라 버킷 필터링
...
Lucene 표현식은 JavaScript 표현식을 바이트코드로 컴파일해 "고성능 사용자 정의 랭킹 및 정렬 함수" 제공 → 낮은 문서당 오버헤드로 속도를 우선시하도록 설계되어 "네이티브 스크립트를 작성한 것보다 더 빠르게" 실행 가능

#### 7.2 구문 기본

표현식은 "JavaScript 구문의 하위 집합: 단일 표현식" 지원. 사용 가능한 변수: 문서 필드(예: `doc['myfield'].value`), 필드 메서드, 스크립트 매개변수, script_score 컨텍스트의 `_score`

#### 7.3 숫자 필드 연산

API는 필드 값과 통계 함수에 대한 접근 제공:
...
날짜 필드는 1970년 1월 1일 이후 밀리초 표현. API가 포함하는 시간 구성 요소:
...
표현식은 `.lat` 및 `.lon` 속성을 통해 지리 좌표에 접근. `haversin()` 함수로 거리 계산:
...
"숫자, 불리언, 날짜 및 geo_point 필드만 접근 가능" → 저장된 필드는 사용 불가
...
검색 템플릿은 변수를 받아 처리하는 저장된 검색 → 최종 사용자에게 쿼리 구문을 노출하지 않고 쿼리 재사용 가능. 커스텀 애플리케이션이나 검색 인터페이스 구현에 특히 유용

#### 8.2 템플릿 생성

템플릿은 `lang: "mustache"`로 저장된 스크립트 생성 API를 사용해 생성:
...
"Mustache 변수는 이중 중괄호로 묶임: `{{my-var}}`" → 실행 중 `params` 객체의 값으로 대체

#### 8.4 주요 작업

##### 유효성 검사

렌더 검색 템플릿 API는 실제 검색을 실행하지 않고 다양한 매개변수로 템플릿을 테스트해 렌더링된 출력 표시
...
검색 템플릿 API는 사용자 정의 `params`로 저장된 템플릿 실행:
...
다중 검색 템플릿 API로 여러 템플릿 검색을 한 번에 실행 가능 → 개별 요청보다 오버헤드가 줄어드는 경우가 많음
...
섹션(`{{#condition}}...{{/condition}}`)과 반전 섹션(`{{^condition}}...{{/condition}}`)으로 템플릿 내에서 if/else 동작 구현 가능
...
템플릿은 중첩 객체를 위한 섹션과 배열 반복을 위한 리스트 지원. 설정 구분자 구문으로 기본 이중 괄호를 사용자 정의 구분자로 변경 가능

지원되지 않음: 부분(Partials)은 Elasticsearch 검색 템플릿에서 사용 불가
...
Painless는 보안을 핵심 설계 원칙으로 하는 "목적에 맞게 구축된" 언어로 동작 → 보안 모델은 여러 계층으로 구성:

1. 세분화된 허용 목록: "허용 목록에 없는 모든 것은 오류 발생" → 무단 작업 방지

2. 운영 체제 샌드박싱: Elasticsearch는 Seccomp(Linux), Seatbelt(macOS) 및 ActiveProcessLimit(Windows)를 사용해 프로세스 분기 또는 외부 프로세스 실행 방지

3. 집계 스크립트 제한: 스크립트 메트릭 집계의 스크립트는 정의된 목록으로 제한되거나 완전히 비활성화 가능

#### 9.2 주요 보안 설정

##### 허용된 스크립트 유형

`script.allowed_types`를 구성해 실행되는 스크립트 제어:
- `inline`: 인라인 스크립트만 허용
- `stored`: 저장된 스크립트만 허용
- `none`: 모든 스크립트 실행 금지

Kibana 사용자 참고: 일부 Kibana 기능이 인라인 스크립트에 의존 → `inline` 스크립트를 허용하도록 설정 필요

##### 허용된 스크립트 컨텍스트

`script.allowed_contexts`로 허용된 실행 컨텍스트 지정(예: "score, update") → 기본값은 모든 컨텍스트 허용

##### 스크립트 메트릭 집계 제어

집계의 스크립트를 다음 설정으로 제한:
...
Painless는 합법적인 작업(검색 점수 및 데이터 처리 등)을 위한 유연성을 유지하면서 클러스터 보안을 손상시킬 수 있는 파일 시스템, 네트워크 및 기타 시스템 리소스에 대한 접근 방지
...
Elasticsearch는 자동 컴파일 캐싱으로 스크립트 성능 최적화 → "컴파일된 스크립트는 캐시에 배치되어 스크립트를 참조하는 요청이 컴파일 페널티를 발생시키지 않음"

#### 10.2 캐시 구성

크기 관리: 캐시는 동시에 접근하는 모든 스크립트를 수용할 수 있도록 충분히 커야 함. 노드 통계로 시스템을 모니터링하고, 캐시 제거가 빈번하게 발생하면서 컴파일 횟수가 증가하는 추세가 보이면 캐시 설정 조정 필요
...
스크립트는 Elasticsearch의 인덱스 구조를 활용할 수 없어 잠재적으로 느린 쿼리 발생 가능. "스크립트는 매우 유용하지만 Elasticsearch의 인덱스 구조나 관련 최적화를 사용할 수 없음"

##### 실용적인 최적화 전략

검색 쿼리 중 값을 계산하는 대신 인제스트 중 계산·저장 권장:

예제 시나리오: 검색 중 `math_score`와 `verbal_score`를 합산하는 스크립트 대신, 문서 인제스트 중 이 합계를 `total_score` 필드로 미리 계산
...
결과: 이 접근 방식은 "인덱스 프로세스를 느리게 하지만 더 빠른 검색을 가능하게 함" → 거의 실시간 쿼리 응답 가능
...
스크립트 API로 저장된 스크립트와 검색 템플릿 관리 가능, 사용 가능한 스크립트 컨텍스트와 언어 조회 기능도 제공
...
- `POST /_scripts/painless/_execute`: Painless 언어를 사용해 스크립트 실행
...
- `id`: 필수, 클러스터 내에서 스크립트 또는 템플릿의 고유 식별자
- `context`: 선택, 스크립트가 실행되는 실행 컨텍스트
  - 지정 시 API가 해당 컨텍스트에서 스크립트를 즉시 컴파일해 오류를 사전 확인

##### 쿼리 매개변수

- context: 실행 컨텍스트(둘 다 지정되면 경로 매개변수로 재정의)
- master_timeout: 마스터 노드에 대한 연결 대기 기간(시간 단위 지원)
- timeout: 응답 대기 기간(시간 단위 지원)

##### 요청 본문

요청에 필요한 `script` 객체 구성 요소:
...
`ScriptEngine`은 Elasticsearch 내에서 사용자 정의 스크립팅 언어를 구현하는 백엔드 역할. `ScriptEngine` 인터페이스로 통합되며, 플러그인은 초기화 시 등록을 위해 `ScriptPlugin` 구현

##### 주요 구현 구성 요소

언어 정의:

엔진은 `getType()`을 통해 사용자 정의 언어 식별자 정의 → 예를 들어 "expert_scripts"가 쿼리의 `lang` 매개변수 값이 됨

스크립트 인식:

`compile()` 메서드는 유효한 스크립트 소스 식별 → 예를 들어 "pure_df"를 스크립트 소스로 인식하면 사용자가 `source` 매개변수에서 참조

사용자 정의 로직:

`execute()` 메서드는 점수 메커니즘 구현

##### 사용 시기

표준 Painless에서 제공하지 않는 저수준 스크립팅 기능이 필요한 경우 사용자 정의 스크립트 엔진 구현:
- 점수 중 용어 빈도 접근이 필요한 스크립트
- 사용자 정의 구문 요구 사항이 있는 특수 언어

#### 12.2 Painless API 참조

Painless는 컨텍스트별로 허용된 메서드와 클래스의 엄격한 목록을 유지 → 모든 스크립트가 안전하게 실행되도록 보장

##### API 구조

문서가 API를 구성하는 두 가지 카테고리:
1. 공유 API - 모든 스크립팅 컨텍스트에서 사용 가능
2. 특수 API - 컨텍스트별 기능

##### 사용 가능한 컨텍스트

참조 문서가 다루는 23개의 스크립팅 컨텍스트:
...
대부분의 메서드는 JRE에서 직접 노출, 일부는 Elasticsearch 또는 Painless 자체에서 제공
...
ES|QL(Elasticsearch Query Language)은 데이터를 필터링·변환·분석하기 위한 파이프라인 기반 쿼리 언어 → 특정 이벤트 검색, 통계 분석, 시각화 생성에 활용 가능

ES|QL은 Elasticsearch에 저장된 데이터와 향후 다른 런타임의 데이터를 필터링·변환·분석하는 강력한 방법 제공. 최종 사용자, SRE 팀, 애플리케이션 개발자, 관리자 모두 쉽게 배우고 사용할 수 있도록 설계

#### 1.2 주요 특징

- 파이프라인 기반: 파이프(`|`)로 데이터를 단계별로 조작·변환. 각 작업의 출력이 다음 작업의 입력이 되는 방식으로 일련의 처리 단계 구성 가능

- 고성능 실행 엔진: 새로운 ES|QL 실행 엔진은 성능을 염두에 두고 설계 → 행 단위가 아닌 블록 단위로 동작, 벡터화 및 캐시 지역성 지향, 특수화 및 멀티스레딩 수용

- 전용 엔드포인트: ES|QL은 Query DSL로 변환되지 않으며, `_query` 엔드포인트를 통해 Elasticsearch 내부에서 직접 실행

- 다양한 사용 환경: Elasticsearch API, Kibana Discover, 대시보드, Dev Tools, Elastic Security 및 Observability의 분석 도구에서 ES|QL 사용 가능
...
ES|QL 쿼리는 소스 명령(source command)으로 시작 → 선택적으로 처리 명령(processing commands)이 파이프 문자(|)로 구분되어 뒤따름
...
쿼리 결과는 마지막 처리 명령이 생성하는 테이블

#### 2.2 구문 규칙

- 대소문자: ES|QL 키워드는 대소문자 구분 없음
- 가독성: 처리 명령마다 줄을 바꾸고 들여쓰기 적용 가능
- 기본 제한: 명시적인 LIMIT를 지정하지 않으면 암시적으로 1,000개 제한 적용
...
설명:
- `FROM` 명령: 인덱스, 데이터뷰 또는 별칭을 선택해 행 테이블로 이벤트 반환
- `LIMIT` 명령: 반환할 행 수 제한
- `KEEP` 명령: 반환할 열 지정
...
특수 문자가 포함된 인덱스 이름이나 식별자는 백틱(`` ` ``)으로 이스케이프. 큰따옴표(`"`)는 문자열 리터럴에만 사용
...
인덱스, 별칭 및 데이터 스트림을 참조할 때 날짜 수학 사용 가능 → 시계열 데이터에 유용
...
ES|QL 명령은 두 가지 유형으로 구분:
- 소스 명령 (Source Commands): 쿼리의 시작점, 데이터를 가져옴
- 처리 명령 (Processing Commands): 입력 테이블을 수정

#### 3.1 소스 명령

##### 3.1.1 FROM

`FROM` 명령은 데이터 스트림, 인덱스 또는 별칭에서 데이터가 포함된 테이블 반환. 결과 테이블의 각 행은 문서를 나타내고, 각 열은 필드에 해당
...
`ROW` 명령은 지정한 값으로 하나 이상의 열이 있는 행 생성 → 테스트에 유용
...
이 명령은 배포 버전, 빌드 날짜, 빌드 해시 반환

##### 3.1.4 TS (Time Series) - 기술 프리뷰

`TS` 명령은 시계열 데이터 스트림에서 메트릭 값을 집계하는 용도

특징:
- 시계열 분석은 시간 버킷별로 메트릭 값을 요약하는 집계 쿼리 기반
- ES|QL 컴퓨트 엔진을 통한 벡터화된 쿼리 실행 추가
- DSL 쿼리 대비 10배 이상의 성능 향상 제공
...
`WHERE` 명령은 지정한 조건이 true인 행만 남긴 테이블 반환
...
`STATS` 명령은 AVG, COUNT, SUM 등의 집계 함수를 사용해 데이터를 그룹화·집계
...
`INLINESTATS` 명령은 그룹별 통계를 계산한 뒤 그 결과를 원래 행에 새 열로 추가 → STATS와 달리 행을 축소하지 않음
...
제한 사항:
- LIMIT 값과 무관하게 쿼리는 10,000개를 초과하는 행을 반환하지 않음 (상한선은 설정으로 변경 가능)
- Kibana Discover에서는 10,000개 이상의 행 미표시
...
`KEEP` 명령은 반환할 열과 순서 지정 → 와일드카드 지원
...
주의 사항:
- 새 이름이 기존 열 이름과 충돌하면 기존 열 제거
- 여러 열을 같은 이름으로 변경하면 가장 오른쪽 열만 유지
...
`DISSECT` 명령은 구분자 기반 패턴으로 문자열을 분해하고 지정한 키를 열로 추출

특징:
- 구분자 기반으로 동작해 GROK보다 빠름
- 데이터 구조가 일관되게 반복될 때 효과적
- 추출된 값은 모두 keyword 타입으로 출력
...
`GROK` 명령은 정규 표현식 기반 패턴으로 문자열을 분석하고 지정한 키를 열로 추출

특징:
- 정규 표현식 기반으로 DISSECT보다 표현력이 강하지만 일반적으로 느림
- 행마다 텍스트 구조가 달라질 때 더 적합
- Oniguruma 정규 표현식 라이브러리 사용
- 사용자 정의 패턴이나 다중 패턴 미지원
...
일부 라인이 일관되게 반복되지만 전체 라인은 그렇지 않은 하이브리드 사용 사례에서 DISSECT와 GROK 함께 사용 가능
...
`ENRICH` 명령은 다른 인덱스의 데이터를 이용해 현재 테이블 보강
...
특징:
- 기본적으로 정책에 정의된 모든 보강 필드가 열로 추가
- `WITH`로 추가할 필드를 명시적으로 지정 가능
- `WITH new_name=<field>` 형태로 열 이름 변경 가능
- 이름 충돌 발생 시 새로 생성된 열이 기존 열을 덮어씀
...
`MV_EXPAND` 명령은 다중값 열의 각 값을 별도 행으로 펼침 → 나머지 열은 복제
...
주의 사항:
- MV_EXPAND로 생성된 출력 행은 순서 보장 안 됨 → 이전 SORT의 정렬 순서를 따르지 않을 수 있음
...
`LOOKUP JOIN` 명령은 ES|QL 쿼리 결과 테이블과 지정한 조회 인덱스의 일치하는 레코드 결합

특징:
- Elasticsearch의 첫 번째 SQL 스타일 JOIN
- LEFT OUTER JOIN으로 동작
- `lookup` 인덱스 모드 필요
- 조회 인덱스는 단일 샤드로 구성
- Elasticsearch 9.2부터 Cross-Cluster Search(CCS) 지원
...
CASE 함수:
- 조건과 값의 쌍을 받아 첫 번째 true 조건에 해당하는 값 반환
- 인수 개수가 홀수이면 마지막 인수가 기본값으로 사용
- 인수 개수가 짝수이고 일치하는 조건이 없으면 null 반환
...
`MATCH` 함수는 지정된 필드에 대해 매치 쿼리 수행 → Elasticsearch Query DSL의 `match` 쿼리와 동일한 동작
...
`QSTR` 함수는 Query DSL의 `query_string` 쿼리와 동일한 기능 제공 → 와일드카드·불리언 로직·다중 필드 검색 등 고급 검색 패턴 지원
...
ES|QL 쿼리 실행은 `POST /_query` 엔드포인트 사용
...
기본적으로 ES|QL은 결과를 행 형태로 반환 → `json`, `yaml`, `cbor`, `smile` 형식에서는 결과를 컬럼형으로 수신 가능
...
Query DSL 쿼리를 `filter` 매개변수에 지정하면 ES|QL 쿼리가 실행될 문서 집합 사전 필터링 가능
...
`profile` 매개변수를 `true`로 설정하면 응답에 쿼리 실행 방식에 대한 추가 프로파일 정보 포함
...
1. Kibana에서 Discover로 이동
2. 애플리케이션 메뉴 바에서 Try ES|QL 선택
3. ES|QL 편집기가 활성화되어 쿼리 작성 가능
...
ES|QL 결과를 기반으로 Kibana에서 시각화 생성 가능:
- 테이블
- 막대 차트
- 라인 차트
- 기타 Lens 시각화
...
- Discover는 최대 10,000개의 행만 표시
- Discover는 최대 50개의 열만 표시
- CSV 내보내기는 최대 10,000개의 행으로 제한
- ES|QL 모드에서는 필터 바 인터페이스 비활성화
...
- LIMIT 명령의 값과 무관하게 쿼리는 최대 10,000개의 행만 반환
- 이 상한은 `esql.query.result_truncation_max_size` 설정으로 조정 가능
- 명시적 LIMIT가 없을 때 적용되는 기본값은 1,000

#### 7.2 지원되지 않는 필드 타입

지원하지 않는 타입의 열을 쿼리하면 오류 반환. 쿼리에서 명시적으로 사용하지 않는 경우, 지원되지 않는 타입의 열은 null로 반환(중첩 필드 제외)
...
ES|QL은 `_source` 필드가 비활성화된 인덱스 미지원

#### 7.4 텍스트 필드 제한

- 전문 검색 함수(MATCH, QSTR)를 사용하지 않는 텍스트 필드 쿼리는 대소문자 구분
- 매핑 기반 분석이 ES|QL 쿼리에 적용되지 않아 오탐지 또는 미탐지 발생 가능
- 권장 사항: 텍스트 필드 대신 keyword 하위 필드를 직접 쿼리

#### 7.5 전문 검색 제한

전문 검색 함수(MATCH, QSTR 등)는 FROM 소스 명령 바로 뒤의 WHERE 절에서 사용 필요 → 다른 처리 명령 이후에 사용하면 유효성 검사 오류 발생
...
필터 없이 많은 인덱스를 한 번에 쿼리하면 Kibana에서 다음과 같은 오류 발생 가능:
```
