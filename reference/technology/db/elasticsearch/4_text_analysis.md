...
standard 분석기가 요구사항에 맞지 않는 경우, Elasticsearch의 다른 내장 분석기를 검토하고 테스트해볼 수 있습니다. 내장 분석기는 별도의 구성 없이 바로 사용할 수 있으며, 일부는 동작을 조정할 수 있는 옵션도 지원합니다.
...
텍스트 분석은 비정형 텍스트를 검색에 최적화된 형식으로 변환합니다. 이 과정을 거쳐 Elasticsearch는 역색인(inverted index)을 구축하고 빠른 전문 검색을 수행합니다.
...
내장 분석기는 이러한 구성 요소들을 다양한 언어와 텍스트 유형에 맞게 미리 패키징한 것입니다. Elasticsearch는 개별 구성 요소도 제공하므로, 이를 조합하여 새로운 사용자 정의 분석기를 만들 수 있습니다.
...
일반적으로, 동일한 형태의 토큰을 사용했을 때 예상치 못하거나 관련 없는 검색 결과가 발생하는 경우에만 별도의 검색 분석기를 지정하는 것이 좋습니다.
...
`analyze` API는 분석기가 생성하는 용어를 확인할 때 매우 유용합니다. 텍스트 문자열을 분석하여 결과 토큰을 반환합니다.
...
토큰이 과도하게 생성되면 노드의 메모리가 부족해질 수 있습니다. `index.analyze.max_token_count` 설정으로 생성 가능한 토큰 수를 제한할 수 있으며, 이 제한을 초과하면 오류가 발생합니다.
...
분석기가 지정되지 않은 모든 `text` 필드에 적용할 기본 분석기를 설정할 수 있습니다:
...
기본 구성 그대로 토크나이저, 토큰 필터, 캐릭터 필터를 사용할 수도 있지만, 각 구성 요소를 별도로 설정한 뒤 사용자 정의 분석기에서 참조할 수도 있습니다.
...
기존 인덱스에 분석기를 추가하려면 인덱스를 먼저 닫고, 분석기를 정의한 다음, 인덱스를 다시 열어야 합니다.
...
`classic` 토크나이저는 영어 문서에 적합한 문법 기반 토크나이저입니다. 두문자어, 회사명, 이메일 주소, 인터넷 호스트명에 대한 특별 처리를 위한 휴리스틱을 갖췄습니다.
...
`lowercase` 필터는 모든 토큰을 소문자로 변환합니다. 이를 통해 대소문자 차이가 쿼리 토큰과 문서 토큰 간의 매칭에 영향을 주지 않게 됩니다.
...
노말라이저(Normalizer)는 분석기와 유사하지만, 단일 토큰만 출력한다는 점에서 다릅니다. 토크나이저가 없으며, 문자 단위로 동작하는 캐릭터 필터와 토큰 필터의 일부만 사용할 수 있습니다.

노말라이저는 전문 텍스트 필드가 아닌 `keyword` 필드에 적용됩니다. 텍스트를 토큰으로 분해하지 않고, 캐릭터 필터와 토큰 필터를 통해 값을 정규화합니다.
...
노말라이저를 사용하면 인덱스에 저장되는 값이 변환됩니다. 원본 값(예: 대문자 "A"가 포함된 "Apple")을 보존하려면 서브필드를 활용하여 원본 값과 정규화된 값을 모두 유지할 수 있습니다.
