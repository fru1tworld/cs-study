...
kNN 검색 API는 먼저 각 샤드에서 `num_candidates` 수만큼 근사 최근접 이웃 후보를 찾습니다. 이후 후보 벡터들과 쿼리 벡터 간의 유사도를 계산하여 각 샤드에서 가장 유사한 k개 결과를 선택하고, 마지막으로 샤드별 결과를 병합하여 전역 top-k 최근접 이웃을 반환합니다.
...
근사 검색은 문서 규모 확장에 유리하므로, 문서가 많거나 크게 늘어날 것으로 예상되는 경우 근사 검색을 사용하는 것이 좋습니다.
...
`knn` 쿼리는 `k` 파라미터가 선택사항이며, 지정하지 않으면 검색 요청의 `size` 값을 따릅니다.
...
HNSW 인덱스를 사용한 근사 kNN 검색에 필터를 적용하면 성능이 저하될 수 있습니다. 필터를 충족하면서 `num_candidates` 수만큼 후보를 확보하기 위해 그래프를 더 많이 탐색해야 하기 때문입니다.
...
여러 `knn` 절과 `query` 절은 논리합(boolean OR)으로 결합됩니다:
...
- 이 겹침은 청크 경계에서 중요한 맥락 정보가 손실되지 않도록 하기 위함입니다.
...
ELSER는 입력 텍스트를 다양한 훈련 데이터에서 함께 등장하는 경향이 있는 용어들로 확장합니다. 이때 확장되는 용어는 단순한 동의어가 아니라, 관련성을 포착하도록 학습된 연관 용어들입니다.
...
kNN 검색 속도는 벡터 차원 수에 선형적으로 비례합니다. 유사도를 계산할 때 두 벡터의 모든 요소를 비교해야 하기 때문입니다.
...
Elasticsearch SQL은 Elasticsearch에 대해 SQL과 유사한 쿼리를 실시간으로 실행할 수 있게 해주는 X-Pack 구성 요소입니다. SQL과 Elasticsearch 양쪽에 익숙한 사용자가 두 환경에서 기본적으로 데이터를 검색하고 집계할 수 있습니다.
...
Elasticsearch를 위해 처음부터 구축되었으며, 각 쿼리는 기본 저장 아키텍처에 따라 관련 노드에서 효율적으로 실행됩니다.

자체 포함(Self-contained)

Elasticsearch를 쿼리하는 데 추가 하드웨어, 프로세스, 런타임, 라이브러리가 필요하지 않습니다. Elasticsearch SQL은 Elasticsearch 클러스터 _내부_에서 실행되어 별도의 구성 요소가 불필요합니다.

경량 및 효율성

Elasticsearch SQL은 검색을 추상화하지 않고 SQL을 그대로 수용하여, 실시간으로 적절한 전문 검색(full-text search)을 수행합니다.
...
Elasticsearch SQL은 SQL 의미론을 가능한 한 따르면서 두 가지 서로 다른 데이터 관리 패러다임을 연결합니다. 다음은 SQL과 Elasticsearch 간의 용어 매핑입니다:
...
Elasticsearch SQL은 "최소 놀라움의 원칙"을 따르며 SQL 의미론을 유지하면서 Elasticsearch의 네이티브 기능을 수용합니다. 덕분에 많은 개념이 Elasticsearch 전반에 걸쳐 자연스럽게 적용됩니다.
...
SQL 검색 API를 사용하면 JSON 문서 형식으로 SQL을 제출하고 결과를 받을 수 있습니다. 기본 엔드포인트는 `/_sql`입니다.
...
지정된 시간 내에 결과가 반환되지 않으면 요청이 비동기 모드로 전환되며 다음을 반환합니다:
...
SQL Translate API는 SQL 쿼리를 Elasticsearch Query DSL 요청으로 변환합니다. SQL 문이 쿼리 수준에서 어떻게 처리되는지 파악할 수 있습니다.
...
요청 본문은 `cursor`를 제외하고 SQL 검색 API와 동일한 파라미터를 지원합니다.
...
Elasticsearch SQL JDBC 드라이버는 JDBC 호출을 Elasticsearch SQL로 변환하는 Type 4 드라이버입니다. 플랫폼에 독립적인 순수 Java 솔루션입니다.
...
드라이버 버전은 Elasticsearch 버전보다 높아서는 안 됩니다. 예를 들어, 9.2.4 드라이버는 Elasticsearch 7.10.0에서 작동하지 않습니다.
...
Elasticsearch SQL ODBC 드라이버는 Elasticsearch를 위한 3.80 호환 ODBC 드라이버입니다. ODBC 함수 호출을 Elasticsearch SQL 작업으로 변환하는 코어 레벨 인터페이스로 동작합니다.
...
드라이버는 ODBC 호환 애플리케이션과 Elasticsearch SQL 기능 사이의 브리지 역할을 하므로, API를 직접 다루지 않아도 Elasticsearch 데이터에 SQL 쿼리를 실행할 수 있습니다.
...
Elasticsearch SQL 기능은 JDBC 및 ODBC 인터페이스를 통해 다양한 서드파티 애플리케이션에서 활용할 수 있습니다:
...
범위: 각 애플리케이션에는 고유한 요구 사항과 라이선스 조건이 있으며, 구성 지원은 Elasticsearch SQL 연결에 한합니다.
...
식별자는 테이블 및 열과 같은 엔티티의 이름을 지정합니다. 인용되지 않은 식별자(예: `ip_address`)와 인용된 식별자(예: `"hosts-*"`)가 있습니다. 인용된 식별자는 명확성과 모호성 제거에 유용하므로, 특히 사용자 입력을 처리할 때 사용을 권장합니다. 키워드와 달리 식별자는 대소문자를 구분합니다.
...
일반 리터럴: 캐스트 연산자나 변환 함수를 통해 생성됩니다.
...
SQL이 정확한 일치가 필요한 `text` 필드를 만나면, 정규화되지 _않은_ 첫 번째 `keyword` 서브필드를 찾아 원래 필드의 _정확한_ 값으로 사용합니다.
...
큰따옴표를 사용하며 Elasticsearch의 표준 다중 대상 표기법을 따릅니다. 와일드카드로 인덱스 포함, 제외, 열거를 지원합니다.
...
크로스 클러스터: 원격 클러스터는 `"<remote_cluster>:<target>"` 구문으로 쿼리할 수 있으며, 양쪽 모두 와일드카드를 지원합니다.

##### SQL LIKE 표기법

작은따옴표를 사용하며 표준 SQL 패턴 매칭 규칙을 따릅니다.
...
중요 제약: 다중 인덱스 쿼리에서 대상으로 지정된 모든 구체적인 테이블은 동일한 매핑을 공유해야 합니다.
...
매우 큰 쿼리는 파싱 단계에서 메모리를 과도하게 소비하여 `ParsingException`을 발생시킬 수 있습니다. 쿼리를 단순화하거나 더 작은 단위로 분할하는 것이 권장됩니다.
...
다중 중첩 문서에는 추가 제약이 있습니다. 쿼리는 서로 다른 수준이나 동일 수준의 여러 중첩 필드를 동시에 참조할 수 없습니다.

#### 13.3 페이지네이션 문제

중첩 필드를 선택할 때 페이지네이션은 정확히 페이지 크기만큼이 아닌, 최소 페이지 크기 이상의 레코드를 반환합니다. Elasticsearch가 중첩 쿼리를 내부 히트가 아닌 루트 문서 단위로 처리하기 때문입니다.

#### 13.4 집계 제약

집계 함수로 정렬하는 경우 최대 65,535행으로 제한됩니다. `ORDER BY` 절은 일반 집계 함수만 참조할 수 있으며, 스칼라 함수나 연산자 조합은 허용되지 않습니다. 또한 `FIRST` 및 `LAST` 집계 함수는 `HAVING` 절에서 사용할 수 없습니다.
...
서브쿼리는 단일 `SELECT` 문으로 평탄화되어야 합니다. `GROUP BY`, `HAVING` 또는 복잡한 조건이 포함된 서브쿼리는 지원되지 않습니다. `PIVOT` 집계는 단일 표현식만 허용하며, `PIVOT`의 `IN` 절에서 서브쿼리는 지원되지 않습니다.

#### 13.7 지리적 제약

`geo_shape` 필드는 doc values가 없어 필터링, 그룹화, 정렬에 사용할 수 없습니다. `geo_point` 필드의 고도(altitude) 값은 인덱싱되거나 저장되지 않습니다.
