...
`.service` unit은 데몬 프로세스를 정의하는 가장 흔한 unit 종류 → systemd가 시작·중지·감시하는 프로세스의 라이프사이클을 기술함
...
`Type=` 은 systemd가 서비스의 시작 완료 시점을 어떻게 판단할지 결정 → 의존하는 다른 unit이 언제 시작되어야 하는지를 정하는 핵심 옵션

#### simple (기본값, 권장하지 않음)

`ExecStart` 의 프로세스가 fork하지 않고 포그라운드로 실행됨 → systemd는 fork() 직후 즉시 unit을 active로 간주

문제: 프로세스가 실제로 준비됐는지(포트 바인딩 완료 등) 확인 불가 → 의존하는 unit이 너무 빨리 시작될 위험

#### exec (권장)

`simple` 과 비슷하지만, systemd는 `execve()` 가 성공할 때까지 대기 → 바이너리가 실제로 로드된 시점을 확인함. 대부분의 경우 `simple` 대신 이 옵션 사용 권장

#### forking

전통적인 데몬 패턴 — 프로세스가 fork하고 부모는 종료, 자식이 백그라운드에서 실행됨. `PIDFile=` 을 함께 지정해 systemd가 자식 PID를 추적할 수 있게 해야 함
...
레거시 데몬에만 사용. 새 서비스는 simple/exec/notify 권장

#### oneshot

한 번 실행하고 끝나는 작업에 사용 (예: 부팅 시 설정 적용). systemd는 프로세스가 종료될 때까지 대기
...
`RemainAfterExit=yes` 를 주면 프로세스 종료 후에도 unit을 active로 표시함

#### notify

서비스가 `sd_notify(3)` API로 시작 완료를 직접 알림. 가장 정확한 방식 → systemd 자체와 nginx, openvpn 등이 지원
...
`Type=notify` 와 같지만 `systemctl reload` 시 SIGHUP 후 `RELOADING=1` / `READY=1` 알림을 기다림 (systemd 253+)

#### dbus

`BusName=` 으로 지정한 D-Bus 이름을 가져올 때 unit이 active가 됨

#### idle

다른 작업이 끝날 때까지 시작을 지연 → 부팅 메시지 충돌 방지용, 거의 쓰이지 않음
...
서비스를 시작하는 명령. 절대 경로 필수. shell 메타문자(`>`, `|`, `*`)는 직접 처리되지 않으므로 셸이 필요하면 `/bin/sh -c "..."` 로 감싸야 함
...
`ExecStart` 직전/직후에 실행됨. 권한 설정, 디렉터리 생성, 헬스체크 등에 사용
...
`systemctl reload` 시 실행할 명령. 보통 SIGHUP 전송
...
서비스를 깔끔하게 종료할 명령. 미지정 시 systemd는 SIGTERM → SIGKILL 순으로 전송
...
- `on-abort`: 의도치 않은 시그널 (SIGABRT 등) 시 재시작
- `always`: 항상 재시작 (오류든 정상이든)

#### 재시작 제어

- `RestartSec=5`: 재시작 전 대기 시간 (기본 100ms)
- `StartLimitIntervalSec=60` + `StartLimitBurst=3`: 60초 안에 3번 이상 시작 실패하면 더 시도 안 함
- `StartLimitAction=`: 한도 초과 시 `reboot`, `poweroff` 등 지정 가능
...
서비스가 30초마다 `sd_notify(0, "WATCHDOG=1")` 을 호출하지 않으면 systemd가 서비스를 SIGABRT(또는 `WatchdogSignal=` 에 지정된 시그널)로 종료하고 failed 상태로 전환. `Restart=on-failure` 등을 설정하면 이후 자동 재시작됨. 데몬이 무한 루프에 빠진 경우를 잡아내는 데 유용
...
셸 스크립트 형식이 아니므로 `export`, 변수 치환, 따옴표 처리 등은 지원 안 되거나 제한적임

---

### PID 파일과 알림

`Type=forking` 일 때만 `PIDFile=` 이 의미 있음. systemd는 이 파일에서 PID를 읽어 메인 프로세스를 추적

```ini
PIDFile=/run/nginx.pid
```

PID 파일은 보안상 시스템 런타임 디렉터리 (`/run`)에 두는 것을 권장

`NotifyAccess=` 는 `Type=notify` 일 때 어느 프로세스의 알림을 수락할지 지정:
...
`systemd.exec` 는 서비스(`.service`), 소켓(`.socket`), 마운트(`.mount`), 스왑(`.swap`) 등 프로세스를 실제로 실행하는 unit의 실행 환경을 통제하는 옵션들을 정의

이 옵션들의 핵심 가치는 서비스 하드닝(hardening). 별도의 SELinux/AppArmor 정책 없이도 systemd만으로 강력한 샌드박싱 구성 가능. 내부적으로는 Linux 커널의 capabilities, namespaces, seccomp, mount propagation 등을 조합해 동작
...
`DynamicUser=yes` 는 다음 설정을 자동으로 적용:
- `RemoveIPC=yes`
...
각각 `/var/lib/myapp`, `/var/cache/myapp`, `/var/log/myapp`, `/etc/myapp`, `/run/myapp` 디렉터리가 자동으로 만들어지고 `User=` 권한이 적용됨

`RuntimeDirectoryPreserve=yes` 를 주면 서비스 재시작 사이에 `/run/myapp` 이 유지됨

---

### Capabilities

Linux capabilities는 root의 권한을 세밀하게 쪼갠 것. systemd는 서비스가 가질 수 있는 capability 제한 가능
...
systemd는 Linux namespaces를 활용해 서비스를 격리 가능
...
- `PrivateDevices=yes`: `/dev` 에 기본 디바이스(null, zero, random 등)만 노출. 디스크·네트워크 디바이스 차단
...
서비스가 자체 네트워크 namespace를 가져 호스트 네트워크 접근 불가. 루프백만 사용 가능. 외부 통신이 필요 없는 서비스에 매우 강력
...
서비스가 자체 user namespace를 가지며 UID/GID 매핑이 격리됨
...
`ProtectSystem=strict`으로 거의 모든 경로를 읽기 전용으로 잠근 뒤, `ReadWritePaths=`로 필요한 경로만 열어주는 패턴이 안전
...
seccomp 기반 syscall 허용 목록·차단 목록
...
- `SystemCallErrorNumber=`: 차단된 syscall이 어떤 에러로 실패할지 지정
...
- `MemoryDenyWriteExecute=yes`: W^X를 강제. JIT 컴파일러를 사용하는 언어(Java, Node.js 등)에서는 비활성화 필요
...
서비스가 사용할 수 있는 소켓 패밀리를 제한. AF_NETLINK 등을 차단해 공격 표면 축소 가능
...
서비스가 새 namespace를 생성하는 것을 차단. 컨테이너 런타임이 아닌 서비스에는 거의 항상 활성화 권장
...
0~10점 (낮을수록 안전). 옵션을 하나씩 켜면서 점수를 낮추는 것이 실전 하드닝의 좋은 출발점
...
이 unit은 `systemd-analyze security` 에서 1~2점대(매우 안전)가 나옴
...
Control Groups(cgroups)는 Linux 커널의 기능으로, 프로세스 그룹의 리소스(CPU, 메모리, IO, 네트워크 등) 사용을 측정·제한·격리함. Docker, Kubernetes, systemd 등 모든 현대 컨테이너·init 시스템의 기반 기술
...
systemd는 PID 1로서 cgroup 트리의 루트를 관리함. 모든 unit은 자동으로 cgroup에 배치됨
...
각 서비스는 `<name>.service` 이름의 cgroup을 가지며, 해당 cgroup 안의 모든 프로세스를 systemd가 추적함. PID 파일 기반의 SysV init보다 훨씬 안정적인 프로세스 추적 가능

---

### Slice 계층

`.slice` unit은 cgroup 트리의 중간 노드 역할. 리소스 정책을 그룹화할 때 유용
...
이렇게 하면 모든 DB 서비스가 한 slice에 묶여 합산 메모리 8GB, CPU 가중치 200으로 제한됨
...
NUMA 시스템에서 cgroup이 사용할 CPU·메모리 노드 지정
...
블록 디바이스 IO 대역폭 제어. cgroups v2에서 큰 폭으로 개선됨
...
> 참고: IO 컨트롤러는 파일시스템 종류와 IO 스케줄러에 따라 동작이 달라짐. ext4/xfs + bfq/mq-deadline 조합이 가장 잘 동작
...
해당 unit이 자식 cgroup을 직접 관리할 수 있도록 권한을 위임함. 컨테이너 런타임(systemd-nspawn, Docker 등)에서 사용하며, 일반 서비스에는 불필요
...
cgroup별 CPU·메모리·IO 사용량을 `top` 처럼 실시간으로 표시
...
`--scope` 는 외부 명령을 새 cgroup으로 감싸서 실행. 즉석에서 리소스 한도를 테스트할 때 유용
