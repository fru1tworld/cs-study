...
부팅 시 비어 있는 tmpfs(RAM 기반). 시스템이 켜진 동안만 의미 있는 런타임 데이터를 담고, 재부팅 시 사라짐. 디스크에 쓸 필요가 없는 PID 파일, 락 파일, 유닉스 도메인 소켓의 자연스러운 거주지.

## 역사적 맥락

옛날 리눅스에서는 같은 목적으로 `/var/run`을 썼음. 문제는:
- `/var`이 마운트되기 전엔 못 씀
- `/var`은 디스크인데 굳이 디스크에 쓸 이유 없음

그래서 부팅 초기부터 사용 가능한 루트 파일시스템 직속 tmpfs `/run`이 생겼고, 지금은 표준임 → `/var/run`은 `/run`의 심볼릭 링크.
...
`/run`이 매 부팅마다 비어 있다는 건 필요한 디렉터리/파일을 부팅 때마다 다시 만들어야 한다는 뜻. systemd의 `tmpfiles.d`가 이를 담당:
...
부팅 시 `systemd-tmpfiles --create`가 이걸 읽어 필요한 노드를 생성함.
