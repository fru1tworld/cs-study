...
리눅스는 everything is a file 철학을 따름 → 디바이스·프로세스 상태·커널 객체·컨테이너의 격리 경계까지 사용자 공간에서는 전부 파일/디렉터리 인터페이스로 노출됨. 여기서는 그 인터페이스들을 항목별로 정리함.
...
- [dev.md](dev.md): `/dev` — 디바이스 파일(null, zero, random, tty, shm 등) · 백엔드 devtmpfs
- [proc.md](proc.md): `/proc` — 프로세스·커널 상태 · 백엔드 procfs
- [sys.md](sys.md): `/sys` — 커널 객체 모델(kobject 트리) · 백엔드 sysfs
- [run.md](run.md): `/run` — 런타임 상태 · 백엔드 tmpfs
- [tmp.md](tmp.md): `/tmp` vs `/var/tmp` vs `/dev/shm` · 백엔드 tmpfs/disk
- [cgroups.md](cgroups.md): cgroups v1/v2, `/sys/fs/cgroup` · 백엔드 cgroup2 FS
- [namespaces.md](namespaces.md): 8가지 namespace, `/proc/<pid>/ns/` · 백엔드 nsfs
...
이 중 `/dev`, `/proc`, `/sys`, `/run`, `/tmp`(부분), `/dev/shm`은 디스크가 아니라 커널·메모리가 만들어내는 가상 파일시스템 → 그 위에 cgroups와 namespaces가 컨테이너의 기반을 이룸.
