...
Pyroscope는 단일 클러스터로 여러 테넌트(조직/팀/환경)를 분리 운영 가능.
...
소규모 환경에서는 멀티테넌시를 비활성화하면 모든 데이터가 `anonymous` 테넌트에 저장됨.
...
Pyroscope 자체에는 사용자/패스워드 개념이 없음. 인증은 보통 다음 중 하나로 처리함.
...
nginx에서 Basic Auth를 처리하고 헤더를 주입함.
...
운영 중에 한도를 변경하려면 Tenant-Settings 컴포넌트를 사용하거나 설정 파일 핫 리로드를 활용함.
...
대규모 멀티 테넌트 환경에서 시끄러운 이웃(noisy neighbor) 문제를 완화함.
...
Pyroscope는 자체 pprof 엔드포인트를 노출함.
...
`mixin` 디렉토리에 공식 대시보드/알람 규칙 존재(Mimir와 유사한 패턴).
...
- 오브젝트 스토리지는 일반적으로 SLA가 높음(S3 99.9999999%) → 별도 백업이 불필요한 경우가 많음
- 고가용성이 필수인 경우 크로스 리전 복제(Cross-Region Replication)를 활성화함

#### Ingester 노드 손실

- RF=3이면 나머지 2개 노드가 데이터를 보유 → 데이터 손실 없음
- WAL이 남아 있으면 재시작 시 자동 복구됨
- WAL까지 손실되면 아직 플러시되지 않은 최근 프로파일 유실 가능

#### 클러스터 전체 복구

- 새 Pyroscope 클러스터 기동
- 같은 오브젝트 버킷을 가리키게 설정
- Compactor가 버킷 인덱스를 재구성함
...
- [09_configuration.md](./4_manage_use_cases_configuration.md) - 설정 항목 상세
- [11_http_api.md](./5_cli_api_visualization.md) - 운영 API
- [03_deployment.md](./1_overview_architecture_deployment.md) - 규모별 배포
...
새로운 버전을 배포한 후 응답 시간이 미묘하게 늘어남. 메트릭 그래프로는 명확히 보이지만, 어느 코드 변경이 원인인지 모름.
...
CPU 사용률이 항상 80%를 넘고 스케일 아웃 비용이 큼. 어디를 최적화해야 할지 모름.
...
서비스 메모리가 시간에 따라 계속 증가하고 결국 OOM Kill됨.
...
서비스의 고루틴 수가 시간이 지나면서 계속 증가함. 결국 메모리/스케줄링 비용 증가로 OOM 또는 latency 폭증 발생.
...
EC2/GCE 비용이 급증했고 일부 마이크로서비스의 CPU 사용량이 크게 늘어남. 가능한 코드 최적화로 노드 수를 줄이고 싶은 상황.
...
P50은 정상인데 P99가 가끔 10배 튐. 트레이스에서는 `process` 스팬이 길게 잡힘.
...
CPU 여유는 충분한데 처리량이 늘지 않음. 멀티 코어가 제대로 활용되지 않는 것으로 의심됨.
...
- [05_flamegraphs.md](./2_profile_types_and_flamegraphs.md) - 분석 도구 사용법
- [12_visualization.md](./5_cli_api_visualization.md) - Grafana 통합
- [04_profile_types.md](./2_profile_types_and_flamegraphs.md) - 어떤 프로파일을 쓸지
...
YAML 파일로 작성하며, CLI 플래그와 환경 변수로 일부 값을 오버라이드 가능.
...
`-config.expand-env=true` 플래그로 활성화함.

#### 핫 리로드

`SIGHUP` 시그널로 일부 항목(limits 등)을 재로드 가능. 단, 모든 항목이 핫 리로드를 지원하지는 않음.
...
kvstore 대안으로 `consul`, `etcd`, `inmemory` 사용 가능.
...
테넌트별 및 글로벌 한도 설정임. (자세한 설명은 [07_manage.md](./4_manage_use_cases_configuration.md))
...
Grafana Labs에 익명 사용 통계를 전송함. 비활성화 가능.
...
- [03_deployment.md](./1_overview_architecture_deployment.md) - 배포 모드별 설정 차이
- [07_manage.md](./4_manage_use_cases_configuration.md) - 운영 가이드
- [11_http_api.md](./5_cli_api_visualization.md) - 상태 점검 API
