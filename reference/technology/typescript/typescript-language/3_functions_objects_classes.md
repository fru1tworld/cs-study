...
- 함수는 모든 애플리케이션의 기본 구성 요소임 → 로컬 함수든, 다른 모듈에서 가져온 함수든, 클래스의 메서드든 동일하게 적용됨
- 함수도 값이며, 다른 값과 마찬가지로 TypeScript는 함수를 호출하는 방법을 설명하는 다양한 방식을 제공함
- 함수를 설명하는 타입 작성 방법을 아래에서 다룸

### 함수 타입 표현식

- 함수를 설명하는 가장 간단한 방법은 _함수 타입 표현식_임
- 이러한 타입은 화살표 함수와 문법적으로 유사함
...
- JavaScript에서 함수는 호출 가능할 뿐만 아니라 속성도 보유 가능
- 그러나 함수 타입 표현식 구문은 속성 선언을 허용하지 않음
- 속성을 가진 호출 가능한 것을 설명하려면, 객체 타입에서 _호출 시그니처_ 작성 가능
...
- JavaScript 함수는 `new` 연산자로도 호출 가능함
- TypeScript는 이들을 보통 새 객체를 생성하기 때문에 _생성자_라고 부름
- 호출 시그니처 앞에 `new` 키워드를 추가하면 _생성 시그니처_ 작성 가능
...
- 입력의 타입이 출력의 타입과 관련되거나, 두 입력의 타입이 어떤 방식으로 관련된 함수를 작성하는 경우가 흔함
- 배열의 첫 번째 요소를 반환하는 함수 예시
...
- 이 함수는 작업을 수행하나, 반환 타입이 `any`인 점이 아쉬움
- 함수가 배열 요소의 타입을 반환하면 더 좋음
- TypeScript에서 _제네릭_은 두 값 사이의 대응을 설명하고 싶을 때 사용됨
- 함수 시그니처에서 _타입 매개변수_를 선언해 대응을 표현
...
- 이 샘플에서 `Type`을 지정할 필요가 없었음 → TypeScript가 타입을 _추론_(자동으로 선택)함
- 여러 타입 매개변수 사용도 가능함
- 예: `map`의 독립형 버전
...
- _어떤_ 종류의 값에서도 작동할 수 있는 제네릭 함수를 작성함
- 두 값을 관련시키고 싶지만, 특정 값의 하위 집합에서만 작동해야 하는 경우도 있음
  - 이 경우, 타입 매개변수가 받아들일 수 있는 종류의 타입을 제한하기 위해 _제약 조건_ 사용 가능
- 두 값 중 더 긴 것을 반환하는 함수 예시 → 숫자인 `length` 속성이 필요함
- `extends` 절을 작성해 타입 매개변수를 해당 타입으로 _제약_
...
- 이 예제의 흥미로운 점
  - TypeScript가 `longest`의 반환 타입을 _추론_하도록 허용함 → 반환 타입 추론은 제네릭 함수에서도 작동함
...
- 이 함수는 괜찮아 보일 수 있음 → `Type`이 `{ length: number }`로 제약되어 있고, 함수가 `Type`이나 해당 제약과 일치하는 값을 반환함
- 문제점: 함수가 전달된 것과 _같은_ 종류의 객체를 반환할 것을 약속하는 것이지, 단지 제약과 일치하는 _어떤_ 객체를 반환하는 것이 아님
- 이 코드가 합법적이라면, 확실히 작동하지 않을 코드를 작성할 수 있게 됨
...
- 제네릭 함수를 작성하는 것은 재미있고, 타입 매개변수에 쉽게 빠져들 수 있음
- 너무 많은 타입 매개변수를 갖거나 필요하지 않은 곳에 제약 조건을 사용하면 추론이 덜 성공적이 되어 함수 호출자를 좌절시킬 수 있음
...
- 두 값을 _관련시키지 않는_ 타입 매개변수 `Func`가 생성됨 → 항상 빨간 플래그임
  - 타입 인수를 지정하려는 호출자가 이유 없이 추가 타입 인수를 수동으로 지정해야 함
  - `Func`는 함수를 읽고 추론하기 더 어렵게 만드는 것 외에는 아무 역할도 하지 않음
...
- 타입 매개변수는 _여러 값의 타입을 관련시키기_ 위한 것임
- 타입 매개변수가 함수 시그니처에서 한 번만 사용되면, 아무것도 관련시키지 않음
  - 추론된 반환 타입도 포함됨 → 예를 들어, `Str`이 `greet`의 추론된 반환 타입의 일부라면, 인수와 반환 타입을 관련시키므로 작성된 코드에서 한 번만 나타나더라도 _두 번_ 사용된 것으로 봄
...
- TypeScript에서 `?`로 매개변수를 _선택적_으로 표시해 모델링 가능
...
- 매개변수가 타입 `number`로 지정되었지만, `x` 매개변수는 실제로 `number | undefined` 타입을 가짐 → JavaScript에서 지정되지 않은 매개변수는 값 `undefined`를 얻기 때문
- 매개변수 _기본값_도 제공 가능
...
- 이것이 _실제로_ 의미하는 것: _`callback`이 하나의 인수로 호출될 수 있다_는 것
- 다시 말해, 함수 정의는 구현이 다음과 같을 수 있다고 말하는 것
...
- TypeScript는 이 의미를 적용해 실제로 가능하지 않은 오류를 발생시킴
...
- TypeScript에서 _오버로드 시그니처_를 작성해 다른 방식으로 호출할 수 있는 함수를 지정 가능
...
- 이 예제: 두 개의 오버로드를 작성함 → 하나는 하나의 인수, 다른 하나는 세 개의 인수를 받음
  - 이 처음 두 시그니처를 _오버로드 시그니처_라고 함
- 그런 다음, 호환 가능한 시그니처로 함수 구현을 작성함
  - 함수에는 _구현_ 시그니처가 있지만, 이 시그니처는 직접 호출 불가
  - 필수 매개변수 뒤에 두 개의 선택적 매개변수가 있는 함수를 작성했지만, 두 개의 매개변수로는 호출 불가

#### 오버로드 시그니처와 구현 시그니처

- 이것은 흔한 혼란의 원인임
- 다음과 같은 코드를 작성하고 왜 오류가 있는지 이해하지 못하는 경우가 많음
...
- 이 함수는 괜찮음 → 문자열이나 배열로 호출 가능
- 그러나 문자열 _또는_ 배열일 수 있는 값으로는 호출 불가 → TypeScript는 함수 호출을 단일 오버로드로만 해결 가능하기 때문
...
- TypeScript는 함수 `user.becomeAdmin`이 외부 객체 `user`에 해당하는 `this`를 가진다는 것을 이해함
- `this`는 많은 경우에 충분할 수 있지만, `this`가 나타내는 객체에 대해 더 많은 제어가 필요한 경우가 많음
- JavaScript 명세는 `this`라는 이름의 매개변수를 가질 수 없다고 명시함 → TypeScript는 해당 구문 공간을 사용해 함수 본문에서 `this`의 타입을 선언할 수 있게 함
...
- 이것은 _빈 객체 타입_ `{ }`과 다르며, 전역 타입 `Object`와도 다름
...
- 이것은 _타입이 지정되지 않은 함수 호출_이며, 안전하지 않은 `any` 반환 타입 때문에 일반적으로 회피 권장
...
- 선택적 매개변수나 오버로드를 사용해 다양한 고정 인수 수를 받아들이는 함수를 만드는 것 외에도, _나머지 매개변수_를 사용하면 _무한한_ 수의 인수를 받는 함수도 정의 가능
...
- 반대로, 스프레드 구문을 사용해 반복 가능한 객체(예: 배열)에서 가변적인 수의 인수를 _제공_ 가능
...
- 나머지 인수를 사용하려면 이전 런타임을 대상으로 할 때 [`downlevelIteration`](/tsconfig#downlevelIteration)을 켜야 할 필요가 있을 수 있음
...
- 함수의 `void` 반환 타입은 일부 특이하지만 예상되는 동작을 생성할 수 있음
- `void` 반환 타입을 가진 문맥적 타이핑은 함수가 무언가를 반환**하지 않도록** 강제하지 **않음**
  - 다시 말해, `void` 반환 타입을 가진 문맥적 함수 타입(`type voidFunc = () => void`)이 구현될 때 _어떤_ 다른 값이든 반환 가능하지만, 무시됨
- 따라서, `() => void` 타입의 다음 구현은 유효함
...
- 이 동작이 존재하여, `Array.prototype.push`가 숫자를 반환하고 `Array.prototype.forEach` 메서드가 `void` 반환 타입을 가진 함수를 기대함에도 다음 코드가 유효함
...
- 알아야 할 또 다른 특별한 경우: 리터럴 함수 정의가 `void` 반환 타입을 가질 때, 해당 함수는 아무것도 반환하지 **않아야** 함
...
- JavaScript에서 데이터를 그룹화하고 전달하는 기본적인 방법은 객체를 통하는 것임
- TypeScript에서 이것들을 _객체 타입_으로 표현함
- 익명일 수 있음
...
- 많은 경우 속성이 설정되어 있을 _수도_ 있는 객체를 다루게 됨
- 그러한 경우, 속성 이름 끝에 물음표(`?`)를 추가해 해당 속성을 _선택적_으로 표시 가능
...
- 이 예제에서, `xPos`와 `yPos` 모두 선택적으로 간주됨
- 둘 중 하나를 제공하도록 선택 가능하므로, 위 `paintShape`에 대한 모든 호출이 유효함
- 선택성이 실제로 말하는 것: 속성이 _설정되면_ 특정 타입을 가져야 한다는 것
- 해당 속성에서 읽기도 가능함 → 다만 [`strictNullChecks`](/tsconfig#strictNullChecks)에서 TypeScript는 잠재적으로 `undefined`라고 알려줌
...
- `readonly` 수정자를 사용한다고 해서 값이 완전히 불변이라는 의미는 아님 → 내부 내용을 변경할 수 없다는 의미가 아님
- 속성 자체를 다시 쓸 수 없다는 것만 의미함
...
- `readonly`가 의미하는 바에 대한 기대를 관리하는 것이 중요함
- TypeScript에서 객체가 어떻게 사용되어야 하는지에 대한 개발 시간 동안의 의도를 알리는 데 유용함
- TypeScript는 두 타입이 호환되는지 확인할 때 속성이 `readonly`인지 고려하지 않음 → 따라서 `readonly` 속성은 별칭을 통해 변경될 수도 있음
...
- [매핑 수정자](/docs/handbook/2/mapped-types.html#mapping-modifiers)를 사용해 `readonly` 속성 제거 가능
...
- `createSquare`에 주어진 인수가 `color` 대신 `colour`로 철자가 되어 있는 점에 주목
- 일반 JavaScript에서 이런 종류의 일은 조용히 실패함
- 이 프로그램이 올바르게 타입화되었다고 주장할 수도 있음 → `width` 속성이 호환되고, `color` 속성이 없으며, 추가 `colour` 속성은 중요하지 않기 때문
- 그러나 TypeScript는 이 코드에 아마도 버그가 있다는 입장을 취함
- 객체 리터럴은 특별한 처리를 받고 다른 변수에 할당하거나 인수로 전달할 때 _초과 속성 검사_를 받음
- 객체 리터럴이 "대상 타입"에 없는 속성이 있으면, 오류가 발생함
...
- `SquareConfig`가 위의 타입으로 `color`와 `width` 속성을 가질 수 있지만, 다른 수의 속성도 가질 수 _있다면_, 다음과 같이 정의 가능
...
- 위와 같은 간단한 코드의 경우, 이러한 검사를 "피하려고" 시도하는 것은 권장하지 않음
- 메서드를 가지고 상태를 유지하는 더 복잡한 객체 리터럴의 경우, 이러한 기술을 염두에 두어야 할 수 있으나, 대부분의 초과 속성 오류는 실제로 버그임
- 즉, 옵션 백과 같은 것에 대해 초과 속성 검사 문제가 발생하면, 일부 타입 선언을 수정해야 할 필요가 있음
  - `color`나 `colour` 속성을 가진 객체를 `createSquare`에 전달하는 것이 괜찮다면, 이를 반영하도록 `SquareConfig`의 정의를 수정해야 함
...
- 이것은 작업을 수행하지만, 여기서 단점은 변경 사항이 순전히 추가적일 때도 `BasicAddress`의 다른 모든 필드를 반복해야 한다는 것
- 대신, 원래 `BasicAddress` 타입을 확장하고 `AddressWithUnit`에 고유한 새 필드만 추가 가능
...
- TypeScript는 주로 기존 객체 타입을 결합하는 데 사용되는 _교차 타입_이라는 또 다른 구문도 제공함
- 교차 타입은 `&` 연산자를 사용해 정의됨
...
- 여기서 `Colorful`과 `Circle`을 교차하여 `Colorful` _과_ `Circle`의 모든 멤버를 가진 새 타입을 생성함
...
- 이것은 상당한 상용구임 → 게다가, 나중에 새 타입과 오버로드를 도입해야 할 수 있음
- 박스 타입과 오버로드가 모두 효과적으로 같기 때문에 이것은 실망스러운 결과임
- 대신, _타입 매개변수_를 선언하는 _제네릭_ `Box` 타입을 만들 수 있음
...
- 나중에 `Box`를 참조할 때, `Type` 대신 _타입 인수_를 제공해야 함
...
- `Box`를 실제 타입에 대한 템플릿으로 생각할 것 → `Type`은 다른 타입으로 대체될 플레이스홀더임
- TypeScript가 `Box<string>`을 보면, `Box<Type>`의 모든 `Type` 인스턴스를 `string`으로 대체하고, `{ contents: string }`과 같은 것으로 작업함
- 다시 말해, `Box<string>`과 이전 `StringBox`는 동일하게 작동함
...
- 이것은 또한 [제네릭 함수](/docs/handbook/2/functions.html#generic-functions)를 대신 사용해 오버로드를 완전히 피할 수 있다는 것을 의미함
...
- 제네릭 객체 타입은 종종 포함하는 요소의 타입과 독립적으로 작동하는 일종의 컨테이너 타입임
- 데이터 구조가 이런 방식으로 작동하는 것이 다른 데이터 타입에서 재사용 가능하도록 이상적임
- 이 핸드북 전체에서 바로 그런 타입으로 작업해 왔음 → `Array` 타입
- `number[]`나 `string[]`과 같은 타입을 작성할 때마다, 이것은 실제로 `Array<number>`와 `Array<string>`의 줄임말임
...
- _튜플 타입_은 정확히 몇 개의 요소를 포함하고 특정 위치에 정확히 어떤 타입을 포함하는지 아는 또 다른 종류의 `Array` 타입임
...
- 선택적 및 나머지 요소가 유용한 이유: TypeScript가 튜플을 매개변수 목록과 대응시킬 수 있기 때문임
- 튜플 타입은 [나머지 매개변수와 인수](/docs/handbook/2/functions.html#rest-parameters-and-arguments)에서 사용 가능하므로
...
- 여기서 `distanceFromOrigin`은 요소를 수정하지 않지만, 변경 가능한 튜플을 기대함
- `point`의 타입이 `readonly [3, 4]`로 추론되었으므로, 해당 타입이 `point`의 요소가 변경되지 않을 것을 보장할 수 없어 `[number, number]`와 호환되지 않음
...
- TypeScript는 ES2015에서 도입된 `class` 키워드에 대한 완전한 지원을 제공함
- 다른 JavaScript 언어 기능과 마찬가지로 TypeScript는 클래스와 다른 타입 간의 관계를 표현할 수 있도록 타입 어노테이션 및 기타 구문을 추가함
...
- 필드는 _초기화자_도 가질 수 있음 → 클래스가 인스턴스화될 때 자동으로 실행됨
...
- [`strictPropertyInitialization`](/tsconfig#strictPropertyInitialization) 설정은 클래스 필드가 생성자에서 초기화되어야 하는지를 제어함
...
- 필드가 _생성자 자체에서_ 초기화되어야 한다는 점에 주의
- TypeScript는 파생 클래스가 해당 메서드를 재정의하고 멤버를 초기화하지 못할 수 있으므로, 생성자에서 호출하는 메서드를 분석해 초기화를 감지하지 않음
- 생성자 외의 수단을 통해 필드를 확실히 초기화하려는 경우(예: 외부 라이브러리가 클래스의 일부를 채워주는 경우), _확정 할당 단언 연산자_ `!`를 사용 가능함
...
- 클래스는 인덱스 시그니처를 선언 가능함 → [다른 객체 타입의 인덱스 시그니처](/docs/handbook/2/objects.html#index-signatures)와 같은 방식으로 작동함
...
- 인덱스 시그니처 타입이 메서드의 타입도 캡처해야 하므로, 이러한 타입을 유용하게 사용하기는 쉽지 않음
- 일반적으로 인덱싱된 데이터를 클래스 인스턴스 자체가 아닌 다른 곳에 저장하는 것을 권장함
...
- 클래스는 여러 인터페이스를 구현할 수도 있음. 예: `class C implements A, B {`

##### 주의 사항

- `implements` 절은 클래스가 인터페이스 타입으로 처리될 수 있는지에 대한 검사일 뿐이라는 것을 이해하는 것이 중요함
- 클래스의 타입이나 메서드를 _전혀_ 변경하지 않음
- 흔한 오류 원인: `implements` 절이 클래스 타입을 변경할 것이라고 가정하는 것 → 실제로는 변경하지 않음
...
- 이 예제에서, 아마도 `s`의 타입이 `check`의 `name: string` 매개변수에 의해 영향을 받을 것으로 예상했을 것임
- 그렇지 않음 → `implements` 절은 클래스 본문이 검사되거나 타입이 추론되는 방식을 변경하지 않음
- 마찬가지로, 선택적 속성이 있는 인터페이스를 구현해도 해당 속성이 생성되지는 않음
...
- `target >= ES2022` 또는 [`useDefineForClassFields`](/tsconfig#useDefineForClassFields)가 `true`이면, 클래스 필드는 부모 클래스 생성자가 완료된 후 초기화되어 부모 클래스가 설정한 값을 덮어씀
...
- 이것은 파생 클래스 필드 초기화가 아직 실행되지 않았기 때문에 기본 클래스 생성자가 자체 생성자 중에 `name`에 대해 자체 값을 보았다는 의미임
...
- `public`이 이미 기본 가시성 수정자이므로, 클래스 멤버에 작성할 _필요_는 없지만, 스타일/가독성을 위해 선택 가능함
...
- 이것은 `private`으로 선언된 필드를 유닛 테스트와 같은 것에 대해 잠재적으로 더 쉽게 접근할 수 있게 하지만, 이러한 필드가 _소프트 private_이며 프라이버시를 엄격하게 적용하지 않는다는 단점이 있음
...
- TypeScript의 `private`과 달리, JavaScript의 [private 필드](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes/Private_class_fields)(`#`)는 컴파일 후에도 private으로 유지되며 대괄호 표기법 접근과 같은 앞서 언급한 탈출구를 제공하지 않아 _하드 private_임
...
- 이러한 구문은 해당 언어가 모든 데이터와 함수를 클래스 내부에 강제하기 때문에 _존재_함 → TypeScript에는 그러한 제한이 없으므로, 필요 없음
- 단일 인스턴스만 있는 클래스는 일반적으로 JavaScript/TypeScript에서 일반 _객체_로 표현됨
...
- 타입은 항상 완전히 지워진다는 점을 기억할 것
- 런타임에 `Box.defaultValue` 속성 슬롯은 _하나_뿐임
- 이것은 `Box<string>.defaultValue`를 설정하면(가능하다면) `Box<number>.defaultValue`도 _변경_한다는 것을 의미함 → 좋지 않은 결과
- 제네릭 클래스의 `static` 멤버는 클래스의 타입 매개변수를 참조할 수 없음

### 클래스에서의 런타임 `this`

- TypeScript가 JavaScript의 런타임 동작을 변경하지 않으며, JavaScript의 런타임 동작이 다소 독특하다는 것을 기억할 것
- JavaScript의 `this` 처리는 실제로 특이함
...
- 간단히 말해서, 기본적으로 함수 내부의 `this` 값은 _함수가 어떻게 호출되었는지_에 따라 달라짐
- 이 예제에서 함수가 `obj` 참조를 통해 호출되었으므로, `this` 값은 클래스 인스턴스가 아닌 `obj`였음
- 이것은 거의 원하지 않는 결과임
- TypeScript는 이런 종류의 오류를 완화하거나 방지하는 몇 가지 방법을 제공함
...
- 메서드나 함수 정의에서 `this`라는 이름의 초기 매개변수는 TypeScript에서 특별한 의미를 가짐
- 이러한 매개변수는 컴파일 중에 지워짐
...
- 클래스에서 `this`라는 특별한 타입은 현재 클래스의 타입을 _동적으로_ 참조함
...
- 이것들을 _매개변수 속성_이라고 하며, 생성자 인수에 가시성 수정자 `public`, `private`, `protected`, 또는 `readonly` 중 하나를 접두사로 붙여 생성함
- 결과 필드는 해당 수정자를 얻음
...
- 클래스 자체의 타입이 주어지면, [InstanceType](/docs/handbook/utility-types.html#instancetypetype) 유틸리티 타입이 이 작업을 모델링함
...
- TypeScript의 클래스, 메서드 및 필드는 _abstract_일 수 있음
- _추상 메서드_ 또는 _추상 필드_는 구현이 제공되지 않은 것임
- 이러한 멤버는 직접 인스턴스화할 수 없는 _추상 클래스_ 내에 존재해야 함
- 추상 클래스의 역할: 모든 추상 멤버를 구현하는 서브클래스의 기본 클래스 역할을 하는 것
- 클래스에 추상 멤버가 없으면 _구체적_이라고 함
- 예제로 확인
...
- 때때로 어떤 추상 클래스에서 파생된 클래스의 인스턴스를 생성하는 일부 클래스 생성자 함수를 받아들이고 싶은 경우가 있음
- 예: 다음 코드를 작성하고 싶을 수 있음
