...
주요 고려사항:
- 복제 계수(Replication factor): 데이터 중복성을 제어합니다. 복제 계수 3은 데이터 손실 없이 브로커 2개의 동시 장애를 허용합니다.
- 파티션 수: 데이터 샤딩과 컨슈머 병렬성을 결정합니다. 폴더 이름의 최대 길이(255자) 제약으로 인해 토픽 이름은 최대 249자로 제한됩니다.
...
이를 통해 개별 브로커 장애뿐 아니라 전체 랙 장애에 대한 내결함성도 확보할 수 있습니다.
...
재할당 도구는 브로커 제거 시 수동 계획이 필요합니다. 관리자는 대상 브로커의 모든 파티션을 나머지 브로커로 옮기는 재할당 계획을 직접 작성해야 하며, 레플리카가 특정 브로커 한 곳에 집중되지 않도록 주의해야 합니다.
...
권장 패턴은 "각 데이터센터에 로컬 Kafka 클러스터를 배포하고, 각 데이터센터의 애플리케이션 인스턴스는 로컬 클러스터와만 통신하며, 클러스터 간 데이터는 미러링으로 동기화"하는 것입니다.
...
모든 위치에서 전체 데이터를 조회해야 하는 애플리케이션에는 "각 데이터센터의 로컬 클러스터로부터 데이터를 집계하여 미러링하는 클러스터"를 별도로 두는 방식이 권장됩니다. 이 집계 클러스터는 전체 데이터셋이 필요한 애플리케이션에 읽기 전용으로 제공됩니다.

#### 2.3 높은 지연 시간 WAN 연결

원격 클러스터를 대상으로 읽기/쓰기를 수행할 수 있지만 추가 지연이 발생합니다. Kafka의 프로듀서·컨슈머 배치 메커니즘은 "높은 지연 시간 연결에서도 높은 처리량"을 달성할 수 있게 해줍니다.

WAN 링크에서 성능을 최적화하려면 `socket.send.buffer.bytes`와 `socket.receive.buffer.bytes` 설정을 사용하여 프로듀서, 컨슈머, 브로커의 TCP 소켓 버퍼 크기를 늘려야 합니다.

#### 2.4 안티패턴 경고

"높은 지연 시간 WAN 링크로 연결된 여러 데이터센터에 걸친 단일 Kafka 클러스터" 구성은 복제 지연 및 가용성 문제로 인해 명시적으로 권장하지 않습니다.
...
Kafka의 지리적 복제 기능을 통해 관리자는 "개별 Kafka 클러스터, 데이터센터, 지리적 지역의 경계를 넘는 데이터 흐름"을 정의할 수 있습니다.

이는 Kafka Connect 프레임워크 위에 구축된 MirrorMaker 2를 통해 수행되며, "서로 다른 Kafka 환경 간에 데이터를 스트리밍 방식으로 복제하는 도구"입니다.
...
원격에서 소비, 로컬에서 생산: 지연 시간을 최소화하기 위해 MirrorMaker 프로세스를 대상 클러스터 가까이에 배치합니다. "Kafka 프로듀서는 일반적으로 Kafka 컨슈머보다 불안정하거나 지연이 높은 네트워크 연결에서 더 큰 어려움을 겪기" 때문입니다.
...
MirrorMaker는 `kafka.connect.mirror` 그룹 아래에 다음 태그가 붙은 메트릭을 발행합니다:
- `source`: 소스 클러스터 별칭
- `target`: 대상 클러스터 별칭
- `topic`: 대상의 복제된 토픽
- `partition`: 복제 중인 파티션
...
"고도로 확장 가능한 이벤트 스트리밍 플랫폼인 Kafka는 다수의 조직에서 중추 시스템으로 활용되며, 다양한 팀과 사업 부문의 수많은 시스템·애플리케이션을 실시간으로 연결합니다."
...
Kafka 관리자는 보안 제어와 결합된 계층적 토픽 명명 체계를 통해 격리된 테넌트 환경을 구축합니다. 일반적으로 사용되는 두 가지 명명 방식은 다음과 같습니다:
...
"특히 멀티 테넌트 환경에서 계층적 토픽 명명 구조를 갖추면 접두사 ACL을 통해 토픽 접근을 편리하게 제어할 수 있다는 이점이 있습니다."
...
이벤트 형식 검증, 잘못된 데이터 차단, 스키마 진화 관리를 위해 스키마 레지스트리를 도입하는 것이 좋습니다. 단, Kafka 자체에는 이 구성 요소가 포함되어 있지 않습니다.
...
Kafka는 24GB RAM을 장착한 듀얼 쿼드코어 Intel Xeon 머신에서 원활하게 작동합니다. 메모리 요구량은 활성 리더·라이터의 버퍼링 규모에 따라 달라지며, "쓰기 처리량 × 30초" 분량의 버퍼링을 기준으로 추정할 수 있습니다.

디스크 처리량이 주요 성능 병목입니다. "8x7200 rpm SATA 드라이브"가 기준 구성으로 언급되며, 일반적으로 드라이브 수가 많을수록 성능이 향상됩니다. 고RPM SAS 드라이브가 실질적인 이점을 제공하는지는 플러시 동작 설정에 따라 달라집니다.

#### 6.2 운영 체제 지원

Kafka는 Linux 및 Solaris를 포함한 Unix 계열 시스템에서 안정적으로 동작합니다. Windows는 개선 가능성이 있으나 아직 완전한 지원이 부족합니다. 몇 가지 핵심 설정을 제외하면 OS 수준의 튜닝은 최소한으로 충분합니다.

#### 6.3 OS 수준 설정

파일 디스크립터: 브로커 프로세스의 시작 기준으로 최소 100,000으로 설정합니다. Kafka는 로그 세그먼트와 네트워크 연결에 파일 디스크립터를 사용합니다. 파티션이 많은 브로커는 "(파티션 수) × (파티션 크기 / 세그먼트 크기)" + 연결 수만큼의 디스크립터가 필요합니다.

소켓 버퍼 크기: 데이터센터 간 고성능 데이터 전송을 가능하게 하려면 최대 소켓 버퍼 크기를 늘립니다.

메모리 매핑 (vm.max_map_count): Linux 기본값은 약 65,535로, 파티션 수가 많은 환경에는 부족합니다. 각 로그 세그먼트는 맵 영역 2개를 필요로 하므로, 파티션 50,000개는 맵 영역 100,000개 이상을 요구하며 이를 초과하면 OutOfMemoryError가 발생합니다.

#### 6.4 디스크 및 파일시스템 전략

Kafka 데이터 전용 드라이브를 여러 개 사용하고, 애플리케이션 로그나 OS 활동과는 분리해야 합니다. 드라이브를 RAID로 묶어 단일 볼륨으로 구성하거나, 각 드라이브를 독립적인 디렉토리로 포맷할 수 있습니다. 파티션은 디렉토리 간에 라운드 로빈 방식으로 분산됩니다.

RAID는 부하 분산 효과가 있지만 쓰기 처리량과 사용 가능한 공간을 줄입니다. RAID 재구축은 I/O를 집중적으로 사용하므로 사실상 서버를 일시적으로 비활성화하여 실제 가용성 이점을 제한합니다.

#### 6.5 플러시 관리

Kafka는 "모든 데이터를 파일시스템에 즉시 기록하지만" 강제 디스크 플러시는 지연시킵니다. "애플리케이션 수준의 fsync를 완전히 비활성화하는 기본 플러시 설정"을 그대로 사용하고 OS 백그라운드 플러싱에 맡기는 방식이 권장됩니다. 이 방식은 "별도의 튜닝 없이도 뛰어난 처리량·지연 시간과 완전한 복구 보장"을 제공합니다.

애플리케이션 수준의 fsync 정책은 비효율성과 지연 증가를 유발합니다. 대부분의 Linux 파일시스템은 fsync 중 쓰기를 차단하는 반면, 백그라운드 플러싱은 세분화된 페이지 단위 잠금을 사용하기 때문입니다.

#### 6.6 파일시스템 선택

EXT4와 XFS가 가장 많이 사용되는 파일시스템입니다. XFS는 최적화된 EXT4 구성 대비 "160ms 대 250ms+"로 우수한 성능을 보이며 디스크 성능 변동도 적습니다.
...
Kafka는 서버 측에 Yammer Metrics, 클라이언트 측에 Kafka Metrics를 사용하며, 두 방식 모두 JMX를 통해 메트릭을 노출하고 외부 모니터링 시스템용 플러그인 리포터를 지원합니다.
...
KRaft(Kafka Raft)는 ZooKeeper를 대체하는 Kafka 클러스터 메타데이터 관리용 합의 프로토콜로, 최신 Kafka 배포에 권장되는 아키텍처입니다.
...
이 명령은 meta.properties 파일과 해당 노드를 유일한 쿼럼 투표자로 설정하는 제어 레코드가 포함된 초기 스냅샷을 생성합니다.
...
`--initial-controllers` 값은 동일한 클러스터 ID를 공유하는 모든 컨트롤러에서 동일하게 지정해야 합니다.
...
마이그레이션에는 브리지 릴리스가 필요합니다. "마지막 브리지 릴리스는 Kafka 3.9입니다." 세부 마이그레이션 절차는 Kafka 3.9 문서의 KRaft/ZooKeeper 마이그레이션 섹션을 참고하세요.
...
Kafka 계층형 스토리지는 데이터 액세스 패턴을 최적화하는 2계층 스토리지 시스템을 구현합니다. 이 설계는 "Kafka 데이터가 대부분 OS 페이지 캐시를 활용하는 테일 읽기 방식으로 스트리밍 소비된다"는 점을 반영하며, 오래된 데이터는 디스크 액세스가 드물게 발생합니다. 시스템은 다음 두 계층을 분리합니다:
...
세그먼트는 롤링 후 원격 스토리지에 자동으로 업로드되며, 이후 로컬 복사본은 보존 정책에 따라 삭제됩니다.
...
Apache Kafka 4.0은 차세대 컨슈머 리밸런스 프로토콜(KIP-848)을 정식(GA)으로 도입했습니다. 이 프로토콜은 "완전히 증분적인 설계 덕분에 리밸런스 시간을 줄이면서" 컨슈머 그룹 확장성을 향상시킵니다.
...
오프라인 방식: 모든 컨슈머를 종료한 뒤 새 설정으로 재시작합니다. 그룹이 빌 때 자동으로 프로토콜이 전환됩니다.

온라인 방식: 롤링 배포로 무중단 전환이 가능합니다. 모든 컨슈머가 업데이트(또는 다운그레이드)될 때까지 그룹은 두 프로토콜을 상호 운용합니다.
...
Apache Kafka 4.0은 트랜잭션 프로토콜을 강화하는 트랜잭션 서버 측 방어(KIP-890)를 도입했습니다. 핵심 개선 사항은 "프로듀서 에포크가 모든 트랜잭션마다 증가하여 각 트랜잭션이 의도한 메시지만 포함하고 중복 메시지가 다음 트랜잭션에 포함되지 않도록 보장"한다는 점입니다.

이 프로토콜은 `transaction.version` 기능 플래그를 통해 기본적으로 활성화되며, 클러스터 생성 시 또는 기능 도구를 통해 동적으로 구성할 수 있습니다.

#### 11.2 업그레이드 및 다운그레이드

새 프로토콜 활성화: 서버에서 `transaction.version=2`를 설정합니다. 프로듀서 클라이언트(4.0+)는 재시작 없이 다음 연결 시 자동으로 향상된 프로토콜을 채택합니다. 트랜잭션 진행 중에는 업그레이드되지 않으며 다음 트랜잭션 시작 시 전환됩니다.

다운그레이드: 업그레이드와 동일한 방식으로 안전하게 진행됩니다. 서버 측 다운그레이드를 인식한 클라이언트는 첫 번째 트랜잭션부터 이전 프로토콜로 되돌아갑니다.

#### 11.3 성능 영향

재설계된 프로토콜은 파티션 추가를 단일 서버 측 작업으로 통합하여 오버헤드를 줄이고 기존의 클라이언트-검증 왕복을 제거합니다. 다만 트레이드오프가 있습니다.

기존에는 클라이언트가 `CONCURRENT_TRANSACTIONS` 오류에 대해 하드코딩된 20ms 재시도 백오프를 사용했습니다. 이제 서버가 재시도를 내부적으로 처리하므로 보고되는 프로듀서 지연 시간이 증가할 수 있습니다. 그러나 종단 간 트랜잭션 지연 시간(커밋까지의 시간)은 변하지 않습니다. 지연이 클라이언트 측 백오프에서 서버 측 처리로 이동했을 뿐입니다.
...
ELR(Eligible Leader Replicas)은 Apache Kafka 4.0에 도입되었으며(4.1에서 기본 활성화) 리더로 승격하기에 안전한 레플리카를 식별하여 복제 신뢰성을 높입니다. 이 기능은 ISR이 `min.insync.replicas` 미만으로 떨어질 때 하이 워터마크 진행을 차단하는 Kafka의 "엄격한 최소 ISR" 규칙을 활용합니다.
...
ELR 정보는 관리 클라이언트의 `DescribeTopicPartitions` API를 통해 조회할 수 있습니다.
...
KRaft는 Apache Kafka의 메타데이터 관리 시스템으로, ZooKeeper를 대체합니다. KRaft의 설정, 업그레이드, 노드 프로비저닝, 운영 방법 및 ZooKeeper에서 KRaft로의 마이그레이션 절차를 다룹니다.
...
> 참고: Combined 서버는 개발 환경 등 소규모 환경에서는 운영이 단순하지만, 프로덕션 환경에서는 권장하지 않습니다.
...
브로커를 버전 3.9.1 이상으로 업그레이드하고 `inter.broker.protocol.version`을 `3.9`로 설정합니다.
...
ZK 브로커를 모두 재시작하면 마이그레이션이 자동으로 시작됩니다.
...
각 브로커의 설정을 아래와 같이 변경한 후 재시작합니다:
