...
모든 데이터 쓰기는 디스크로 플러시하지 않아도 즉시 영구 로그에 기록됩니다. 커널의 페이지캐시가 이 전송을 처리합니다.

#### 상수 시간으로 충분합니다

전통적인 메시징 시스템은 메타데이터 관리를 위해 BTree 또는 유사한 구조의 컨슈머별 큐를 사용하므로 O(log N) 연산이 필요합니다. 디스크 탐색(seek)은 각각 약 10밀리초가 소요되며, 고정 캐시에서 데이터가 증가할수록 성능 저하가 초선형적으로 발생합니다.
...
버스트성 랜덤 메시지 쓰기가 컨슈머 방향의 선형 흐름으로 변환됩니다.
...
클라이언트는 랜덤 로드 밸런싱 또는 시맨틱 파티셔닝 함수를 통해 파티션 발행을 제어합니다. 파티션 키를 지정하면 해당 키의 모든 데이터가 동일한 파티션으로 전달되어 컨슈머의 지역성 민감(locality-sensitive) 처리가 가능합니다.
...
Kafka 컨슈머는 파티션 리더 브로커에 fetch 요청을 보내면서 로그 오프셋을 지정합니다. 브로커는 해당 위치부터 로그 청크를 반환하므로 컨슈머가 소비 위치를 직접 제어할 수 있고 재처리를 위한 데이터 되감기도 가능합니다.

#### Push vs. Pull

Kafka는 프로듀서가 브로커에 푸시하고 컨슈머가 브로커에서 풀하는 전통적인 풀 모델을 따릅니다. 푸시 기반 시스템은 브로커가 전송 속도를 제어하기 때문에 다양한 컨슈머를 지원하기 어렵습니다. 소비 속도가 생산 속도보다 느려지면 컨슈머가 감당할 수 없게 되어 사실상 서비스 거부 상태가 됩니다.
...
소비 추적은 핵심 성능 요소입니다. 대부분의 시스템은 소비 메타데이터를 브로커에 즉시 또는 컨슈머 확인 시점에 저장합니다. 그러나 소비 상태에 대한 브로커-컨슈머 간 합의는 간단하지 않습니다.

소비 즉시 기록 방식은 컨슈머가 전달 후 실패할 경우 처리되지 않은 메시지가 건너뛰어질 위험이 있습니다. 확인 기반 방식은 손실을 방지하지만 복잡성이 증가합니다. 컨슈머가 메시지를 처리한 후 확인 전에 충돌하면 복구 시 중복 소비가 발생합니다.
...
리밸런스 프로토콜은 그룹 코디네이터가 멤버에게 임시 엔티티 ID를 할당하는 방식에 의존합니다. 멤버가 재시작하여 다시 참여하면 ID가 바뀌어 리밸런싱이 트리거됩니다. 상태 저장 애플리케이션에서는 이로 인해 태스크 할당이 뒤섞여 긴 상태 복구 기간과 부분적 비가용성이 발생합니다.
...
이는 발행 및 소비 보장 문제로 나뉩니다. "정확히 한 번"을 주장하는 시스템은 실제 동작이 실패 시나리오, 다중 컨슈머 프로세스, 디스크 손실 가능성에 따라 달라지므로 주의 깊게 살펴봐야 합니다.
...
0.11.0.0 이후, 멱등성(idempotent) 프로듀서 전달은 재전송이 중복을 만들지 않음을 보장합니다. 브로커는 프로듀서 ID를 할당하고 시퀀스 번호를 사용하여 중복을 제거합니다. 프로듀서는 원자적 다중 파티션 트랜잭션도 지원하여 모든 메시지가 성공하거나 모두 실패하도록 보장합니다.
...
중단된 트랜잭션은 컨슈머 위치를 되돌립니다(컨슈머는 자동으로 되감기보다 커밋된 오프셋을 다시 가져와야 합니다). 처리된 데이터는 "read_committed" 격리 수준을 사용하는 다른 컨슈머에게 보이지 않습니다(기본값인 "read_uncommitted"는 중단된 트랜잭션 데이터를 포함한 모든 메시지를 표시합니다).
...
Kafka 트랜잭션은 다른 메시징 시스템과 다릅니다. 컨슈머와 프로듀서가 분리되어 있으며 프로듀서만 트랜잭션을 수행합니다. 다만 프로듀서는 전체적인 정확히 한 번 동작을 위해 트랜잭션으로 컨슈머 위치를 업데이트할 수 있습니다.
...
컨슈머는 구독할 토픽을 동적으로 설정합니다. 페칭 시 컨슈머는 일치하는 토픽-파티션에서 레코드를 수신하며, 레코드에는 시간 제한된 획득 잠금(기본 30초, 그룹 설정 `share.record.lock.duration.ms`로 제어)이 걸립니다. 잠긴 레코드는 다른 컨슈머에서 사용할 수 없습니다.
...
브로커는 `group.share.partition.max.record.locks`를 통해 공유 그룹의 토픽-파티션별 획득 레코드 수를 제한합니다. 이 제한에 도달하면 획득된 레코드가 타임아웃으로 줄어들 때까지 페칭이 일시적으로 중단됩니다.

---

### 복제(Replication)

Kafka는 설정 가능한 서버 수에 걸쳐 토픽 파티션 로그를 복제하며, 토픽마다 복제 팩터를 지정할 수 있습니다. 이를 통해 서버 장애 시 복제본으로 자동 페일오버가 이루어지면서 메시지 가용성을 유지합니다.

복제를 선택 사항으로 취급하는 일부 시스템과 달리 Kafka는 복제를 기본값으로 구현합니다. 단일 복제본 토픽도 복제 팩터가 1인 복제된 토픽입니다.

복제 단위는 토픽 파티션입니다. 정상 조건에서 각 파티션은 하나의 리더와 0개 이상의 팔로워를 가집니다. 리더를 포함한 모든 복제본이 복제 팩터를 구성합니다. 모든 쓰기는 파티션 리더로 전달되고, 읽기는 리더 또는 팔로워에서 처리됩니다. 파티션 수는 일반적으로 브로커 수보다 많으며 리더는 고르게 분산됩니다. 팔로워 로그는 리더 로그와 동일한 오프셋, 메시지, 순서를 가집니다(리더가 일시적으로 아직 복제되지 않은 메시지를 가질 수 있습니다).
...
두 조건을 모두 충족하는 노드는 "동기화 중(in sync)"으로 간주되며 리더가 동기화 복제본(ISR) 세트로 추적합니다. ISR 세트는 클러스터 메타데이터에 유지되며 변경 시 업데이트됩니다. 조건을 충족하지 못하면 해당 브로커가 ISR에서 제거됩니다. 죽은 팔로워는 세션 만료로 제거되고, 뒤처진 팔로워는 `replica.lag.time.max.ms`로 결정되는 복제 지연 초과 시 제거됩니다.
...
리더 선출 효율성은 비가용성 시간을 결정하므로 매우 중요합니다. 단순한 구현이라면 장애 노드가 보유한 파티션마다 개별 선출을 실행합니다. 하지만 "컨트롤러" 노드가 브로커 등록과 장애 감지를 관리하며 남은 ISR 멤버를 리더로 선출합니다. 리더십 알림을 배치 처리하여 파티션이 많아도 선출이 빠르고 효율적으로 이루어집니다. 컨트롤러 장애 시에는 새 컨트롤러 선출이 트리거됩니다.
...
이러한 사례는 실시간 변경 피드가 필요하지만, 머신 충돌이나 데이터 리로드, 재처리 시 전체 데이터 로드가 필요하기도 합니다.
...
이 브로커 설정은 `server.properties`에서 재정의하지 않으면 클러스터의 모든 토픽에 적용됩니다. 클리너는 압축 시간 지연을 통해 최소한의 압축되지 않은 "헤드" 양을 유지할 수 있습니다:
...
대용량을 생산·소비하거나 높은 속도로 요청을 보내는 프로듀서·컨슈머는 브로커 리소스를 독점하고 네트워크를 포화시켜 다른 클라이언트와 브로커에 DoS 공격 수준의 영향을 줄 수 있습니다. 쿼터는 이러한 문제로부터 보호합니다. 이는 멀티테넌트 클러스터에서 잘못 동작하는 클라이언트 세트가 잘 동작하는 사용자 경험을 저하시키는 것을 방지하는 데 중요합니다. 서비스 운영 Kafka는 계약에 따른 API 제한을 적용합니다.

#### 클라이언트 그룹

Kafka 클라이언트의 사용자는 보안 클러스터에서 인증된 사용자 주체(principal)입니다. 인증되지 않은 클러스터는 설정 가능한 `PrincipalBuilder`를 통해 사용자를 그룹화합니다. client-id는 의미 있는 애플리케이션 이름을 사용하는 논리적 클라이언트 그룹화입니다. (user, client-id) 튜플은 주체와 ID를 모두 공유하는 보안 클라이언트 그룹을 정의합니다.
...
쿼터 위반 시 브로커는 클라이언트가 쿼터 이하로 돌아오는 데 필요한 지연 시간을 계산하여 즉시 응답을 반환합니다. 이때 페치 응답에는 데이터가 포함되지 않습니다. 브로커는 클라이언트 채널을 음소거하여 지연이 경과할 때까지 추가 요청 처리를 막습니다. 응답을 수신한 Kafka 클라이언트도 지연 동안 추가 요청을 보내지 않습니다. 즉, 스로틀된 클라이언트 요청은 양측 모두에서 차단됩니다.
