...
Vector는 다양한 운영 체제와 플랫폼에서 쉽게 설치하고 운영할 수 있도록 설계되었습니다.
...
다양한 환경에서 Vector 인스턴스를 시작, 중지, 재시작, 리로드하는 방법을 설명합니다.
...
MSI로 Windows에 Vector를 설치한 경우 다음 명령을 사용할 수 있습니다.
...
프로세스 관리자 없이 Vector 실행 파일을 직접 관리합니다.
...
`--watch-config` (또는 `-w`) 플래그를 지정하면 설정 파일이 변경될 때 Vector가 자동으로 리로드됩니다.
...
SIGINT 또는 SIGTERM 수신 후 정상 종료를 기다리는 최대 시간(초)입니다. 이 시간이 지나면 Vector는 강제 종료됩니다.
...
`data_dir`은 Vector 상태 데이터(디스크 버퍼, 파일 체크포인트 등)를 저장하는 디렉토리입니다.
...
Vector는 관찰 가능성 데이터를 처리하는 도구이면서 자체적으로도 높은 관찰 가능성을 제공합니다. `internal_logs`와 `internal_metrics` 두 소스를 통해 Vector가 생성하는 로그와 메트릭을 일반 소스 데이터처럼 처리할 수 있습니다.
...
`internal_logs` 소스로 전달되는 로그는 로그 레벨에 따라 결정됩니다. 기본값은 `info`입니다.
...
여러 Vector 인스턴스의 내부 메트릭을 같은 목적지로 보낼 때는 메트릭 시리즈 충돌을 방지하기 위해 고유한 태그를 추가해야 합니다:
...
핫 패스 로그 속도 제한:
Vector는 핫 패스의 로그 이벤트에 속도 제한을 적용하여, I/O 포화나 서비스 장애 위험 없이 세밀한 인사이트를 얻을 수 있습니다.

VRL에서의 로깅:
VRL의 `log` 함수는 기본적으로 속도 제한을 구현합니다. 이를 통해 VRL 프로그램이 `log`를 호출할 때 실수로 I/O를 포화시키는 상황을 방지합니다.

---

### 3. 검증 (Validating)

Vector는 설정 유효성을 검사하는 `validate` 서브커맨드를 제공합니다. 검증 성공 시 종료 코드 0, 실패 시 종료 코드 78로 종료합니다.
...
`validate` 서브커맨드는 지정된 설정에 대해 다음 검사를 수행합니다:
...
4. 환경 검사: Vector가 설정된 토폴로지를 지원할 수 있는 환경에서 실행되고 있는지 확인
   - 모든 컴포넌트가 실행에 필요한 사전 조건을 갖추고 있는지 확인 (예: 데이터 디렉토리 존재 및 쓰기 가능 여부)
...
헬스체크 엔드포인트에 접근할 수 없는 환경(예: 로컬 워크스테이션)에서 다른 환경 검사는 유지하면서 헬스체크만 건너뛰려면 `--skip-healthchecks` 플래그를 사용합니다:
...
Vector는 Rust로 작성되어 런타임이나 가상 머신을 포함하지 않습니다. 별도의 조정 없이도 기본적으로 모든 시스템 리소스를 최대한 활용합니다.

#### 4.1 Profile-Guided Optimization (PGO)

PGO(Profile-Guided Optimization)는 런타임 프로파일을 기반으로 프로그램을 최적화하는 컴파일러 기법입니다.

##### 성능 향상

테스트 결과, 일부 Vector 워크로드에서 초당 처리 로그 이벤트가 최대 15% 향상될 수 있습니다.
...
Vector는 백프레셔와 데이터 손실을 관리하는 버퍼링 모델을 사용합니다.
...
Vector는 적응형 동시성(Adaptive Concurrency)을 사용하여 다운스트림 서비스 상태에 따라 동시성을 자동으로 조정합니다.

---

### 5. GraphQL API

Vector는 GraphQL API를 통해 자체 관찰 가능성을 제공합니다. 이 API는 v0.11.0에 도입되었습니다.
...
API가 활성화되면 브라우저에서 GraphQL Playground에 접속할 수 있습니다:
...
컴포넌트 메트릭과 토폴로지 정보를 htop과 유사한 대시보드 스타일로 표시합니다:
...
파이프라인 컴포넌트를 흐르는 이벤트를 실시간으로 관찰합니다:
...
설정에 정의된 토폴로지를 DOT 형식의 그래프로 출력합니다:
...
Vector는 처리 파이프라인의 변환(transform)을 단위 테스트할 수 있습니다.

#### 6.1 기본 개념

- 변환에 입력 세트를 제공합니다.
- VRL 어설션으로 출력이 예상과 일치하는지 확인합니다.
...
Boolean 표현식을 첫 번째 인수로 받습니다. 표현식이 false이면 테스트가 실패합니다:
...
권장사항: 단위 테스트에서 `assert`와 `assert_eq`는 bang(`!`) 구문을 사용하여 인폴리블(infallible)하게 작성하세요. 조건이 실패하면 VRL 프로그램이 즉시 중단됩니다.
...
Vector가 1.0 미만인 동안에는 마이너 버전마다 호환되지 않는 변경 사항이 포함될 수 있으므로 마이너 버전을 순차적으로 업그레이드하는 것이 좋습니다. 구체적인 변경 사항은 각 업그레이드 가이드에 명시되어 있습니다.
...
Vector는 에이전트로 배포할 수 있을 만큼 효율적이면서 동시에 애그리게이터로 배포할 수 있을 만큼 강력합니다.
...
에이전트 역할은 단일 호스트의 모든 데이터를 수집하도록 설계되었습니다. 에이전트는 데몬(Daemon)과 사이드카(Sidecar) 두 가지 방식으로 배포할 수 있습니다.
...
사이드카 역할은 Vector를 개별 서비스에 결합하여 단일 서비스의 데이터를 수집합니다. 데몬 방식보다 효율성은 낮지만 관리가 더 단순할 수 있으며, 수집 책임을 서비스 소유자에게 위임할 수 있습니다.
...
애그리게이터 역할은 중앙 처리에 적합하도록 설계되었으며, 여러 업스트림 소스로부터 데이터를 수집하고 호스트 간 집계 및 분석을 수행합니다.
...
토폴로지는 Vector 인스턴스들이 어떻게 연결되는지, 그리고 데이터가 어떻게 흐르는지를 정의합니다. Vector는 세 가지 주요 배포 토폴로지를 지원합니다.
...
중앙 집중형 토폴로지는 분산형과 스트림 기반 사이의 균형점으로, 많은 사용 사례에서 단순성·안정성·제어 측면의 적절한 절충안이 됩니다.

이 방식에서는 Vector가 에이전트와 애그리게이터 역할을 모두 수행합니다. 클라이언트 노드의 에이전트가 중앙 Vector 서비스로 데이터를 전달하고, 중앙 서비스가 다운스트림 통신을 담당합니다.
...
스트림 기반 토폴로지는 내구성과 복원력이 가장 뛰어난 토폴로지로, Kafka 같은 스트림 서비스 운영 경험이 있는 팀이 대규모 데이터 스트림에 활용합니다.
...
적합한 배포 전략은 조직의 요구 사항, 규모, 팀 역량에 따라 달라집니다.
...
실제 환경에서는 여러 토폴로지를 조합해 사용할 수 있습니다.
...
- 기존 에이전트를 변경하지 않고도 복잡한 환경에 쉽게 통합할 수 있음
- 기업 및 대규모 조직에 특히 적합
...
- vector source 및 sink 컴포넌트를 통해 Vector 네이티브 프로토콜을 사용하여 성능 향상
...
이 섹션은 데이터, 프로세스, 호스트, 네트워크 계층 전반에 걸쳐 심층 방어 전략을 적용한 Vector 배포를 위한 보안 가이드를 제공합니다.
...
데이터 수신 장애:
해결책: "Vector의 엔드투엔드 승인 기능 활성화." 승인이 전송되기 전에 데이터를 지속합니다.
