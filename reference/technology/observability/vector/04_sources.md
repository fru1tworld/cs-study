...
| `glob_minimum_cooldown_ms` | int | 1000 | 파일 탐색 호출 간격(밀리초) |
...
Vector는 파일 읽기 위치를 추적하는 체크포인트 파일을 유지합니다. 덕분에 Vector가 재시작되어도 마지막으로 읽은 위치부터 이어서 처리할 수 있습니다.
...
- exclude 우선: 컨테이너가 include와 exclude 모두 일치하면 제외됩니다.
- 접두사 매칭: 옵션들은 접두사 매칭을 사용합니다. `foo`는 `foo`, `foobar`, `foo-123` 등과 일치합니다.
- 기본 동작: 별도 설정이 없으면 Vector는 모든 컨테이너에서 로그를 수집합니다.
...
리모트 라이트 프로토콜은 메트릭 태그, 타임스탬프, 숫자 값만 전송하며, 원래 메트릭 타입(카운터, 히스토그램 등)에 대한 명시적 정보는 포함되지 않습니다. 따라서 이 소스는 다음 규칙으로 메트릭 타입을 추론합니다:
...
acknowledgements를 활성화하면 소스가 Splunk HEC 인덱서 승인 프로토콜을 사용하며, 클라이언트는 데이터가 대상 싱크에 전달되었는지 확인할 수 있습니다.
...
AWS Kinesis Firehose는 HTTP를 통해서만 데이터를 전달합니다. TLS 종단 처리는 로드 밸런서를 앞단에 두거나 TLS 옵션을 직접 구성하여 해결해야 합니다.
...
`gcp_pubsub` 소스는 스트림을 통과하는 트래픽을 모니터링하여 동시 활성 스트림 수를 자동으로 관리합니다. 스트림이 전체 응답(`full_response_size` 기준)을 수신하면 "busy" 상태로 표시됩니다. 소스는 주기적으로 모든 활성 연결을 폴링하며, 모든 활성 스트림이 busy 상태이고 활성 스트림 수가 `max_concurrency`보다 적으면 새 스트림을 추가로 시작합니다.
...
인프라에 이미 fluent 에이전트(Fluentd 또는 Fluent Bit)를 운용 중이라면 이 소스를 통해 해당 데이터를 간편하게 Vector로 수집할 수 있습니다.
...
Logstash에서 Vector 인스턴스로 데이터를 전달하려면 lumberjack 출력 플러그인을 구성합니다. Logstash 측에서 SSL 설정이 필요합니다.
...
원시 바이트 스트림에서 이벤트 경계를 구분하는 방식을 구성합니다:
