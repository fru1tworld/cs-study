...
풍부한 컨텍스트 데이터를 가진 "이벤트 전용 세계(event-only world)"가 이상적이지만, 현실의 서비스들은 다양한 품질의 로그와 메트릭을 생성합니다. Vector는 이러한 기존 표준을 수용하도록 설계되어, 레거시 시스템과 새로운 옵저버빌리티 접근 방식 사이의 가교 역할을 합니다.

이 데이터 모델은 시스템 간 상호운용성을 실현합니다. 예를 들어, 적절한 내부 메트릭 데이터 구조 없이는 StatsD 소스와 Prometheus 싱크를 결합한 파이프라인을 구성할 수 없습니다. 이 분류 체계 덕분에 Vector는 서로 다른 옵저버빌리티 시스템 간 변환을 정확하게 수행할 수 있습니다.

---

## 로그 이벤트 (Log Events)

Vector의 로그 이벤트는 특정 시점에 발생한 사건을 구조화된 데이터로 표현합니다. 핵심 원칙은 스키마 중립성(schema neutrality)으로, Vector는 특정 필드를 강제하지 않아 다양하고 변화하는 데이터 스키마와의 호환성을 보장합니다.

### 스키마

Vector는 스키마 중립적이며 특정 스키마를 요구하지 않습니다. 이 유연성 덕분에 레거시 형식과 앞으로 등장할 데이터 형식 모두를 수정 없이 지원할 수 있습니다.
...
| 타입 | 설명 |
|------|------|
| Strings (문자열) | UTF-8로 인코딩되며, 크기는 시스템 메모리에 의해서만 제한됩니다. |
| Integers (정수) | 부호 있는 64비트 정수 값입니다. |
| Floats (부동소수점) | IEEE 754 64비트 표준을 따릅니다. |
| Booleans (불리언) | 이진 true/false 값입니다. |
| Timestamps (타임스탬프) | Rust `DateTime` 구조체로 UTC 값으로 저장됩니다. 타임존 정보가 없으면 Vector는 로컬 시간으로 간주하고 자동으로 UTC로 변환합니다. |
...
JSON처럼 공식적인 타임스탬프 정의가 없는 형식의 경우, Vector는 타임스탬프를 원시 형태로 수집한 뒤 `remap` 변환과 VRL의 `parse_timestamp` 함수를 사용해 타입 변환을 수행합니다.
...
Vector는 메트릭을 구조화된 로그가 아닌 일급 시민(first-class citizen)으로 취급합니다. Vector의 메트릭 데이터 모델은 Prometheus, StatsD 등 다양한 시스템의 메트릭 타입을 통합하여 시스템 간 정확한 상호운용성을 보장합니다.
...
증가하거나 리셋될 수 있지만 감소할 수 없는 값입니다. 증분 값은 양수 float이어야 합니다.
...
특정 시점의 값을 나타내며 위아래로 자유롭게 변동합니다. 메모리 사용량, CPU 사용률처럼 현재 상태를 추적하는 메트릭에 적합합니다.
...
타이머라고도 불리며, 요청 지속 시간과 같은 관측값을 샘플링해 설정 가능한 버킷으로 분류합니다. 전체 카운트와 합계를 함께 제공합니다.
...
글로벌 히스토그램과 요약을 지원하는 서비스를 위한 샘플링된 값의 분포를 나타냅니다. 샘플 값과 통계 타입(histogram 또는 summary)이 모두 필요합니다.
...
히스토그램과 유사하지만 슬라이딩 시간 윈도우 기준으로 설정 가능한 분위수(quantile)를 계산합니다. 관측 카운트, 합계, 분위수 값을 포함합니다.
...
| 종류 | 설명 |
|------|------|
| incremental | 이전 값 대비 변화량을 나타냅니다. 시간에 따라 누적 합산됩니다. |
| absolute | 특정 시점의 절댓값을 나타냅니다. 합산하지 않고 그대로 사용됩니다. |
...
2. Transforms (변환): 이벤트를 파싱, 필터링, 집계, 변환합니다.
...
Vector의 `remap` 변환과 VRL을 사용해 이벤트 타입 간 변환을 수행할 수 있습니다.

### 로그를 메트릭으로 변환

`log_to_metric` 변환을 사용해 로그 이벤트에서 메트릭을 생성할 수 있습니다:
...
`metric_to_log` 변환을 사용해 메트릭 이벤트를 로그로 변환할 수 있습니다:
