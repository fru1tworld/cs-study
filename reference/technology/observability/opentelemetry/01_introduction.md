...
OpenTelemetry(OTel) = 분산 시스템의 텔레메트리(Telemetry) 데이터를 생성·수집·내보내기 위한 벤더 중립적 표준. CNCF 인큐베이팅을 거쳐 졸업(graduated)한 프로젝트 → Kubernetes 다음으로 활발한 CNCF 프로젝트.

핵심 정의:
- API + SDK + Protocol + Collector의 통합 패키지
- 백엔드(Jaeger, Tempo, Prometheus, Datadog 등)에 종속되지 않는 계측 코드
- Traces, Metrics, Logs를 일관된 모델로 다룸 (최근 Profiles 추가)

OpenTelemetry는 "관측 가능성(Observability)을 위한 USB-C"에 비유 — 어떤 백엔드를 꽂아도 동작하는 표준 인터페이스.
...
백엔드를 바꾸면 모든 애플리케이션 코드의 계측을 다시 작성해야 하는 문제 발생.

### 2. 신호 분리 문제

Trace, Metric, Log가 서로 다른 라이브러리·포맷·전송 채널을 사용 → 상관관계 추적 어려움. 같은 요청의 trace와 log를 연결하려면 trace_id를 수동으로 주입 필요.
...
OpenTelemetry는 Semantic Conventions로 이를 표준화.
...
네 번째 시그널 Profiles가 실험적으로 도입. CPU/메모리 프로파일링 데이터를 표준 형식으로 다룸 (eBPF·pprof와 연계).

---

## 구성 요소

OpenTelemetry는 다음 레이어로 구성:
...
벤더 중립적 텔레메트리 파이프라인, 별도의 프로세스 또는 사이드카로 동작.
...
OpenTelemetry Protocol. 텔레메트리 시그널을 전송하는 표준 와이어 프로토콜.
...
각 단계는 선택적. 가장 단순한 구성은 SDK가 직접 백엔드로 OTLP를 보내는 방식, 가장 복잡한 구성은 Agent Collector → Gateway Collector → 여러 백엔드로 분기하는 방식.
...
OTLP 외에도 Collector가 다음을 수신 가능 → 점진적 마이그레이션 가능:
- Jaeger (Thrift, gRPC)
- Zipkin
...
OpenTelemetry는 다루는 범위가 넓음 → 단계적 접근이 효율적.
