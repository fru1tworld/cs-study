...
클라이언트는 **요청 스트림**(클라이언트 시작 양방향 QUIC 스트림)에서 HTTP 요청을 전송하며, **스트림당 단일 요청**을 사용합니다.
...
HTTP/3는 HPACK 대신 **QPACK 압축**을 사용합니다:
...
HTTP/3에서 CONNECT 메서드는 전체 연결 대신 **개별 스트림**에 터널을 설정합니다.
...
HTTP/3는 HTTP의 시맨틱을 유지하면서 **QUIC의 장점을 활용하여 성능과 보안을 획기적으로 개선**합니다.
