...
**Infra Standard**는 WHATWG에서 관리하는 Living Standard로, 다른 모든 웹 표준 문서들이 공유하는 **공통 기반 정의(common infrastructure)**를 제공한다. 이 표준은 웹 API를 직접 정의하지 않지만, HTML Standard, DOM Standard, Fetch Standard, URL Standard 등 다른 표준들이 사용하는 기본 개념, 자료 구조, 알고리즘 표기법을 정의한다.
