...
파싱의 시작 상태이다. 첫 번째 문자가 ASCII 영문자이면 소문자로 변환하고 버퍼에 추가한 후 **Scheme State**로 전이한다.
...
첫 문자가 영문자가 아니고, state override가 없으면 **No Scheme State**로 전이한다.
...
- 버퍼에 모인 문자열이 스킴이 된다
- 특수 스킴인지 확인한다
- 이후 `//`가 오면 **Authority State**로, 그렇지 않으면 다른 적절한 상태로 전이한다
...
`?` 이후의 쿼리 문자열을 파싱한다. `#`을 만나면 쿼리 파싱 종료 후 **Fragment State**로 전이한다.
