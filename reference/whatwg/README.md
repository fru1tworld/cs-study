...
> **"Priority of Constituencies"** 는 WHATWG의 가장 중요한 원칙이다. 의사결정에서 충돌이 발생할 때, **최종 사용자의 이익**이 다른 모든 이해관계자보다 우선한다.
...
2000년대 초반, W3C는 웹의 미래를 **XHTML 2.0**에 걸었다. XHTML 2.0은 다음과 같은 특징을 가졌다:
...
- **Opera의 Ian Hickson**과 **Mozilla의 Brendan Eich** 등이 "기존 HTML을 점진적으로 확장하여 웹 애플리케이션 기능을 추가하자"는 제안을 발표
- **W3C는 이를 투표로 부결** (찬성 8, 반대 14)
- W3C는 XHTML 2.0 노선을 고수하기로 결정

이 결정에 실망한 Apple, Mozilla, Opera의 개발자들이 독자적으로 **WHATWG를 결성**했다.
...
**Ian "Hixie" Hickson**은 WHATWG의 핵심 인물이다:

- Opera에서 근무하다가 이후 Google로 이적
- WHATWG 설립 시부터 **HTML 스펙의 단독 에디터**로 활동
...
WHATWG에 기여하려면 **Participant Agreement**에 서명해야 한다:
...
**Living Standard**는 WHATWG가 채택한 표준 문서 관리 모델이다. 전통적인 버전 기반 표준(W3C Recommendation Track)과 근본적으로 다른 접근이다.
...
Living Standard 모델에서도 **특허 정책(Patent Policy)** 을 위해 주기적으로 **Review Draft**를 발행한다:
