...
브라우저가 HTML 문자열을 DOM 트리로 변환하는 과정은 **토크나이저**와 **트리 구성** 두 단계로 나뉜다.
...
토크나이저는 **80개 이상의 상태**를 가지며, 각 문자를 읽을 때마다 상태가 전이된다.
...
`<table>`, `<tbody>`, `<tr>`, `<td>` 내부에 허용되지 않는 노드가 등장하면, 해당 노드를 테이블의 **부모 요소 앞**으로 이동시킨다. 이를 **foster parenting**이라 한다.
...
동일 출처(origin) 단위로 키-값 데이터를 저장한다. 값은 항상 **문자열**이다.
...
**같은 출처의 다른 탭**에서 `localStorage`가 변경될 때 발생한다.
...
메인 스레드와 분리된 **백그라운드 스레드**에서 스크립트를 실행한다. DOM에 접근할 수 없다.
...
데이터를 복사 대신 **소유권을 이전**하여 성능을 높인다. 전송 후 원본은 사용 불가가 된다.
...
**같은 출처**의 모든 탭/창/iframe/워커 간 간편한 메시지 브로드캐스트를 제공한다.
...
서버에서 클라이언트로의 **단방향 실시간 스트림**이다. HTTP 기반이며 자동 재연결을 지원한다.
...
HTML에 기계가 읽을 수 있는 **구조화된 데이터**를 내장하는 메커니즘이다. 주로 **Schema.org** 어휘와 함께 사용한다.
...
> 검색엔진은 Microdata, JSON-LD, RDFa 중 **JSON-LD**를 선호하는 추세이지만, Microdata는 HTML과 콘텐츠가 직접 연결되어 동기화 문제가 적다는 장점이 있다.
