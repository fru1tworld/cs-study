...
설정 파일의 위치는 설치 방법에 따라 다릅니다. 대부분의 Linux 기반 시스템에서는 다음 위치에 설정 파일이 있습니다:
...
Vector는 워크플로우에 맞게 YAML, TOML, JSON 형식을 지원합니다. 파일 확장자를 기반으로 형식을 자동으로 감지합니다.
...
YAML이 기본 설정 형식입니다. Vector 팀은 TOML 설정이 컴포넌트 수가 늘어날수록 가독성이 급격히 떨어진다는 점을 발견했습니다. 문서와 CLI 기본값을 YAML로 전환함으로써 사용자가 TOML의 한계에 도달한 후 마이그레이션하는 것이 아니라 처음부터 YAML을 사용하도록 유도합니다.

기존 TOML 및 JSON 설정은 이 결정에 영향을 받지 않으며 평소와 같이 작동합니다.

### 형식 변환

Vector는 TOML/JSON 형식의 설정을 YAML로 변환할 때 출발점으로 활용할 수 있는 `vector convert-config` 명령을 제공합니다.

```bash
vector convert-config --input vector.toml --output vector.yaml
```

참고: 이 명령은 최선을 다하지만 다음과 같은 제약이 있습니다:
- 주석을 보존하지 않습니다.
- 기본값과 동일한 경우 해당 값을 명시적으로 출력하지 않을 수 있습니다.
- 변환된 설정을 반드시 검토하고 필요에 따라 수정하십시오.
...
Remap 변환은 Vector의 관찰 데이터 처리 능력을 강화하는 핵심 요소입니다. Vector Remap Language(VRL)라는 간결한 언어를 제공하여 파이프라인을 통과하는 이벤트 데이터를 파싱, 조작, 보강할 수 있습니다.
...
이 값은 내부 메트릭 스크레이프 간격(기본값 5분)보다 크게 설정하여 메트릭이 충분히 오래 유지되어 수집 및 캡처될 수 있도록 합니다.
...
싱크에서 활성화되면, 해당 싱크에 연결된 소스 중 종단 간 확인을 지원하는 소스는 연결된 모든 싱크에서 이벤트가 확인될 때까지 소스 측 확인을 보류합니다.

### require_healthy

시작 시 싱크가 정상 상태로 보고되어야 하는지 여부를 설정합니다. 활성화 상태에서 싱크가 비정상으로 보고되면 Vector는 시작 중에 종료됩니다.
...
환경 변수를 리터럴 문자열로 처리하려면 `$`를 이중으로 작성하여 이스케이프할 수 있습니다:
...
Vector는 개행 문자가 포함된 환경 변수를 거부하여 보간 관련 보안 취약점을 차단합니다. 이를 통해 다중 행 설정 블록 주입도 방지됩니다.

### 엄격 모드

CLI는 환경 변수 보간에 대한 엄격 모드를 제공합니다. 활성화되면 설정 파일에 누락된 환경 변수가 있을 때 경고 대신 오류를 발생시켜 해당 설정 파일 로드를 실패시킵니다. 이 옵션은 deprecated 상태이며, 향후 버전에서 누락된 환경 변수를 경고로 처리하는 동작을 제거하면서 함께 삭제될 예정입니다. 기본값은 true입니다.

### 설정 위치를 지정하는 환경 변수

Vector는 설정 파일 경로를 지정하기 위해 다음 환경 변수를 지원합니다:
- `VECTOR_CONFIG`
- `VECTOR_CONFIG_YAML`
- `VECTOR_CONFIG_JSON`
- `VECTOR_CONFIG_TOML`
...
`--config-dir`로 설정 디렉토리를 지정하면, Vector는 컴포넌트 유형(예: transforms, sources)에 해당하는 하위 디렉토리를 자동으로 탐색하고 해당 디렉토리의 파일이 그 유형의 컴포넌트를 정의한다고 유추합니다. 설정 파일의 파일 이름이 컴포넌트 ID로 사용됩니다.
...
Vector가 디렉토리 구조에서 컴포넌트 유형과 ID를 자동으로 유추하므로 설정 내에 별도로 지정해야 할 항목이 줄어듭니다.
...
유효성 검사 모드를 "relaxed"로 설정하면 일치하는 입력이 없는 와일드카드가 포함된 설정도 오류 없이 허용됩니다.
...
Vector가 실행 중인 상태에서 설정을 변경한 경우, SIGHUP 신호를 보내 설정을 다시 로드하도록 할 수 있습니다:

```bash
kill -SIGHUP $(pidof vector)
```

SIGHUP 신호를 보낼 수 없는 환경에서는 `--watch-config` 플래그를 사용하면 파일이 변경될 때마다 자동으로 설정을 다시 로드합니다.
...
설정 변경을 폴링할 간격을 지정할 수 있습니다. `--watch-config-method`가 `poll`로 설정된 경우에만 적용됩니다. 기본값은 60초입니다.
...
`--watch-config` 플래그는 풍부화 테이블 파일의 변경 사항도 함께 감시합니다. HTTP 싱크의 TLS `crt_file` 및 `key_file`도 `--watch-config` 활성화 시 감시 대상에 포함되므로, 해당 파일이 변경되면 Vector 재시작 없이 설정 리로드가 트리거됩니다.

### 빈 설정 지원

Vector는 컴포넌트가 없는 빈 설정으로도 실행할 수 있습니다. 나중에 실제 컴포넌트로 채울 스텁 설정을 미리 로드하는 용도로 활용할 수 있으며, 설정 파일 감시와 함께 사용할 때 유용합니다.
...
`vector validate` 명령의 출력은 가독성이 개선되어 사용하기 편리합니다. `linkerd validate` 명령에서 영감을 받았습니다.
...
특정 입력 이벤트에 대한 변환 컴포넌트의 출력을 검증하여 설정 동작이 회귀하지 않도록 보장합니다. 협업이 필요한 미션 크리티컬 프로덕션 파이프라인에서 특히 강력한 기능입니다.
...
`insert_at` 지시문은 테스트 입력을 변환에 직접 주입합니다. Vector 단위 테스트는 관찰 데이터 소스를 모의(mock)하고 해당 모의 소스에 변환이 예상대로 반응하는지 검증하는 방식으로 동작한다고 이해하면 됩니다.
...
설정에 단위 테스트를 추가하는 것을 권장하며, 빌드 파이프라인에 통합하여 활용할 수도 있습니다. 조건 없이 단위 테스트 출력을 정의하면 변환의 입력 및 출력 이벤트를 그대로 출력하여 동작을 확인할 수 있습니다.
...
외부 백엔드에서 시크릿을 가져와 Vector 설정에 평문으로 저장하지 않을 수 있습니다. 여러 백엔드를 동시에 설정할 수 있습니다.
...
`SECRET[<backend_name>.<secret_key>]` 형식으로 시크릿 참조를 작성하면 Vector가 해당 백엔드에서 시크릿을 조회하여 플레이스홀더를 실제 값으로 대체합니다.
...
외부 프로세스를 호출하여 시크릿을 Vector에 안전하게 로드하는 방식입니다. Vault와 같은 서비스와 통합하여 자격 증명을 제공하는 데 활용할 수 있습니다.
...
VRL에 "secret" 함수가 추가되었습니다. 임의의 시크릿을 저장할 수 있으며, 기존의 메타데이터 함수 대신 사용하는 것이 권장됩니다. 문자열 키-값 쌍의 맵을 안전하게 저장합니다.

### 보안 고려 사항

과거에는 환경 변수를 통한 주입이 선호되었지만, 호스트 사용자가 Vector 프로세스의 /proc 파일 시스템을 읽을 수 있는 경우 환경 변수 값이 노출될 수 있어 보안상 문제가 있습니다.

### 디스크 버퍼의 시크릿

Vector 0.34.0부터 이벤트에 포함된 시크릿이 디스크 버퍼에 저장됩니다. 이 시크릿은 암호화되지 않은 상태로 저장됩니다.

### 모범 사례

- 평문 시크릿을 사용하지 마십시오.
- Vector 설정 파일에 평문 시크릿을 포함하지 마십시오.
- Vector의 데이터 디렉토리는 프로세스를 실행하는 사용자/그룹만 읽기/쓰기할 수 있도록 권한을 최대한 엄격하게 설정하십시오.
- Unix 기반 플랫폼에서 Vector는 기본적으로 디스크 버퍼 디렉토리와 파일의 권한을 프로세스 사용자만 읽기/쓰기 가능하도록 설정합니다.
...
오버플로 동작을 활용하면 버퍼 토폴로지를 구성할 수 있습니다. 순차적으로 배열된 두 개 이상의 버퍼로 구성되며, 하나의 버퍼가 가득 차면 다음 버퍼로 넘칩니다.

### 메모리 버퍼

인메모리 버퍼는 내구성이 없습니다. Vector는 이벤트가 처리될 때까지 소스 측 확인을 보류하는 종단 간 확인 등의 기능을 제공하지만, Vector 프로세스나 호스트가 충돌하면 인메모리 버퍼의 모든 이벤트가 손실됩니다.
...
데이터 내구성이 처리 성능보다 우선인 경우 디스크 버퍼를 사용하면 Vector 재시작(충돌 포함) 중에도 버퍼링된 이벤트를 보존할 수 있습니다. Vector가 재시작되면 기본적으로 중단된 지점부터 처리를 재개합니다.
...
디스크 버퍼는 이벤트를 디스크에 기록할 때 자동으로 체크섬을 계산하며, 읽기 중 손상이 감지되면 디코딩 가능한 이벤트를 자동으로 복구합니다.

파일 시스템에서 디스크 버퍼는 최대 파일 크기 128MiB로 성장하고 완전히 처리되면 삭제되는 추가 전용 로그 파일처럼 보입니다.

### 오버플로 버퍼 토폴로지

인메모리 버퍼만 사용하면 가용 메모리 한도에 제약을 받고, 디스크 버퍼만 사용하면 싱크에 문제가 없는 상황에서도 처리량이 저하됩니다. 오버플로 모드를 사용하면 우선적으로 인메모리 버퍼를 활용하고, 필요한 경우에만 디스크 버퍼로 자동 폴백합니다.
...
중요: 인메모리 버퍼에 여유 공간이 생기면, 디스크 버퍼에 이벤트가 남아 있더라도 새로 버퍼링할 이벤트는 인메모리 버퍼로 먼저 이동합니다. 따라서 인메모리 버퍼의 새 이벤트가 디스크 버퍼의 이전 이벤트보다 먼저 처리될 수 있습니다. 오버플로 동작을 사용하는 버퍼 토폴로지에서는 이벤트 순서가 보장되지 않습니다.
...
- 싱크 수가 많을 경우 메모리를 늘리거나 디스크 버퍼로 전환하는 것을 고려하십시오.
- 디스크 버퍼는 성능이 낮으므로 가능하면 메모리를 늘리는 편이 좋습니다.
- 디스크 처리량은 예상 최대 처리량의 최소 2배로 구성하여 충분한 여유를 확보하십시오.
- 일반적인 시작점으로 vCPU당 2GiB의 메모리가 권장됩니다.
- 인메모리 배치 및 버퍼링으로 인해 싱크 수에 비례하여 메모리 사용량이 증가합니다.
...
Docker 컨테이너에서 Vector를 실행할 때는 `0.0.0.0`에 바인딩해야 합니다. 그렇지 않으면 API가 컨테이너 외부에서 접근되지 않습니다.
...
Vector에는 사용 가능한 쿼리를 탐색하고 API를 직접 실행해볼 수 있는 GraphQL 플레이그라운드가 내장되어 있습니다. `/playground` 경로에서 접근할 수 있습니다.

### Vector Tap과 함께 사용

`vector tap` 명령은 Vector API를 통해 동작합니다. Vector 인스턴스에 API가 활성화되어 있으면 `vector tap`을 사용할 수 있습니다. `--url` 옵션으로 기본값이 아닌 API 주소를 지정할 수 있습니다.
...
이 검사는 싱크 초기화 시 수행됩니다. 헬스체크가 실패하면 오류가 기록되지만 Vector는 시작을 계속합니다.

### 글로벌 헬스체크 설정

헬스체크는 모든 싱크에 대해 전역적으로 활성화하거나 비활성화할 수 있으며, 싱크별로 개별 재정의도 가능합니다.
...
시작 시 싱크가 정상 상태로 보고되어야 하는지 여부를 설정합니다. 활성화 상태에서 싱크가 비정상으로 보고되면 Vector는 시작을 중단하고 종료됩니다.
...
Azure Blob Storage에서 비계정 SAS를 사용하는 경우 헬스체크가 실패합니다. 이 경우 해당 싱크의 `healthcheck.enabled`를 `false`로 설정하여 헬스체크를 비활성화해야 합니다.

### Vector API 헬스체크 엔드포인트

Vector는 API를 통해 헬스체크 엔드포인트도 제공합니다. Vector가 실행 중인지 확인하는 데 유용하며, 성공 응답은 Vector가 초기화되어 정상 실행 중임을 나타냅니다.
...
트래픽 유형에 따라 각각 다른 프록시를 설정할 수 있습니다. 프록시를 거치지 않아야 할 특정 호스트도 지정할 수 있습니다.
...
설정 파일 또는 컴포넌트 수준에서 프록시를 별도로 설정하면 환경 변수보다 우선 적용됩니다. 환경 변수는 소문자 형식이 대문자 형식보다 우선순위가 높습니다.
...
HTTP 프록시가 필요한 경우 Vector의 글로벌 프록시 옵션을 사용하면 모든 HTTP 트래픽을 손쉽게 프록시를 통해 라우팅할 수 있습니다.
...
메모리 테이블은 로그 이벤트를 처리하며 VRL 객체를 입력으로 받습니다. 각 키-값 쌍은 테이블에 별도 항목으로 저장되어 값과 키가 연결됩니다.
...
메모리 풍부화 테이블은 데이터를 주입하기 위해 싱크로 사용해야 하며, 이후 다른 풍부화 테이블과 동일한 방식으로 쿼리할 수 있습니다.
...
`find_enrichment_table_records`는 복잡한 사용 사례에서 여러 행을 배열 형식으로 반환합니다.
...
시작 시 오류를 방지하려면 풍부화 테이블 이름이 소스, 변환, 싱크 등 다른 컴포넌트 이름과 충돌하지 않도록 주의하십시오. 충돌이 발생하면 풍부화 테이블을 고유한 이름으로 변경하여 해결할 수 있습니다.
...
Vector는 외부 소스의 로그와 메트릭을 처리하는 것과 동일한 방식으로 Vector 자체가 생성하는 로그와 메트릭을 처리할 수 있도록 `internal_logs`와 `internal_metrics` 두 가지 소스를 제공합니다.
...
여러 Vector 인스턴스가 동일한 대상으로 internal_metrics를 전송하는 경우, 메트릭 시리즈가 충돌하지 않도록 각 인스턴스를 식별하는 고유한 태그를 메트릭에 추가하는 것이 일반적입니다.
...
내부 메트릭 스크레이프 간격(기본값 5분)보다 크게 설정하여 메트릭이 수집 및 캡처되기에 충분히 오래 유지되도록 합니다.

### 관찰 전략

Vector는 일관성 있고 상호 연관된 텔레메트리 데이터를 보장하는 이벤트 기반 관찰 전략을 사용합니다. 핫 경로에서는 로그 이벤트를 속도 제한하여 IO 포화나 서비스 장애 위험 없이 세밀한 가시성을 확보합니다.

---

## 설정 스키마 생성

Vector는 파이프라인 실행 외에도 다양한 작업을 위한 CLI 하위 명령을 제공합니다.

### 스키마 생성

Vector 바이너리로 스키마를 생성하여 IDE에 제공하면 자동 완성을 활성화할 수 있습니다:

```bash
vector generate-schema -o vector-v0.45.0-schema.json
```

이를 통해 IDE가 실시간 제안을 제공하여 Vector 문서를 직접 방문하는 빈도를 줄일 수 있습니다.
