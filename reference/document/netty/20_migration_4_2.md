...
> 이 문서는 Netty 공식 Wiki의 "Netty 4.2 Migration Guide" 페이지를 한국어로 번역한 것입니다.
...
(Netty 팀의) 우리 입장에서 가장 큰 변경 중 하나는 최소 Java 버전 요구사항을 Java 6에서 Java 8로 변경한다는 점입니다. 이 업그레이드는 우리에게 큰 삶의 질 개선이지만, Java 8보다 오래된 자바 버전을 여전히 사용하는 정기 유지보수 자바 프로젝트가 거의 남아 있지 않으므로 다른 분들에게는 영향이 거의 없을 것입니다.

## 권장 업그레이드 절차

다음은 실전 경험으로 다듬은, Netty 4.2로 시스템을 업그레이드하는 권장 절차입니다. 여기서 설명하는 절차는 다양한 시스템에 적용할 수 있도록 고수준이고 일반적입니다. 본 마이그레이션 가이드의 다른 절도 함께 읽고 자기 시스템에 관련 있는 세부 사항과 팁을 확인하세요.

시작 전에, 의존성 관리 모범 사례를 따르는 것이 권장됩니다. 특히 가능하면 shading은 피해야 합니다. shading은 Maven/Gradle 같은 의존성 관리 도구로부터 의존성을 숨기기 때문입니다. 또한 의존성 버전을 제어하기 위해 `netty-bom` 같은 BOM 파일을 사용하세요. 이런 관행을 따르면 런타임에 버전이 뒤섞이는 일을 피할 수 있습니다.

Netty 4.2 업그레이드는 시스템 규모와 복잡도에 따라 여러 스프린트와 배포 사이클이 걸릴 수 있습니다. 프로젝트를 끝까지 완료할 시간과 자원을 확보할 수 있도록 계획을 세우세요.
...
이 기본값은 시스템 프로퍼티로 덮어 Netty 4.1 동작으로 되돌릴 수 있습니다.

```
io.netty.handler.ssl.defaultEndpointVerificationAlgorithm=NONE
```

이 override의 목적은 시스템이 Netty 4.2 마이그레이션을 진행하는 데 도움을 주기 위함입니다. 의도적으로 endpoint identification을 비활성화하려는 시스템은 `SslContextBuilder` 옵션을 통해 그렇게 해야 합니다.

**⚠️ `adaptive` 메모리 할당자가 새 기본값입니다.**

Netty 4.1에서 `adaptive` 메모리 할당자는 실험적이었습니다. 이제 `pooled` 대신 새 기본값으로 만들었습니다.

대부분의 워크로드는 메모리 사용량이 줄어들고, 성능은 pooled 할당자와 비슷하거나 약간 더 좋을 것으로 봅니다. adaptive 할당자는 관찰된 워크로드에 맞춰 잘 동작하도록 자신을 자동으로 튜닝합니다. 또한 처음부터 가상 스레드와도 잘 동작하도록 설계되었습니다.

다만 모든 애플리케이션이 할당자를 사용하는 방식과 압박 방식이 다르므로, adaptive 할당자가 어떤 경우에는 더 나쁘게 동작할 수도 있습니다. Netty 4.1에서처럼 pooled 할당자를 계속 쓰고 싶은 통합자는 다음 시스템 프로퍼티로 기본 할당자를 변경할 수 있습니다.
...
Netty 4.2에는 문제를 일으킬 가능성은 낮지만 그래도 언급해야 할 호환성 변경이 다수 포함되어 있습니다.
...
이제 통합자는 전송별 `IoHandlerFactory`를 `MultiThreadedEventLoopGroup` 생성자에 전달해야 합니다.
...
또한 (올바른 `IoHandle` 타입을 구현한다면) 채널 외의 것을 등록할 수 있게 되어, Netty 컴포넌트를 재사용하고 오늘 시점에 존재하지 않는 다른 `IoHandle` 구현에 대한 지원을 추가할 가능성이 많아졌습니다. 예를 들어 io_uring으로 파일 IO를 구현하는 데 사용할 수 있습니다.

**✅ SelfSignedCertificate 대신 netty-pkitesting 사용**

Netty 4.1에는 우리가 TLS 구현 테스트에 사용하는 `SelfSignedCertificate` 클래스가 포함되어 있습니다. 이것이 그렇게 널리 사용될 의도는 아니었으며, 범용 PKI·TLS 테스트 도구로는 부적합한 여러 API 결정을 갖고 있습니다.
