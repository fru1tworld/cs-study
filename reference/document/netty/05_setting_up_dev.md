...
> 이 문서는 Netty 공식 Wiki의 "Setting up development environment" 페이지를 한국어로 번역한 것입니다.
...
Windows를 사용하는 경우, 코드를 체크아웃할 때 LF가 자동으로 CRLF로 변환됩니다.

```sh
git config --global core.autocrlf true
```

macOS를 사용하는 경우, 코드를 커밋할 때 CRLF가 자동으로 LF로 변환됩니다.
...
코드를 체크아웃한 뒤 `./mvnw install -DskipTests -T1C`로 빌드 환경이 정상 동작하는지 확인합니다. 이 명령은 로컬 Maven 캐시도 함께 채워주므로 이후 개별 모듈을 빌드할 때 도움이 됩니다.
...
64비트 운영체제를 사용한다면 64비트 버전의 IntelliJ IDEA를 사용하세요. 예를 들어 64비트 Windows를 쓰더라도 시작 메뉴 단축키는 32비트 바이너리를 가리킬 수 있습니다. 이 경우 설치 디렉터리에서 `idea64.exe`를 찾아 실행해야 합니다. 그러지 않으면 IntelliJ가 `io.netty:netty-tcnative:windows-x86_32`를 찾을 수 없다는 오류를 냅니다.

### `Use --release …` 컴파일러 옵션 해제

Netty 4.1 이하 버전은 Java 6을 최소 자바 버전으로 삼고 있어 Java 6 바이트코드로 컴파일해야 합니다. 다만 성능을 위해 코드베이스에는 더 새로운 Java API 호출도 들어 있고, 런타임 버전 검사로 적절히 가드되어 있습니다. 이 때문에 Netty는 `--source`와 `--target` 플래그로 컴파일해야 합니다. 그런데 IntelliJ는 기본적으로 `--release` 플래그를 사용하며, 이 플래그는 API 버전 검사도 함께 활성화합니다.

`--release` 플래그를 사용하면 IntelliJ가 Netty 프로젝트 빌드 시 컴파일 오류를 내므로, [Compiler Settings 다이얼로그](https://www.jetbrains.com/help/idea/troubleshooting-common-maven-issues.html#check_compiler_settings)에서 이 옵션을 해제해야 합니다.
...
수정 사항이 inspection 경고를 도입하지 않도록 하세요. 거짓 양성(false positive)이라고 판단되면 IDE가 안내하는 대로 `@SuppressWarnings` 애노테이션이나 `noinspection` 라인 주석으로 경고를 억제하세요. inspector 사용에 대한 자세한 내용은 [웹 도움말](http://www.jetbrains.com/idea/webhelp/inspecting-source-code.html)을 참고하세요.
