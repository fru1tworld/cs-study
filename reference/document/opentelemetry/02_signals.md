...
> OpenTelemetry가 다루는 텔레메트리 데이터의 기본 단위인 **Signal** 을 정리합니다.
...
표준을 따르면 백엔드 대시보드·알람이 자동으로 동작합니다 (Tempo의 service map, Datadog APM의 endpoint 분석 등).
...
Trace가 "어디서 시간을 썼는가"를 함수 단위로 깊이 있게 보여줍니다 → continuous profiling이 OTLP 생태계로 통합되는 단계.

본 문서 시리즈에서는 코어 6편에서 깊이 다루지 않습니다.
