...
> Span의 데이터 모델, Context 전파, Sampling을 정리합니다.
...
부모로 했다면 부모가 둘이 되어 트리가 깨집니다. Link는 트리는 그대로 두고 다중 참조를 표현합니다.
...
수동 호출이 필요한 경우는 비표준 전송이거나 직접 구현한 RPC 정도.
...
모든 요청을 저장하면 비용이 폭발 → **일부만 선택해서 저장**. 두 가지 큰 전략:
...
단점: 모든 Span을 일단 Collector에 모아야 함 → 네트워크·메모리 비용. 같은 trace의 모든 Span이 같은 Collector 인스턴스에 모여야 함 (보통 Load Balancing exporter로 trace_id hash 라우팅).
...
Tail sampling은 비트가 아니라 Collector 안에서 처리되므로 와이어 포맷에는 영향 없음.
...
대부분의 가치는 자동 계측에서 나옴. 수동 계측은 **비즈니스 의미가 있는 작업** 에만 추가.
...
→ 백엔드에서 attribute 기반 필터링·집계가 강력해짐.
