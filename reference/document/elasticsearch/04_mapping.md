...
동적 매핑을 사용하면 Elasticsearch가 문서의 필드 데이터 타입을 자동으로 감지하고 매핑을 생성합니다. 새로운 필드를 자동으로 감지하고 추가하는 것을 동적 매핑이라고 합니다.
...
> 참고: JSON은 long과 integer, double과 float을 구분하지 않으므로, 파싱된 모든 부동 소수점 숫자는 `float`(dynamic: true 기본값) 또는 `double`(dynamic: runtime)로 매핑되고, 모든 파싱된 정수는 `long`으로 매핑됩니다.
...
동적 매핑은 시작 단계에 유용하지만, 프로덕션 환경에서는 직접 명시적 매핑을 정의하는 것이 좋습니다. 인덱스 생성 시 필드 매핑을 정의하거나, 기존 인덱스에 새 필드를 추가할 수 있습니다.
...
위 예제에서 `employee-id`는 `keyword` 필드이며 `index` 파라미터가 `false`로 설정되어 있습니다. 즉, 값은 저장되지만 인덱싱되지 않아 검색에 사용할 수 없습니다.
...
스크립트가 없으면 Elasticsearch는 쿼리 시점에 `_source`에서 런타임 필드와 동일한 이름의 필드를 찾아 값이 있으면 반환합니다.
...
런타임 필드는 인덱스 매핑에 동일한 이름으로 정의된 필드보다 우선합니다. 이를 통해 필드 자체를 수정하지 않고도 기존 필드를 섀도잉하여 다른 값을 계산할 수 있습니다.
...
`_source` 필드는 인덱싱 시점에 전달된 원본 JSON 문서 본문을 담습니다. `_source` 필드 자체는 인덱싱되지 않아 검색할 수 없지만, get 또는 search 같은 조회 요청 시 반환될 수 있도록 저장됩니다.
...
`doc_values`는 문서 인덱싱 시점에 생성되는 온디스크 데이터 구조로, 위와 같은 데이터 접근 패턴을 지원합니다. `_source`와 동일한 값을 저장하지만, 정렬 및 집계에 더 효율적인 열 기반 형식으로 저장합니다.
...
동적 필드 매핑은 필드에 구체적인 값이 있을 때만 추가됩니다. 필드에 `null`이나 빈 배열이 있으면 동적 필드 매핑이 추가되지 않습니다. `dynamic_template`에 `null_value` 옵션이 있는 경우, 해당 필드에 구체적인 값을 가진 첫 번째 문서가 인덱싱된 후에만 적용됩니다.
...
인덱스에 별칭을 정의하면 리인덱싱이 필요할 때 애플리케이션이 인덱스 변경을 인지하지 않아도 백그라운드에서 데이터를 리인덱싱할 수 있습니다.
