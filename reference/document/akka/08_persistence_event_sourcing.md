...
> 이 문서는 Akka 공식 문서의 "Event Sourcing" 섹션을 한국어로 번역한 것입니다.
...
Akka의 이벤트 소싱(event sourcing)은 상태를 가진 액터(stateful actor)가 상태 자체를 저장하는 대신, **상태 변화를 나타내는 이벤트(event)만을 저장**함으로써 자신의 상태를 영속화(persist)할 수 있게 해줍니다. 공식 문서의 표현을 빌리면, "액터에 의해 영속화되는 것은 이벤트뿐이며, 액터의 실제 상태는 저장되지 않는다(only the events that are persisted by the actor are stored, not the actual state of the actor)".
...
이 동작(behavior)은 Command 타입에 대해 엄격하게 타입이 지정(strictly typed)되며, 이는 Akka의 타입 시스템(type system)에 의해 강제됩니다.
...
커맨드(command)는 **영속화되지 않는(non-persistent) 메시지**로서, 현재 상태(current state)에 대해 검증(validate)됩니다. 검증이 성공하면, 커맨드의 효과(effect)를 나타내는 이벤트(event)가 생성됩니다. 이 이벤트들은 영속화된 뒤에, 액터의 상태를 변경하기 위해 적용(apply)됩니다.
...
이 사이드 이펙트들은 영속화가 성공한 뒤 **순차적으로(sequentially)** 실행되며, **"최대 한 번(at-most-once)"** 의 기준으로 동작합니다. 만약 persist가 실패하면 이 사이드 이펙트들은 실행되지 않습니다.
...
상태(state)는 가능한 한 **불변(immutable)** 이어야 합니다. 만약 상태가 가변(mutable)이어야 한다면, 상태 팩토리 함수(state factory function)를 받는 `withMutableState` 팩토리 메서드를 사용하여, 복구 도중에 항상 새로운 인스턴스(new instance)가 생성되도록 보장해야 합니다.
...
이벤트 어댑터(event adapter)는 저널 저장(journal storage)을 위해 이벤트를 다른 타입으로 변환합니다. `EventAdapter`를 확장하며 다음 세 가지 메서드를 구현합니다.

- **toJournal**: 이벤트를 저장 형식(storage format)으로 변환
- **fromJournal**: 저장 형식을 다시 이벤트로 변환
- **manifest**: 버전 정보(version information)를 제공

어댑터는 복구 도중에 형식 변환을 처리함으로써 **스키마 진화(schema evolution)** 를 지원합니다. 즉, 애플리케이션이 시간이 지나며 이벤트 구조가 바뀌더라도, 과거에 저장된 이벤트를 새로운 형식으로 변환하여 읽을 수 있게 해 줍니다.
...
문서는 유연성(flexibility)과 스키마 진화(schema evolution) 지원을 위해 **Jackson 직렬화(Jackson serialization)** 를 권장합니다. 이것이 중요한 이유는, 이벤트가 애플리케이션이 진화하는 동안에도 계속해서 읽을 수 있어야(readable) 하기 때문입니다. 한 번 저장된 이벤트는 영구적으로 남으므로, 미래의 코드 버전에서도 역직렬화(deserialize)할 수 있어야 합니다.
...
이 절은 Akka에서 이벤트 소싱 엔티티를 구조화하는 모범 사례(best practice)를 다룹니다. 네 가지 주요 아키텍처 패턴을 중심으로 구성됩니다.

### 상태 안의 이벤트 핸들러 (Event handlers in the state)

이 가이드는 이벤트 처리 로직(event handling logic)을 상태 클래스(state class) 자체 안에 배치하는 방식을 보여 줍니다. 은행 계좌(bank account) 도메인 예제를 사용하여, 생애 주기(lifecycle)의 서로 다른 단계를 나타내는 별도의 상태 클래스를 사용합니다.
...
이 접근법은 상태를 핵심 비즈니스 로직(core business logic)을 담은 도메인 객체(domain object)로 취급하면서, 이벤트 적용 로직을 관련된 상태 타입에 국한시킵니다. 또한 예상치 못한 이벤트가 부적절한 상태에 도착했을 때 예외(exception)를 던짐으로써, 잘못된 상태 전이(invalid state transition)를 방지합니다.

### 상태 안의 커맨드 핸들러 (Command handlers in the state)

앞선 패턴을 확장하여, 커맨드 처리(command handling) 또한 상태 클래스 안의 `applyCommand` 메서드를 통해 내장할 수 있습니다. 이는 비즈니스 로직을 한층 더 중앙집중화하여, 각 상태가 들어오는 커맨드에 어떻게 반응할지를 스스로 정의하도록 합니다.
...
이 방식은 커맨드 처리가 비즈니스 상태 생애 주기와 밀접하게 대응될 때 잘 동작합니다. 서로 다른 상태가 동일한 커맨드에 대해 자연스럽게 다르게 반응하기 때문입니다.
...
이 접근법은 빈 상태에 의미 있는 연산이나 데이터가 없을 때 보일러플레이트(boilerplate)를 줄여 줍니다. 보통 첫 번째 커맨드가 정상 운영이 시작되기 전에 첫 상태를 생성하게 됩니다.
...
가변 상태를 사용할 때, `emptyState` 메서드는 매 호출마다 새로운 인스턴스(fresh instance)를 생성해야 합니다. 이는 실패 복구(failure recovery) 도중 올바른 동작을 보장하기 위함입니다.
...
`onCommand`와 `onEvent` 메서드는 중첩된(nested) switch 식(expression)을 사용하여, 빌더 기반(builder-based) 등록을 대체합니다. 이 접근법은 보일러플레이트를 제거하면서도 컴파일 타임의 완전성 검사(exhaustiveness checking)를 보장합니다. 레코드(record)는 이 패턴과 자연스럽게 짝을 이루어 간결한 타입 정의를 가능하게 합니다.
...
스냅샷 저장소가 설정되지 않은 채로 운영하는 것도 유효하지만, 액터가 스냅샷 작업을 시도하기 전까지 Akka는 경고(warning)를 로깅합니다.
...
복제 이벤트 소싱(Replicated Event Sourcing)은 표준 `EventSourcedBehavior`를 확장하여, 단일 작성자 원칙(single-writer principle)을 강제하는 대신 **각 엔티티의 여러 활성 복제본(multiple active replicas)** 을 가능하게 합니다. 이를 통해 여러 위치에 걸쳐 "액티브-액티브(active-active)" 및 "핫 스탠바이(hot standby)" 배포 패턴을 유지하면서도 최종 일관성(eventual consistency)을 달성할 수 있습니다.
...
**핵심적인 트레이드오프**: 이벤트 핸들러는 동시 이벤트(concurrent event)를 처리해야 합니다. "단일 작성자 보장(single-writer guarantee)이 유지되지 않기" 때문입니다. 네트워크 분할(partition)이나 장애 동안 복제가 지연될 수 있으므로, 상태는 "최종 일관성(eventually consistent)"을 갖게 됩니다.
...
이벤트 핸들러에서의 직접적인 사이드 이펙트는 "일반적으로 권장되지 않습니다." 왜냐하면 "이벤트 핸들러는 재생(replay) 시와 복제된 이벤트를 소비할 때도 사용되어", 의도치 않은 재실행(re-execution)을 유발하기 때문입니다.
...
Akka Projections는 복제된 엔티티와 함께 동작하지만, "프로젝션은 모든 복제본으로부터 모든 이벤트(all events from all replicas)를 받게 되며", 복제본마다 다른 순서로 도착할 수 있다는 점을 인지해야 합니다.
...
CQRS(Command Query Responsibility Segregation, 커맨드 쿼리 책임 분리)는 데이터를 변경하는 작업(커맨드, command)과 데이터를 읽는 작업(쿼리, query)을 서로 다른 모델로 분리하는 아키텍처 패턴입니다.

Akka 공식 문서에서 이 페이지는 비교적 간결하며, 핵심 메시지는 다음과 같습니다.
