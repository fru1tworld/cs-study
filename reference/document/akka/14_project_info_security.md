...
> 이 문서는 Akka 공식 문서의 "Project Information", "Security Announcements" 섹션을 한국어로 번역한 것입니다.
...
즉, 애플리케이션을 다시 컴파일하지 않고도 의존하는 Akka 라이브러리의 버전을 단순히 교체하는 것만으로 업그레이드를 수행할 수 있다는 것이 핵심 원칙입니다.
...
Akka는 모든 풀 리퀘스트(pull request)에 대해 바이너리 호환성을 강제하기 위해 **MiMa**(Lightbend가 유지 관리하는 도구, Migration Manager for Scala)를 사용합니다.
...
액터(Actor), 액터 시스템(ActorSystem), 머티리얼라이저(Materializer)는 "명시적으로 해제되어야 하는 리소스를 점유(bind)합니다." 프레임워크는 생성될 때마다 그에 대응하는 `stop`, `terminate` 또는 `shutdown` 호출을 요구합니다. 그 이유는 "액터는 독립적으로 존재하면서 자신만의 생명을 갖도록 의도되었기(Actors are meant to have a life of their own, existing independently)" 때문입니다.

**Q. JVM 애플리케이션 또는 Scala REPL이 "멈춘(hanging)" 것처럼 보이는 이유 (JVM application or Scala REPL "hanging")**

시스템이 자동으로 종료되지 않는 이유는 "JVM은 명시적으로 중단(stopped)되기 전까지는 종료되지 않기(the JVM will not exit until it is stopped)" 때문입니다. 개발자는 정상적인 종료가 이루어지도록 "실행 중인 애플리케이션 또는 Scala REPL 세션 내의 모든 액터 시스템(ActorSystem)을 종료(shutdown)"해야 합니다.

### 3.2 액터 관련 (Actors Section)

**Q. 왜 `OutOfMemoryError`가 발생하나요? (Why OutOfMemoryError?)**

흔한 원인은 메시지 흐름 제어(message flow control)의 실패입니다. "순수하게 푸시(push) 기반인 시스템에서 메시지 소비자(consumer)가 대응하는 메시지 생산자(producer)보다 잠재적으로 느릴 경우, 반드시 어떤 형태의 메시지 흐름 제어를 추가해야(you must add some kind of message flow control)" 합니다.
...
프로젝트는 잠재적인 취약점을 **공개 공시(public disclosure) 이전에** 전용 비공개 보안 이메일 주소(dedicated private security email address)로 알릴 것을 강력히 권장합니다. 이 주소로 전송된 보고는 전담 보안 팀(specialized security team)의 검토를 받게 되며, 이 팀은 신속한 수정을 보장하기 위해 보고자와 협력합니다.

- **연락처(Contact):** 해당 페이지는 (이메일 보호 인코딩으로 표시된) 보안 이메일 주소를 참조합니다.
...
### 5.1 페이지 개요 (Page Overview)

이 페이지는 Akka 코어 문서(버전 2.10.19)의 마이그레이션 가이드(Migration Guides) 허브로서, 메이저 버전 간 업그레이드를 수행하는 개발자를 위한 중심 참조 지점 역할을 합니다.

### 5.2 제공되는 마이그레이션 가이드 (Available Migration Guides)

이 페이지는 다음 마이그레이션 경로(migration path)들을 나열합니다.
...
6. **이전 마이그레이션 가이드(Older Migration Guides)** (그 이전 버전을 위한 아카이브)

### 5.3 주요 요소 (Key Elements)

**탐색 구조(Navigation Structure):**

이 페이지는 "프로젝트 정보(Project Information)" 섹션의 일부이며, 다음과 같은 관련 주제로의 링크를 포함합니다.

- 바이너리 호환성 규칙(Binary Compatibility Rules)
- 롤링 업데이트와 버전(Rolling Updates and Versions)
- 이슈 추적(Issue Tracking)

**헤더 공지(Header Notice):**

문서는 신규 사용자에게 다음과 같은 메시지로 안내합니다. "Akka가 처음이신가요? Akka SDK로 시작하세요(New to Akka? Start with the Akka SDK)."

**언어 지원(Language Support):**

문서의 탭(tab)을 통해 Scala, Java와 sbt, Maven, Gradle 빌드 도구 사이를 전환할 수 있습니다.

**관련 콘텐츠(Related Content):**

인접한 링크로 "Lombok을 사용한 불변성(Immutability using Lombok)" 가이드가 있으며, 다음으로 권장되는 읽을거리는 "마이그레이션 가이드 2.9.x → 2.10.x"입니다.

**라이선스 정보(License Information):**

"Akka는 Business Source License 1.1 하에서 제공됩니다(Akka is available under the Business Source License 1.1)."
