...
> 이 문서는 Akka 공식 문서의 "Getting Started" 및 "General Concepts" 섹션을 한국어로 번역한 것입니다.
...
이 장에서는 동시성/분산 컴퓨팅에 대해 자주 쓰이지만 의미가 혼동되는 용어들을 정리합니다.
...
이전의 "액터 시스템(Actor Systems)" 섹션에서는 액터들이 어떻게 계층을 형성하며, 애플리케이션을 구축할 때 가장 작은 단위(smallest unit)가 되는지를 설명했습니다. 이 섹션에서는 그러한 액터 하나를 따로 떼어 살펴보면서, 액터를 구현할 때 마주치게 되는 개념들을 설명합니다. 모든 세부 사항을 담은 보다 심층적인 레퍼런스는 "액터 소개(Introduction to Actors)"를 참고하시기 바랍니다.
...
액터는 상태(State), 행동(Behavior), 메일박스(Mailbox), 자식 액터(Child Actors), 슈퍼바이저 전략(Supervisor Strategy)을 담는 컨테이너입니다. 이 모든 것은 액터 참조(Actor Reference) 뒤에 캡슐화됩니다. 주목할 만한 한 가지 측면은, 액터가 명시적인 생명 주기(explicit lifecycle)를 가진다는 점입니다. 액터는 더 이상 참조되지 않더라도 자동으로 파괴(destroy)되지 않습니다. 액터를 생성한 후에는, 그것이 결국 종료(terminate)되도록 보장하는 것이 사용자의 책임입니다. 이는 또한 "액터가 종료될 때(When an Actor Terminates)" 리소스가 어떻게 해제되는지를 사용자가 제어할 수 있게 해 줍니다.

### 7.1 액터 참조(Actor Reference)

아래에서 자세히 설명하듯, 액터 모델의 이점을 누리기 위해서는 액터 객체가 외부로부터 차단(shielded)되어야 합니다. 따라서 액터는 외부에 대해 액터 참조(actor reference)를 통해 표현되며, 이 참조는 제약 없이 자유롭게 전달될 수 있는 객체입니다. 이렇게 내부 객체(inner object)와 외부 객체(outer object)로 분리함으로써, 원하는 모든 연산에 대한 투명성(transparency)이 가능해집니다. 즉, 다른 곳의 참조를 갱신할 필요 없이 액터를 재시작하는 것, 실제 액터 객체를 원격 호스트(remote host)에 배치하는 것, 액터가 어디에서 실행되든 상관없이 메시지를 보내는 것이 가능합니다. 그러나 가장 중요한 측면은, 액터가 스스로 어리석게 그 정보를 공개하지 않는 한, 외부에서 액터 내부를 들여다보고 그 상태(state)를 손에 넣는 것이 불가능하다는 점입니다.

액터 참조는 파라미터화(parameterized)되어 있으며, 지정된 타입(specified type)의 메시지만 보낼 수 있습니다.

### 7.2 상태(State)

액터 객체는 일반적으로 액터가 처할 수 있는 상태를 반영하는 여러 변수를 포함합니다. 이는 명시적인 상태 기계(state machine)일 수도 있고, 카운터(counter), 리스너 집합(set of listeners), 대기 중인 요청(pending requests) 등일 수도 있습니다. 이 데이터들은 액터를 가치 있게 만드는 것이며, 다른 액터에 의한 손상(corruption)으로부터 보호되어야 합니다. 좋은 소식은, Akka 액터가 개념적으로 각자 자신만의 경량 스레드(light-weight thread)를 가지며, 이 스레드는 시스템의 나머지로부터 완전히 차단된다는 점입니다. 이는 곧 락(lock)을 사용하여 접근을 동기화할 필요 없이, 동시성에 대해 전혀 신경 쓰지 않고 액터 코드를 작성할 수 있음을 의미합니다.
...
내부 상태(internal state)는 액터의 동작에 핵심적이기 때문에, 일관성 없는 상태(inconsistent state)는 치명적입니다. 따라서 액터가 실패(fail)하여 그 슈퍼바이저(supervisor)에 의해 재시작(restart)될 때, 상태는 액터를 처음 생성할 때와 마찬가지로 처음부터 새로(from scratch) 생성됩니다. 이는 시스템의 자가 치유(self-healing) 능력을 가능하게 하기 위함입니다.

선택적으로, 수신한 메시지를 영속화(persisting)하고 재시작 후 이를 재생(replaying)함으로써, 액터의 상태를 재시작 이전의 상태로 자동 복구할 수 있습니다(이벤트 소싱(Event Sourcing) 참고).

### 7.3 행동(Behavior)

메시지가 처리될 때마다, 그 메시지는 액터의 현재 행동(current behavior)에 대해 매칭됩니다. 행동(Behavior)이란, 그 시점에 메시지에 반응하여 취할 행동을 정의하는 함수(function)를 의미합니다. 예를 들어, 클라이언트가 인가(authorized)되었으면 요청을 전달하고 그렇지 않으면 거부하는 식입니다. 이 행동은 시간이 지나면서 변경될 수 있습니다. 예컨대 시간이 흐르면서 서로 다른 클라이언트가 인가를 얻거나, 액터가 "서비스 불가(out-of-service)" 모드로 들어갔다가 나중에 복귀할 수 있기 때문입니다. 이러한 변경은 행동 로직에서 읽어 들이는 상태 변수(state variables)에 인코딩하거나, 다음 메시지에 사용할 다른 행동을 반환함으로써 런타임에 함수 자체를 교체하는 방식으로 달성됩니다. 다만, 액터 객체의 생성(construction) 시점에 정의된 초기 행동(initial behavior)은 특별합니다. 액터가 재시작되면 그 행동이 이 초기 행동으로 재설정(reset)되기 때문입니다.
...
각 액터는 잠재적으로 부모(parent)입니다. 만약 하위 작업(sub-task)을 위임하기 위해 자식을 생성한다면, 그 자식들을 자동으로 슈퍼비전(supervise)하게 됩니다. 자식들의 목록(list of children)은 액터의 컨텍스트(context) 안에서 유지되며, 액터는 그 목록에 접근할 수 있습니다. 목록에 대한 수정은 자식을 스폰(spawning)하거나 정지(stopping)함으로써 이루어지며, 이러한 동작들은 즉시 반영됩니다. 실제 생성 및 종료 동작은 비동기적(asynchronous) 방식으로 무대 뒤에서 일어나므로, 부모를 "블로킹"하지 않습니다.

### 7.6 슈퍼바이저 전략(Supervisor Strategy)

액터의 마지막 구성 요소는 예상치 못한 예외(unexpected exceptions), 즉 실패(failures)를 처리하기 위한 전략입니다. 그러면 장애 처리(fault handling)는 Akka에 의해 투명하게 수행되며, 각 실패에 대해 "장애 내성(Fault Tolerance)"에서 기술된 전략 중 하나가 적용됩니다.

### 7.7 액터가 종료될 때(When an Actor Terminates)

일단 액터가 종료되면, 즉 재시작으로 처리되지 않는 방식으로 실패하거나, 스스로 정지하거나, 슈퍼바이저에 의해 정지되면, 그 액터는 자신의 리소스를 해제하고 메일박스에 남아 있는 모든 메시지를 시스템의 "데드 레터 메일박스(dead letter mailbox)"로 흘려보냅니다. 이 데드 레터 메일박스는 그것들을 `DeadLetters`로서 `EventStream`으로 전달합니다. 그런 다음 메일박스는 액터 참조 내에서 시스템 메일박스(system mailbox)로 교체되어, 모든 새 메시지를 `DeadLetters`로서 `EventStream`으로 리디렉션합니다. 다만 이는 최선 노력(best effort) 기반으로 이루어지므로, "보장된 전달(guaranteed delivery)"을 구성하기 위해 이에 의존해서는 안 됩니다.

---

## 8. 슈퍼비전과 모니터링(Supervision and Monitoring)

이 장에서는 슈퍼비전(supervision)의 개념, 제공되는 기본 요소(primitives)와 그 의미론(semantics)을 개괄합니다. 이것이 실제 코드로 어떻게 변환되는지에 대한 세부 사항은 supervision을 참고하시기 바랍니다.
...
위에서 설명한 부모와 자식 사이의 특별한 관계와는 대조적으로, 각 액터는 다른 어떤 액터든 모니터링(monitor)할 수 있습니다. 액터는 생성과 함께 완전히 살아있는(fully alive) 상태로 등장하고 재시작은 영향을 받는 슈퍼바이저 외부에서 보이지 않으므로, 모니터링에 이용할 수 있는 유일한 상태 변화는 살아있음(alive)에서 죽음(dead)으로의 전이뿐입니다. 따라서 모니터링은 한 액터를 다른 액터에 묶어서 그 다른 액터의 종료(termination)에 반응할 수 있게 하기 위해 사용됩니다. 이는 실패(failure)에 반응하는 슈퍼비전과는 대조적입니다.

생명 주기 모니터링은 모니터링 액터가 수신할 `Terminated` 메시지를 사용하여 구현되며, 기본 행동은 별도로 처리되지 않을 경우 특별한 `DeathPactException`을 던지는 것입니다. `Terminated` 메시지 수신을 시작하려면 `ActorContext.watch(targetActorRef)`를 호출합니다. 수신을 중지하려면 `ActorContext.unwatch(targetActorRef)`를 호출합니다. 한 가지 중요한 속성은, 모니터링 요청과 대상의 종료가 일어나는 순서와 무관하게 메시지가 전달된다는 점입니다. 즉, 등록 시점에 대상이 이미 죽어 있더라도 여전히 메시지를 받게 됩니다.
...
메시지가 처리되는 동안(즉, 메일박스에서 꺼내져 현재 행동에 넘겨진 동안) 예외가 던져지면, 그 메시지는 손실(lost)됩니다. 그것이 메일박스에 다시 놓이지 않는다(not put back on the mailbox)는 점을 이해하는 것이 중요합니다. 따라서 메시지 처리를 재시도(retry)하고 싶다면, 예외를 잡아내고(catching the exception) 흐름을 재시도하는 식으로 직접 처리해야 합니다. 재시도 횟수에는 반드시 상한(bound)을 두어야 합니다. 시스템이 라이브락(livelock)에 빠지는 것(즉, 진전 없이 많은 CPU 사이클을 소비하는 것)을 원하지 않기 때문입니다.
...
프레임워크는 "보장된 전달(guaranteed delivery)"이라는 개념 자체가 모호하기 때문에 이를 주장하기를 의도적으로 피합니다. 문서가 언급하듯, "보장(guaranteed)"이 무엇을 의미하는지 판단하려면 다음 중 어느 것을 의미하는지 명확히 해야 합니다: 메시지가 네트워크로 전송되는 것, 호스트에 의해 수신되는 것, 메일박스에 놓이는 것, 처리가 시작되는 것, 또는 처리가 성공적으로 완료되는 것. 각 수준은 서로 다른 비용과 과제를 가집니다.

Akka는 "누수 추상화(leaky abstraction)"를 만드는 대신, 이러한 실패 가능성(fallibility)을 명시적으로 받아들입니다. 송신자가 성공을 알 수 있는 유일하게 신뢰할 수 있는 방법은, 수신자로부터 비즈니스 수준의 확인 응답 메시지(business-level acknowledgment message)를 받는 것입니다.
