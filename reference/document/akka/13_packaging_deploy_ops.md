...
> 이 문서는 Akka 공식 문서의 "Packaging", "Deploying", "Operating" 섹션을 한국어로 번역한 것입니다.
...
이후에는 sbt-native-packager 공식 문서의 [`JavaAppPackaging`](https://www.scala-sbt.org/sbt-native-packager/archetypes/java_app/index.html) 안내를 따르면 됩니다.
...
두 경우 모두 `reference.conf`(그리고 `version.conf`)를 이어붙이도록(append) 설정하여, 팻 jar 안에 포함된 여러 Akka 모듈의 기본 설정이 올바르게 병합되도록 합니다.
...
Akka 클러스터를 컨테이너 오케스트레이션(container orchestration) 환경에 배포하는 구체적인 방법은 "Deploying Akka Cluster to Kubernetes(쿠버네티스에 Akka 클러스터 배포하기)" 가이드를 참고하는 것이 권장됩니다.
...
- **롤링 업데이트(Rolling Updates)**: 무중단 배포를 위해 Akka Management에서 제공하는 "Kubernetes Rolling Updates" 기능과 `app-version` 관련 기능을 활성화하는 것이 권장됩니다.
...
즉, 노드의 물리적 CPU 수(16)나 요청 값(2)에 그대로 의존하지 않고, 애플리케이션 특성에 맞는 적절한 값(여기서는 4)을 명시적으로 지정하는 방식입니다.
...
이 절은 Akka 클러스터를 실제로 운영(operating)하는 절차를 다루며, 패키징, 배포, 롤링 업데이트 가이드를 보완합니다.
...
여러 Akka 프로젝트와 빌드 도구 설정을 포함한 종합적인 샘플은 [Akka Edge Documentation](https://doc.akka.io/libraries/akka-edge/current/lightweight-deployments.html#graalvm-native-image)에서 확인할 수 있으며, 빌드 도구(sbt, Maven, Gradle)별 구체적인 네이티브 이미지 빌드 설정도 이 문서를 참고하는 것이 권장됩니다.
...
로깅에 `akka-slf4j`를 사용하는 경우, "선택한 구체 로거(concrete logger)가 추가 설정을 필요로 할 가능성이 매우 높습니다."

Akka는 특정 로거를 강제하지는 않지만, 샘플에서는 `logback-classic`이 흔하게 사용되며, Akka는 이에 대한 리플렉션 메타데이터를 제공합니다. 다만 logback을 사용하는 프로젝트는 네이티브 이미지 플래그 `--initialize-at-build-time=ch.qos.logback`이 필요합니다.
