...
> 이 문서는 Akka 공식 문서의 "Akka Guide Tutorial" 섹션을 한국어로 번역한 것입니다.
...
이 페이지는 Java DSL과 Scala DSL이 함께 번들로 제공된다는 점을 언급합니다. 또한 임포트(import) 제안 충돌을 방지하기 위해 Eclipse 및 IntelliJ에 대한 IDE 설정을 권장합니다.

> 이 번역 문서에서는 스타일 지침에 따라 본문 예제로 **Scala 코드**를 보존하여 제시합니다.
...
Akka를 사용하면 액터 시스템 인프라를 구축하고 기본적인 동작 관리를 위한 저수준(low-level) 코드를 작성하는 부담에서 벗어날 수 있습니다. 이 장점을 충분히 이해하기 위해서는, 여러분이 코드에서 개발하는 액터들과 Akka가 내부적으로 생성하고 유지 관리하는 액터들 사이의 관계, 그리고 액터 생명주기(actor lifecycle)와 장애(failure)가 어떻게 처리되는지를 이해하는 것이 중요합니다.
...
이제 액터 계층 구조의 형태를 이해했으니, "왜 이런 계층 구조가 필요한가? 어떤 목적을 수행하는가?"라는 질문을 할 수 있습니다.

계층 구조는 액터 생명주기를 안전하게 감독(oversee)하는 중요한 기능을 수행합니다. 다음에 이 측면을 살펴보고, 이 이해가 어떻게 코드 품질을 향상시킬 수 있는지 탐구해 보겠습니다.

### 첫 번째 액터 (The First Actor)

> 이 절은 위 실험 코드에서 다룬 첫 번째 액터 생성과 자식 생성에 대한 내용을 포함합니다. (위 `PrintMyActorRefActor` / `Main` 예제 참조)

### 액터 생명주기 (The Actor Lifecycle)

액터는 생성될 때 존재하게 되며, 이후 사용자의 요청에 따라 정지(halt)될 수 있습니다. 액터가 정지되면, 그 모든 자식들도 재귀적으로(recursively) 함께 정지됩니다. 이 특성은 리소스 정리(resource cleanup)를 현저하게 간소화하며, 닫히지 않은 소켓(socket)이나 파일에서 비롯되는 리소스 누수(resource leak)를 방지하는 데 도움을 줍니다. 실제로 저수준 멀티스레드 프로그래밍을 다룰 때, 다양한 동시성 리소스의 생명주기를 관리하는 것은 종종 간과되는 어려운 과제입니다.
...
장애 발생 후, 감독되는 액터는 `PreRestart` 시그널을 받으며, 이는 재시작 작업이 시작되기 전에 처리됩니다. 그 후 액터가 재시작되고, 장애 정보는 감독 전략으로 전달되어 대응 방식을 결정합니다.
...
액터 계층 구조와 동작을 이해하고 나면, 최상위 IoT 시스템 구성 요소를 어떻게 액터로 매핑할 것인가 하는 질문으로 이어집니다. 사용자 가디언(user guardian)은 애플리케이션 전체를 나타내는 액터로서, IoT 시스템에서 하나의 최상위 액터입니다. 디바이스와 대시보드를 관리하는 구성 요소들은 이 액터의 자식이 되어, 예제 사용 사례 아키텍처를 트리 구조(tree structure)로 리팩터링할 수 있게 합니다.
...
이전 주제들에서는 액터 시스템을 **거시적으로(in the large)** 바라보는 방법, 즉 구성 요소들을 어떻게 표현해야 하는지, 액터들을 계층 구조 안에 어떻게 배치해야 하는지를 설명했습니다. 이번 파트에서는 디바이스 액터를 구현함으로써 액터를 **미시적으로(in the small)** 살펴봅니다.
...
이 아키텍처는 다음을 제공합니다. 그룹 내에서의 장애 격리(isolation of failures), 한 그룹에 속한 디바이스 조회의 간소화, 각 그룹이 동시에 실행되는 전용 액터를 가지므로 병렬성(parallelism) 증가, 개별 디바이스 장애의 격리, 그리고 네트워크 연결이 개별 디바이스 액터와 직접 통신할 수 있게 함으로써 온도 측정값 수집에서의 병렬성 증가입니다.
...
이 문서는 그룹 내 모든 디바이스 액터를 조회하는 것을 목표로 하는 **스캐터-개더 조회 패턴(scatter-gather query pattern)**을 구현하는 방법을 설명합니다. 이것은 핵심적인 과제를 다룹니다. 즉, **그룹의 멤버십은 동적(dynamic)이라는 점입니다. 각 센서 디바이스는 언제든지 정지할 수 있는 액터로 표현됩니다.**
...
조회 로직을 그룹 액터에 내장하는 대신, 이 설계는 **단일 조회(single query)를 나타내고, 그룹 액터를 대신하여 조회를 완료하는 데 필요한 작업을 수행하는 액터**를 생성합니다. 이 접근 방식은 그룹 디바이스 액터를 단순하게 유지하고, 조회 기능을 격리하여(in isolation) 더 잘 테스트할 수 있게 해 줍니다.
