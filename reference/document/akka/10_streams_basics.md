...
> 이 문서는 Akka 공식 문서의 "Streams" 섹션을 한국어로 번역한 것입니다.
...
또한, 처리해야 할 데이터셋이 전체를 한 번에 메모리에 올려 처리하기에는 너무 큰 경우가 흔하며, 이런 경우 데이터를 클러스터(cluster) 전반에 걸쳐 분산 처리해야 할 수도 있습니다. 그럼에도 불구하고 데이터를 스트림처럼 다루는 것은 합리적인 접근 방식입니다.

액터(actor)는 메시지 스트림을 처리해 지식(knowledge)이나 정보를 전달하는 데 사용될 수 있지만, 안정적이고 견고한 액터 간 스트리밍을 구현하는 일은 지루하고 오류가 발생하기 쉽습니다(error-prone). 개발자는 버퍼(buffer)나 메일박스(mailbox)가 넘치지 않도록 막아야 하고, 메시지가 유실될 가능성을 재전송(retransmission)을 통해 관리해야 합니다. 데이터를 빠뜨리면 수신된 데이터에 빈틈(gap)이 생기기 때문입니다.
...
소스를 실행하려면 소비 함수(consumer function)로 소스를 보완(complement)하는 `runForeach()`를 호출합니다. 이 메서드는 `Future[Done]`을 반환합니다.
...
Akka Streams의 두드러진 특징 중 하나는, 소스(Source)뿐만 아니라 다른 모든 요소들도 청사진(blueprint)처럼 재사용할 수 있다는 점입니다. 예를 들어, 문자열을 받아 줄바꿈을 붙여 파일에 쓰는 싱크를 다음과 같이 만들 수 있습니다.
...
복잡하고 비선형적인 스트림 구조에는 `GraphDSL`이 가장 편리한 API를 제공합니다. 소스를 `Broadcast`를 통해 여러 싱크에 연결할 때는 엣지(edge) 연산자 `~>`를 사용합니다.
...
사용자는 어떠한 스트림 처리 토폴로지(topology)든 표현할 수 있는 도구를 제공받으면서, 동시에 다음과 같은 본질적인 측면들을 모델링할 수 있습니다. 백프레셔(back-pressure), 버퍼링(buffering), 변환(transformation), 그리고 실패 복구(failure recovery)가 그것입니다. 그리고 구축된 모든 구성물은 더 큰 맥락(larger context) 안에서 재사용 가능한 상태로 남아 있습니다.
...
따라서 마무리(finalization)가 필요한 객체의 정리(cleanup)는 Akka Streams의 기능 바깥에서 사용자가 직접 처리해야 합니다.
...
Akka는 구현하기 어려운 Reactive Streams 조각들 대신, 더 단순한 추상화인 `GraphStage`와 연산자(operator)를 사용하도록 권장합니다. 상호 운용은 `Sink.asPublisher`나 `fromSubscriber` 같은 메서드를 통해 여전히 사용할 수 있습니다.
...
소스, 싱크, 그리고 여러 연산자를 모두 연결하여 `RunnableGraph`를 구성한 이후라 하더라도, 그것이 구체화(materialize)되기 전까지는 어떤 데이터도 흐르지 않습니다.
...
구체화(materialization)는 스트림 기술(RunnableGraph)을 가져와서 그것이 실행되기 위해 필요한 모든 자원을 할당(allocate)하는 과정입니다. 이는 일반적으로 처리를 구동하는 액터(Actor)를 시작하는 것을 포함하지만, 그것에만 국한되지 않습니다. 파일이나 소켓 연결(socket connection)을 여는 것 등을 의미할 수도 있습니다.
...
병렬 처리(parallel processing)를 위해서는 비동기 경계(asynchronous boundary)를 삽입해야 하며, 이는 `Attributes.asyncBoundary` 또는 `.async` 메서드를 통해 가능합니다.
...
시스템 범위(system-scoped)의 구체화기를 명시적으로 전달하면, 스트림이 그것을 생성한 액터보다 더 오래 살아남게(outlive) 할 수 있습니다. 주의: 액터 내부에서 새로운 구체화기를 생성하면 자원 누수(resource leak) 위험이 있습니다. 대신 구체화기를 전달받거나 액터 컨텍스트(actor context)를 통해 생성하십시오.
...
`GraphDSL`은 복잡한 그래프를 위한 고급 합성(advanced composition)을 제공합니다. 두 DSL(선형 DSL과 `GraphDSL`)의 차이는 표면적인 것일 뿐이며, 그들이 다루는 개념(concept)은 모든 DSL에 걸쳐 동일합니다(uniform).
...
이 섹션은 Akka Streams에서 버퍼(buffer)가 어떻게 동작하는지를 다룹니다. 특히 상류(upstream)와 하류(downstream)의 처리율(rate)이 서로 다르거나, 처리량(throughput)에 급격한 스파이크(spike)가 발생하는 경우를 중점적으로 다룹니다.
...
공식 문서는 다음과 같이 설명합니다. 이는 이 플로우의 출력이, 생산자가 충분히 빠르면 드리프트(drift)를 0으로 보고하고, 그렇지 않으면 더 큰 드리프트를 보고하게 됨을 의미합니다.
