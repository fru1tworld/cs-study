...
> 이 문서는 Akka 공식 문서의 "Persistence Query", "Persistence Plugins", "Durable State" 섹션을 한국어로 번역한 것입니다.
...
이 API는 저널 구현체가 자신의 최고의 기능들을 노출할 수 있도록 의도적으로 느슨하게(loosely) 명세되어 있습니다. 각 읽기 저널(read journal)은 자신이 지원하는 쿼리 타입을 명시적으로 문서화해야 합니다.
...
높은 이벤트 볼륨이나 복원력(resilience) 요구 사항을 위해, "클러스터 샤딩(Cluster Sharding)을 이벤트 태깅과 함께 사용하는 것은 이벤트를 클러스터 전반에 샤딩(shard)하는 데 훌륭한 적합성을 갖는다." 이를 통해 수평 확장과 노드 장애로부터의 복구가 가능해집니다.
...
"반응형 데이터베이스 드라이버(Reactive database drivers, R2DBC)는 PostgreSQL, H2(최소한의 인-프로세스 메모리 또는 파일 기반 데이터베이스로서), Yugabyte 같은 관계형 데이터베이스를 지원한다." 이 구현체는 "Akka Persistence의 최신 기능 추가 사항을 지원하며, 일반적으로 JDBC 기반 플러그인보다 권장된다"는 점에서 추천됩니다.
...
스키마 진화(schema evolution)는 이벤트 소싱을 사용하는 장기간 운영되는 프로젝트에서 매우 중요해집니다. 비즈니스 요구 사항과 도메인에 대한 이해가 발전함에 따라, 영속화된 이벤트의 데이터 구조도 그에 맞춰 적응해야 합니다. 스키마 적응을 요구하는 성공적인 프로젝트는 활발한 사용과 지속적인 비즈니스 발전을 의미합니다.
...
이벤트 소싱은 스키마 진화에 대해 특정한 도전 과제를 만들어냅니다:

- 시스템은 대규모 마이그레이션을 요구하지 않고 계속 운영되어야 합니다.
- 시스템은 스토리지로부터 오래된 이벤트를 읽어와 현재 형식으로 애플리케이션 로직에 제시해야 합니다.
- 이벤트는 복구(recovery)나 쿼리 도중 최신 버전으로 투명하게(transparently) 승격(promote)되어야 합니다.
- 비즈니스 로직은 여러 이벤트 버전의 존재를 알 필요가 없어야 합니다.
...
직렬화 형식(serialization format) 선택은 어떤 진화가 쉬운지 또는 어려운지에 상당한 영향을 미칩니다. 중요한 요소는 다음과 같습니다:
...
**중요한 노트:** "직렬화(Serialization)는 Akka Persistence 자체에 의해 자동으로 처리되지 않는다." 프레임워크는 직렬화기를 제공하지만, 그 사용 여부는 저널 플러그인이 결정합니다. 일부 저널은 Akka Serialization 대신 자신의 데이터 저장소에 고유한 형식(JSON 등)을 사용할 수 있습니다.

#### 직렬화 엔벨로프(Envelope) 구조

프레임워크는 사용자 페이로드(payload)를 영속성 메타데이터를 포함하는 엔벨로프(envelope)로 감쌉니다. 저널이 래퍼(wrapper) 타입에 대해 제공된 Protobuf 직렬화기를 사용하는 경우, 사용자 이벤트 페이로드는 설정된 직렬화기로 직렬화됩니다(지정하지 않으면 기본적으로 Java 직렬화).
...
"클러스터 샤딩(Cluster Sharding)은 영속 액터를 클러스터 전반에 분산시키고 id로 주소를 지정(address)하는 데 훌륭한 적합성을 갖는다." 이는 `persistenceId` 당 오직 하나의 활성 엔티티 인스턴스만 존재하도록 보장하며, 사용 가능한 노드에서 액터를 신속하게 재시작함으로써 노드 장애 전반에 걸친 복원력을 개선합니다.
...
영속 액터는 명령 핸들러로부터 새로운 동작(behavior)을 반환할 수 없습니다. 동작은 복구 도중 신중하게 재구성되어야 하는 액터 상태를 구성하기 때문입니다. 대신, 현재 상태에 따라 서로 다른 함수가 명령을 처리하는 상태 기반 명령 처리(state-based command handling)를 구현하십시오. 이는 유한 상태 기계(finite state machine)에 유용합니다.

#### 가변 상태 고려 사항(Mutable State Considerations)

가변 상태(mutable state)의 경우, 실패 후 재시작 시 상태 인스턴스가 재생성되도록 보장하기 위해 `emptyStateFactory: () => State` 매개변수를 받는 `DurableStateBehavior.withMutableState` 또는 관련 팩토리를 사용하십시오.

---

## 5. 영속 상태 스타일 가이드(Durable State Style)

이 가이드는 타입드 Akka 애플리케이션에서 명령 처리와 상태 관리를 어떻게 구조화하는지를 보여주며, 영속 상태(Durable State) 엔티티를 위한 스타일링 접근법을 다룹니다.
...
이 패턴은 컴파일러 검증을 통해 "이벤트 소스 동작 핸들러 메서드에서 모든 종류의 명령과 이벤트를 처리했음"을 보장합니다.
...
함수형(functional) 위임 패턴과 객체지향(object-oriented) 위임 패턴 사이의 선택은 애플리케이션 복잡성과 팀 선호도에 따라 달라지며, 각각은 코드 조직화와 테스트 용이성(testability)에 대해 고유한 이점을 제공합니다.
...
이 페이지는 영속 상태 동작(Durable State Behaviors)에 대한 쿼리 인터페이스를 비동기 스트림(asynchronous stream)을 통해 제공하는 영속 상태(Durable State)용 Akka Persistence Query를 문서화합니다. 이는 CQRS 아키텍처 패턴에서 읽기 측(read side)을 가능하게 합니다.
...
**노트:** 의존성은 https://account.akka.io/token 에 따른 보안 토큰화 URL 접근(secure, tokenized URL access)을 요구합니다.

---

### 6.3 소개

이 쿼리 모듈은 비동기 스트림을 사용하여 영속 상태 동작(Durable State Behaviors)에 대한 인터페이스를 제공합니다. 이는 CQRS 패턴에서 읽기 측을 구현하는 것을 지원하며, (Akka persistence를 통한) 쓰기와 쿼리를 분리합니다.
