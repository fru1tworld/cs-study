...
> 이 문서는 `man systemd-journald.service` 의 내용을 한국어로 정리한 것입니다.
...
`/dev/kmsg` 에서 읽어 들임. `dmesg` 와 같은 데이터지만 journal에서는 시간 동기화·메타데이터가 더 풍부합니다.
...
각 파일은 자체 인덱스를 가진 바이너리 형식이며 mmap 기반으로 빠른 접근 가능합니다.
...
대부분의 현대 systemd 시스템 기본 설정. rsyslog/syslog-ng 미설치.
