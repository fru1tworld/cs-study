...
> 이 문서는 Scala 3(3.7.0) 공식 마이그레이션 가이드를 한국어로 번역/정리한 것입니다.
> 원본: https://docs.scala-lang.org/scala3/guides/migration/compatibility-intro.html
...
Scala 3는 "Scala 생태계에서 호환성(compatibility)이라는 측면에서 판도를 바꾸는(game changer) 존재"이며, 이는 모든 Scala 프로그래머의 일상적인 개발 경험을 크게 향상시킬 것이라고 공식 가이드는 설명합니다.

Scala 3는 **언어의 핵심 기반(core foundations)을 완전히 재설계(complete redesign)** 한 결과물입니다. 그럼에도 불구하고, 실제 마이그레이션의 난이도는 **Scala 2 버전들 사이를 이동하는 것과 비슷한 수준**이며, 오히려 상호운용성(interoperability) 기능 덕분에 일부 측면에서는 더 개선되었습니다.
...
이 문서는 마이그레이션을 위해 알아야 할 호환성의 차원, 알려진 비호환성 목록, 그리고 이를 처리하기 위한 컴파일러 마이그레이션 모드와 도구를 다룹니다.
...
클래스패스 수준 호환성(classpath compatibility)이란, **서로 다른 컴파일러로 빌드된 모듈/라이브러리를 같은 클래스패스(classpath)에 놓고 함께 사용할 수 있는지**를 다룹니다. 이는 시그니처(signature)를 읽어 타입 검사(type check)를 수행하는 능력에 달려 있습니다.
...
- 언피클러는 커뮤니티 차원에서 광범위하게 테스트되었으며 **프로덕션(production) 사용에 안전**한 것으로 간주됩니다.
...
- Scala 3는 Scala 2와 동일한 JVM 바이트코드(bytecode) 모델로 컴파일되며, **프로덕션 배포(production deployment)에 안전**합니다.
- 런타임 동작과 성능 특성은 Scala 2.13과 사실상 동등한 수준이며, 동일한 표준 라이브러리를 공유하므로 런타임 의미(runtime semantics)도 일관됩니다.
...
- Scala 2의 매크로 시스템(`scala.reflect` / `def macro` 기반)은 **Scala 3에서 더 이상 동작하지 않으며 완전히 교체**되었습니다.
...
- 따라서 Scala 2 매크로를 사용하는 라이브러리는 **Scala 3용으로 매크로를 재구현(reimplement)** 해야 합니다.
...
`scalac`는 Scala 컴파일러의 실행 파일이며 GitHub에서 다운로드하거나 Coursier로 설치할 수 있습니다.
...
이를 통해 개발자는 컴파일을 중단시키지 않고도 더 이상 사용되지 않는(deprecated) 코드를 식별할 수 있습니다.
...
이 옵션들은 마이그레이션에 국한되지 않고 Scala 3 학습 전반(general)에 도움이 됩니다.

---

## 9. Scala 3의 주요 신기능 요약

마이그레이션 시 Scala 3가 제공하는 새로운 기능을 이해하면, 옛 구문을 어떤 새 구문으로 대체할지 판단하는 데 도움이 됩니다. (출처: Scala 3 Book - Scala Features)
