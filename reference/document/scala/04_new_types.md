...
> 이 문서는 Scala 3(3.7.0) 공식 문서의 "New Types" 섹션을 한국어로 번역한 것입니다.
> 원본: https://docs.scala-lang.org/scala3/reference/new-types/
...
각 주제는 더 넓은 Scala 3 타입 시스템 프레임워크 안에서 상세한 설명과 사용 예제를 제공합니다.
...
Scala 3는 타입에 대한 `&` 연산자를 사용하여 교집합 타입(intersection type)을 도입합니다. 문서에 따르면, 타입 `S & T`는 **"동시에 타입 `S`이면서 타입 `T`인 값들을 표현"**합니다.

`&` 연산자가 도입됨에 따라, 교집합 타입은 (서로 다른 위치에 동일한 멤버를 갖는 여러 클래스를 혼합하는 경우 종종 발생하던) 컴파일 타임 클래스 합성(class composition)의 일부 제약을 대체합니다.
...
**공유 멤버 해소(Shared Member Resolution)**: 두 타입이 동일한 멤버를 정의하는 경우, 컴파일러는 해당 멤버 타입들의 교집합을 사용합니다. 문서는 다음과 같은 예제로 이를 설명합니다. `A`와 `B` 모두에 나타나는 `children` 메서드는 결과적으로 `List[A] & List[B]` 타입을 갖게 되며, 이는 공변성(covariance)으로 인해 `List[A & B]`로 단순화됩니다.
...
컴파일러는 교집합 타입의 멤버에 대한 구현을 자동으로 생성하지 **않습니다**. 대신, 여러 타입을 확장(extends)하는 구체 클래스(concrete class)가 교집합의 타입 요구 사항에 부합하는 구현을 명시적으로 제공해야 합니다. 레퍼런스가 명시하듯이, 값이 **구성(constructed)되는** 시점에서 상속받은 모든 멤버가 올바르게 정의되었는지를 보장해야 합니다.
...
타입 람다의 의미론(semantics)과 구현에 대한 더 깊은 기술적 이해가 필요한 경우, 명세 문서(specification document)에서 더 자세한 내용을 확인할 수 있습니다.

> **참고**: 타입 람다는 레퍼런스 문서의 "New Types" 섹션 안에서 합집합 타입(Union Types)과 매치 타입(Match Types) 사이에 위치합니다.
...
문서는 **"의존 함수 타입이란 결과가 함수의 파라미터에 의존하는 함수 타입"**이라고 설명합니다. 이전에는 Scala가 의존 메서드(dependent method)는 지원했지만, 이를 함수 값(function value)으로 표현하는 문법이 없었습니다. Scala 3는 이 간극을 메웁니다.
...
다형 함수 타입(polymorphic function type)은 함수가 타입 파라미터(type parameter)를 받을 수 있게 하여, 다형 메서드(polymorphic method)와 일급 함수 값(first-class function value) 사이의 간극을 메웁니다. 문서가 명시하듯이, **"다형 함수 타입이란 타입 파라미터를 받는 함수 타입"**입니다.
...
다형 함수 타입은 타입 람다(type lambda)와 근본적으로 다릅니다. 문서에 명시된 대로, **"타입 람다는 타입(types) 안에서 적용되는 반면, 다형 함수는 항(terms) 안에서 적용"**됩니다. 타입 람다는 타입 표현식 안에서 타입 수준(type level)으로 동작하는 반면, 다형 함수는 메서드 본문 안에서 런타임(runtime)에 호출되며 `bar[Int]`처럼 명시적인 타입 인자(type argument)를 받습니다.
