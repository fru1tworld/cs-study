...
> 이 문서는 libbpf 라이브러리의 사용법을 정리한 것입니다.
...
BPF 프로그램을 로드·관리하는 표준 C 라이브러리. 커널 BPF 서브시스템 메인테이너들이 같이 관리합니다.
...
현재는 BCC 자체 일부 도구도 내부적으로 libbpf로 마이그레이션 중 (`/usr/share/bcc/libbpf-tools/`).

---

## Skeleton

`bpftool gen skeleton` 으로 생성되는 헤더. BPF 오브젝트의 사용자 공간 인터페이스를 자동으로 만들어줌.
...
`bpftool` 은 BPF 객체를 명령줄에서 다루는 도구. libbpf와 같은 코드를 일부 공유.
...
ebpf-go는 Pure Go 구현 (C 의존성 없음). 가장 인기.
...
여전히 활발히 사용. CO-RE는 일부만 지원.

#### bpfd

(실험적) BPF daemon. CRD 기반 attach.
