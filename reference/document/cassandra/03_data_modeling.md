...
> 이 문서는 Apache Cassandra 공식 문서의 "Data Modeling" 섹션을 한국어로 번역한 것입니다.
...
즉, 관계형 데이터베이스에서처럼 도메인 엔티티(entity)를 먼저 모델링하고 나중에 쿼리를 작성하는 방식이 아니라, 애플리케이션이 수행할 쿼리를 먼저 식별한 뒤 그 쿼리를 지원하기 위한 테이블을 설계합니다. 이를 **쿼리 우선(query-first)** 모델링이라고 합니다.

### 관계형 모델과의 핵심 차이

전통적인 데이터베이스가 외래 키(foreign key)를 사용하여 연관 테이블에 걸쳐 데이터를 정규화(normalize)하는 것과 달리, Cassandra는 여러 테이블에 데이터를 중복 저장하는 **비정규화(denormalization)** 를 사용합니다. 이 설계 선택은 저장 효율성보다 읽기 성능(read performance)을 우선시합니다. Cassandra는 "관계형 데이터베이스를 위한 관계형 데이터 모델링을 지원하지 않습니다."
...
개념적 데이터 모델(conceptual data model)은 도메인(domain)에 대한 추상적인 뷰입니다. 이는 **기술 독립적(technology independent)** 이며, 특정 데이터베이스 시스템에 종속되지 않습니다. 공식 문서는 Cassandra의 분산 해시테이블(distributed hashtable) 구조로 매핑할 관계형 도메인 모델을 제시하면서 개념적 데이터 모델링을 소개합니다.
...
문서는 "모두에게 익숙한 도메인을 선택"함으로써 도메인의 복잡성보다 Cassandra의 동작 메커니즘에 집중할 수 있다고 강조합니다. 호텔 예약 예제는 데이터 구조와 설계 패턴을 보여주기에 충분한 복잡성을 가지면서도 이해하기 쉽다는 균형을 갖추고 있습니다.
...
논리적 데이터 모델링(logical data modeling) 단계에서는 애플리케이션 쿼리를 기반으로 테이블을 설계합니다. 문서에 따르면 "개념적 모델의 엔티티와 관계를 포착하면서, 각 쿼리마다 하나의 테이블을 포함하는 논리적 모델을 생성"해야 합니다.
...
**기본 키 설계(Primary Key Design)**: 기본 키는 "각 파티션에 얼마나 많은 데이터가 저장될지, 그리고 그 데이터가 디스크에 어떻게 조직될지"를 결정합니다. 이는 다시 Cassandra가 읽기를 처리하는 속도에 영향을 주므로, 기본 키 설계는 매우 중요합니다. 기본 키는 다음으로 구성됩니다.
...
물리적 데이터 모델링(physical data modeling) 단계는 논리적 모델 개발 이후에 진행됩니다. 문서에 따르면 "논리적 데이터 모델이 정의되면, 물리적 모델을 만드는 것은 비교적 간단한 과정"입니다.
...
성능 문제를 방지하기 위해 파티션 크기를 평가해야 합니다. Cassandra의 하드 리밋(hard limit)은 파티션당 20억(2 billion) 셀(cell)이지만, "그 한계에 도달하기 훨씬 전에 성능 문제에 부딪힐 가능성이 높습니다."
...
- **Npk** = 파티션 키 컬럼의 개수 (number of primary key columns)
...
문서는 이 방법이 "추가적인 애플리케이션 로직"을 필요로 하지만, 파티션이 실용적인 한계를 초과하는 것을 방지한다고 언급합니다.
