...
> 이 문서는 Apache Cassandra 공식 문서의 "Getting Started" 섹션을 한국어로 번역한 것입니다.
> 원본: https://cassandra.apache.org/doc/latest/cassandra/getting-started/

이 섹션은 Apache Cassandra를 처음 사용하는 방법을 다루며, Cassandra를 처음 접한다면 가장 먼저 읽어야 할 내용입니다. 설치, 설정, 데이터 삽입 및 조회, 클라이언트 드라이버(driver), 프로덕션(production) 권장 사항을 차례로 설명합니다.
...
가장 빠르게 Cassandra를 사용해 볼 수 있는 방법입니다. 사전에 Docker(macOS의 Docker Desktop, Windows의 Docker Desktop, 또는 Linux의 docker)가 설치되어 있어야 합니다.
...
tarball 설치는 가장 간단한 방법 중 하나입니다. tarball은 모든 내용을 하나의 위치에 풀어 놓으며, 바이너리와 설정 파일이 하위 디렉터리에 위치합니다. 또한 root 권한이 필요하지 않습니다.
...
다음과 같은 로그가 나타나면 클라이언트 연결을 받을 준비가 된 것입니다.
...
`Status` 열이 `UN`(Up/Normal)으로 표시되면 노드가 정상적으로 동작하고 있는 것입니다.
...
Cassandra의 동작은 여러 설정 파일을 통해 제어합니다. 이 파일들의 위치는 설치 방식에 따라 다릅니다.
...
- **`listen_address`**: 노드 간 통신을 가능하게 하는 IP 주소이며, 기본값은 localhost입니다.
- **`listen_interface`**: `listen_address` 대신 네트워크 인터페이스를 지정하는 대안 속성입니다.
...
logback 로거는 기본적으로 다음과 같이 동작합니다.

- `system.log`에 INFO 레벨로 기록
- `debug.log`에 DEBUG 레벨로 기록
- 포그라운드(foreground)로 실행할 경우 콘솔에 INFO 레벨로 출력

로깅은 `logback.xml`을 수정하거나 `nodetool setlogginglevel` 명령으로 변경할 수 있습니다.

---

## 데이터 삽입 및 조회

Cassandra와 상호작용하는 기본 API는 **CQL(Cassandra Query Language)**입니다. CQL은 다음 세 가지 방법으로 사용할 수 있습니다.
...
`cqlsh`는 모든 Cassandra 배포판에 포함된 명령줄 도구로, `bin` 디렉터리에 위치합니다. 노드에 접속하려면 다음과 같이 실행합니다.

```bash
$ bin/cqlsh localhost
```

노드를 지정하지 않으면 기본값으로 `localhost`에 접속합니다. 접속 후에는 CQL 명령을 대화형(interactive)으로 실행할 수 있습니다.
...
위 예시는 `system.local` 테이블에서 클러스터 이름(`cluster_name`)과 수신 주소(`listen_address`)를 조회하여 클러스터 메타데이터를 확인하는 모습입니다.

### 그 밖의 자료

`CREATE KEYSPACE`, `CREATE TABLE`, `INSERT`, `SELECT` 등 CQL의 전체 문법은 공식 문서의 [CQL 레퍼런스 섹션](https://cassandra.apache.org/doc/latest/cassandra/developing/cql/ddl.html)을 참고하십시오. 또한 사용하려는 프로그래밍 언어에 맞는 클라이언트 드라이버의 문서도 함께 확인하는 것이 좋습니다.

---

## 클라이언트 드라이버

커뮤니티에서 다양한 언어의 클라이언트 드라이버를 제공합니다. 드라이버를 선택하기 전에 해당 드라이버가 지원하는 Cassandra 버전과 기능을 반드시 확인해야 합니다.
...
프로덕션 클러스터를 구축할 때는 단일 노드 개발 환경과 다른 여러 사항을 고려해야 합니다. 아래는 공식 문서가 권장하는 핵심 사항들입니다.

### 토큰(Tokens)과 가상 노드(vnodes)

토큰 개수(`num_tokens`)는 클러스터의 탄력성(elasticity)과 가용성(availability) 사이의 균형을 맞추는 데 중요합니다. `num_tokens`는 `cassandra.yaml`에서 설정합니다.

- **1 token**: 가용성과 클러스터 최대 크기가 최대이고 피어(peer) 수가 가장 적지만, 확장이 유연하지 못합니다.
- **4 tokens**: 30개를 초과하는 노드 클러스터에 권장됩니다. 균형을 맞추기 위해 약 20%의 추가 노드가 필요합니다.
- **8 tokens**: 성능에 거의 영향을 주지 않으면서 워크로드를 분산합니다.
- **16 tokens**: 탄력적인 클러스터에 가장 적합하지만, 50개 이상의 노드 클러스터에서는 문제가 될 수 있습니다.

균형 잡힌 토큰 할당을 위해 `allocate_tokens_for_local_replication_factor`를 설정하는 것이 중요합니다.

### 리드 어헤드(Read Ahead)

리드 어헤드는 하드웨어에 따라 디스크 I/O 성능을 최적화하는 방식이 다릅니다.

- **회전식 디스크(spinning disk)**: 64KB로 시작하는 것을 권장합니다.
- **SSD**: 4KB로 시작하는 것을 권장합니다.

파티션(partition)이 작거나 단일 파티션 키(single-partition-key) 테이블에서는 리드 어헤드가 오히려 역효과를 내어, 키/값(key/value) 워크로드에서 지연 시간(latency)과 처리량(throughput) 성능을 최대 5배까지 저하시킬 수 있습니다.

리눅스의 `blockdev` 도구로 조정하며, 단위는 512바이트 섹터(sector)입니다.

### 압축(Compression)

기본 압축 알고리즘은 `LZ4Compressor`입니다. `chunk_length_in_kb` 파라미터(Cassandra 4.0부터 기본값 16KB)는 성능에 큰 영향을 미칩니다. 청크(chunk)가 지나치게 크면 읽기 증폭(read amplification)이 발생합니다.

### 컴팩션(Compaction)

컴팩션 전략(strategy)은 워크로드 특성에 따라 선택합니다. 하나의 클러스터 안에서도 테이블마다 서로 다른 컴팩션 전략을 사용할 수 있습니다.

### 암호화 및 보안(Encryption & Security)

프로덕션 클러스터를 구축할 때 노드 간(peer-to-peer) 암호화와 클라이언트-서버(client-server) 암호화를 함께 설정하는 것이 훨씬 좋습니다. 배포 이후에 이를 적용하는 것은 상당한 위험을 동반합니다.

### 복제 전략(Replication Strategy)

프로덕션 클러스터는 반드시 `NetworkTopologyStrategy`를 사용해야 하며, `SimpleStrategy`는 사용하지 않아야 합니다. `NetworkTopologyStrategy`는 다중 랙(multi-rack) 및 다중 데이터센터(multi-datacenter) 구성을 가능하게 합니다.

### 스니치(Snitch) 설정

클러스터가 프로비저닝(provisioning)된 이후에 랙을 올바르게 설정하거나 변경하는 작업은 지원되지 않는 과정입니다. 따라서 처음부터 올바르게 구성해야 합니다.

- **하이브리드(hybrid) 환경**: `GossipingPropertyFileSnitch` 권장
...
- [Installing Cassandra](https://cassandra.apache.org/doc/latest/cassandra/getting-started/installing.html)
