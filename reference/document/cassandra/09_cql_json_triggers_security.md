...
> 이 문서는 Apache Cassandra 공식 문서의 CQL "JSON", "Triggers", "Security" 섹션을 한국어로 번역한 것입니다.
...
Cassandra 2.2부터 `SELECT` 및 `INSERT` 문에 대한 JSON 지원이 도입되었습니다. 이 지원은 CQL API를 근본적으로 변경하지 않으며(예를 들어 스키마는 여전히 강제됩니다), 단지 JSON 문서를 편리하게 다룰 수 있는 수단을 제공할 뿐입니다.

---

### SELECT JSON

`SELECT` 문에서 `JSON` 키워드를 사용하면 각 행(row)을 하나의 JSON 인코딩된 맵(map)으로 반환할 수 있습니다. 결과 맵의 키(key)는 일반 결과 집합(result set)의 컬럼명(column name)과 동일합니다. 예를 들어 `SELECT JSON a, ttl(b) FROM ...` 와 같은 쿼리는 `"a"`와 `"ttl(b)"`를 키로 가지는 맵을 생성합니다.

한 가지 유의할 점이 있습니다. 대문자를 포함하는 대소문자 구분(case-sensitive) 컬럼명은 큰따옴표(double quote)로 둘러싸입니다. 이는 `INSERT JSON` 동작과의 대칭성(symmetry)을 위한 것입니다. 예를 들어 `SELECT JSON myColumn FROM ...` 는 따옴표가 이스케이프된(escaped) 맵 키 `"\"myColumn\""`를 결과로 만듭니다.
...
기본적으로 생략된(omitted) 컬럼은 `NULL`로 설정되어, 기존에 존재하던 값을 제거하고 툼스톤(tombstone)을 생성합니다. `DEFAULT UNSET`을 사용하면 생략된 컬럼에 대해 기존 값을 보존(preserve)합니다.

---

### Cassandra 데이터 타입의 JSON 인코딩

Cassandra는 가능한 경우 데이터 타입을 네이티브(native) JSON 표현으로 표현하고 수용합니다. 또한 단일 필드(single-field) 타입의 경우 CQL 리터럴(literal) 형식과 일치하는 문자열 표현도 수용합니다. 컬렉션(collection), 튜플(tuple), 사용자 정의 타입(user-defined type)과 같은 복합 타입(compound type)은 반드시 네이티브 JSON 컬렉션(맵 또는 리스트)이나 JSON 인코딩된 문자열 표현을 사용해야 합니다.
...
`fromJson()` 함수는 `INSERT JSON`과 유사하게 동작하지만, 단일 컬럼 값(single column value)에 대해 사용됩니다. 이 함수는 `INSERT` 문의 `VALUES` 절(clause)에서만 사용하거나, `UPDATE`, `DELETE`, `SELECT` 문에서 컬럼 값으로만 사용할 수 있습니다. 즉, `SELECT` 문의 셀렉션 절(selection clause)에서는 사용할 수 없습니다.

---

### toJson() 함수

`toJson()` 함수는 `SELECT JSON`과 유사하게 동작하지만, 개별 컬럼 값(individual column value)에 대해 사용됩니다. 이 함수는 `SELECT` 문의 셀렉션 절(selection clause)에서만 사용할 수 있습니다.

---

## 트리거(Triggers)

트리거(trigger)는 표준 식별자(identifier) 구문을 따르는 이름으로 식별됩니다. 트리거를 구성하는 실제 로직(logic)은 모든 Java(JVM) 언어로 작성할 수 있으며, 데이터베이스 외부에 존재합니다.

트리거 코드는 Cassandra 설치 디렉터리의 `lib/triggers` 하위 디렉터리(subdirectory)에 배치합니다. 이 코드는 클러스터(cluster) 시작 시점에 로드되며, 클러스터에 참여하는 모든 노드(node)에 존재합니다.

테이블에 정의된 트리거는 요청된 DML 문이 실행되기 전(before)에 발동(fire)됩니다. 이는 트랜잭션(transaction)의 원자성(atomicity)을 보장합니다.
...
CQL은 데이터베이스 역할(database role)을 사용하여 개별 사용자(user)와 사용자 그룹(group)을 모두 표현합니다. 역할의 구문은 다음과 같이 정의됩니다:
...
기본적으로 역할은 `LOGIN` 권한(privilege)이나 `SUPERUSER` 상태(status)를 가지지 않습니다.

데이터베이스 리소스(resource)에 대한 권한(permission)은 역할에 부여(grant)됩니다. 리소스 타입에는 키스페이스(keyspace), 테이블(table), 함수(function), 그리고 역할 자체가 포함됩니다. 역할은 다른 역할에 할당(assign)되어 계층적(hierarchical) 권한 구조를 형성할 수 있습니다. 이러한 계층 구조에서 권한과 `SUPERUSER` 상태는 상속(inherit)되지만, `LOGIN` 권한은 상속되지 않습니다.

역할이 `LOGIN` 권한을 가지면, 클라이언트는 접속(connection) 시 해당 역할로 식별(identify)될 수 있습니다. 접속이 유지되는 동안 클라이언트는 해당 역할에 할당된 모든 역할과 권한을 획득(acquire)합니다.

데이터베이스 역할 리소스에 대한 `CREATE` 권한을 가진 클라이언트만이 `CREATE ROLE` 요청을 발행(issue)할 수 있습니다(단, 클라이언트가 `SUPERUSER`인 경우는 예외). Cassandra의 역할 관리(role management)는 플러그형(pluggable)이며, 커스텀(custom) 구현체는 위에 나열된 옵션의 일부만 지원할 수도 있습니다.

역할명이 영숫자(alphanumeric)가 아닌 문자를 포함하는 경우 따옴표로 묶어야 합니다.

##### 내부 인증을 위한 자격 증명 설정(Setting credentials for internal authentication)

내부 인증(internal authentication)을 위한 비밀번호(password)를 설정하려면 `WITH PASSWORD` 절을 사용하고, 비밀번호를 작은따옴표(single quotation mark)로 감싸야 합니다.
...
`network_authorizer`가 구성된 경우, `ACCESS TO DATACENTERS` 절 뒤에 접근 가능한 데이터센터의 집합 리터럴(set literal)을 지정하여 로그인 역할(login role)을 특정 데이터센터로 제한할 수 있습니다. 데이터센터를 생략하면 암묵적으로(implicitly) 모든 데이터센터에 대한 접근이 허용됩니다. 명확성을 위해 `ACCESS TO ALL DATACENTERS` 절을 사용할 수 있으나, 기능적인 차이는 없습니다.

##### 조건부 역할 생성(Creating a role conditionally)

이미 존재하는 역할을 생성하려고 시도하면 유효하지 않은 쿼리(invalid query) 조건이 발생합니다. 단, `IF NOT EXISTS` 옵션을 사용하면 예외입니다. 이 옵션을 사용하고 역할이 이미 존재하는 경우, 해당 문은 아무 동작도 하지 않습니다(no-op).
...
`network_authorizer`가 구성된 경우, `ACCESS TO DATACENTERS` 절에 접근 가능한 데이터센터의 집합 리터럴을 지정하여 로그인 역할을 특정 데이터센터로 제한할 수 있습니다. 데이터센터 제한을 제거하려면 `ACCESS TO ALL DATACENTERS` 절을 사용합니다.

`ALTER ROLE` 문을 실행하기 위한 조건은 다음과 같습니다:

- 다른 역할의 `SUPERUSER` 상태를 변경하려면 클라이언트가 `SUPERUSER` 상태를 가지고 있어야 합니다.
- 클라이언트는 자신이 현재 보유한 어떤 역할의 `SUPERUSER` 상태도 변경할 수 없습니다.
- 클라이언트는 로그인 시 식별된 역할의 일부 속성(예: `PASSWORD`)만 수정할 수 있습니다.
- 역할 속성을 수정하려면 해당 역할에 대한 `ALTER` 권한이 부여되어 있어야 합니다.
...
`DROP ROLE`을 실행하려면 클라이언트가 해당 역할에 대한 `DROP` 권한을 가지고 있어야 합니다. 또한 클라이언트는 로그인 시 식별된 자신의 역할을 `DROP`할 수 없습니다. 마지막으로, `SUPERUSER` 상태를 가진 클라이언트만이 다른 `SUPERUSER` 역할을 `DROP`할 수 있습니다.

존재하지 않는 역할을 삭제하려고 시도하면 유효하지 않은 쿼리 조건이 발생합니다. 단, `IF EXISTS` 옵션을 사용하면 예외입니다. 이 옵션을 사용하고 역할이 존재하지 않는 경우, 해당 문은 아무 동작도 하지 않습니다(no-op).

**참고(Note):** `DROP ROLE`은 의도적으로 열려 있는 사용자 세션(user session)을 종료하지 않습니다. 현재 연결된 세션은 계속 연결된 상태로 유지되며, 인가(authorization)가 필요하지 않은 모든 데이터베이스 작업을 수행할 수 있는 능력을 유지합니다. 다만 인가가 활성화되어 있으면, 삭제된 역할의 권한 또한 철회(revoke)되며 이는 cassandra-yaml 파일에 구성된 캐싱(caching) 옵션의 영향을 받습니다. 만약 삭제된 역할이 이후 새로운 권한이나 역할이 부여된 채로 다시 생성된다면, 여전히 연결되어 있는 클라이언트 세션은 새로 부여된 권한과 역할을 획득하게 됩니다.
...
이 문은 `report_writer` 역할을 `alice`에게 부여합니다. `report_writer`에 부여된 모든 권한은 `alice`도 획득하게 됩니다.

역할은 방향성 비순환 그래프(directed acyclic graph, DAG)로 모델링되므로, 순환적(circular) 부여는 허용되지 않습니다. 다음 예시들은 오류(error) 조건을 발생시킵니다:
...
이 문은 `report_writer` 역할을 `alice`로부터 철회합니다. `alice`가 `report_writer` 역할을 통해 획득했던 모든 권한도 함께 철회됩니다.

---

#### LIST ROLES

시스템에 알려진 모든 역할(또는 특정 역할에 부여된 역할)은 `LIST ROLES` 문으로 열거(enumerate)할 수 있습니다:
...
다음 예시는 `alice`에게 부여된 모든 역할을 열거하며, 전이적으로(transitively) 획득한 역할도 포함합니다:

```
LIST ROLES OF alice;
```

다음 예시는 `bob`에게 직접(directly) 부여된 모든 역할만 나열하며, 전이적으로 획득한 역할은 포함하지 않습니다:
...
Cassandra 2.2에서 역할(role) 개념이 도입되기 전에는, 인증(authentication)과 인가(authorization)가 `USER`라는 개념에 기반하고 있었습니다. 하위 호환성(backward compatibility)을 위해 레거시(legacy) 구문이 보존되었으며, `USER` 중심 문들은 `ROLE` 기반의 등가 문(equivalent)에 대한 동의어(synonym)가 되었습니다. 다시 말해, 사용자를 생성/수정하는 것은 단지 역할을 생성/수정하는 또 다른 구문일 뿐입니다.
...
`CREATE USER` 명령은 `LOGIN` 옵션이 `true`인 `CREATE ROLE`과 동등합니다. 다음 문 쌍(pair)들은 서로 등가입니다:
...
이 문은 `LIST ROLES`와 등가이지만, 출력에는 `LOGIN` 권한을 가진 역할만 포함된다는 점에 유의해야 합니다.
...
리소스에 대한 권한은 역할에 부여됩니다. Cassandra에는 여러 종류의 리소스 타입이 있으며, 각각은 계층적(hierarchical)으로 모델링됩니다:

- 데이터 리소스(Data resource), 즉 키스페이스(Keyspace)와 테이블(Table)의 계층은 `ALL KEYSPACES` → `KEYSPACE` → `TABLE` 구조를 가집니다.
- 함수 리소스(Function resource)는 `ALL FUNCTIONS` → `KEYSPACE` → `FUNCTION` 구조를 가집니다.
- 역할을 표현하는 리소스는 `ALL ROLES` → `ROLE` 구조를 가집니다.
- JMX ObjectName을 표현하는 리소스(MBean/MXBean 집합에 매핑됨)는 `ALL MBEANS` → `MBEAN` 구조를 가집니다.

권한은 이러한 계층 구조의 어느 레벨에서든 부여될 수 있으며, 아래쪽으로 흘러내립니다(flow downwards). 체인 상위에 있는 리소스에 권한을 부여하면, 하위에 있는 모든 리소스에 동일한 권한이 자동으로 부여됩니다. 예를 들어 `KEYSPACE`에 `SELECT`를 부여하면, 해당 `KEYSPACE` 내의 모든 `TABLE`에 자동으로 부여됩니다. 마찬가지로 `ALL FUNCTIONS`에 권한을 부여하면 키스페이스 범위와 무관하게 정의된 모든 함수에 부여됩니다. 특정 키스페이스에 한정된 모든 함수에 권한을 부여하는 것도 가능합니다.

권한 변경 사항은 기존 클라이언트 세션에도 즉시 반영됩니다. 권한 변경 후 접속을 다시 맺을 필요가 없습니다.
...
모든 권한이 모든 리소스 타입에 적용되는 것은 아닙니다. 예를 들어 `EXECUTE`는 함수나 mbean의 맥락에서만 의미가 있습니다. 테이블을 표현하는 리소스에 `EXECUTE`를 부여하는 것은 무의미합니다. 적용할 수 없는 리소스에 권한을 `GRANT`하려고 시도하면 오류 응답(error response)이 발생합니다.
...
이 예시는 `data_reader` 역할을 가진 사용자에게 모든 키스페이스의 임의의 테이블에 대해 `SELECT` 문을 실행할 수 있는 권한을 부여합니다:

```
GRANT MODIFY ON KEYSPACE keyspace1 TO data_writer;
```

이는 `data_writer` 역할을 가진 사용자에게 `keyspace1` 키스페이스의 모든 테이블에 대해 `UPDATE`, `INSERT`, `DELETE`, `TRUNCATE` 쿼리를 수행할 수 있는 권한을 제공합니다:

```
GRANT DROP ON keyspace1.table1 TO schema_owner;
```

이는 `schema_owner` 역할을 가진 사용자에게 특정 테이블 `keyspace1.table1`을 `DROP`할 수 있는 권한을 부여합니다:

```
GRANT EXECUTE ON FUNCTION keyspace1.user_function( int ) TO report_writer;
```

이 명령은 `report_writer` 역할을 가진 사용자에게 함수 `keyspace1.user_function( int )`를 사용하는 `SELECT`, `INSERT`, `UPDATE` 쿼리를 실행할 수 있는 권한을 부여합니다:
...
`CREATE KEYSPACE`, `CREATE TABLE`, `CREATE FUNCTION`, `CREATE AGGREGATE`, `CREATE ROLE` 문을 통해 리소스가 생성되면, 생성자(creator, 해당 문을 발행한 데이터베이스 사용자가 식별된 역할)는 새 리소스에 대해 적용 가능한 모든 권한을 자동으로 부여받습니다.
...
정상적인 드라이버(driver) 동작에서의 역할 때문에, 특정 테이블은 `SELECT` 권한을 철회할 수 없습니다. 다음 테이블들은 할당된 역할과 무관하게 모든 인가된 사용자에게 항상 사용 가능합니다:
...
`alice`에게 부여된 모든 권한을 표시하며, 다른 역할로부터 전이적으로 획득한 권한도 포함합니다:

```
LIST ALL PERMISSIONS ON keyspace1.table1 OF bob;
```

`keyspace1.table1`에 대해 `bob`에게 부여된 모든 권한을 표시하며, 다른 역할로부터 전이적으로 획득한 권한도 포함합니다. 또한 `keyspace1.table1`에 적용될 수 있는 리소스 계층 상위의 권한도 포함됩니다. 예를 들어 `bob`이 `keyspace1`에 대한 `ALTER` 권한을 가지고 있다면, 이 권한도 쿼리 결과에 포함됩니다. `NORECURSIVE` 스위치를 추가하면 결과가 `bob` 또는 `bob`의 역할 중 하나에 직접 부여된 권한으로만 제한됩니다:

```
LIST SELECT PERMISSIONS OF carlos;
```

`carlos` 또는 `carlos`의 역할 중 하나에 부여된 권한을 표시하며, 임의의 리소스에 대한 `SELECT` 권한으로 제한합니다.
