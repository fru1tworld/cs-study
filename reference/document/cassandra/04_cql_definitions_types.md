...
> 이 문서는 Apache Cassandra 공식 문서의 CQL "Definitions" 및 "Data Types" 섹션을 한국어로 번역한 것입니다.
...
모든 구문은 아래에 나열되어 있으며 이 문서의 나머지 부분에서 설명됩니다(위 링크 참고).
...
- 카운터 업데이트는 본질적으로 [멱등(idempotent)](https://en.wikipedia.org/wiki/Idempotence)이지 않습니다. 그 중요한 결과로, 카운터 업데이트가 예기치 않게 실패하면(타임아웃이나 코디네이터 노드와의 연결 손실 등) 클라이언트는 업데이트가 적용되었는지 여부를 알 방법이 없습니다. 특히 업데이트를 재실행(replay)하면 과다 집계(over count)로 이어질 수도, 아닐 수도 있습니다.
...
위의 `+0000` 은 RFC 822 형식의 4자리 시간대 지정자입니다. `+0000` 은 GMT를 가리킵니다. 미국 태평양 표준시(US Pacific Standard Time)는 `-0800` 입니다. 원한다면 시간대를 생략할 수 있으며(`'2011-02-03 04:05:00'`), 이 경우 날짜는 코디네이팅 Cassandra 노드에 설정된 시간대에 있는 것으로 해석됩니다. 그러나 시간대 설정이 예상대로 되어 있다고 가정하는 데에는 본질적인 어려움이 있으므로, 가능하다면 타임스탬프에는 항상 시간대를 명시하는 것이 권장됩니다.
...
`duration` 타입의 값은 가변 길이의 부호 있는 정수 3개로 인코딩됩니다. 첫 번째 정수는 개월 수를, 두 번째는 일 수를, 세 번째는 나노초 수를 나타냅니다. 이는 한 달의 일 수가 달라질 수 있고, 일광 절약 시간제(daylight saving)에 따라 하루가 23시간 또는 25시간이 될 수 있기 때문입니다. 내부적으로 개월 수와 일 수는 32비트 정수로, 나노초 수는 64비트 정수로 디코딩됩니다.
...
- list에서 해당 위치에 기존 요소가 있는 특정 위치의 값을 설정. list에 해당 위치가 없으면 오류가 발생합니다.

```cql
UPDATE plays SET scores[1] = 7 WHERE id = '123-afde';
```

- list에서 해당 위치에 기존 요소가 있는 요소를 위치로 제거. list에 해당 위치가 없으면 오류가 발생합니다. 또한 이 연산은 list에서 요소를 제거하므로 list 크기가 한 요소만큼 줄어들며, 뒤따르는 모든 요소의 위치가 하나씩 앞당겨집니다.
