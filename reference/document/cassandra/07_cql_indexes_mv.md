...
> 이 문서는 Apache Cassandra 공식 문서의 CQL "Secondary Indexes", "Materialized Views", "SASI" 섹션을 한국어로 번역한 것입니다.
...
- `CREATE INDEX` 문을 실행하면, 컬럼에 이미 데이터가 들어 있는 경우 해당 데이터는 인덱스 생성 시점에 인덱싱됩니다. 인덱스가 생성된 후에는 해당 컬럼의 데이터가 변경될 때마다 인덱스가 자동으로 갱신됩니다.
...
- 보조 인덱스(2i)는 각 노드에 로컬(local)로 구성되므로, 인덱스 컬럼만으로 질의하면 코디네이터(coordinator)가 모든 노드에 흩어져 질의를 보내야 할 수 있어 비용이 높습니다.
...
SASI라는 이름은 Cassandra의 한 번 쓰고(write-once) 변경 불가능하며(immutable) 정렬된(ordered) 데이터 모델을 활용하여, Memtable을 디스크로 플러시(flush)할 때 인덱스를 함께 구성한다는 점에서 유래했습니다("SSTable Attached Secondary Index").
...
**핵심 아키텍처**: SASI는 Cassandra의 한 번 쓰고 변경 불가능하며 정렬된 데이터 모델을 활용하여, Memtable을 디스크로 플러시할 때 인덱스를 함께 구성합니다. 이것이 "SSTable Attached Secondary Index"라는 이름의 유래입니다.
...
**질의 처리(Query Processing)**: 검색 질의마다 인스턴스화되는 `QueryPlan`이 SASI 질의 구현의 핵심입니다. 이는 Cassandra의 내부 표현인 `IndexExpression`들을 연산 트리(operation tree)로 변환하여 질의 최적화를 수행한 뒤, `DecoratedKey`를 생성하는 이터레이터(iterator)를 통해 실행합니다.
