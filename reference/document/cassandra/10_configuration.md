...
Apache Cassandra의 설정은 여러 설정 파일로 구성되며, 각 파일은 서로 다른 역할을 담당합니다. 주요 설정 파일은 다음과 같습니다.
...
파티셔너(partitioner)는 행 그룹(파티션 키 기준)을 클러스터 노드들에 분산시킵니다. 파티셔너는 모든 데이터를 다시 로드하지 않는 한 변경할 수 **없습니다**. 기본 파티셔너는 `Murmur3Partitioner`입니다.
...
OS와 드라이브가 작업을 재정렬(reorder)할 수 있도록 큐 깊이를 충분히 확보하려면 `(16 * 드라이브 수)`로 설정하는 것이 좋습니다.

- **기본값:** `32`

#### concurrent_writes

시스템의 코어 수에 따라 달라지며, `(8 * 코어 수)`를 기준으로 삼는 것이 좋습니다.
...
바인딩할 주소 또는 인터페이스로, 다른 Cassandra 노드들에게 이 주소로 연결하도록 알립니다. 여러 노드가 서로 통신할 수 있게 하려면 반드시 이 값을 변경해야 합니다!
...
다음 예시는 세 개의 데이터센터를 구성한 경우를 보여줍니다.
...
이 구조를 통해 클러스터 토폴로지 전반에 걸쳐 적절한 복제본 분산과 효율적인 요청 라우팅이 가능합니다.
...
JVM 설정을 동적으로 계산해야 할 때는 이 파일이 적합하며, 정적(static) 설정에는 `jvm-*` 옵션 파일을 사용하는 것이 좋습니다.
...
`jvm-*` 파일은 **정적(static) JVM 설정만** 저장하는 반면, 레거시 `cassandra-env.sh` bash 스크립트는 JVM 설정을 시스템 특성에 따라 동적으로 계산해야 할 때 여전히 유용합니다.
