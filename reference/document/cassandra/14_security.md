...
> 이 문서는 Apache Cassandra 공식 문서의 "Security" 및 "Audit Logging" 섹션을 한국어로 번역한 것입니다.
...
Cassandra는 세 가지 주요 보안 구성 요소를 제공합니다. 즉, 클라이언트 통신 및 노드 간 통신을 위한 TLS/SSL 암호화(encryption), 클라이언트 인증(authentication), 그리고 권한 부여(authorization)입니다. 기본적으로 이 기능들은 모두 비활성화되어 있으며, 이는 상당한 공격 표면(attack surface)을 노출시킵니다. 클러스터를 올바르게 보안 설정하려면 세 가지 구성 요소를 모두 이해하는 것이 필수적입니다.

이 세 가지 보안 영역은 다음과 같습니다.
...
암호화를 활성화하면 JVM의 기본 프로토콜(protocols)과 암호 스위트(cipher suites)가 사용됩니다. 이 값들은 `cassandra.yaml`에서 재정의(override)할 수 있으나, 특정 정책상 필요하거나 취약한 암호(vulnerable ciphers)를 비활성화해야 하는 경우가 아니라면 권장되지 않습니다.

FIPS 준수(FIPS-compliant) 설정은 `cassandra.yaml` 설정 파일이 아니라 JVM 수준(JVM level)에서 구성해야 합니다.

Cassandra는 JKS 및 PKCS12를 포함한 여러 키스토어(keystore) 형식과 PEM 표준을 지원합니다. 조직은 `ISslContextCreationFactory` 인터페이스를 구현하거나 그 public 하위 클래스를 확장하여 SSL 컨텍스트 생성 방식을 커스터마이징할 수 있습니다. `ssl_context_factory` 설정은 `server_encryption_options`와 `client_encryption_options` 두 섹션 모두에 적용됩니다.

Java가 지원하는 키스토어를 사용하는 SSL 통신에 필요한 키스토어(keystore) 및 트러스트스토어(truststore) 파일 생성에 대한 정보는 키스토어 생성에 관한 Java 공식 문서를 참고하십시오.
...
키스토어/트러스트스토어 파일이 갱신되면, Cassandra는 이후의 연결을 위해 해당 파일들을 자동으로 다시 로드합니다. 갱신된 파일은 기존 설정과 동일한 비밀번호(password)를 사용해야 합니다.

인증서 핫 리로딩은 `nodetool reloadssl` 명령을 사용하여 트리거할 수도 있습니다. Cassandra가 변경된 인증서를 즉시 인식하도록 하려면 이 명령을 사용하십시오.
...
인증을 클러스터 전체에 활성화하기 전에, 클라이언트 애플리케이션에 자격 증명을 미리 구성해 두어야 합니다. 초기 구성은 클라이언트 트래픽으로부터 격리된 노드에서 수행해야 합니다. 다음 절차를 따르십시오.
...
기본 슈퍼유저 자격 증명은 `QUORUM` 일관성(consistency)으로 읽히는 반면, 다른 사용자들은 `LOCAL_ONE`을 사용합니다. 추가 슈퍼유저를 생성하면 성능과 보안이 향상됩니다.
...
권한은 화이트리스트(whitelist)로 모델링되며, 기본 가정(default assumption)은 주어진 역할이 어떠한 데이터베이스 리소스에도 접근 권한이 없다는 것입니다. 이것이 의미하는 바는, 일단 노드에서 권한 부여가 활성화되면 필요한 권한이 부여되기 전까지 모든 요청이 거부된다는 것입니다.
...
다른 노드들이 구성되기 전까지는 영향을 받지 않으므로 클라이언트 중단(disruption)을 피할 수 있습니다.
...
인증과 권한 부여는 `system_auth` 테이블에서 자주 읽기를 수행하므로 추가적인 부하(load)를 발생시킵니다. 이러한 읽기는 클라이언트 작업의 임계 경로(critical path)에서 발생하므로 서비스 품질(quality of service)에 영향을 줄 수 있습니다. 인증 데이터 캐싱(auth data caching)은 이러한 영향을 완화합니다.
...
- **갱신 주기(Refresh Rate)**: 기저 데이터(underlying data) 변경 사항을 반영하기 위한 백그라운드 읽기(background read) 빈도를 제어합니다. 비동기 갱신(async refresh) 중에는 캐시가 다소 오래된(stale) 데이터를 제공할 수 있습니다.
...
JMX 접근 제어는 CQL 인증 및 권한 부여와는 별도로 구성됩니다. 두 가지 제공자(provider)를 사용할 수 있습니다. 즉, 표준 JMX 보안(standard JMX security)과 Cassandra의 통합 인증 서브시스템(integrated auth subsystem)입니다.
...
표준 JMX 인증의 대안으로, Cassandra의 네이티브 인증 및 권한 부여 제공자를 사용할 수 있습니다. 이 접근 방식은 더 큰 유연성과 보안을 제공하지만 한 가지 중요한 주의 사항이 있습니다. 즉, 노드가 링(ring)에 합류하기 전까지는 사용할 수 없다는 것입니다. 이는 인증 서브시스템이 그 시점까지 완전히 구성되지 않기 때문입니다.

그러나 모니터링 목적상, 특히 부트스트랩(bootstrap) 중에 JMX 접근을 확보하는 것이 결정적으로 중요한 경우가 많습니다. 따라서 가능한 경우, 부트스트랩 중에는 로컬 전용 JMX 인증을 사용하고, 노드가 링에 합류하여 초기 설정이 완료된 후 원격 연결이 필요하면 통합 인증으로 전환하는 것이 권장됩니다.

CQL 인증에 사용되는 데이터베이스 역할(database roles)로 JMX 접근을 제어할 수 있으며, `cqlsh`를 통한 중앙 집중식 관리(centralized management)가 가능합니다. `GRANT PERMISSION` 문을 통해 특정 MBean 작업에 대한 세밀한 제어(fine-grained control)가 가능합니다.
...
커스텀 Java 암호화 공급자(Java Crypto Provider) 지정 기능은 CASSANDRA-18624의 일부로 구현되었습니다.
...
`DefaultCryptoProvider`는 Amazon Corretto Crypto provider를 설치하며, 이는 JRE 설치판의 기본 암호화 공급자보다 훨씬 더 성능이 우수한 것으로 입증되었습니다.

Amazon Corretto Crypto Provider는 x86_64 및 aarch64 플랫폼에서 동작합니다. 설치에 실패하면 기본 JRE 암호화 공급자로 폴백(fall back)합니다. `fail_on_missing_provider`를 "true"로 설정하면 공급자가 제대로 설치되지 않았을 때 강제로 실패하게 됩니다.
...
설치 시 `AbstractCryptoProvider`는 공급자가 이미 설치되어 있는지 확인합니다. 설치되어 있고 첫 번째 위치(first)에 배치되어 있으면, 메시지가 로깅됩니다. 설치되어 있으나 첫 번째가 아니고 `fail_on_missing_provider`가 true이면, 예외가 발생하고 노드가 시작에 실패합니다. 설치가 성공하지 못한 경우에도 동일한 동작이 발생합니다.

### 플랫폼별 라이브러리

플랫폼별 라이브러리(platform-specific libraries)는 `cassandra.in.sh`에 의해 Cassandra의 클래스패스에 자동으로 추가됩니다. 디렉터리에는 각 아키텍처용 JAR 파일이 들어 있는 `lib/aarch64`와 `lib/x86_64`가 포함됩니다. 플랫폼 판별은 `uname -m` 명령의 출력을 사용합니다.
...
Cassandra의 감사 로깅(audit logging)은 들어오는 모든 CQL 명령 요청과 Cassandra 노드에 대한 인증(로그인 성공/실패)을 로깅합니다. 두 가지 구현체를 사용할 수 있습니다. 즉, 성능을 위해 커뮤니티가 권장하는 `BinAuditLogger`와 `FileAuditLogger`입니다.
...
준비된 문(prepared statements)을 실행하면, prepare 호출에서 클라이언트가 제공한 대로 쿼리가 로깅됩니다. 준비된 문 실행을 위해 바인딩된 실제 값(actual values bound)은 감사 로그에 나타나지 않습니다.

**로깅되는 속성(Logged Attributes):**

각 구현체는 다음 정보에 접근할 수 있습니다. 즉, 사용자(user), 호스트(host), 소스 IP 주소(source IP address), 소스 포트(source port), 타임스탬프(timestamp), 요청 유형(request type), 카테고리(category), 키스페이스(keyspace), 스코프(scope), 그리고 작업(operation, 실제 CQL 명령)입니다.
...
- `roll_cycle`: 로그 롤링 주기(기본값: `FAST_HOURLY` / 문서상 `HOURLY`로 표기되기도 함).
...
`FileAuditLogger`는 SLF4J 로거를 통해 텍스트 기반으로 감사 이벤트를 기록합니다. 이를 사용하려면 `logback.xml`에 감사 이벤트를 위한 별도의 어펜더(appender)를 구성하여, 로깅을 `audit/audit.log`로 보내고 커스텀 롤링 정책(rolling policy) 및 보존(retention) 설정을 적용해야 합니다.
