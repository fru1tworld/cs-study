...
> 이 문서는 `man systemd.resource-control` 와 cgroups v2 문서를 한국어로 정리한 것입니다.
...
각 서비스는 `<name>.service` 라는 cgroup을 갖고, 그 cgroup의 모든 프로세스는 systemd가 추적합니다. 이는 PID 파일 기반의 SysV init보다 훨씬 안정적인 프로세스 추적을 제공합니다.
...
이렇게 하면 모든 DB 서비스가 한 slice에 묶여 합산 8GB 메모리, 가중치 200으로 제한됩니다.
...
[Service]
Delegate=yes
DelegateControllers=cpu memory io
```

이 unit이 자식 cgroup을 직접 관리하도록 위임. 컨테이너 런타임(systemd-nspawn, Docker 등)이 사용. 일반 서비스는 필요 없음.
...
cgroup별 CPU/메모리/IO 사용량을 `top` 처럼 보여줌.
...
`--scope` 는 외부 명령을 새 cgroup으로 감싸 실행. 즉석에서 리소스 한도 테스트에 유용.
