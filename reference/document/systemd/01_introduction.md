...
> 이 문서는 systemd 공식 사이트와 man 페이지의 개요 부분을 한국어로 정리한 것입니다.
...
서비스를 어떻게(how) 시작할지 스크립트로 작성하는 대신, **무엇(what)을 원하는지** 를 unit 파일이라는 단순한 INI 형식으로 선언합니다.
...
PID 1은 Linux 커널이 부팅 마지막 단계에서 실행하는 첫 번째 사용자 공간 프로세스입니다. PID 1이 죽으면 커널 패닉이 발생하므로 매우 안정적이어야 합니다.
...
- Linux 커널 4.15 이상 권장 (cgroups v2 안정 지원)
- glibc 2.16 이상
- 주요 아키텍처: x86_64, aarch64, riscv64, armv7, ppc64le, s390x
