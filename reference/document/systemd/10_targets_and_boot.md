...
> 이 문서는 `man systemd.target` 과 `man bootup` 의 내용을 한국어로 정리한 것입니다.
...
SysV init의 **런레벨(runlevel)** 과 비슷한 개념을 일반화한 것:
...
이 명령은 심볼릭 링크를 바꿔줍니다. 다음 부팅부터 적용.
...
각 target은 **이전 단계의 완료를 보장** 합니다. 예를 들어 `multi-user.target` 에 등록된 서비스는 `basic.target` 이 도달한 후에만 시작됩니다.
...
`network-online.target` 은 단독으로 활성화되지 않으므로 `Wants=` 도 함께 명시해야 합니다.

---

## Isolate

`systemctl isolate <target>` 은 **지정한 target과 그 의존성만 활성 상태로 만들고, 나머지는 모두 중지** 합니다.
...
target이 `AllowIsolate=yes` 를 가져야 isolate 대상이 될 수 있습니다.
...
이제 `systemctl start myapp.target` 하나로 스택 전체를 다룰 수 있습니다.

### Drop-in으로 target 확장

기존 `multi-user.target` 에 의존성 추가하고 싶다면:
...
부팅 시간 측정에는 `systemd-analyze` 가 가장 편리합니다.
